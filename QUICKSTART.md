...
| **1,720+ molecular descriptors across 6 categories** (~492 built-in + 1,019 via the `addcore_3d` plugin + 20 via the `constitutional_property` plugin + 107 via the `topological_connectivity` plugin + 86 via the `walk_path_information` plugin). | `milia_pipeline/descriptors/` — registry, 6+ files. README § Key Features → "Molecular Descriptors" enumerates the 6 categories with counts. |
