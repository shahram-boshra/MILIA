...
| **3,000+ molecular descriptors across 6 categories** (~492 built-in + 1,019 via the `addcore_3d` plugin + 20 via the `constitutional_property` plugin + 107 via the `topological_connectivity` plugin + 86 via the `walk_path_information` plugin + 177 via the `matrix_spectral` plugin + 606 via the `autocorrelation_2d` plugin + 316 via the `estate_atomtype` plugin + 150 via the `cats2d` plugin + 45 via the `eta` plugin + 50 via the `cpsa_geometric_3d` plugin + 25 via the `eht_electronic` plugin). | `milia_pipeline/descriptors/` — registry, 6+ files. README § Key Features → "Molecular Descriptors" enumerates the 6 categories with counts. |
...
| **Three-tier plugin architecture** (transformations, descriptors, models) with YAML manifests. | `milia_pipeline/plugins/` — one kind-container per kind (`transformations/`, `descriptors/`, `models/`), each with a `user_template/` scaffold. README § Key Features → "Three-Tier Plugin Architecture" + § Datasets → "Adding a dataset" subsection describes the three-file extension pattern. |
