...
### 3.9 Documentation Build System — ✅ IMPLEMENTED *(activates when pushed to Read the Docs)*

**Source**: Sphinx, Read the Docs, pyOpenSci. Scientific Python Development Guide (`learn.scientific-python.org/development/tutorials/docs/`). pyOpenSci Python Package Guide — Documentation section.

**Tool choice**: **Sphinx** (not MkDocs). Evidence: pyOpenSci — "The most common tool for building documentation in the Python ecosystem currently is Sphinx"; Scientific Python Dev Guide — "we will use Sphinx as it is still more popular with scientific libraries"; pyOpenSci default template uses Sphinx with pydata-sphinx-theme.

**✅ DONE**: Created `docs/` directory with Sphinx build system. 7 files created + `.gitignore` updated + downstream files updated.

**Files created**:
- **`docs/conf.py`**: Sphinx configuration. Extensions: `autodoc`, `autosummary`, `napoleon`, `intersphinx`, `viewcode`, `myst_parser`, `sphinx_copybutton`. Theme: `pydata-sphinx-theme` (pyOpenSci standard). All project metadata read dynamically from installed package metadata via `importlib.metadata` (falls back to direct `__init__.py` import). Intersphinx maps to Python, NumPy, PyTorch, PyG docs.
- **`docs/index.md`**: Landing page with toctree (User Guide, API Reference, Project sections). MyST Markdown syntax per Scientific Python Dev Guide.
- **`docs/getting-started.md`**: Installation and quick-start guide. Content sourced from `README.md` and `CONTRIBUTING.md`.
- **`docs/api/index.md`**: API reference with `autosummary :recursive:` directive for auto-generated module docs. 11-module table sourced from project structure.
- **`docs/changelog.md`**: Includes root `CHANGELOG.md` via MyST `{include}` directive.
- **`docs/contributing.md`**: Includes root `CONTRIBUTING.md` via MyST `{include}` directive.
- **`docs/requirements.txt`**: Pinned doc build deps — `sphinx>=8.1,<8.2` (8.2 drops Python 3.10 per sphinx-doc/sphinx#13325), `pydata-sphinx-theme>=0.16`, `myst-parser>=4.0`, `sphinx-copybutton>=0.5`.
- **`docs/_static/.gitkeep`**: Placeholder for custom CSS/assets.
- **`.readthedocs.yaml`**: RTD v2 config — Python 3.12, `docs/conf.py`, `docs/requirements.txt`.

**Downstream updates**:
- **`.gitignore`**: Removed `docs/` exclusion (per §2.4 note: "When the documentation build system is adopted, `docs/` should be removed from `.gitignore`"). `docs/_build/` already excluded by GitHub Python template (line 78).
- **`Makefile`**: Added 3 targets (`docs`, `docs-serve`, `docs-clean`) + `docs/_build` in `clean-all`.
- **`pyproject.toml`**: Added `[project.optional-dependencies].docs` group; updated Documentation URL to `https://milia.readthedocs.io`.
- **`CONTRIBUTING.md`**: Added "Documentation" section with build instructions, writing guidelines, NumPy-style docstring standard.
- **`MANIFEST.in`**: Added `prune docs` (docs source not needed in sdist; hosted separately on RTD).

**Post-push setup** (one-time, after first push to GitHub):
1. Connect repository to Read the Docs at `readthedocs.org/dashboard/import/`.
2. RTD will auto-detect `.readthedocs.yaml` and build docs on each push to `main`.
...
├── .readthedocs.yaml                    # ✅ CREATED — Read the Docs build config (§3.9, activates when connected to RTD)
...
├── docs/                                # ✅ CREATED — Sphinx documentation build system (§3.9)
│   ├── conf.py                          #   Sphinx configuration (autodoc, MyST, pydata-sphinx-theme)
│   ├── index.md                         #   Landing page with toctree
│   ├── getting-started.md               #   Installation and quick-start guide
│   ├── api/index.md                     #   API reference (autosummary :recursive:)
│   ├── changelog.md                     #   Includes root CHANGELOG.md
│   ├── contributing.md                  #   Includes root CONTRIBUTING.md
│   ├── requirements.txt                 #   Pinned doc build dependencies
│   └── _static/.gitkeep                 #   Custom CSS/assets placeholder
