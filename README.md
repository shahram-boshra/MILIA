...
| `descriptors/` | 6+ files | 3,000+ molecular descriptors across 6 categories (~492 RDKit baseline + 1,019 via the `addcore_3d` plugin + 20 via the `constitutional_property` plugin + 107 via the `topological_connectivity` plugin + 86 via the `walk_path_information` plugin + 177 via the `matrix_spectral` plugin + 606 via the `autocorrelation_2d` plugin + 316 via the `estate_atomtype` plugin + 150 via the `cats2d` plugin + 45 via the `eta` plugin + 50 via the `cpsa_geometric_3d` plugin + 25 via the `eht_electronic` plugin; + 307 via the opt-in `cdk_substructure` plugin) with thread-safe singleton registry, caching, conformer generation, block-cache for vector blocks, and plugin support |
