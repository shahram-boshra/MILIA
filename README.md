...
milia --list-descriptors             # 2,951+ descriptors: 418 built-in + 1,019 via addcore_3d + 20 via constitutional_property + 107 via topological_connectivity + 86 via walk_path_information + 177 via matrix_spectral + 606 via autocorrelation_2d + 316 via estate_atomtype + 150 via cats2d + 45 via eta
...
| `datasets/` | 11 files | Registry-based PyTorch Geometric datasets with Protocol contracts, compile-time validation, and 11 concrete implementations spanning the VQM24 family (DFT, DMC, Wavefunction), QM9, the ANI family (ANI-1x, ANI-1ccx, ANI-2x), rMD17, xxMD, QDπ, and QM40 — see the [Datasets](#datasets) section |
...
| `descriptors/` | 6+ files | 2,900+ molecular descriptors across 6 categories (~492 RDKit baseline + 1,019 via the `addcore_3d` plugin + 20 via the `constitutional_property` plugin + 107 via the `topological_connectivity` plugin + 86 via the `walk_path_information` plugin + 177 via the `matrix_spectral` plugin + 606 via the `autocorrelation_2d` plugin + 316 via the `estate_atomtype` plugin + 150 via the `cats2d` plugin + 45 via the `eta` plugin) with thread-safe singleton registry, caching, conformer generation, block-cache for vector blocks, and plugin support |
