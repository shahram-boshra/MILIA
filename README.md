...
milia --list-descriptors             # 1,450+ descriptors: 418 built-in + 1,019 via addcore_3d + 20 via constitutional_property
...
| `descriptors/` | 6+ files | 1,530+ molecular descriptors across 6 categories (~492 RDKit baseline + 1,019 via the `addcore_3d` plugin + 20 via the `constitutional_property` plugin) with thread-safe singleton registry, caching, conformer generation, block-cache for vector blocks, and plugin support |
