...
milia --list-descriptors             # 1,550+ descriptors: 418 built-in + 1,019 via addcore_3d + 20 via constitutional_property + 107 via topological_connectivity
...
| `descriptors/` | 6+ files | 1,630+ molecular descriptors across 6 categories (~492 RDKit baseline + 1,019 via the `addcore_3d` plugin + 20 via the `constitutional_property` plugin + 107 via the `topological_connectivity` plugin) with thread-safe singleton registry, caching, conformer generation, block-cache for vector blocks, and plugin support |
