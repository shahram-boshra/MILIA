...
milia --list-descriptors             # 1,400+ descriptors: 418 built-in + 1,019 via the addcore_3d plugin
...
| `descriptors/` | 6+ files | 1,500+ molecular descriptors across 6 categories (~492 RDKit baseline + 1,019 via the `addcore_3d` plugin) with thread-safe singleton registry, caching, conformer generation, block-cache for vector blocks, and plugin support |
