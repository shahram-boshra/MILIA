...
class TestLocalHelperFunctions:
    ...
    def test_get_config_value_missing_key(self, mock_config):
        ...
        with pytest.raises(ConfigurationError, match="Missing required config key"):
            config_constants._get_config_value(
                mock_config, 'nonexistent_key', str
            )
    ...
    def test_get_config_value_tuple_of_types(self, mock_config):
        ...
        value = config_constants._get_config_value(
            mock_config, 'dataset_type', (str, type(None))
        )
        assert value == 'DFT'
...
class TestHandlerConstants:
    ...
    def test_supported_handler_types(self):
        ...
        expected_types = ['DFT', 'DMC', 'Wavefunction', 'QM9', 'ANI1x', 'ANI1CCX', 'RMD17', 'ANI2x', 'XXMD', 'QDPi']
        assert config_constants.SUPPORTED_HANDLER_TYPES == expected_types
...
class TestGetHandlerConstants:
    ...
    def test_get_handler_constants_dft(self, mock_get_dataset_config):
        ...
        setattr(config_constants, 'HAR2EV', 27.211386245988)
        setattr(config_constants, 'ATOMIC_ENERGIES_HARTREE', {'H': -0.5, 'C': -37.8})
        setattr(config_constants, 'HEAVY_ATOM_SYMBOLS_TO_Z', {'C': 6, 'N': 7})
    ...
    def test_get_handler_constants_caching(self, mock_get_dataset_config):
        ...
        setattr(config_constants, 'HAR2EV', 27.211)
        setattr(config_constants, 'ATOMIC_ENERGIES_HARTREE', {'H': -0.5})
        setattr(config_constants, 'HEAVY_ATOM_SYMBOLS_TO_Z', {'C': 6})
...
class TestHandlerCompatibility:
    ...
    def test_check_handler_feature_support_true(self):
        ...
        result = config_constants.check_handler_feature_support(
            'DFT', 'vibrational_analysis'
        )
...
class TestGetTransformationConstants:
    ...
    def test_get_transformation_constants_caching(self):
        ...
        with patch('milia_pipeline.config.config_constants._check_transformation_system_availability') as mock:
...
class TestHandlerTransformCompatibility:
    ...
    def test_get_handler_transform_compatibility_compatible(self):
        ...
        status = config_constants.get_handler_transform_compatibility(
            'DFT', 'NormalizeFeatures'
        )
...
class TestExperimentalSetupValidation:
    ...
    def test_validate_experimental_setup_compatible(self, mock_get_setup):
        ...
        result = config_constants.validate_experimental_setup_for_handler(
            'DFT', 'baseline'
        )
    ...
    def test_validate_experimental_setup_invalid_handler(self):
        """Test that invalid handler raises error."""
        with pytest.raises(HandlerNotAvailableError):
            config_constants.validate_experimental_setup_for_handler(
                'INVALID', 'baseline'
            )
...
class TestLegacyCompatibility:
    ...
    def test_get_legacy_compatible_constants_basic(self):
        ...
        setattr(config_constants, 'HEAVY_ATOM_SYMBOLS_TO_Z', {'C': 6})
        setattr(config_constants, 'HAR2EV', 27.211)
        setattr(config_constants, 'RAW_NPZ_FILENAME_CACHED', 'file.npz')
        setattr(config_constants, 'RAW_DATA_DOWNLOAD_URL_CACHED', 'url')
        setattr(config_constants, 'DATASET_ROOT_DIR_CACHED', '/dir')
        setattr(config_constants, 'PROCESSED_DATA_FILENAME', 'file.pt')
        ...
        try:
            ...
            # Cleanup
            for attr in ['ATOMIC_ENERGIES_HARTREE', 'HEAVY_ATOM_SYMBOLS_TO_Z', 'HAR2EV',
                        'RAW_NPZ_FILENAME_CACHED', 'RAW_DATA_DOWNLOAD_URL_CACHED',
                        'DATASET_ROOT_DIR_CACHED', 'PROCESSED_DATA_FILENAME']:
                try:
    ...
    def test_get_legacy_compatible_constants_with_handler(self, mock_is_supported, mock_get_handler):
    ...
    def test_ensure_handler_constant_compatibility(self):
        ...
        try:
            with patch('milia_pipeline.config.config_constants.get_handler_constants') as mock:
                ...
                with patch('milia_pipeline.config.config_constants.validate_handler_environment') as mock_validate:
                    mock_validate.return_value = {'handler_environment_ready': True}
                    
                    with patch('milia_pipeline.config.config_constants.get_supported_handler_types') as mock_supported:
            ...
            # Cleanup
            for attr in ['ATOMIC_ENERGIES_HARTREE', 'HEAVY_ATOM_SYMBOLS_TO_Z', 'HAR2EV',
                        'RAW_NPZ_FILENAME_CACHED', 'DATASET_ROOT_DIR_CACHED']:
                try:
                    delattr(config_constants, attr)
                except AttributeError:
                    pass
...
class TestDebuggingDiagnostics:
    ...
    def test_get_complete_constants_debug_info(self):
        ...
        try:
            with patch('milia_pipeline.config.config_constants.get_handler_constants') as mock_handler:
                ...
                with patch('milia_pipeline.config.config_constants.validate_complete_environment') as mock_validate:
                    mock_validate.return_value = {'complete_environment_ready': True}
                    
                    with patch('milia_pipeline.config.config_constants.ensure_complete_compatibility') as mock_compat:
                        mock_compat.return_value = True
                        
                        with patch('milia_pipeline.config.config_constants.get_supported_handler_types', 
                                  return_value=['DFT', 'DMC']):
                            # Also need to mock _dataset_constants which is used directly in the function
                            with patch.object(config_constants, '_dataset_constants', ('file.npz', 'url', '/dir'), create=True):
            ...
            for attr in ['ATOMIC_ENERGIES_HARTREE', 'HEAVY_ATOM_SYMBOLS_TO_Z', 
                        'HAR2EV', 'PROCESSED_DATA_FILENAME']:
                try:
                    delattr(config_constants, attr)
                except AttributeError:
                    pass
    ...
    def test_validate_complete_environment(self, mock_check_transform, mock_validate_handler):
        ...
        with patch('milia_pipeline.config.config_constants.get_transformation_constants') as mock_get:
...
class TestLazyLoading:
    ...
    def test_lazy_load_processed_data_filename(self):
        ...
        setattr(config_constants, 'RAW_NPZ_FILENAME_CACHED', 'DFT_all.npz')
        setattr(config_constants, 'PROCESSED_DATA_FILENAME', 'DFT_all.pt')
...
class TestRegistryWrappers:
    ...
    def test_registry_list_all_fallback(self):
        ...
        try:
            ...
            with patch('milia_pipeline.config.config_constants._init_registry', return_value=False):
                with patch('milia_pipeline.config.config_constants._discover_dataset_types_from_filesystem', 
    ...
    def test_registry_get_not_available(self):
        ...
        try:
            ...
            with patch('milia_pipeline.config.config_constants._init_registry', return_value=False):
                with patch('milia_pipeline.config.config_constants.registry_list_all', return_value=['DFT']):
                    with pytest.raises(HandlerNotAvailableError):
    ...
    def test_registry_is_registered_fallback(self):
        ...
        try:
            ...
            with patch('milia_pipeline.config.config_constants._init_registry', return_value=False):
                with patch('milia_pipeline.config.config_constants.registry_list_all', 
                          return_value=['DFT', 'DMC']):
...
class TestCacheInvalidation:
    ...
    def test_setup_registry_cache_invalidation_success(self):
        ...
        with patch('milia_pipeline.config.config_constants._init_registry', return_value=True):
            with patch('milia_pipeline.config.config_constants.get_default_registry', 
    ...
    def test_ensure_cache_invalidation_registered(self):
        ...
        try:
            ...
            with patch('milia_pipeline.config.config_constants._setup_registry_cache_invalidation', 
                      return_value=True):
...
class TestDynamicHandlerFunctions:
    ...
    def test_is_handler_type_supported_true(self):
        ...
        with patch('milia_pipeline.config.config_constants._ensure_cache_invalidation_registered'):
            with patch('milia_pipeline.config.config_constants._init_registry', return_value=True):
                with patch('milia_pipeline.config.config_constants._REGISTRY_AVAILABLE', True):
                    with patch('milia_pipeline.config.config_constants.registry_is_registered', 
                              return_value=True):
...
class TestAdditionalDynamicHandlerFunctions:
    ...
    @patch('milia_pipeline.config.config_constants._init_registry')
    @patch('milia_pipeline.config.config_constants.registry_is_registered')
    def test_get_handler_molecule_creation_strategy_valid(self, mock_is_reg, mock_init, mock_ensure):
...
class TestWavefunctionHandlerConstants:
    ...
    def test_get_wavefunction_handler_constants(self, mock_init_registry, mock_get_dataset_config):
        ...
        setattr(config_constants, 'HEAVY_ATOM_SYMBOLS_TO_Z', {'C': 6, 'N': 7})
        setattr(config_constants, 'BOHR_TO_ANGSTROM', 0.529177)
        ...
        try:
            ...
            with patch('milia_pipeline.config.config_constants.is_handler_type_supported', 
                      return_value=True):
                with patch('milia_pipeline.config.config_constants.registry_is_registered',
                          return_value=True):
...
class TestGlobalPathsConfig:
    ...
    def test_get_dataset_constants_uses_global_paths(self, mock_load_config):
        ...
        mock_config = {
            ...
            'global_paths': {
                'working_root_dir': '/custom/data/path'
            }
        }
        mock_load_config.return_value = mock_config
