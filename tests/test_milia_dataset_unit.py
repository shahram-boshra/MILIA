...
class TestPhase6InsightExtractionMethods(BaseTestCase):
    ...
    def test_extract_vibrational_insights_with_stats(self):
        ...
        statistics = {}
        handler_stats = {
            "vibrational_refinement": {"molecules_refined": 50, "average_frequency_reduction": 0.05}
        }
        ...
        self.assertEqual(
            result["vibrational_insights"]["vibrational_processing"]["molecules_refined"], 50
        )
    ...
    def test_extract_orbital_insights_with_stats(self):
        ...
        self.assertEqual(
            result["orbital_insights"]["orbital_processing"]["molecules_with_orbitals"], 75
        )
...
class TestPhase6BackwardCompatibility(BaseTestCase):
    ...
    def test_legacy_dmc_insights_replaced_by_uncertainty_insights(self):
        ...
        stats = {}
        handler_stats = {"uncertainty_statistics": {"count": 10, "mean": 0.1}}
    ...
    def test_legacy_dft_insights_replaced_by_vibrational_insights(self):
        ...
        stats = {}
        handler_stats = {"vibrational_refinement": {"molecules_refined": 10}}
        result = dataset._extract_vibrational_specific_insights(stats, handler_stats)
        self.assertIn("vibrational_insights", result)
        self.assertNotIn("dft_insights", result)
        self.assertEqual(
            result["vibrational_insights"]["vibrational_processing"]["molecules_refined"], 10
        )
...
class TestStandardTransformsSupport(BaseTestCase):
    ...
    def test_priority_1_uses_get_combined_transforms_as_dicts(
        self, mock_get_setup, mock_get_combined
    ):
    ...
    def test_priority_3_uses_get_combined_transforms_as_dicts(
        self, mock_get_config, mock_get_setup, mock_get_combined
    ):
    ...
    @patch("milia_pipeline.config.config_accessors.get_combined_transforms_as_dicts")
    @patch("milia_pipeline.config.config_accessors.get_experimental_setup")
    def test_combined_transforms_standard_plus_experimental(
        self, mock_get_setup, mock_get_combined
    ):
...
class TestNPZDataLoading(BaseTestCase):
    ...
    def test_determine_required_keys_with_handler(self):
        ...
        result = dataset._determine_required_keys_via_handler_enhanced(
            self.dataset_config, self.processing_config
        )
...
class TestMoleculeProcessing(BaseTestCase):
    ...
    def test_conversion_error_handling(self):
        ...
        self.assertIn(
            "handler_processing_errors", dataset._processing_statistics.get("error_statistics", {})
        )
...
class TestFactoryMethods(BaseTestCase):
    ...
    def test_create_with_containers_accepts_experimental_setup(self):
        ...
        with patch(
            "milia_pipeline.datasets.milia_dataset.get_experimental_setup"
        ) as mock_get_setup:
            mock_setup = Mock()
