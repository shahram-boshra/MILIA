...
class ValidatingMockDFTHandler(ValidatingMockHandler):
    ...
    def __init__(self, should_validate=True, strict_validation=True):
        super().__init__(dataset_type='DFT', should_validate=should_validate, strict_validation=strict_validation)
...
class ValidatingMockDMCHandler(ValidatingMockHandler):
    ...
    def __init__(self, should_validate=True, strict_validation=True, uncertainty_enabled=False):
        super().__init__(dataset_type='DMC', should_validate=should_validate, strict_validation=strict_validation)
        self.dataset_config.is_uncertainty_enabled = uncertainty_enabled
        if uncertainty_enabled:
            self.dataset_config.uncertainty_config = {
                'uncertainty_field_name': 'std',
...
class TestDatasetRequirements(unittest.TestCase):
    ...
    def test_get_requirements_creates_handler_when_none(self):
        ...
        with patch('milia_pipeline.molecules.molecule_validator.create_dataset_config_from_global') as mock_dataset_config, \
             patch('milia_pipeline.molecules.molecule_validator.create_filter_config_from_global') as mock_filter_config, \
             patch('milia_pipeline.molecules.molecule_validator.create_processing_config_from_global') as mock_proc_config, \
             patch('milia_pipeline.molecules.molecule_validator.create_dataset_handler') as mock_create_handler:
...
class TestValidatorCreation(unittest.TestCase):
    ...
    def test_create_validator_with_explicit_config(self):
        ...
        with patch('milia_pipeline.molecules.molecule_validator.create_filter_config_from_global') as mock_filter_config, \
             patch('milia_pipeline.molecules.molecule_validator.create_processing_config_from_global') as mock_proc_config, \
             patch('milia_pipeline.molecules.molecule_validator.create_dataset_handler') as mock_create_handler:
...
class TestLegacyValidation(unittest.TestCase):
    ...
    def test_legacy_validation_success(self, mock_create_handler,
                                      mock_proc_config, mock_filter_config, 
                                      mock_dataset_config, mock_get_dataset_type):
...
class TestPhase6RefactoredFunctions(unittest.TestCase):
    ...
    def test_02_validate_structure_dmc_creates_dataset_specific_error_on_failure(self):
        ...
        with self.assertRaises((DatasetSpecificHandlerError, HandlerError, MoleculeProcessingError)):
            validate_molecular_structure(
                atoms=atoms,
                coordinates=coordinates,
                molecule_index=self.molecule_index,
    ...
    def test_08_get_requirements_uses_feature_based_categorization(self):
        ...
        # DFT requirements
        dft_reqs = get_dataset_requirements(
            dataset_type='DFT',
            handler=self.dft_handler
        )
        ...
        # DMC requirements
        dmc_reqs = get_dataset_requirements(
            dataset_type='DMC',
            handler=self.dmc_handler
        )
