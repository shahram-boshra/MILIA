...
class MockHPOConfig:
    ...
    def __init__(
        ...
        pruner: Optional[MockPrunerConfig] = None,
        sampler: Optional[MockSamplerConfig] = None,
        study: Optional[MockStudyConfig] = None,
...
def mock_backend():
    ...
    backend.get_all_trials.return_value = [
        {'number': 0, 'params': {'lr': 0.001}, 'value': 0.05, 'state': 'COMPLETE', 'duration': 60.0},
...
class TestHPOManagerInit:
    ...
    def test_init_with_enabled_config(self, mock_backend):
        ...
        with patch('milia_pipeline.models.hpo.hpo_manager.HPOConfig', MockHPOConfig):
            with patch('milia_pipeline.models.hpo.hpo_manager.get_backend', return_value=mock_backend):
    ...
    def test_init_enabled_calls_get_backend(self, mock_backend):
        ...
        with patch('milia_pipeline.models.hpo.hpo_manager.HPOConfig', MockHPOConfig):
            with patch('milia_pipeline.models.hpo.hpo_manager.get_backend', return_value=mock_backend) as mock_get_backend:
                from milia_pipeline.models.hpo.hpo_manager import HPOManager
...
class TestHPOManagerOptimize:
    ...
    def test_optimize_creates_pruner(self, mock_backend, mock_dataset):
        ...
        with patch('milia_pipeline.models.hpo.hpo_manager.HPOConfig', MockHPOConfig):
            with patch('milia_pipeline.models.hpo.hpo_manager.get_backend', return_value=mock_backend):
                with patch('milia_pipeline.models.hpo.hpo_manager.get_factory', return_value=MagicMock()):
                    from milia_pipeline.models.hpo.hpo_manager import HPOManager
...
class TestHPOManagerCreateObjective:
    ...
    def test_objective_suggests_params(self, mock_backend, mock_dataset, mock_trial, mock_trainer):
        ...
        with patch('milia_pipeline.models.hpo.hpo_manager.HPOConfig', MockHPOConfig):
            with patch('milia_pipeline.models.hpo.hpo_manager.get_backend', return_value=mock_backend):
                with patch('milia_pipeline.models.hpo.hpo_manager.get_factory', return_value=MagicMock()):
                    with patch('milia_pipeline.models.hpo.hpo_manager.Trainer', return_value=mock_trainer):
                        with patch('milia_pipeline.models.hpo.hpo_manager.create_hpo_callback', return_value=MagicMock()):
                            from milia_pipeline.models.hpo.hpo_manager import HPOManager
...
class TestHPOManagerGetBestTrial:
    ...
    def test_get_best_trial_success(self, mock_backend, mock_study):
        ...
        mock_backend.get_all_trials.return_value = [
            {'number': 0, 'params': {'lr': 0.01}, 'value': 0.1, 'state': 'COMPLETE', 'duration': 60},
            {'number': 1, 'params': {'lr': 0.001}, 'value': 0.05, 'state': 'COMPLETE', 'duration': 70},
        ]
        mock_backend.get_best_value.return_value = 0.05
...
class TestHPOManagerGetAllTrials:
    ...
    def test_get_all_trials_success(self, mock_backend, mock_study):
        ...
        expected_trials = [
            {'number': 0, 'params': {'lr': 0.001}, 'value': 0.05, 'state': 'COMPLETE', 'duration': 60},
...
class TestHPOManagerGetStudyStatistics:
    ...
    def test_get_study_statistics_success(self, mock_backend, mock_study):
        ...
        mock_backend.get_all_trials.return_value = [
            {'number': 0, 'params': {'lr': 0.001}, 'value': 0.05, 'state': 'COMPLETE', 'duration': 60.0},
            {'number': 1, 'params': {'lr': 0.01}, 'value': 0.1, 'state': 'COMPLETE', 'duration': 70.0},
            {'number': 2, 'params': {'lr': 0.1}, 'value': None, 'state': 'PRUNED', 'duration': 30.0},
    ...
    def test_save_results_json_content(self, mock_backend, mock_study, tmp_path):
        ...
        with patch('milia_pipeline.models.hpo.hpo_manager.HPOConfig', MockHPOConfig):
            with patch('milia_pipeline.models.hpo.hpo_manager.get_backend', return_value=mock_backend):
                ...
                with open(saved_paths['best_params_path'], 'r') as f:
                    best_params = json.load(f)
                assert best_params == {'lr': 0.001, 'hidden_channels': 128}
                
                # Verify trials content
                with open(saved_paths['trials_path'], 'r') as f:
                    trials = json.load(f)
                assert len(trials) == 1
...
class TestHPOManagerTrainFinalModel:
    ...
    def test_train_final_model_no_best_params_raises(self, mock_backend):
        ...
        with patch('milia_pipeline.models.hpo.hpo_manager.HPOConfig', MockHPOConfig):
            with patch('milia_pipeline.models.hpo.hpo_manager.get_backend', return_value=mock_backend):
                ...
                assert "best" in str(exc_info.value).lower() or "optimize" in str(exc_info.value).lower()
    ...
    def test_train_final_model_no_datasplitter_raises(self, mock_backend):
        ...
        with patch('milia_pipeline.models.hpo.hpo_manager.HPOConfig', MockHPOConfig):
            with patch('milia_pipeline.models.hpo.hpo_manager.get_backend', return_value=mock_backend):
                with patch('milia_pipeline.models.hpo.hpo_manager.Trainer', mock_trainer):
                    with patch('milia_pipeline.models.hpo.hpo_manager.DataSplitter', None):
                        ...
                        assert "datasplitter" in str(exc_info.value).lower() or "split" in str(exc_info.value).lower()
            ...
            with patch('milia_pipeline.models.hpo.hpo_manager.get_backend', return_value=mock_backend):
                with patch('milia_pipeline.models.hpo.hpo_manager.Trainer', mock_trainer):
                    with patch('milia_pipeline.models.hpo.hpo_manager.DataSplitter', mock_datasplitter):
                        with patch('milia_pipeline.models.hpo.hpo_manager.get_factory', None):
                            from milia_pipeline.models.hpo.hpo_manager import HPOManager
    ...
    def test_train_final_model_accepts_config_dict(self, mock_backend, mock_dataset):
        ...
        mock_factory.create_model_with_info.return_value = (real_model, {'uses_edge_features': False})
        ...
        with patch('milia_pipeline.models.hpo.hpo_manager.HPOConfig', MockHPOConfig):
            with patch('milia_pipeline.models.hpo.hpo_manager.get_backend', return_value=mock_backend):
                with patch('milia_pipeline.models.hpo.hpo_manager.Trainer', mock_trainer_class):
                    with patch('milia_pipeline.models.hpo.hpo_manager.DataSplitter', mock_datasplitter):
                        with patch('milia_pipeline.models.hpo.hpo_manager.get_factory', mock_get_factory):
                            with patch('torch_geometric.loader.DataLoader', return_value=mock_dataloader):
                                from milia_pipeline.models.hpo.hpo_manager import HPOManager
                                
                                config = MockHPOConfig(enabled=True)
                                manager = HPOManager(config)
                                manager.best_params = {'hyperparameters.hidden_channels': 64, 'optimizer.lr': 0.001}
...
class TestSearchSpaceFiltering:
    ...
    def setup_mock_registry(self):
        ...
        MockModelRegistry.register_mock_model("GCN", {
        ...
        MockModelRegistry.register_mock_model("GAT", {
            "in_channels": {"type": "integer", "required": True},
            "hidden_channels": {"type": "integer", "default": 64},
            "num_layers": {"type": "integer", "default": 2},
            "out_channels": {"type": "integer", "required": True},
    ...
    def sample_search_space_with_heads(self):
        ...
        return {
            "hyperparameters": {
                ...
                "num_layers": MockSearchSpaceParamConfig(
                    type=MockParamType.INT, low=2, high=6
                ),
                "dropout": MockSearchSpaceParamConfig(
                    type=MockParamType.FLOAT, low=0.0, high=0.6
                ),
        ...
        with patch('milia_pipeline.models.hpo.hpo_manager.HPOConfig', MockHPOConfig):
            with patch('milia_pipeline.models.hpo.hpo_manager.get_backend', return_value=mock_backend):
                with patch('milia_pipeline.models.hpo.hpo_manager._REGISTRY_AVAILABLE', True):
                    with patch('milia_pipeline.models.hpo.hpo_manager.ModelRegistry', MockModelRegistry):
                        from milia_pipeline.models.hpo.hpo_manager import HPOManager
            ...
            with patch('milia_pipeline.models.hpo.hpo_manager.get_backend', return_value=mock_backend):
                with patch('milia_pipeline.models.hpo.hpo_manager._REGISTRY_AVAILABLE', True):
                    with patch('milia_pipeline.models.hpo.hpo_manager.ModelRegistry', MockModelRegistry):
                        ...
                        filtered = manager._filter_search_space_for_model(
                            "GAT", sample_search_space_with_heads
                        )
            ...
            with patch('milia_pipeline.models.hpo.hpo_manager.get_backend', return_value=mock_backend):
                with patch('milia_pipeline.models.hpo.hpo_manager._REGISTRY_AVAILABLE', True):
                    with patch('milia_pipeline.models.hpo.hpo_manager.ModelRegistry', MockModelRegistry):
                        ...
                        assert "lr" in filtered.get("optimizer", {}), \
                            "Optimizer 'lr' should be preserved"
    ...
    def test_registry_unavailable_fallback(self, mock_backend, sample_search_space_with_heads):
        ...
        with patch('milia_pipeline.models.hpo.hpo_manager.HPOConfig', MockHPOConfig):
            with patch('milia_pipeline.models.hpo.hpo_manager.get_backend', return_value=mock_backend):
                with patch('milia_pipeline.models.hpo.hpo_manager._REGISTRY_AVAILABLE', False):
                    with patch('milia_pipeline.models.hpo.hpo_manager.logger') as mock_logger:
                        from milia_pipeline.models.hpo.hpo_manager import HPOManager
            ...
            with patch('milia_pipeline.models.hpo.hpo_manager.get_backend', return_value=mock_backend):
                with patch('milia_pipeline.models.hpo.hpo_manager._REGISTRY_AVAILABLE', True):
                    with patch('milia_pipeline.models.hpo.hpo_manager.ModelRegistry', MockModelRegistry):
                        ...
                        # Call filter
                        _ = manager._filter_search_space_for_model(
                            "GCN", sample_search_space_with_heads
                        )
                        
                        # Verify original not mutated
                        assert set(sample_search_space_with_heads["hyperparameters"].keys()) == original_hp_keys, \
    ...
    def test_loss_params_filtered_for_task_type(self, mock_backend):
        ...
        mock_loss_registry.get_valid_params.return_value = {'reduction': {}}  # MSE only accepts reduction
        
        with patch('milia_pipeline.models.hpo.hpo_manager.HPOConfig', MockHPOConfig):
            with patch('milia_pipeline.models.hpo.hpo_manager.get_backend', return_value=mock_backend):
                with patch('milia_pipeline.models.hpo.hpo_manager._REGISTRY_AVAILABLE', True):
                    with patch('milia_pipeline.models.hpo.hpo_manager.ModelRegistry', MockModelRegistry):
                        with patch('milia_pipeline.models.hpo.hpo_manager.LossRegistry', mock_loss_registry):
                            from milia_pipeline.models.hpo.hpo_manager import HPOManager
                            ...
                            assert "reduction" in filtered.get("loss", {}), \
                                "'reduction' is valid for MSE loss"
    ...
    def test_loss_params_preserved_when_no_task_type(self, mock_backend):
        ...
        search_space_with_loss = {
            ...
            "loss": {
                "alpha": MockSearchSpaceParamConfig(
                    type=MockParamType.FLOAT, low=0.0, high=1.0
                ),
            },
        }
    ...
    def test_loss_filtering_logs_removed_params(self, mock_backend):
        ...
        mock_loss_registry = MagicMock()
        mock_loss_registry.get_valid_params.return_value = {'reduction': {}}  # Only 'reduction' is valid
                ...
                with patch('milia_pipeline.models.hpo.hpo_manager._REGISTRY_AVAILABLE', True):
                    with patch('milia_pipeline.models.hpo.hpo_manager.ModelRegistry', MockModelRegistry):
                        ...
                        with patch.object(
                            ...
                            import importlib
                            import milia_pipeline.models.hpo.hpo_manager as hpo_mod
                ...
                with patch('milia_pipeline.models.hpo.hpo_manager._REGISTRY_AVAILABLE', True):
                    with patch('milia_pipeline.models.hpo.hpo_manager.ModelRegistry', MockModelRegistry):
                        with patch('milia_pipeline.models.hpo.hpo_manager.logger') as mock_logger:
                            ...
                            assert "heads" in info_msg.lower() or "removed" in info_msg.lower() or "filter" in info_msg.lower()
    
    def test_model_with_no_metadata_returns_unfiltered(self, mock_backend, sample_search_space_with_heads):
        ...
        with patch('milia_pipeline.models.hpo.hpo_manager.HPOConfig', MockHPOConfig):
            with patch('milia_pipeline.models.hpo.hpo_manager.get_backend', return_value=mock_backend):
                with patch('milia_pipeline.models.hpo.hpo_manager._REGISTRY_AVAILABLE', True):
                    with patch('milia_pipeline.models.hpo.hpo_manager.ModelRegistry', MockRegistryNoMetadata):
                        with patch('milia_pipeline.models.hpo.hpo_manager.logger') as mock_logger:
                            from milia_pipeline.models.hpo.hpo_manager import HPOManager
                ...
                with patch('milia_pipeline.models.hpo.hpo_manager.get_factory', return_value=MagicMock()):
                    with patch('milia_pipeline.models.hpo.hpo_manager._REGISTRY_AVAILABLE', True):
                        with patch('milia_pipeline.models.hpo.hpo_manager.ModelRegistry', MockModelRegistry):
                            ...
                            config = MockHPOConfig(
                                enabled=True,
                                n_trials=10,
                                search_space=search_space_with_heads
                            )
                            manager = HPOManager(config)
...
class TestPrepareDataForTaskHpo:
    ...
    def test_accepts_target_selection_config_parameter(self):
        ...
        result_train, result_val, num_classes = _prepare_data_for_task_hpo(
            train_data, val_data, 'node_regression', None, None  # discretize_config, target_selection_config
        )
    ...
    def test_resolves_target_selection_config_for_task(self):
        ...
        try:
            from milia_pipeline.models.factory.target_selection_config import TargetSelectionConfig, TargetLevel
            config = TargetSelectionConfig.from_config({
                'target_level': 'auto',
                'target_source': 'auto',
                'strict': True
...
class TestExtractTargetsFromSource:
    ...
    def test_handles_1d_tensor(self):
        ...
        result = _extract_targets_from_source(
            data_subset, 'custom_attr', [0, 2], 'y', 'train'
        )
...
class TestEdgeCasesComprehensive:
    ...
    def test_get_best_trial_with_duplicate_values(self, mock_backend, mock_study):
        ...
        mock_backend.get_all_trials.return_value = [
            {'number': 0, 'params': {'lr': 0.01}, 'value': 0.05, 'state': 'COMPLETE', 'duration': 60},
            {'number': 1, 'params': {'lr': 0.001}, 'value': 0.05, 'state': 'COMPLETE', 'duration': 70},  # Same value
        ]
        mock_backend.get_best_value.return_value = 0.05
    ...
    def test_prepare_data_calls_resolve_for_task(self):
        ...
        with patch('milia_pipeline.models.hpo.hpo_manager._TARGET_SELECTION_AVAILABLE', True):
            _prepare_data_for_task_hpo(
                train_data, val_data, 'graph_regression', None, mock_config
            )
