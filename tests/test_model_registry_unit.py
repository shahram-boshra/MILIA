...
class TestModelRegistrationPydanticModel:
    ...
    def test_all_fields_present(self):
        ...
        expected_fields = {
            'name', 'model_class', 'metadata', 'is_builtin', 
            'plugin_name', 'registered_at'
        }
...
class TestAutoDiscovery:
    ...
    def test_discovery_handles_none_metadata(self, isolated_registry):
        ...
        isolated_registry._discovery_stats = {
            'total_attempted': 0, 'successful': 0, 'failed': 0, 'last_discovery': None
        }
...
class TestModelRegistration:
    ...
    def test_register_with_plugin_name(
    ...
    def test_register_duplicate_without_force(
        ...
        result = fresh_registry.register_model(
            "Duplicate", mock_torch_module, sample_metadata
        )
    ...
    def test_register_updates_category(
        self, fresh_registry, mock_torch_module, sample_metadata
    ):
...
class TestModelUnregistration:
    ...
    def test_unregister_existing_model(
        self, fresh_registry, mock_torch_module, sample_metadata
    ):
...
class TestListAndFilter:
    ...
    def test_list_available_models_by_category(
        self, fresh_registry, mock_torch_module
    ):
        ...
        basic_models = fresh_registry.list_available_models(
            category=ModelCategory.BASIC_GNN
        )
        assert "Model1" in basic_models
        assert "Model2" not in basic_models
    
    def test_list_available_models_by_task(
        self, fresh_registry, mock_torch_module
    ):
        ...
        node_models = fresh_registry.list_available_models(
            task_type="node_classification"
        )
        assert "TaskModel" in node_models
        
        link_models = fresh_registry.list_available_models(
            task_type="link_prediction"
        )
        assert "TaskModel" not in link_models
    
    def test_list_available_models_by_heterogeneous(
        self, fresh_registry, mock_torch_module
    ):
        ...
        hetero_models = fresh_registry.list_available_models(
            supports_heterogeneous=True
        )
        assert "HeteroModel" in hetero_models
        assert "HomoModel" not in hetero_models
    
    def test_list_available_models_by_tags(
        self, fresh_registry, mock_torch_module
    ):
        ...
        both_tags = fresh_registry.list_available_models(
            tags=["attention", "spectral"]
        )
...
class TestPluginManagement:
    ...
    def test_builtin_and_custom_disjoint(
        self, fresh_registry, mock_torch_module, sample_metadata
    ):
...
class TestStatisticsAndReporting:
    ...
    def test_get_statistics_counts(
        self, fresh_registry, mock_torch_module, sample_metadata
    ):
    ...
    def test_get_availability_report_success_rate(self, fresh_registry):
        ...
        if report['total_attempted'] > 0:
            expected_rate = (
                report['total_registered'] / report['total_attempted'] * 100
            )
...
class TestGlobalInstanceAndConvenienceFunctions:
    ...
    def test_get_model_info_convenience_function(
        self, mock_torch_module, sample_metadata
    ):
    ...
    def test_get_model_info_structure(self, mock_torch_module, sample_metadata):
        ...
        expected_keys = {
            'name', 'class', 'description', 'category', 'supported_tasks',
            'is_builtin', 'plugin_name', 'paper_url', 'tags',
            'requires_edge_features', 'requires_edge_weights',
            'supports_heterogeneous', 'registered_at'
        }
...
class TestErrorHandling:
    ...
    def test_register_with_invalid_metadata_type(
        self, fresh_registry, mock_torch_module
    ):
    ...
    def test_register_internal_with_lock(
        self, fresh_registry, mock_torch_module, sample_metadata
    ):
