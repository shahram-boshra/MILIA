...
class TestRegistryIntegration:
        ...
        with patch('milia_pipeline.datasets.registry.list_all') as mock_list_all:
            with patch('milia_pipeline.datasets.registry.get') as mock_get:
                with patch('milia_pipeline.datasets.registry.is_registered') as mock_is_registered:
                    with patch('milia_pipeline.datasets.registry.get_default_registry') as mock_get_default_registry:
    ...
    def test_registry_get_not_available(self):
        ...
        with patch.object(accessors_module, '_registry_get_safe', return_value=None):
            with patch('milia_pipeline.config.config_accessors._registry_list_all_safe', return_value=['DFT', 'DMC']):
    ...
    def test_is_valid_dataset_type_true(self):
        ...
        with patch('milia_pipeline.config.config_accessors._registry_is_registered_safe', return_value=True):
    ...
    def test_is_valid_dataset_type_false(self):
        ...
        with patch('milia_pipeline.config.config_accessors._registry_is_registered_safe', return_value=False):
...
class TestDatasetTypeAccessors:
    ...
    def test_get_dataset_type_success(self, mock_load_config):
        ...
        with patch('milia_pipeline.config.config_accessors._registry_list_all_safe', return_value=['DFT', 'DMC', 'Wavefunction']):
    ...
    def test_get_dataset_type_not_registered(self, mock_load_config):
        ...
        with patch('milia_pipeline.config.config_accessors._registry_list_all_safe', return_value=['DMC', 'Wavefunction']):
    ...
    def test_validate_dataset_type_invalid_raises(self):
        ...
        with patch('milia_pipeline.config.config_accessors.registry_is_registered', return_value=False):
            with patch('milia_pipeline.config.config_accessors.registry_list_all', return_value=['DFT']):
                with pytest.raises(ConfigurationError) as exc_info:
...
class TestHandlerAccessors:
    ...
    def test_get_handler_type_with_config(self):
        ...
        with patch('milia_pipeline.config.config_accessors.get_dataset_config', return_value={'handler_type': 'DMC'}):
    ...
    def test_get_handler_type_not_registered(self, mock_load_config):
        ...
        with patch('milia_pipeline.config.config_accessors.get_dataset_config', side_effect=ConfigurationError('Not found', config_key='test')):
            with pytest.raises(ConfigurationError):
    ...
    def test_is_handler_type_handler_not_available(self, mock_load_config):
        ...
        with patch('milia_pipeline.config.config_accessors._registry_is_registered_safe', return_value=True):
            ...
            with patch('milia_pipeline.config.config_accessors.get_handler_type', side_effect=HandlerNotAvailableError('Error', requested_dataset_type='DFT')):
    ...
    def test_get_required_properties_success(self, mock_registry_class):
        ...
        with patch('milia_pipeline.config.config_accessors._registry_is_registered_safe', return_value=True):
            with patch('milia_pipeline.config.config_accessors.registry_get', return_value=mock_registry_class):
...
class TestFeatureAccessors:
    ...
    def test_get_supported_features_with_handler_type(self, mock_registry_class):
        ...
        with patch('milia_pipeline.config.config_accessors._registry_is_registered_safe', return_value=True):
            with patch('milia_pipeline.config.config_accessors.registry_get', return_value=mock_registry_class):
                with patch('milia_pipeline.config.config_accessors.get_supported_features', side_effect=HandlerNotAvailableError("Error", requested_dataset_type='DFT')):
                    result = get_supported_features('DFT')
    ...
    def test_get_dataset_appropriate_structural_features_no_handler_method(self, mock_registry_class):
    ...
    def test_validate_structural_features_for_dataset_valid(self, mock_registry_class):
        ...
        with patch('milia_pipeline.config.config_accessors._registry_is_registered_safe', return_value=True):
            with patch('milia_pipeline.config.config_accessors.registry_get', return_value=mock_registry_class):
                with patch('milia_pipeline.config.config_accessors._is_valid_dataset_type', return_value=True):
    ...
    def test_validate_structural_features_for_dataset_not_registered(self):
        ...
        with patch('milia_pipeline.config.config_accessors._is_valid_dataset_type', return_value=False):
...
class TestMoleculeCreationAccessors:
    ...
    def test_get_molecule_creation_strategy_success(self, mock_load_config):
        ...
        assert result in ['direct', 'identifier_coordinate_based']  # Accept actual implementation default
...
class TestUtilityFunctions:
    ...
    def test_config_accessors_with_no_config_file(self):
        ...
        with patch('milia_pipeline.config.config_accessors.load_config', side_effect=FileNotFoundError("No config")):
...
class TestTransformationConfiguration:
    ...
    def test_list_experimental_setups_error(self):
        ...
        with patch('milia_pipeline.config.config_accessors.load_config', side_effect=Exception("Error")):
            result = list_experimental_setups()
            assert isinstance(result, list)
    ...
    def test_get_available_transforms_import_error(self):
        ...
        with patch('milia_pipeline.config.config_accessors.get_graph_transforms', return_value=None):
            result = get_available_transforms()
            assert isinstance(result, list)
...
class TestDescriptorConfiguration:
    ...
    def test_get_selected_descriptors_disabled(self):
        ...
        with patch('milia_pipeline.config.config_accessors.get_descriptor_config', return_value={'enabled': False}):
            result = get_selected_descriptors()
            assert result == []
    
    def test_get_selected_descriptors_no_categories(self):
        """Test get_selected_descriptors with no categories."""
        with patch('milia_pipeline.config.config_accessors.get_descriptor_config', return_value={'enabled': True, 'default_categories': []}):
            result = get_selected_descriptors()
            assert result == []
    ...
    def test_get_selected_descriptors_import_error(self):
        ...
        with patch('milia_pipeline.descriptors.descriptor_registry.DescriptorRegistry', side_effect=ImportError("Module not found")):
            result = get_selected_descriptors()
            assert isinstance(result, list)
    ...
    def test_get_selected_descriptors_unknown_category(self, mock_load_config):
        ...
        with patch('milia_pipeline.config.config_accessors.get_descriptor_config', 
...
class TestTransformationConfigurationExtended:
    ...
    def test_get_default_experimental_setup_not_specified(self):
        ...
        with patch('milia_pipeline.config.config_accessors.load_config', return_value={'transformations': {}}):
            result = get_default_experimental_setup()
            assert result is not None  # Returns ExperimentalSetup object
...
class TestDescriptorConfigurationExtended:
    ...
    def test_is_descriptors_enabled_with_partial_config(self):
        ...
        with patch('milia_pipeline.config.config_accessors.load_config', return_value={'descriptors': {}}):
            result = is_descriptors_enabled()
            assert isinstance(result, bool)
    ...
    def test_get_selected_descriptors_mixed_categories(self, mock_load_config):
        ...
        with patch('milia_pipeline.config.config_accessors.get_descriptor_config',
                   return_value={'enabled': True, 'default_categories': ['molecular', 'invalid'], 
    ...
    def test_get_selected_descriptors_registry_unavailable(self):
        ...
        with patch('milia_pipeline.descriptors.descriptor_registry.DescriptorRegistry', side_effect=Exception("Error")):
            result = get_selected_descriptors()
            assert isinstance(result, list)
    ...
    def test_descriptor_config_missing_nested_keys(self):
        ...
        with patch('milia_pipeline.config.config_accessors.load_config', 
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_malformed_config_handling(self):
        ...
        with patch('milia_pipeline.config.config_accessors.load_config', return_value={'invalid': 'structure'}):
            try:
    ...
    def test_error_messages_clarity(self):
        ...
        with patch('milia_pipeline.config.config_accessors.load_config', side_effect=FileNotFoundError("Config not found")):
            try:
                get_dataset_type()
    ...
    def test_fallback_chain_completeness(self, mock_registry_class):
        ...
        with patch('milia_pipeline.config.config_accessors.registry_get', side_effect=Exception("Error")):
...
class TestPerformanceAndOptimization:
    ...
    def test_registry_initialization_performance(self):
        ...
        import time
        start = time.time()
        with patch('milia_pipeline.config.config_accessors.get_default_registry', return_value=Mock()):
            _init_registry()
    ...
    def test_large_property_list_handling(self, mock_registry_class):
        ...
        mock_registry_class.get_required_properties.return_value = ['prop' + str(i) for i in range(1000)]
        with patch('milia_pipeline.config.config_accessors._registry_is_registered_safe', return_value=True):
            with patch('milia_pipeline.config.config_accessors.registry_get', return_value=mock_registry_class):
                result = get_required_properties('DFT')
...
class TestLoggingAndMonitoring:
    ...
    def test_error_logging(self):
        ...
        with patch('milia_pipeline.config.config_accessors.load_config', side_effect=Exception("Test error")):
            try:
                get_dataset_type()
            except:
...
class TestStandardTransformsAccessors:
    ...
    def test_get_standard_transforms_returns_list(self, mock_transformation_config_with_standard):
        ...
        with patch('milia_pipeline.config.config_accessors.get_transformation_config', 
                   return_value=mock_transformation_config_with_standard):
            result = get_standard_transforms()
            assert isinstance(result, list)
    
    def test_get_standard_transforms_returns_transform_specs(self, mock_transformation_config_with_standard):
    ...
    def test_get_standard_transforms_returns_enabled_only(self, mock_transformation_config_with_standard):
    ...
    def test_get_standard_transforms_preserves_order(self, mock_transformation_config_with_standard):
    ...
    def test_get_standard_transforms_as_dicts_returns_list(self, mock_transformation_config_with_standard):
    ...
    def test_get_standard_transforms_as_dicts_returns_dicts(self, mock_transformation_config_with_standard):
    ...
    def test_get_standard_transforms_as_dicts_has_required_keys(self, mock_transformation_config_with_standard):
    ...
    def test_get_standard_transforms_as_dicts_correct_values(self, mock_transformation_config_with_standard):
    ...
    def test_get_standard_transforms_as_dicts_empty_when_none(self, mock_transformation_config_no_standard):
    ...
    def test_get_combined_transforms_returns_transform_specs(self, mock_transformation_config_with_standard):
    ...
    def test_get_combined_transforms_uses_default_setup(self, mock_transformation_config_with_standard):
    ...
    def test_get_combined_transforms_specific_setup(self):
        ...
        setup1 = ExperimentalSetup(name='setup1', transforms=[
            TransformSpec(name='Setup1Transform', enabled=True)
        ])
        setup2 = ExperimentalSetup(name='setup2', transforms=[
            TransformSpec(name='Setup2Transform', enabled=True)
        ])
    ...
    def test_get_combined_transforms_as_dicts_returns_list(self, mock_transformation_config_with_standard):
    ...
    def test_get_combined_transforms_as_dicts_returns_dicts(self, mock_transformation_config_with_standard):
    ...
    def test_get_combined_transforms_as_dicts_has_required_keys(self, mock_transformation_config_with_standard):
    ...
    def test_get_combined_transforms_as_dicts_correct_order(self, mock_transformation_config_with_standard):
    ...
    def test_has_standard_transforms_false_all_disabled(self):
        ...
        setup = ExperimentalSetup(name='test', transforms=[
            TransformSpec(name='Exp', enabled=True)
        ])
        config = TransformationConfig(
...
class TestStructuralFeaturesAccessors:
    ...
    def test_is_structural_features_enabled_true(self, mock_load_config):
        ...
        with patch('milia_pipeline.config.config_accessors.get_structural_features_config',
    ...
    def test_should_pass_coordinates_to_structural_features(self, mock_load_config):
        ...
        with patch('milia_pipeline.config.config_accessors.get_data_config',
    ...
    def test_should_enable_stereochemistry_preprocessing(self, mock_load_config):
        ...
        # Mock get_data_config since the mock_load_config doesn't have data_config key
        with patch('milia_pipeline.config.config_accessors.get_data_config',
                   return_value={'structural_feature_integration': {'enable_stereochemistry_preprocessing': True}}):
    ...
    def test_get_feature_compatibility_report_structure(self, mock_load_config):
        ...
        with patch('milia_pipeline.config.config_accessors.get_structural_features_config',
                   return_value={'atom': [], 'bond': []}):
            with patch('milia_pipeline.config.config_accessors.create_dataset_config_container'):
                with patch('milia_pipeline.config.config_accessors.create_filter_config_container'):
                    with patch('milia_pipeline.config.config_accessors.create_processing_config_container'):
...
class TestConfigValueAccessors:
    ...
    def test_get_data_config_returns_dict(self, mock_load_config):
        ...
        mock_config_with_data = {
            ...
            'data_config': {
                'property_selection': {
                    'DFT': {'scalar_graph_targets_to_include': ['Etot']}
                },
                'common_settings': {}
            }
        }
        with patch('milia_pipeline.config.config_accessors.load_config', return_value=mock_config_with_data):
