                ...
                with patch('torch.cuda.memory_allocated', return_value=1024**3):
                    with patch('torch.cuda.memory_reserved', return_value=2 * 1024**3):
                        with patch('torch.cuda.max_memory_allocated', return_value=1.5 * 1024**3):
                            with patch('torch.cuda.max_memory_reserved', return_value=2.5 * 1024**3):
...
class TestMemoryConfig:
    ...
    def test_to_dict_includes_all_fields(self):
        ...
        expected_keys = [
            'mixed_precision', 'precision', 'gradient_checkpointing',
            'pin_memory', 'non_blocking', 'empty_cache_interval',
            'garbage_collect_interval', 'max_memory_allocated', 'growth_interval'
        ]
...
class TestMemoryOptimizerInitialization:
    ...
    def test_silent_initialization(self, caplog):
        ...
        init_logs = [r for r in caplog.records 
                     if "MemoryOptimizer initialized" in r.message]
...
class TestEstimateModelMemory:
    ...
    def test_estimate_memory_batch_size_scaling(self, simple_model):
        ...
        estimates_batch_1 = estimate_model_memory(
            simple_model,
            input_size=(10,),
            batch_size=1
        )
        
        estimates_batch_8 = estimate_model_memory(
            simple_model,
            input_size=(10,),
            batch_size=8
        )
...
class TestLogging:
    ...
    def test_optimizer_init_logs_on_verbose(self, caplog):
        ...
        assert any("MemoryOptimizer initialized" in record.message 
    ...
    def test_gradient_checkpointing_logs(self, caplog, simple_model):
        ...
        assert any("gradient checkpointing" in record.message.lower() 
                   for record in caplog.records)
