...
def clear_plugin_loader():
    ...
    from milia_pipeline.descriptors.descriptor_plugin_system import DescriptorPluginLoader
...
class TestDescriptorPluginIntegration:
    ...
    def test_example_plugin_descriptors(self):
        ...
        assert registry.has_descriptor("HeteroatomRatio"), "HeteroatomRatio descriptor not registered"
...
""")
    ...
    def test_plugin_enable_disable(self):
        ...
        assert plugin_loader.is_enabled("example_descriptors"), "Plugin should be enabled after enable_plugin call"
    ...
    def test_plugin_info_retrieval(self):
        ...
        assert info["plugin_name"] == "example_descriptors", f"Expected plugin_name='example_descriptors', got {info['plugin_name']}"
        assert "version" in info, "Plugin info should contain version"
        assert "author" in info, "Plugin info should contain author"
        assert "descriptor_declarations" in info, "Plugin info should contain descriptor_declarations"
...
""")
        ...
        if registry.has_descriptor("CollisionDesc"):
            ...
            assert value in [0.0, 1.0], f"Expected collision resolution to give 0.0 or 1.0, got {value}"
