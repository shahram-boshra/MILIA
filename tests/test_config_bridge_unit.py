...
class TestCallbackConfig:
    ...
    def test_custom_callback(self):
        ...
        config = CallbackConfig(
            enabled=True,
            params={"monitor": "val_loss", "patience": 10}
        )
...
class TestCallbacksConfig:
    ...
    def test_custom_callbacks(self):
        ...
        config = CallbacksConfig(
            ...
            model_checkpoint=CallbackConfig(
                enabled=True,
                params={"save_top_k": 1}
            )
        )
...
class TestAccelerationConfig:
    ...
    def test_validation_propagates_device_errors(self):
        ...
        with pytest.raises(PydanticValidationError):
            AccelerationConfig(
                enabled=True,
                device=DeviceConfig(type="invalid")
            )
...
class TestModelConfig:
    ...
    def test_is_phase_enabled_acceleration(self):
        ...
        config_disabled = ModelConfig(
            enabled=True,
            acceleration=AccelerationConfig(enabled=False)
        )
    ...
    def test_is_phase_enabled_hpo(self):
        ...
        config = ModelConfig(
            enabled=True,
            hpo=HPOConfigBridge(enabled=True)
        )
        assert config.is_phase_enabled("hpo") is True
        
        config_disabled = ModelConfig(
            enabled=True,
            hpo=HPOConfigBridge(enabled=False)
        )
...
class TestEdgeCases:
    ...
    def test_data_split_exact_sum(self):
        ...
        config = DataSplitConfig(
            train_ratio=0.333333333333,
            val_ratio=0.333333333333,
            test_ratio=0.333333333334
        )
    ...
    def test_acceleration_validation_disabled_ignores_invalid_device(self):
        ...
        with pytest.raises(PydanticValidationError, match="Invalid device type"):
            AccelerationConfig(
                enabled=False,
                device=DeviceConfig(type="invalid_device")
            )
