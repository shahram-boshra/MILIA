...
class TestModuleInitialization:
    ...
    def test_module_version_format(self):
        ...
        assert len(version_parts) >= 3, f"Version '{__version__}' should have at least MAJOR.MINOR.PATCH"
...
class TestPublicAPIExports:
    ...
    EXPECTED_EXPORT_CATEGORIES = {
        'version': ['__version__'],
        'registry': ['DescriptorRegistry', 'DescriptorRegistration', 'registry', 
                     'get_descriptor', 'has_descriptor', 'list_descriptors', 'auto_discover_rdkit'],
        'categories': ['DescriptorCategory', 'DescriptorMetadata', 'get_descriptors_by_category',
                       'get_descriptor_metadata', 'requires_3d_coordinates', 'requires_partial_charges',
                       'get_all_descriptor_names', 'get_category_descriptor_names',
                       'filter_descriptors_by_requirements', 'get_descriptor_count_by_category',
                       'validate_descriptor_coverage', 'DESCRIPTOR_METADATA_MAP', 'ALL_DESCRIPTORS',
                       'DESCRIPTORS_BY_CATEGORY'],
        'calculator': ['DescriptorCalculator', 'CalculationResult', 'BatchCalculationResult'],
        'validator': ['DescriptorValidator', 'ValidationResult', 'validate_value',
                      'check_requirements', 'filter_by_requirements', 'validator'],
        'integration': ['descriptors_to_tensor', 'add_descriptors_to_pyg_data',
                        'merge_descriptors_with_features', 'extract_descriptors_from_pyg_data',
                        'validate_descriptor_integration', 'get_descriptor_statistics'],
        'plugin': ['DescriptorPluginLoader', 'DescriptorPluginMetadata', 'DescriptorDeclaration',
                   'plugin_loader', 'discover_plugins', 'validate_plugin', 'list_plugins', 'get_plugin_info'],
    }
    ...
    def test_all_items_importable(self):
        ...
        assert not missing_exports, f"These exports are listed in __all__ but not accessible: {missing_exports}"
...
class TestRegistrySystem:
    ...
    def test_registry_singleton_exists(self):
        ...
        assert isinstance(registry, DescriptorRegistry), "registry must be DescriptorRegistry instance"
...
class TestConstants:
    ...
    def test_descriptor_metadata_map_values(self):
        ...
        for name, meta in DESCRIPTOR_METADATA_MAP.items():
            ...
            assert isinstance(meta, DescriptorMetadata), f"Value for {name} must be DescriptorMetadata"
...
class TestPyTorchGeometricIntegration:
    ...
    def test_add_descriptors_preserves_original_data(self, sample_pyg_data, sample_descriptors_dict):
        ...
        assert data.edge_index.shape == original_edge_shape, "Original edge_index should be preserved"
    ...
    def test_merge_descriptors_with_features(self, sample_descriptors_dict):
        ...
        data = merge_descriptors_with_features(
            fresh_data,
            sample_descriptors_dict
        )
    ...
    def test_merge_descriptors_dimension_calculation(self, sample_descriptors_dict):
        ...
        assert data.x.shape[1] == expected_dim, f"Expected dim {expected_dim}, got {data.x.shape[1]}"
    ...
    def test_extract_descriptors_from_pyg_data(self, sample_descriptors_dict):
        ...
        assert len(extracted) == len(sample_descriptors_dict), "Should have same number of descriptors"
    ...
    def test_validate_descriptor_integration_returns_tuple(self, sample_pyg_data, sample_descriptors_dict):
    ...
    def test_get_descriptor_statistics_single_molecule(self, sample_pyg_data, sample_descriptors_dict):
...
class TestPluginSystem:
    ...
    def test_list_plugins_returns_strings_or_dicts(self):
        ...
        for p in plugins:
            assert isinstance(p, (str, dict)), f"Plugin entry should be string or dict, got {type(p)}"
    ...
    @patch('milia_pipeline.descriptors.descriptor_plugin_system.DescriptorPluginLoader.discover_plugins')
    ...
    @patch('milia_pipeline.descriptors.descriptor_plugin_system.DescriptorPluginLoader.validate_plugin')
...
class TestModuleConsistency:
    ...
    def test_dataclass_exports_consistent(self):
        ...
        reg = DescriptorRegistration(
            name="Test", function=lambda x: x, 
            metadata=meta, is_builtin=True
        )
        ...
        calc = CalculationResult(
            success=True, value=1.0, descriptor_name='Test',
            error_message=None, computation_time=0.0
        )
        ...
        plugin_meta = DescriptorPluginMetadata(
            plugin_name='test', version='1.0.0', author='test'
        )
    ...
    def test_singleton_types(self):
        ...
        assert isinstance(plugin_loader, DescriptorPluginLoader), "plugin_loader must be DescriptorPluginLoader"
...
class TestDynamicAPICoverage:
    ...
    def test_all_class_exports_have_docstrings(self):
        ...
        class_names = [
            'DescriptorRegistry', 'DescriptorRegistration', 'DescriptorCategory',
            'DescriptorMetadata', 'DescriptorCalculator', 'CalculationResult',
            'BatchCalculationResult', 'DescriptorValidator', 'ValidationResult',
            'DescriptorPluginLoader', 'DescriptorPluginMetadata', 'DescriptorDeclaration'
        ]
    ...
    def test_all_function_exports_have_docstrings(self):
        ...
        function_names = [
            'get_descriptor', 'has_descriptor', 'list_descriptors',
            'get_descriptors_by_category', 'get_descriptor_metadata',
            'requires_3d_coordinates', 'requires_partial_charges',
            'get_all_descriptor_names', 'get_category_descriptor_names',
            'filter_descriptors_by_requirements', 'get_descriptor_count_by_category',
            'validate_descriptor_coverage', 'validate_value', 'check_requirements',
            'filter_by_requirements', 'descriptors_to_tensor', 'add_descriptors_to_pyg_data',
            'merge_descriptors_with_features', 'extract_descriptors_from_pyg_data',
            'validate_descriptor_integration', 'get_descriptor_statistics',
            'discover_plugins', 'validate_plugin', 'list_plugins', 'get_plugin_info',
            'auto_discover_rdkit'
        ]
