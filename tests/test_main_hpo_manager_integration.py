...
class MockPyGData:
    ...
    def __init__(
        ...
        edge_y: Optional[torch.Tensor] = None,
    ):
        ...
        self.edge_index = edge_index if edge_index is not None else torch.tensor(
...
class MockPyGDataset:
    ...
    def __init__(self, data_list: Optional[List[MockPyGData]] = None, task_type: Optional[str] = None):
        ...
        search_space={
            ...
            'loss': {
                'alpha': MockSearchSpaceParamConfig(
                    type=MockParamType.FLOAT, low=0.1, high=0.9
                ),
            }
        }
    )
...
def graph_regression_data():
    ...
    data_list = [
        MockPyGData(y=torch.tensor([float(i)])) for i in range(10)
    ]
...
class TestHPOManagerMainModuleIntegration:
    ...
    def test_hpo_data_prep_matches_main_data_prep_for_graph_tasks(self, graph_regression_data, mock_logger):
        ...
        hpo_train, hpo_val, _ = _prepare_data_for_task_hpo(
...
class TestEndToEndHPOWorkflow:
    ...
    def test_param_extraction_categories(self):
        ...
        model_params, optimizer_params, scheduler_params, loss_params, training_params = _extract_param_categories(flat_params)
...
class TestTaskSpecificDataFlow:
    ...
    def test_classification_full_flow(self):
        ...
        data_list = [
            MockPyGData(y=torch.tensor([i % 3], dtype=torch.long))
            for i in range(10)
        ]
...
class TestLossRegistryAdvancedFeatures:
    ...
    def test_all_regression_losses_available(self):
        ...
        for loss_name in regression_losses:
            assert loss_name in LossRegistry.list_available(), f"Missing regression loss: {loss_name}"
    ...
    def test_all_classification_losses_available(self):
        ...
        for loss_name in classification_losses:
            assert loss_name in LossRegistry.list_available(), f"Missing classification loss: {loss_name}"
...
class TestSchedulerRegistryAdvancedFeatures:
    ...
    def test_all_step_schedulers_available(self, mock_model):
        ...
        for sched_name in step_schedulers:
            assert sched_name in SchedulerRegistry.list_available(), f"Missing scheduler: {sched_name}"
...
class TestHPOManagerConvenienceFunctions:
    ...
    def test_get_best_params_raises_error_when_no_optimization(self, mock_get_backend):
        ...
        from milia_pipeline.models.hpo.hpo_manager import get_best_params, HPOManager
        from milia_pipeline.exceptions import HPOError
...
class TestTaskTypeMappingCompleteness:
    ...
    def test_classification_tasks_map_to_cross_entropy(self):
        ...
        for task in classification_tasks:
            assert _get_loss_name_for_task(task) == 'cross_entropy', f"Task {task} should map to cross_entropy"
...
class TestSearchSpaceFilteringIntegration:
    ...
    def test_filter_search_space_without_registry(self, mock_get_backend):
        ...
        search_space = {
            'hyperparameters': {
                'hidden_channels': MockSearchSpaceParamConfig(type=MockParamType.INT, low=32, high=256),
