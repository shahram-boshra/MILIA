...
class MockTargetSelectionConfig:
    ...
    def __init__(
        ...
        resolved_source_attr: Optional[str] = None,
    ):
    ...
    def resolve(self, available_names: Optional[List[str]] = None, total_count: Optional[int] = None):
...
class TestPrepareForTaskDispatch:
    ...
    def test_dispatch_unknown_task_type(self, train_data_list, val_data_list, test_data_list, caplog):
    ...
    def test_dispatch_uses_provided_logger(self, train_data_list, val_data_list, test_data_list, mock_logger):
    ...
    def test_dispatch_with_target_selection_config(
        self, train_node_data_list, val_node_data_list, test_node_data_list, mock_target_selection_config
    ):
        """Test dispatch with target_selection_config parameter."""
        # This should work with or without TARGET_SELECTION_AVAILABLE
        train, val, test, num_classes = TaskDataPreparer.prepare_for_task(
            train_node_data_list, val_node_data_list, test_node_data_list,
...
class TestPrepareGraphRegression:
    ...
    def test_graph_regression_logs_debug_message(
        ...
        assert "graph_regression" in caplog.text.lower() or "no transform needed" in caplog.text.lower()
...
class TestPrepareGraphClassification:
    ...
    def test_graph_classification_with_int_targets_returns_unchanged(
        ...
        train, val, test, num_classes = TaskDataPreparer.prepare_for_task(
            train_data_int_targets, val_data_int_targets, test_data_int_targets,
    ...
    def test_graph_classification_discretize_fit_failure(self):
        ...
        with patch(
            ...
            with pytest.raises(DataCompatibilityError, match="Failed to fit"):
                TaskDataPreparer.prepare_for_task(
                    data, data[:3], data[:2],
                    task_type='graph_classification'
                )
...
class TestPrepareNodeClassification:
    ...
    def test_node_classification_with_int_targets(
        self, sample_node_level_data_int_target
    ):
...
class TestPrepareEdgeRegression:
    ...
    def test_edge_regression_with_existing_edge_y(
        ...
        with caplog.at_level(logging.INFO):
            train, val, test, num_classes = TaskDataPreparer.prepare_for_task(
                train_edge_data_list, val_edge_data_list, test_edge_data_list,
...
class TestPrepareEdgeClassification:
    ...
    def test_edge_classification_checks_multiple_edge_attrs(self):
        ...
        train, val, test, num_classes = TaskDataPreparer.prepare_for_task(
            data_edge_value, data_edge_value, data_edge_value,
            task_type='edge_classification'
        )
    ...
    def test_edge_classification_with_edge_label(self):
        ...
        train, val, test, num_classes = TaskDataPreparer.prepare_for_task(
            data_with_edge_y, data_with_edge_y[:3], data_with_edge_y[:2],
            task_type='edge_classification'
        )
...
class TestPrepareLinkPrediction:
    ...
    def test_link_prediction_with_existing_edge_label(
        self, sample_link_prediction_data, caplog
    ):
...
class TestGetDiscretizeTargetsClass:
    ...
    def test_get_discretize_targets_logs_error_on_failure(self, caplog):
        ...
        def simulate_all_imports_fail(task_type):
            from milia_pipeline.models.training.data_preparation import logger, DataCompatibilityError
            logger.error(
...
class TestConvenienceFunctions:
    ...
    def test_prepare_data_for_task_delegates_to_class(
        ...
        with patch.object(TaskDataPreparer, 'prepare_for_task') as mock_prepare:
            ...
            mock_prepare.assert_called_once_with(
                train_data_list, val_data_list, test_data_list,
                'graph_regression', None, None
            )
    
    def test_prepare_data_for_task_with_all_args(
        self, train_data_list, val_data_list, test_data_list, mock_logger, mock_target_selection_config
    ):
    ...
    def test_prepare_data_for_task_preserves_data_integrity(self):
        ...
        train, val, test, num_classes = prepare_data_for_task(
            original_data, original_data[:3], original_data[:2],
            task_type='graph_classification'
        )
...
class TestDataCompatibilityError:
    ...
    def test_data_compatibility_error_with_missing_features(self):
        ...
        error = DataCompatibilityError(
            "Test error",
            missing_features=['edge_attr', 'pos']
        )
...
class TestTargetSelectionConfigIntegration:
    ...
    def test_target_selection_config_resolve_for_task_called(self):
        ...
        with patch(
            'milia_pipeline.models.training.data_preparation.TARGET_SELECTION_AVAILABLE',
            True
        ):
            TaskDataPreparer.prepare_for_task(
                data, data[:3], data[:2],
        ...
        for task_type in task_types:
            ...
            try:
                result = TaskDataPreparer.prepare_for_task(
                    data_list, data_list[:3], data_list[:2],
                    task_type=task_type
                )
...
class TestModuleInitialization:
    ...
    def test_module_logger_is_named_correctly(self):
        ...
        assert logger.name == 'milia_pipeline.models.training.data_preparation' or \
               logger.name.endswith('data_preparation')
