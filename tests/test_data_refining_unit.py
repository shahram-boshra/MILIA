...
class MockDFTHandler:
    ...
    def __init__(self, dataset_type='DFT'):
        ...
        self.dataset_config = DatasetConfig(dataset_type=dataset_type) if CONFIG_IMPORTS_SUCCESSFUL else Mock()
        # ProcessingConfig requires scalar_graph_targets parameter
        self.processing_config = ProcessingConfig(scalar_graph_targets=['Etot']) if CONFIG_IMPORTS_SUCCESSFUL else Mock()
...
class MockDMCHandler:
    ...
    def __init__(self):
        ...
        self.dataset_config = DatasetConfig(dataset_type='DMC') if CONFIG_IMPORTS_SUCCESSFUL else Mock()
        # ProcessingConfig requires scalar_graph_targets parameter
        self.processing_config = ProcessingConfig(scalar_graph_targets=['Etot']) if CONFIG_IMPORTS_SUCCESSFUL else Mock()
...
class TestHandlerCreation(unittest.TestCase):
    ...
    def test_create_dft_handler(self):
        ...
        try:
            ...
            self.assertTrue(
                'processing' in error_msg or 'import' in error_msg
                or 'not available' in error_msg or 'handler' in error_msg,
...
class TestmiliaProcessing(unittest.TestCase):
    ...
    def test_diagnose_vibrational_data_structure(self):
        ...
        diagnostics = diagnose_vibrational_data_structure(
            vibmode_data,
            molecule_index=0
        )
        ...
        self.assertIn('shape', diagnostics['overall_structure'])  # shape is nested inside overall_structure
...
class TestLoggingStatusFunctions(unittest.TestCase):
    ...
    def test_log_data_refinement_status_dft(self):
        ...
        # Should not raise
        log_data_refinement_status(
            raw_props, 0, self.logger, dataset_config
        )
...
class TestEdgeCasesAndErrors(unittest.TestCase):
    ...
    def test_configuration_error_on_invalid_config(self):
        ...
        with self.assertRaises((ValueError, ConfigurationError, TypeError, AttributeError, HandlerNotAvailableError)):
            create_refinement_handler(
    ...
    def test_complete_dft_refinement_workflow(self):
        ...
        try:
            ...
            try:
                result = refine_molecular_data_with_handler(
                    handler=handler,
                    raw_properties_dict={'freqs': [100.0, 200.0], 'vibmodes': [np.array([[1.0, 0.0, 0.0]]), np.array([[0.0, 1.0, 0.0]])]},
                    molecule_index=0
                )
                self.assertIsNotNone(result)
            except Exception as e:
...
class TestMigrateRefinementCallToHandler(unittest.TestCase):
    ...
    def test_migrate_without_handler_creates_one(self):
        ...
        try:
            ...
            self.assertIsInstance(result, dict)
        except (HandlerNotAvailableError, MigrationError, LegacyCodeError, 
                HandlerConfigurationError, ValueError, TypeError, NameError):
...
class TestApplyDatasetSpecificRefinement(unittest.TestCase):
    ...
    def test_apply_refinement_raises_on_quality_failure(self):
        ...
        with self.assertRaises((PropertyEnrichmentError, DataProcessingError, HandlerOperationError)):
            apply_dataset_specific_refinement(
                raw_properties_dict=self.raw_props,
                molecule_index=0,
                dataset_config=self.dataset_config,
...
class TestDatasetSpecificHandlerErrorHandling(unittest.TestCase):
    ...
    def test_handler_stats_reraises_dataset_specific_error(self):
        ...
        with self.assertRaises(DatasetSpecificHandlerError):
            _handler_get_refinement_statistics(
                handler=handler,
                refinement_results=[{'is_refined': True}]
            )
