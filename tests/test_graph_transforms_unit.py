...
class MockData:
    ...
    def __repr__(self):
        ...
        if self.edge_attr is not None:
            attrs.append(f"edge_attr={list(self.edge_attr.shape) if hasattr(self.edge_attr, 'shape') else 'present'}")
        if self.edge_index is not None:
            attrs.append(f"edge_index={list(self.edge_index.shape) if hasattr(self.edge_index, 'shape') else 'present'}")
...
class TestTransformRegistry:
    ...
    def test_get_transforms_by_category(self):
        ...
        structure_transforms = registry.list_by_category('structural')  # Changed 'structure' to 'structural'
...
class TestTransformComposer:
    ...
    def test_create_transform_sequence_simple(self, mock_registry, mock_validator):
        ...
        with patch('milia_pipeline.transformations.graph_transforms.Compose', MockCompose), \
            ...
            try:
                ...
                assert 'TransformCompositionError' in str(type(e).__name__) or isinstance(e, Exception)
...
class TestTransformComposerEdgeAttrIntegration:
    ...
    def test_composer_set_sample_data(self, mock_registry, mock_validator, mock_data_with_edge_attr):
    ...
    def test_compose_transforms_with_sample_data_param(self, mock_registry, mock_validator, mock_data_with_edge_attr):
        ...
        with patch('milia_pipeline.transformations.graph_transforms.Compose', MockCompose), \
            ...
            try:
                ...
                compose = composer.compose_transforms(
                    configs, 
                    sample_data=mock_data_with_edge_attr
                )
...
class TestGraphTransformsEdgeAttrIntegration:
    ...
    def test_create_transform_sequence_accepts_sample_data(self, mock_data_with_edge_attr):
        ...
        try:
            result = gt.create_transform_sequence(
                configs, 
                sample_data=mock_data_with_edge_attr
            )
...
class TestAddSelfLoopsMetadataUpdate:
    ...
    def test_add_self_loops_modifies_edge_attr_in_metadata(self):
        ...
        if info is not None and info.dependencies is not None:
            ...
            assert 'edge_attr' in modifies, "AddSelfLoops should list edge_attr in modifies_attributes"
...
class TestParameterConstraint:
    ...
    def test_parameter_constraint_validate_pattern_valid(self):
        ...
        constraint = ParameterConstraint(
            type='pattern',
            description='Lowercase alphanumeric',
            constraint_value='^[a-z0-9]+$'
        )
...
class TestParameterMetadata:
    ...
    def test_parameter_metadata_has_default(self):
        ...
        # With default value
        metadata_with_default = ParameterMetadata(
            name='p',
            default_value=0.5
        )
        ...
        metadata_no_default = ParameterMetadata(
            name='p',
    ...
    def test_parameter_metadata_is_optional_with_basic_type(self):
        ...
        metadata_required = ParameterMetadata(
            name='p',
            type_hint=float
        )
...
class TestDatasetAwareValidator:
    ...
    def test_dataset_validator_dft_validation(self, dataset_validator):
        ...
        result_context = dataset_validator.validate_for_dataset(
            configs, 'DFT', context
        )
    ...
    def test_dataset_validator_unknown_dataset_warning(self, dataset_validator):
        ...
        result_context = dataset_validator.validate_for_dataset(
            configs, 'UnknownDataset', context
        )
        
        # Should add a warning for unknown dataset type
        warnings = [issue for issue in result_context.issues 
...
class TestTransformRegistryCompatibility:
    ...
    def test_registry_check_compatibility(self, mock_registry):
        ...
        is_compatible, reason = mock_registry.check_compatibility(
...
class TestTransformComposerSampleDataMethods:
    ...
    def test_composer_set_sample_data_with_edge_attr(self, mock_registry, mock_validator, mock_data_with_edge_attr):
    ...
    def test_composer_set_sample_data_without_edge_attr(self, mock_registry, mock_validator, mock_data_without_edge_attr):
...
class TestConfigurationValidatorStandardTransforms:
    ...
    def test_is_valid_v3_format_with_experimental_setups_only(self):
        ...
        config = {
            'experimental_setups': {
                'baseline': {'transforms': [{'name': 'AddSelfLoops'}]}
            },
            'research_context': 'molecular_property_prediction',
            'dataset_optimization': {'dataset_type': 'DFT'}
        }
    ...
    def test_validate_v3_config_standard_transforms_only(self):
        ...
        assert any('experimental_setups' in w.lower() or 'standard_transforms' in w.lower() for w in warnings)
    ...
    def test_validate_v3_config_default_setup_with_standard_fallback(self):
        ...
        config = {
            ...
            'experimental_setups': {
                'other_setup': {'transforms': [{'name': 'GCNNorm'}]}
            },
...
class TestConfigurationBridgeStandardTransforms:
    ...
    def test_convert_legacy_config_with_standard_transforms(self, mock_registry):
        ...
        config = {
            ...
            'experimental_setups': {
                'baseline': {'transforms': [{'name': 'GCNNorm'}]}
            },
    ...
    def test_convert_legacy_config_order_standard_before_experimental(self, mock_registry):
        ...
        config = {
            ...
            'experimental_setups': {
                'test_setup': {
                    'transforms': [
                        {'name': 'Transform_Exp_1'},
                        {'name': 'Transform_Exp_2'}
                    ]
                }
            },
