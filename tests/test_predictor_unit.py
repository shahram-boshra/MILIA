...
class TestPredictorInitialization:
    ...
    def test_init_auto_detects_cpu_when_cuda_unavailable(self, mock_cuda, mock_model, working_root_dir):
    ...
    def test_init_stores_task_type(self, mock_model, working_root_dir):
        ...
        predictor = Predictor(model=mock_model, working_root_dir=working_root_dir, task_type='graph_regression')
    ...
    def test_init_with_classification_task_type(self, mock_model, working_root_dir):
        ...
        predictor = Predictor(model=mock_model, working_root_dir=working_root_dir, task_type='graph_classification')
    ...
    def test_init_with_real_model(self, simple_pyg_model, working_root_dir):
        ...
        predictor = Predictor(model=simple_pyg_model, working_root_dir=working_root_dir, device=torch.device('cpu'))
    ...
    def test_init_stores_model_info(self, mock_model, working_root_dir):
        ...
        predictor = Predictor(model=mock_model, working_root_dir=working_root_dir, model_info=model_info)
...
class TestPredictorFromCheckpoint:
    ...
    def test_from_checkpoint_passes_device(
        ...
        device = torch.device('cpu')
        predictor = Predictor.from_checkpoint("test.pt", working_root_dir=working_root_dir, device=device)
    ...
    def test_from_checkpoint_classification_task(
        self, mock_loader_class, mock_cm_class, mock_model, mock_classification_checkpoint, working_root_dir
    ):
...
class TestPredictorPredict:
    ...
    def test_predict_moves_data_to_device(self, mock_model, simple_pyg_data, cpu_device, working_root_dir):
        """Test predict moves data to correct device."""
        # Setup mock to track data device
        mock_model.return_value = torch.tensor([[0.5]])
        predictor = Predictor(model=mock_model, working_root_dir=working_root_dir, device=cpu_device)
    ...
    def test_predict_returns_tensor_by_default(self, mock_model, simple_pyg_data, cpu_device, working_root_dir):
    ...
    def test_predict_returns_numpy_when_requested(self, mock_model, simple_pyg_data, cpu_device, working_root_dir):
    ...
    def test_predict_runs_in_no_grad_context(self, mock_model, simple_pyg_data, cpu_device, working_root_dir):
    ...
    def test_predict_with_real_model_single_data(self, simple_pyg_model, simple_pyg_data, cpu_device, working_root_dir):
        """Test predict with real model and single Data."""
        predictor = Predictor(model=simple_pyg_model, working_root_dir=working_root_dir, device=cpu_device)
    ...
    def test_predict_with_real_model_batch(self, simple_pyg_model, pyg_batch, cpu_device, working_root_dir):
...
class TestPredictorForward:
    ...
    def test_forward_passes_x_and_edge_index(self, mock_model, simple_pyg_data, cpu_device, working_root_dir):
    ...
    def test_forward_passes_batch_when_present(self, mock_model, pyg_batch, cpu_device, working_root_dir):
    ...
    def test_forward_passes_pos_when_present(self, mock_model, pyg_data_with_pos, cpu_device, working_root_dir):
    ...
    def test_forward_does_not_pass_missing_batch(self, mock_model, simple_pyg_data, cpu_device, working_root_dir):
    ...
    def test_forward_does_not_pass_missing_pos(self, mock_model, simple_pyg_data, cpu_device, working_root_dir):
...
class TestPredictorPostprocess:
    ...
    def test_postprocess_no_change_for_regression(self, mock_model, simple_pyg_data, cpu_device, working_root_dir):
        """Test _postprocess returns output unchanged for regression tasks."""
        output = torch.tensor([[0.5]])
        mock_model.return_value = output
        predictor = Predictor(model=mock_model, working_root_dir=working_root_dir, device=cpu_device, task_type='graph_regression')
        
        result = predictor._postprocess(output, simple_pyg_data)
        
        assert torch.equal(result, output)
    
    def test_postprocess_no_change_for_none_task_type(self, mock_model, simple_pyg_data, cpu_device, working_root_dir):
        """Test _postprocess returns output unchanged when task_type is None."""
        output = torch.tensor([[0.1, 0.7, 0.2]])
        mock_model.return_value = output
        predictor = Predictor(model=mock_model, working_root_dir=working_root_dir, device=cpu_device, task_type=None)
        
        result = predictor._postprocess(output, simple_pyg_data)
        
        assert torch.equal(result, output)
    
    def test_postprocess_argmax_for_classification(self, mock_model, simple_pyg_data, cpu_device, working_root_dir):
        """Test _postprocess applies argmax for classification tasks."""
        output = torch.tensor([
            [0.1, 0.7, 0.2],  # Class 1
            [0.8, 0.1, 0.1],  # Class 0
        ])
        predictor = Predictor(model=mock_model, working_root_dir=working_root_dir, device=cpu_device, task_type='graph_classification')
    ...
    def test_postprocess_argmax_for_node_classification(self, mock_model, simple_pyg_data, cpu_device, working_root_dir):
        ...
        predictor = Predictor(model=mock_model, working_root_dir=working_root_dir, device=cpu_device, task_type='node_classification')
    ...
    def test_postprocess_handles_binary_classification(self, mock_model, simple_pyg_data, cpu_device, working_root_dir):
        """Test _postprocess handles binary classification (2 classes)."""
        output = torch.tensor([
            [0.7, 0.3],
            [0.2, 0.8],
        ])
        predictor = Predictor(model=mock_model, working_root_dir=working_root_dir, device=cpu_device, task_type='classification')
    ...
    def test_postprocess_no_argmax_for_single_class(self, mock_model, simple_pyg_data, cpu_device, working_root_dir):
    ...
    def test_postprocess_case_insensitive_classification(
        ...
        for task_type in ['Classification', 'CLASSIFICATION', 'Graph_Classification']:
            predictor = Predictor(model=mock_model, working_root_dir=working_root_dir, device=cpu_device, task_type=task_type)
            result = predictor._postprocess(output, simple_pyg_data)
...
class TestPredictorPredictBatch:
    ...
    def test_predict_batch_correct_output_shape(self, simple_pyg_model, pyg_dataset, cpu_device, working_root_dir):
    ...
    def test_predict_batch_preserves_order(self, cpu_device, working_root_dir):
        ...
        dataset = [
            Data(x=torch.randn(3, 16), edge_index=torch.tensor([[0, 1], [1, 0]]))
            for _ in range(5)
        ]
    ...
    def test_predict_batch_uses_dataloader_correctly(self, mock_model, pyg_dataset, cpu_device, working_root_dir):
    ...
    def test_predict_batch_with_small_dataset(self, simple_pyg_model, cpu_device, working_root_dir):
        ...
        small_dataset = [
            Data(x=torch.randn(4, 16), edge_index=torch.tensor([[0, 1], [1, 0]]))
            for _ in range(3)
        ]
...
class TestPredictConvenienceFunction:
    ...
    def test_predict_passes_device(self, mock_from_checkpoint, simple_pyg_data, working_root_dir):
        ...
        device = torch.device('cpu')
        result = predict("model.pt", simple_pyg_data, working_root_dir=working_root_dir, device=device)
    ...
    @patch('milia_pipeline.models.post_training.inference.predictor.Predictor.from_checkpoint')
    def test_predict_calls_predictor_predict(self, mock_from_checkpoint, simple_pyg_data, working_root_dir):
    ...
    @patch('milia_pipeline.models.post_training.inference.predictor.Predictor.from_checkpoint')
    def test_predict_passes_return_numpy(self, mock_from_checkpoint, simple_pyg_data, working_root_dir):
        """Test predict() passes return_numpy to predictor.predict."""
        mock_predictor = MagicMock()
        mock_predictor.predict.return_value = np.array([[0.5]])
        mock_from_checkpoint.return_value = mock_predictor
        
        result = predict("model.pt", simple_pyg_data, working_root_dir=working_root_dir, return_numpy=True)
        
        mock_predictor.predict.assert_called_once_with(simple_pyg_data, return_numpy=True)
    
    @patch('milia_pipeline.models.post_training.inference.predictor.Predictor.from_checkpoint')
    def test_predict_with_path_object(self, mock_from_checkpoint, simple_pyg_data, working_root_dir):
    ...
    @patch('milia_pipeline.models.post_training.inference.predictor.Predictor.from_checkpoint')
    def test_predict_returns_prediction_result(self, mock_from_checkpoint, simple_pyg_data, working_root_dir):
...
class TestDeviceHandling:
    ...
    def test_prediction_output_on_correct_device(self, mock_model, simple_pyg_data, cpu_device, working_root_dir):
    ...
    def test_data_moved_to_predictor_device(self, mock_model, simple_pyg_data, cpu_device, working_root_dir):
    ...
    @patch('torch.cuda.is_available', return_value=False)
    def test_cpu_fallback_when_cuda_unavailable(self, mock_cuda, mock_model, simple_pyg_data, working_root_dir):
    ...
    def test_explicit_device_overrides_auto_detection(self, mock_model, simple_pyg_data, working_root_dir):
...
class TestEdgeCasesAndErrors:
    ...
    def test_predict_batch_with_varying_graph_sizes(self, simple_pyg_model, cpu_device, working_root_dir):
    ...
    def test_predict_preserves_model_parameters(self, simple_pyg_model, simple_pyg_data, cpu_device, working_root_dir):
    ...
    def test_numpy_output_on_cpu(self, simple_pyg_model, simple_pyg_data, cpu_device, working_root_dir):
...
class TestClassificationTaskTypes:
    ...
    def test_various_classification_task_types(
        self, mock_model_for_classification, simple_pyg_data, cpu_device, working_root_dir, task_type
    ):
...
class TestTypeAnnotations:
    ...
    def test_predict_accepts_data_type(self, simple_pyg_model, simple_pyg_data, cpu_device, working_root_dir):
    ...
    def test_predict_accepts_batch_type(self, simple_pyg_model, pyg_batch, cpu_device, working_root_dir):
    ...
    def test_predict_batch_accepts_list(self, simple_pyg_model, pyg_dataset, cpu_device, working_root_dir):
...
class TestIntegrationStyle:
    ...
    def test_full_prediction_workflow(self, simple_pyg_model, pyg_dataset, cpu_device, working_root_dir):
    ...
    def test_multiple_predictions_consistency(self, simple_pyg_model, simple_pyg_data, cpu_device, working_root_dir):
...
class TestStructuralFeaturesConfig:
    ...
    def test_structural_features_config_returns_config_when_present(self, mock_model, working_root_dir):
        """Test property returns config from model_info.data_info."""
        expected_config = {'atom': ['atomic_num', 'degree'], 'bond': ['bond_type']}
        model_info = {
            'data_info': {
                'structural_features_config': expected_config
            }
        }
        predictor = Predictor(
    ...
    def test_structural_features_config_returns_none_when_no_model_info(self, mock_model, working_root_dir):
        """Test property returns None when model_info is empty."""
        predictor = Predictor(model=mock_model, working_root_dir=working_root_dir)
        
        assert predictor.structural_features_config is None
    
    def test_structural_features_config_returns_none_when_no_data_info(self, mock_model, working_root_dir):
    ...
    def test_structural_features_config_returns_none_when_no_config_key(self, mock_model, working_root_dir):
...
class TestResolvePath:
    ...
    def test_resolve_path_relative_resolves_against_working_root(self, mock_model, working_root_dir):
...
class TestSavePredictions:
    ...
    def test_save_predictions_csv_with_identifiers(self, mock_model, working_root_dir):
        ...
        result_path = predictor.save_predictions(
            predictions, "with_ids.csv", format='csv',
            include_inputs=True, input_identifiers=identifiers
        )
    ...
    def test_save_predictions_json_with_identifiers(self, mock_model, working_root_dir):
        ...
        result_path = predictor.save_predictions(
            predictions, "with_ids.json", format='json',
            include_inputs=True, input_identifiers=identifiers
        )
...
class TestForward3DMolecularModel:
    ...
    def test_forward_3d_model_raises_on_missing_z(self, cpu_device, working_root_dir, caplog):
        ...
        assert any("3D molecular model requires 'z'" in record.message
    ...
    def test_forward_3d_model_raises_on_missing_pos(self, cpu_device, working_root_dir, caplog):
        ...
        assert any("3D molecular model requires 'pos'" in record.message
                    or "Signature introspection failed" in record.message
                    for record in caplog.records)
