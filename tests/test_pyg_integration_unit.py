...
class TestValidatePygData:
    ...
    def test_negative_edge_indices(self):
        ...
        data = Data(
            x=torch.randn(5, 10), edge_index=torch.tensor([[0, -1], [1, 2]], dtype=torch.long)
        )
        result = validate_pyg_data(data)
    ...
    def test_nan_in_node_features(self):
        ...
        x = torch.randn(5, 10)
        x[0, 0] = float("nan")
        data = Data(x=x, edge_index=torch.tensor([[0, 1], [1, 2]], dtype=torch.long))
        result = validate_pyg_data(data)
...
class TestCheckDataCompatibility:
    ...
    def test_missing_node_features(self):
        ...
        data = Data(edge_index=torch.tensor([[0, 1], [1, 2]], dtype=torch.long))
        compatible, missing = check_data_compatibility(data, requires_node_features=True)
    ...
    def test_missing_edge_index(self):
        ...
        data = Data(x=torch.randn(5, 10))
        compatible, missing = check_data_compatibility(data, requires_edge_index=True)
    ...
    def test_missing_edge_features(self, minimal_graph_data):
        ...
        compatible, missing = check_data_compatibility(
            minimal_graph_data, requires_edge_features=True
        )
    ...
    def test_missing_edge_weights(self, minimal_graph_data):
        ...
        compatible, missing = check_data_compatibility(
            minimal_graph_data, requires_edge_weights=True
        )
    ...
    def test_multiple_missing_requirements(self):
        ...
        data = Data(x=torch.randn(5, 10))
        compatible, missing = check_data_compatibility(
            data, requires_edge_index=True, requires_edge_features=True, requires_edge_weights=True
        )
...
class TestInferNumFeatures:
    ...
    def test_infer_1d_node_features(self):
        ...
        data = Data(x=torch.randn(5), edge_index=torch.tensor([[0, 1], [1, 2]], dtype=torch.long))
...
class TestInferOutChannels:
    ...
    def test_graph_regression_no_y_attribute(self):
        ...
        data = Data(
            x=torch.randn(10, 16), edge_index=torch.tensor([[0, 1], [1, 2]], dtype=torch.long)
        )
        out_channels = infer_out_channels(data, "graph_regression")
    ...
    def test_node_classification_from_dataset(self):
        ...
        data_list = [
            Data(
                ...
                edge_index=torch.tensor([[0, 1], [1, 2]], dtype=torch.long),
                y=torch.tensor(
                    [i % 3 for i in range(10)], dtype=torch.long
                ),  # 3 classes, shape (10,)
            )
            for _ in range(50)
        ]
...
class TestCreateDataloader:
    ...
    def test_dataloader_with_kwargs(self):
        ...
        loader = create_dataloader(dataset, batch_size=2, shuffle=False, drop_last=True)
...
class TestComputeGraphStatistics:
    ...
    def test_degree_distribution(self):
        ...
        data = Data(
            x=torch.randn(5, 10),
            edge_index=torch.tensor([[0, 0, 0, 1, 2], [1, 2, 3, 3, 4]], dtype=torch.long),
        )
...
class TestEdgeCases:
    ...
    def test_single_edge_graph(self):
        ...
        data = Data(x=torch.randn(2, 5), edge_index=torch.tensor([[0], [1]], dtype=torch.long))
...
class TestIntegration:
    ...
    def test_full_validation_workflow(self, valid_graph_data):
        ...
        compatible, _ = check_data_compatibility(valid_graph_data, requires_edge_features=True)
        assert compatible is True
...
class TestGetBatchInfoExtended:
    ...
    def test_unbatched_data_without_x_raises(self):
        ...
        data = Data(edge_index=torch.tensor([[0, 1], [1, 2]], dtype=torch.long), num_nodes=4)
...
class TestPrintDatasetSummaryExtended:
    ...
    def test_summary_without_edge_statistics(self, capsys):
        ...
        data_list = [Data(x=torch.randn(5, 10)) for _ in range(3)]
        dataset = SimpleDataset(data_list)
...
class TestInferOutChannelsLogging:
    ...
    def test_link_prediction_logs_debug(self, caplog):
        ...
        data = Data(
            x=torch.randn(5, 8), edge_index=torch.tensor([[0, 1], [1, 2]], dtype=torch.long)
        )
...
class TestComputeDatasetStatisticsExtended:
    ...
    def test_dataset_with_num_nodes_fallback(self):
        ...
        data_list = [
            Data(edge_index=torch.tensor([[0, 1], [1, 2]], dtype=torch.long), num_nodes=5)
            for _ in range(3)
        ]
        dataset = SimpleDataset(data_list)
        stats = compute_dataset_statistics(dataset)
...
class TestValidatePygDataEdgeWeights:
    ...
    def test_edge_weights_without_edge_index(self):
        ...
        data = Data(x=torch.randn(5, 10), edge_weight=torch.randn(3))
        result = validate_pyg_data(data)
