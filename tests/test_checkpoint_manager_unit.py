...
def mock_optimizer():
    ...
    optimizer.state_dict = Mock(
        return_value={"state": {}, "param_groups": [{"lr": 0.001, "weight_decay": 0.0}]}
    )
...
class TestResolveCheckpointPathMethod:
    ...
    def test_resolve_checkpoint_path_expanduser(self, checkpoint_manager, temp_checkpoint_dir):
...
class TestSaveMethod:
    ...
    def test_save_minimal_parameters(self, checkpoint_manager, mock_model, temp_checkpoint_dir):
        """Test save with minimal required parameters."""
        filepath = temp_checkpoint_dir / "minimal.pt"

        result_path = checkpoint_manager.save(filepath=filepath, model=mock_model)

        assert result_path == filepath.resolve()
        assert filepath.exists()

    def test_save_returns_path(self, checkpoint_manager, mock_model, temp_checkpoint_dir):
    ...
    def test_save_with_string_path(self, checkpoint_manager, mock_model, temp_checkpoint_dir):
    ...
    def test_save_without_optimizer(self, checkpoint_manager, mock_model, temp_checkpoint_dir):
    ...
    def test_save_without_scheduler(self, checkpoint_manager, mock_model, temp_checkpoint_dir):
    ...
    def test_save_with_extra_data(self, checkpoint_manager, mock_model, temp_checkpoint_dir):
    ...
    def test_save_logging(self, checkpoint_manager, mock_model, temp_checkpoint_dir, caplog):
...
class TestSaveMethodPathResolution:
    ...
    def test_save_with_relative_path(self, checkpoint_manager, mock_model, temp_checkpoint_dir):
    ...
    def test_save_returns_resolved_path(self, checkpoint_manager, mock_model, temp_checkpoint_dir):
...
class TestLoadMethodPathResolution:
    ...
    def test_load_with_relative_path(self, checkpoint_manager, mock_model, temp_checkpoint_dir):
...
class TestSaveMethodWithRealModels:
    ...
    def test_save_real_model(self, checkpoint_manager, real_model, temp_checkpoint_dir):
    ...
    def test_save_real_model_and_scheduler(
        self, checkpoint_manager, real_model, real_optimizer, real_scheduler, temp_checkpoint_dir
    ):
...
class TestLoadMethod:
    ...
    def test_load_returns_dict(self, checkpoint_manager, mock_model, temp_checkpoint_dir):
    ...
    def test_load_contains_saved_data(self, checkpoint_manager, mock_model, temp_checkpoint_dir):
        """Test load returns checkpoint with saved data."""
        filepath = temp_checkpoint_dir / "saved.pt"
        checkpoint_manager.save(filepath=filepath, model=mock_model, epoch=50, global_step=2500)
    ...
    def test_load_with_map_location_cpu(self, checkpoint_manager, mock_model, temp_checkpoint_dir):
        """Test load with map_location=cpu."""
        filepath = temp_checkpoint_dir / "map_cpu.pt"
        checkpoint_manager.save(filepath=filepath, model=mock_model)

        checkpoint = checkpoint_manager.load(filepath=filepath, map_location=torch.device("cpu"))
    ...
    def test_load_with_map_location_string(
        ...
        checkpoint = checkpoint_manager.load(filepath=filepath, map_location="cpu")
    ...
    def test_load_logging_info(self, checkpoint_manager, mock_model, temp_checkpoint_dir, caplog):
    ...
    def test_load_preserves_extra_data(self, checkpoint_manager, mock_model, temp_checkpoint_dir):
        """Test load preserves extra_data from save."""
        filepath = temp_checkpoint_dir / "extra_data.pt"
        checkpoint_manager.save(filepath=filepath, model=mock_model, custom_field="custom_value")
...
class TestLoadMethodEdgeCases:
    ...
    def test_load_checkpoint_with_empty_version_info(self, checkpoint_manager, temp_checkpoint_dir):
...
class TestIsV2CheckpointMethod:
    ...
    def test_is_v2_checkpoint_version_less_than_2(self, checkpoint_manager):
        ...
        checkpoint_v19 = {"version_info": {"checkpoint_format_version": "1.9"}}
        assert checkpoint_manager.is_v2_checkpoint(checkpoint_v19) is False

        # Version '1.0' should be < '2.0'
        checkpoint_v10 = {"version_info": {"checkpoint_format_version": "1.0"}}
    ...
    def test_is_v2_checkpoint_exact_v2(self, checkpoint_manager):
        ...
        checkpoint = {"version_info": {"checkpoint_format_version": "2.0"}}
...
class TestGetHyperParametersMethod:
    ...
    def test_get_hyper_parameters_from_v1_checkpoint(self, checkpoint_manager, v1_checkpoint):
...
class TestGetModelNameMethod:
    ...
    def test_get_model_name_from_v2_checkpoint(self, checkpoint_manager, v2_checkpoint):
        """Test get_model_name extracts model name from v2.0 checkpoint."""
        result = checkpoint_manager.get_model_name(v2_checkpoint)
        assert result == "GCN"

    def test_get_model_name_from_v1_checkpoint(self, checkpoint_manager, v1_checkpoint):
    ...
    def test_get_model_name_various_model_names(self, checkpoint_manager):
        ...
        for model_name in model_names:
            checkpoint = {"hyper_parameters": {"model_name": model_name}}
            result = checkpoint_manager.get_model_name(checkpoint)
...
class TestRoundTrip:
    ...
    def test_round_trip_minimal(self, checkpoint_manager, mock_model, temp_checkpoint_dir):
    ...
    def test_round_trip_real_model(
        ...
        # Save
        checkpoint_manager.save(
            filepath=filepath, model=real_model, optimizer=real_optimizer, epoch=10
        )
    ...
    def test_round_trip_preserves_tensor_values(
        ...
        # Verify exact values
        for key in original_state:
            torch.testing.assert_close(original_state[key], loaded["model_state_dict"][key])
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_save_with_empty_model_state_dict(self, checkpoint_manager, temp_checkpoint_dir):
    ...
    def test_save_with_large_state_dict(self, checkpoint_manager, temp_checkpoint_dir):
        ...
        large_model.state_dict = Mock(
            return_value={f"layer{i}.weight": torch.randn(1000, 1000) for i in range(10)}
        )
    ...
    def test_save_with_negative_epoch(self, checkpoint_manager, mock_model, temp_checkpoint_dir):
...
class TestIntegrationWithRealModelLoading:
    ...
    def test_load_state_dict_into_real_model(
        ...
        torch.testing.assert_close(new_model.linear.weight, torch.ones(5, 10))
        torch.testing.assert_close(new_model.linear.bias, torch.full((5,), 0.5))
...
class TestMultipleSaveOperations:
    ...
    def test_multiple_saves_same_file(self, checkpoint_manager, mock_model, temp_checkpoint_dir):
    ...
    def test_multiple_saves_different_files(
        ...
        for i in range(5):
            ...
            checkpoint_manager.save(filepath=filepath, model=mock_model, epoch=i)
...
class TestDocumentationCompliance:
    ...
    def test_usage_example_from_docstring(self, mock_model, mock_optimizer, temp_checkpoint_dir):
        """
