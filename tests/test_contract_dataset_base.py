...
class TestRegistryPopulation:
    ...
    def test_expected_datasets_are_registered(self, registered_names: list[str]):
        ...
        assert not missing, (
            f"Expected datasets missing from registry: {missing}. Registered: {registered_set}"
        )
...
class TestBaseDatasetSubclassContract:
    ...
    def test_is_subclass_of_base_dataset(self, registered_classes: dict[str, type[BaseDataset]]):
...
class TestDatasetMetadataContract:
    ...
    def test_metadata_name_is_non_empty_string(
        ...
        for _name, cls in registered_classes.items():
            ...
            assert len(cls.metadata.name) > 0, f"{cls.__name__}.metadata.name is empty"
...
class TestDatasetSchemaContract:
    ...
    def test_required_properties_is_non_empty_tuple(
        ...
        for _name, cls in registered_classes.items():
            ...
            assert len(rp) > 0, f"{cls.__name__}.schema.required_properties is empty"
            for prop in rp:
...
class TestConfigKeyContract:
    ...
    def test_config_keys_are_unique(self, registered_classes: dict[str, type[BaseDataset]]):
        """No two datasets should share the same config_key."""
        seen: dict[str, str] = {}
        for name, cls in registered_classes.items():
            key = cls.config_key
            if key in seen:
                pytest.fail(f"Duplicate config_key '{key}' shared by '{seen[key]}' and '{name}'")
            seen[key] = name
...
class TestAbstractMethodContract:
    ...
    def test_get_required_properties_returns_list_of_strings(
        ...
        for _name, cls in registered_classes.items():
            ...
            assert len(result) > 0, f"{cls.__name__}.get_required_properties() returned empty list"
            for item in result:
                assert isinstance(item, str) and item, (
                    f"{cls.__name__}.get_required_properties() contains invalid entry: {item!r}"
                )
    ...
    def test_get_feature_support_matches_features(
        ...
        for _name, cls in registered_classes.items():
            ...
            assert from_method == from_features, (
                f"{cls.__name__}.get_feature_support() does not match features.to_dict()"
            )
    ...
    def test_missing_schema_raises_type_error(self):
        ...
        with pytest.raises(TypeError, match="missing required class attributes"):
            ...
            class BadDataset2(BaseDataset):
                metadata = DatasetMetadata(name="Bad2", version="1.0", description="bad")
                # schema is missing
                features = DatasetFeatures()
    ...
    def test_empty_config_key_raises_type_error(self):
        ...
        with pytest.raises(TypeError, match="config_key"):
            ...
            class BadDataset4(BaseDataset):
                metadata = DatasetMetadata(name="Bad4", version="1.0", description="bad")
                schema = DatasetSchema(required_properties=("a",))
                features = DatasetFeatures()
                config_key = ""  # empty
...
class TestCreateHandlerContract:
    ...
    def test_create_handler_is_classmethod(self, registered_classes: dict[str, type[BaseDataset]]):
        ...
        for _name, cls in registered_classes.items():
            ...
            method = cls.create_handler
            assert callable(method), f"{cls.__name__}.create_handler is not callable"
...
class TestDatasetSpecificProperties:
    ...
    def test_all_coordinate_based_datasets_have_atomization_energy(self, registered_classes):
...
class TestListAllClasses:
    ...
    def test_list_all_classes_returns_list_of_types(self, default_registry: DatasetRegistry):
    ...
    def test_list_all_classes_count_matches_list_all(self, default_registry: DatasetRegistry):
