...
class MockHPOManager:
    ...
    def optimize(
        self, model_name, dataset, base_hyperparameters=None, trainer_kwargs=None, callbacks=None
    ):
...
class MockGNNArchitectureSpace:
    ...
    activation_types: list[MockActivationType] = field(
        default_factory=lambda: [
            MockActivationType.RELU,
            MockActivationType.GELU,
            MockActivationType.ELU,
        ]
    )
...
def mock_arch_space_gcn_only():
    ...
    return MockGNNArchitectureSpace(layer_types=[MockLayerType.GCN], allow_mixed_layers=False)
...
def mock_hpo_config_with_search_space():
    ...
    return MockHPOConfig(
        ...
        search_space={
            "optimizer": {
                "lr": MockSearchSpaceParamConfig(type=MockParamType.LOGUNIFORM, low=1e-5, high=1e-2)
            }
        },
    )
...
class TestNASManagerInitWithNASConfig:
    ...
    def test_nasmanager_init_with_nas_config(self):
        ...
        from milia_pipeline.models.hpo.nas.nas_manager import NASConfig, NASManager
        from milia_pipeline.models.hpo.nas.search_space import GNNArchitectureSpace
...
class TestNASManagerBuildModel:
    ...
    def test_build_model_returns_module(self, sample_architecture):
        ...
        model = nas.build_model(architecture=sample_architecture, in_channels=10, out_channels=1)
    ...
    def test_build_model_heterogeneous_architecture(self):
        ...
        model = nas.build_model(architecture=mixed_arch, in_channels=10, out_channels=1)
...
class TestHeterogeneousGNNInit:
    ...
    def test_heterogeneous_gnn_init_creates_layers(self, sample_architecture):
        ...
        model = HeterogeneousGNN(architecture=sample_architecture, in_channels=10, out_channels=1)
...
class TestHeterogeneousGNNCreatePooling:
    ...
    def test_create_pooling_attention(self, sample_architecture_with_attention):
        ...
        model = HeterogeneousGNN(
            architecture=sample_architecture_with_attention, in_channels=10, out_channels=1
        )
...
class TestCreateNASManager:
    ...
    def test_create_nas_manager_returns_nas_manager(self):
        ...
        from milia_pipeline.models.hpo.nas.nas_manager import NASManager, create_nas_manager
...
class TestIntegration:
    ...
    def test_full_workflow_search_and_build(self, mock_dataset):
        ...
        model = nas._build_heterogeneous_model(
            architecture=best_arch, in_channels=10, out_channels=1
        )
...
class TestEdgeCasesAndErrors:
    ...
    def test_heterogeneous_gnn_no_batch_norm(self):
        ...
        model = HeterogeneousGNN(architecture=arch, in_channels=10, out_channels=1)
