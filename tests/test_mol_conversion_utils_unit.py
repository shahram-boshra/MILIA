...
from milia_pipeline.molecules import mol_conversion_utils
...
class TestCreateRdkitMol(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_handler.get_molecule_creation_strategy.return_value = "identifier_coordinate_based"
    ...
    def test_create_rdkit_mol_atom_count_mismatch(
        self, mock_error_class, mock_context, mock_chem, mock_validate_structure, mock_validate_handler
    ):
...
class TestCreateRdkitMolStrategyRouting(unittest.TestCase):
    ...
    def test_strategy_identifier_coordinate_based_routes_to_inchi(
        ...
        mock_mol = Mock()
        mock_mol.GetNumAtoms.return_value = 3
        mock_mol.GetAtomWithIdx.side_effect = lambda idx: Mock(GetAtomicNum=Mock(return_value=8 if idx == 0 else 1))
        mock_chem.MolFromInchi.return_value = mock_mol
        mock_chem.AddHs.return_value = mock_mol
        mock_chem.Conformer.return_value = Mock()
        mock_chem.SANITIZE_SETAROMATICITY = 1
    ...
    def test_strategy_coordinate_based_uses_rdDetermineBonds(
        self, mock_context, mock_get_constants, mock_determine_bonds, 
        mock_chem, mock_validate_structure, mock_validate_handler
    ):
...
class TestMolToPygData(unittest.TestCase):
    ...
    def test_mol_to_pyg_data_success(self, mock_context, mock_from_rdmol, mock_validate_handler):
        ...
        result = mol_to_pyg_data(rdkit_mol=mock_mol,
            logger=self.logger,
            molecule_index=0,
            handler=self.mock_handler
        )
    ...
    def test_mol_to_pyg_data_none_mol_raises_error(self, mock_validate_handler):
        ...
        with self.assertRaises(vqm_exceptions.HandlerOperationError) as context:
            mol_to_pyg_data(rdkit_mol=None,
                logger=self.logger,
                molecule_index=0,
                handler=self.mock_handler
            )
    ...
    def test_mol_to_pyg_data_conversion_failure(
        ...
        with self.assertRaises(Exception):
            mol_to_pyg_data(rdkit_mol=mock_mol,
                logger=self.logger,
                molecule_index=0,
                handler=self.mock_handler
            )
...
class TestEnrichPygDataFromHandler(unittest.TestCase):
    ...
    def test_enrich_pyg_data_handler_exception_reraises_known_errors(self, mock_validate_handler):
        ...
        self.mock_handler.enrich_pyg_data.side_effect = vqm_exceptions.DatasetSpecificHandlerError(
            message="Handler enrichment failed",
            dataset_type="DFT",
...
class TestCreateMolWithDatasetSupport(unittest.TestCase):
    ...
    def test_create_mol_with_dataset_support_full_pipeline(
        self, mock_context, mock_from_rdmol, mock_chem, mock_validate_structure, mock_validate_handler
    ):
...
class TestValidateHandlerForConversion(unittest.TestCase):
    ...
    def test_validate_handler_valid_with_all_required_methods(self):
        ...
        validate_handler_for_conversion(handler=mock_handler, operation='test_operation', molecule_index=0)
    ...
    def test_validate_handler_none_raises_error(self):
        ...
        with self.assertRaises(vqm_exceptions.HandlerNotAvailableError) as context:
            validate_handler_for_conversion(handler=None, operation='test_operation', molecule_index=0)
...
class TestGetConversionContextInfo(unittest.TestCase):
    ...
    def test_get_conversion_context_info_full(self):
        ...
        context = get_conversion_context_info(handler=mock_handler, molecule_index=42, identifier="test_mol")
    ...
    def test_get_conversion_context_info_minimal(self):
        ...
        context = get_conversion_context_info(handler=mock_handler, molecule_index=0
        , identifier="test_mol")
    ...
    def test_get_conversion_context_info_none_handler(self):
        ...
        context = get_conversion_context_info(handler=None, molecule_index=5
        , identifier="test_mol")
...
class TestApplyHandlerSpecificRdkitProcessing(unittest.TestCase):
    ...
    def test_apply_processing_dft_dataset(self):
        ...
        from milia_pipeline.molecules.mol_conversion_utils import apply_handler_specific_rdkit_processing
        
        result = apply_handler_specific_rdkit_processing(rdkit_mol=mock_mol,
            handler=mock_handler,
            logger=logger,
            molecule_index=0
        )
...
class TestIntegrationScenarios(unittest.TestCase):
    ...
    def test_full_conversion_workflow(
        self, mock_context, mock_from_rdmol, mock_chem,
        mock_validate_atomic, mock_validate_structure, mock_validate_handler
    ):
...
def suite():
    ...
    test_suite.addTests(unittest.TestLoader().loadTestsFromTestCase(TestCreateRdkitMolStrategyRouting))
    ...
    test_suite.addTests(unittest.TestLoader().loadTestsFromTestCase(TestCreateMolWithDatasetSupport))
    test_suite.addTests(unittest.TestLoader().loadTestsFromTestCase(TestValidateHandlerForConversion))
    test_suite.addTests(unittest.TestLoader().loadTestsFromTestCase(TestGetConversionContextInfo))
    test_suite.addTests(unittest.TestLoader().loadTestsFromTestCase(TestApplyHandlerSpecificRdkitProcessing))
    test_suite.addTests(unittest.TestLoader().loadTestsFromTestCase(TestGetHandlerConversionStatistics))
    test_suite.addTests(unittest.TestLoader().loadTestsFromTestCase(TestEnhanceConversionErrorContext))
    test_suite.addTests(unittest.TestLoader().loadTestsFromTestCase(TestValidateConversionPrerequisites))
    test_suite.addTests(unittest.TestLoader().loadTestsFromTestCase(TestIntegrationScenarios))
    test_suite.addTests(unittest.TestLoader().loadTestsFromTestCase(TestEdgeCasesAndBoundaryConditions))
