...
class MinimalDataset:
    ...
    def __init__(
        ...
        for i in range(num_samples):
            ...
            self._data.append(MinimalGraphData(
                num_nodes=num_nodes,
...
class TestCLIArchitectureArguments(unittest.TestCase):
    ...
    def test_architecture_config_argument(self):
        ...
        args = self._parse_only([
            '--train', '--mode', 'custom',
            '--architecture-config', '/path/to/arch.yaml'
        ])
...
class TestConfigurationLoadingWithTraining(unittest.TestCase):
    ...
    def test_args_namespace_has_all_training_attributes(self):
        ...
        expected_attrs = [
            'train', 'hpo', 'mode', 'model_name', 'task_type',
            'epochs', 'batch_size', 'learning_rate', 'checkpoint',
            'evaluate_only', 'n_trials', 'hpo_timeout', 'cv_folds',
            'sampler', 'pruner', 'resume_study', 'hpo_backend',
            'custom_architecture', 'architecture_config', 'builder_type',
            'ensemble', 'ensemble_config', 'ensemble_strategy', 'fusion_method'
        ]
        
        for attr in expected_attrs:
            self.assertTrue(
                hasattr(args, attr),
                f"Args missing expected attribute: {attr}"
            )
...
class TestMinimalFixtures(unittest.TestCase):
    ...
    def test_minimal_dataset_reproducibility(self):
        ...
        for i in range(len(dataset1)):
            self.assertTrue(
                torch.allclose(dataset1[i].x, dataset2[i].x),
                f"Sample {i} node features differ"
            )
...
class TestDataSplitterIntegration(unittest.TestCase):
    ...
    def test_random_split_reproducibility(self):
        ...
        train1, val1, test1 = self.DataSplitter.random_split(
            self.dataset, random_seed=42
        )
        train2, val2, test2 = self.DataSplitter.random_split(
            self.dataset, random_seed=42
        )
    ...
    def test_random_split_different_seeds(self):
        ...
        train1, _, _ = self.DataSplitter.random_split(
            self.dataset, random_seed=42
        )
        train2, _, _ = self.DataSplitter.random_split(
            self.dataset, random_seed=123
        )
...
class TestStandardTrainingFlow(unittest.TestCase):
    ...
    def test_get_optimizer_sgd(self):
        ...
        training_config = {
            'optimizer': {
                'name': 'sgd',
                'params': {'lr': 0.01, 'weight_decay': 0.0001}
            }
        }
...
class TestModelCreationIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_data = MinimalGraphData(
            num_nodes=10,
            num_features=16,
            num_edges=20
        )
...
class TestHPOConfiguration(unittest.TestCase):
    ...
    def test_search_space_param_config_loguniform(self):
        ...
        param = self.SearchSpaceParamConfig(
            type=self.ParamType.LOGUNIFORM,
            low=1e-5,
            high=1e-2
        )
    ...
    def test_search_space_param_config_categorical(self):
        ...
        param = self.SearchSpaceParamConfig(
            type=self.ParamType.CATEGORICAL,
...
class TestHPOManagerIntegration(unittest.TestCase):
    ...
    def test_create_hpo_manager_convenience(self):
        ...
        manager = self.__class__.create_hpo_manager(
            n_trials=5,
            backend='optuna'
        )
...
class TestHPOCallbacks(unittest.TestCase):
    ...
    def test_optuna_pruning_callback_initialization(self, mock_trial_class):
        ...
        callback = self.OptunaPruningCallback(
            trial=mock_trial,
            monitor='val_loss'
        )
...
class TestErrorHandlingIntegration(unittest.TestCase):
    ...
    def test_handle_training_mode_returns_error_code_on_unavailable(self):
        ...
        if not self.models_available:
            result = self.handle_training_mode(
                args, self.mock_logger, self.dataset, config
            )
...
class TestHandleTrainingModeIntegration(unittest.TestCase):
    ...
    def test_handle_training_mode_returns_int(self):
        ...
        if self.models_available:
            ...
            result = self.handle_training_mode(
                args, self.mock_logger, self.dataset, self.config
            )
...
class TestComponentAvailabilitySummary(unittest.TestCase):
    ...
    def test_print_component_availability(self):
        ...
        # Models Module
        try:
            from milia_pipeline.models import ModelFactory, Trainer, DataSplitter
        ...
        # HPO Module
        try:
            from milia_pipeline.models.hpo import HPOManager, HPOConfig, OPTUNA_AVAILABLE
