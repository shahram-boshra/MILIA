#!/usr/bin/env python3
"""
PRODUCTION-READY Unit Test Suite for milia_pipeline/handlers/implementations/ani1ccx.py

Module under test: ani1ccx.py
- ANI1ccxDatasetHandler: Handler for ANI-1ccx quantum chemistry datasets
  ...
  - ANI-1ccx-specific: coordinate_based strategy (NO InChI/SMILES), neutral molecules
    (charge=0), ccsd_energy (CCSD(T)/CBS) as primary target, dft_energy as optional
    secondary, forces/charges/dipole processing, object-array dtype normalization,
    atomization energy calculation (Hartree to eV), Hirshfeld/CM5 charge handling

Test path on local machine: ~/ml_projects/milia/tests/test_handler_impl_ani1ccx_unit.py
Module path on local machine: ~/ml_projects/milia/milia_pipeline/handlers/implementations/ani1ccx.py
...
def _make_dataset_config(**overrides):
    ...
    cfg.dataset_type = overrides.get("dataset_type", "ANI1ccx")
...
def _make_filter_config(**overrides):
    """Build a minimal mock FilterConfig for ANI-1ccx handler tests."""
...
def _make_processing_config(**overrides):
    """Build a minimal mock ProcessingConfig for ANI-1ccx handler tests."""
    cfg = Mock(name="ProcessingConfig")
    cfg.scalar_graph_targets = overrides.get("scalar_graph_targets", ["ccsd_energy"])
...
def _make_handler(**overrides):
    """Build an ANI1ccxDatasetHandler instance with configurable mocked configs."""
...
def _make_pyg_data(**overrides):
    """Build a minimal PyG Data object for ANI-1ccx enrichment tests."""
...
def _make_raw_properties(**overrides):
    """Build a realistic raw_properties_dict for ANI-1ccx molecule tests.

    ANI-1ccx primary energy is ccsd_energy (CCSD(T)/CBS), not 'energy'.
    """
    num_atoms = overrides.get("num_atoms", 5)
    props = {
...
class TestGetRequiredProperties(unittest.TestCase):
    ...
    def test_includes_core_ani1ccx_properties(self):
        """Required properties include ANI-1ccx core: ccsd_energy, atoms, coordinates."""
        handler = _make_handler()
...
class TestGetMolecularCharge(unittest.TestCase):
    ...
    def test_returns_zero_regardless_of_properties(self):
        handler = _make_handler()
        props = {"ccsd_energy": -40.518, "atoms": np.array([6, 1, 1])}
...
class TestValidateMoleculeDataSuccess(unittest.TestCase):
    ...
    @patch("milia_pipeline.handlers.implementations.ani1ccx.validate_molecular_structure")
    def test_positive_ccsd_energy_logs_warning(self, mock_vs):
        """Positive CC energy logs a warning but does not raise."""
        mock_vs.return_value = None
        handler = _make_handler()
        props = _make_raw_properties(ccsd_energy=5.0)
        with self.assertLogs("test.ani1ccx", level="WARNING") as cm:
            handler.validate_molecule_data(props, molecule_index=0, identifier="test")
        self.assertTrue(any("positive CC energy" in msg for msg in cm.output))

    @patch("milia_pipeline.handlers.implementations.ani1ccx.validate_molecular_structure")
    def test_negative_ccsd_energy_no_warning(self, mock_vs):
...
class TestValidateMoleculeDataErrors(unittest.TestCase):
    ...
    def test_missing_ccsd_energy_raises(self):
        handler = _make_handler()
        with self.assertRaises(HandlerValidationError) as ctx:
            handler.validate_molecule_data(_make_raw_properties(ccsd_energy=None), molecule_index=0)
        self.assertIn("ccsd_energy", str(ctx.exception))
...
class TestProcessPropertyValueEnergy(unittest.TestCase):
    ...
    def test_ccsd_energy_float_passthrough(self):
        handler = _make_handler()
        self.assertEqual(handler.process_property_value("ccsd_energy", -40.518, 0), -40.518)

    def test_ccsd_energy_ndarray_scalar_extracted(self):
        handler = _make_handler()
        result = handler.process_property_value("ccsd_energy", np.array(-40.518), 0)
        self.assertIsInstance(result, float)
        self.assertAlmostEqual(result, -40.518, places=3)

    def test_ccsd_energy_nan_float_passes_through(self):
        """Plain float NaN passes through: NaN check is only inside np.ndarray branch."""
        handler = _make_handler()
        result = handler.process_property_value("ccsd_energy", float("nan"), 0)
        # The code only checks NaN when value is np.ndarray, so plain float NaN passes through
        self.assertTrue(math.isnan(result))

    def test_ccsd_energy_nan_ndarray_returns_none(self):
        handler = _make_handler()
        self.assertIsNone(handler.process_property_value("ccsd_energy", np.array(float("nan")), 0))

    def test_dft_energy_float_passthrough(self):
        handler = _make_handler()
        self.assertEqual(handler.process_property_value("dft_energy", -39.123, 0), -39.123)

    def test_dft_energy_ndarray_scalar_extracted(self):
        handler = _make_handler()
        result = handler.process_property_value("dft_energy", np.array(-39.123), 0)
        self.assertIsInstance(result, float)
        self.assertAlmostEqual(result, -39.123, places=3)

    def test_dft_energy_nan_ndarray_returns_none(self):
        handler = _make_handler()
        self.assertIsNone(handler.process_property_value("dft_energy", np.array(float("nan")), 0))
    ...
    def test_none_value_passthrough(self):
        handler = _make_handler()
        self.assertIsNone(handler.process_property_value("ccsd_energy", None, 0))
...
class TestAddScalarTargetsInternal(unittest.TestCase):
    ...
    def test_int_scalar_target(self):
        ...
        handler = _make_handler(processing_config=pc)
        data = _make_pyg_data()
        handler._add_scalar_targets_internal(data, _make_raw_properties(ccsd_energy=-40), 0, "test")
...
class TestEnrichPygData(unittest.TestCase):
    ...
    def test_sets_dataset_type(self):
        ...
        self.assertEqual(result.dataset_type, "ANI1ccx")
...
class TestGetProcessingStatistics(unittest.TestCase):
    ...
    def test_basic_stats_structure(self):
        ...
        self.assertEqual(stats["dataset_type"], "ANI1ccx")
    ...
    def test_experimental_setup_in_stats(self):
        ...
        self.assertEqual(stats["experimental_context"]["dataset_type"], "ANI1ccx")
...
class TestGetTransformRecommendationsPrivate(unittest.TestCase):
    ...
    def test_distance_recommendation_with_graph_nn(self):
        recs = _make_handler()._get_transform_recommendations(["GCNNorm", "GATConv"])
    ...
    def test_empty_transforms_get_all_recommendations(self):
        recs = _make_handler()._get_transform_recommendations([])
        # Empty transforms should produce recommendations for norm + geometric
...
def run_comprehensive_suite():
    ...
    print("")
    print("=" * 80)
    print("PRODUCTION-READY TEST SUITE RESULTS - handlers/implementations/ani1ccx.py")
    print("=" * 80)
