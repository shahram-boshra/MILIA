...
def setup_module(module):
    ...
    spec = importlib.util.spec_from_file_location(
        "milia_pipeline.models.builders.validation",
        validation_path
    )
...
class TestValidateMethod:
    ...
    def test_validate_calls_all_sub_validators(self, simple_layers):
        ...
        with patch.object(validator, 'validate_channel_flow') as mock_channel, \
...
class TestValidateChannelFlow:
    ...
    def test_channel_flow_out_features_vs_out_channels(self):
        ...
        result1 = validator.validate_channel_flow(
            layers=layers1,
            in_channels=16,
            out_channels=1
        )
        ...
        result2 = validator.validate_channel_flow(
            layers=layers2,
            in_channels=16,
            out_channels=1
        )
...
class TestConvenienceFunctions:
    ...
    def test_validate_data_compatibility_function(self, sample_data):
        ...
        result = validate_data_compatibility(
            architecture=architecture,
            sample_data=sample_data
        )
...
class TestEdgeCases:
    ...
    def test_validation_with_unknown_layer_type(self):
        ...
        with pytest.raises(ValueError):
            validator.validate_channel_flow(
                layers=layers,
                in_channels=16,
                out_channels=1
            )
