...
class TestParamTypeEnum:
    ...
    def test_all_param_types_defined(self):
        ...
        expected_types = {'INT', 'FLOAT', 'CATEGORICAL', 'LOGUNIFORM', 
...
class TestPrunerTypeEnum:
    ...
    def test_all_pruner_types_defined(self):
        ...
        expected_types = {'MEDIAN', 'PERCENTILE', 'HYPERBAND', 
                         'SUCCESSIVE_HALVING', 'THRESHOLD', 'PATIENT', 'NONE'}
...
class TestSamplerTypeEnum:
    ...
    def test_all_sampler_types_defined(self):
        ...
        expected_types = {'TPE', 'RANDOM', 'CMAES', 'GRID', 
                         'NSGAII', 'MOTPE', 'QMCSAMPLER'}
...
class TestStudyConfigValidCreation:
    ...
    def test_storage_with_mysql_url(self):
        ...
        assert config.storage == "mysql://user:pass@localhost/db"
    ...
    def test_storage_with_postgresql_url(self):
        ...
        assert config.storage == "postgresql://user:pass@localhost/db"
...
class TestMultiObjectiveStudyConfigValidationErrors:
    ...
    def test_invalid_direction_raises_error(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            MultiObjectiveStudyConfig(
                directions=("minimize", "invalid_direction"),
                metrics=("val_loss", "val_accuracy")
            )
        error_str = str(exc_info.value)
...
class TestEdgeCases:
    ...
    def test_categorical_with_mixed_type_choices(self):
        ...
        config = SearchSpaceParamConfig(
            type=ParamType.CATEGORICAL,
            choices=['string', 123, 0.5, True, None]
        )
...
class TestComplexConfigurations:
        ...
        search_space = {
            'model': {
                'hidden_channels': SearchSpaceParamConfig(
                    type=ParamType.INT, low=32, high=256
                ),
                'num_layers': SearchSpaceParamConfig(
                    type=ParamType.INT, low=1, high=5
                ),
                'dropout': SearchSpaceParamConfig(
                    type=ParamType.FLOAT, low=0.0, high=0.5
                ),
            ...
            'optimizer': {
                'lr': SearchSpaceParamConfig(
                    type=ParamType.LOGUNIFORM, low=1e-5, high=1e-2
                )
            }
        }
    ...
    def test_nested_search_space_categories(self):
        ...
        search_space = {
            'encoder': {
                'hidden': SearchSpaceParamConfig(type=ParamType.INT, low=32, high=128)
            },
            'decoder': {
                'hidden': SearchSpaceParamConfig(type=ParamType.INT, low=32, high=128)
            },
            'regularization': {
                ...
                'weight_decay': SearchSpaceParamConfig(type=ParamType.LOGUNIFORM, low=1e-6, high=1e-3)
            }
        }
...
class TestErrorContextInformation:
    ...
    def test_validation_error_contains_constraint_info(self):
        ...
        assert "optuna" in error_str.lower() or "ray_tune" in error_str.lower() or "backend" in error_str.lower()
    ...
    def test_search_space_error_includes_type_info(self):
        ...
        assert "low" in error_str.lower() or "high" in error_str.lower() or "int" in error_str.lower()
...
class TestBaseModelHashing:
    ...
    def test_configs_can_be_used_in_sets(self, valid_int_param_config):
        ...
        identical = SearchSpaceParamConfig(
            type=ParamType.INT, low=32, high=256, step=32
        )
