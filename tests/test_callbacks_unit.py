...
def mock_trainer():
    ...
    trainer.model.parameters = Mock(return_value=[
        Mock(grad=Mock(data=Mock(norm=Mock(return_value=torch.tensor(1.0)))))
    ])
    ...
    trainer.scheduler.state_dict = Mock(return_value={
        'last_epoch': 0,
        'base_lrs': [0.001]
    })
...
class TestModelCheckpoint:
    ...
    def test_filename_pattern_formatting(self, mock_trainer, temp_checkpoint_dir):
        ...
        assert len(checkpoints) == 1
        assert "model_005_val_loss_0.12.pt" == checkpoints[0].name
    ...
    def test_best_model_path_updated_on_improvement_min_mode(self, mock_trainer, temp_checkpoint_dir):
    ...
    def test_best_model_path_updated_on_improvement_max_mode(self, mock_trainer, temp_checkpoint_dir):
    ...
    def test_best_model_score_not_updated_on_worse_checkpoint(self, mock_trainer, temp_checkpoint_dir):
    ...
    def test_best_model_path_returns_best_pt_when_save_best_true(self, mock_trainer, temp_checkpoint_dir):
    ...
    def test_best_model_path_returns_checkpoint_path_when_save_best_false(self, mock_trainer, temp_checkpoint_dir):
...
class TestTensorBoardLogger:
    ...
    def test_on_train_begin_with_tensorboard_available(self, mock_trainer, temp_tensorboard_dir):
        ...
        with patch.object(tb, '_tensorboard_available', True):
            with patch(
    ...
    def test_on_train_begin_handles_unavailable_tensorboard(self, mock_trainer, temp_tensorboard_dir, caplog):
...
class TestCallbackFactory:
    ...
    def test_from_config_skips_tensorboard_when_unavailable(self, temp_working_dir, caplog):
        ...
        assert "tensorboard not installed" in caplog.text or "Skipping TensorBoardLogger" in caplog.text
    ...
    def test_register_custom_callback_with_path_params(self):
        ...
        CallbackFactory.register_custom_callback(
            'custom_with_path',
            CustomCallback,
            path_params={'output_dir': 'custom_output'}
        )
        
        try:
            assert 'custom_with_path' in CallbackFactory._path_params
            assert CallbackFactory._path_params['custom_with_path'] == {'output_dir': 'custom_output'}
        finally:
            # Cleanup
