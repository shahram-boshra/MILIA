...
class TestDescriptorPerformance:
    ...
    def test_calculator_initialization_options(self, mock_registry, mock_validator):
        ...
        calc_no_cache = DescriptorCalculator(
            registry=mock_registry, validator=mock_validator, enable_cache=False
        )
        assert calc_no_cache.enable_cache is False

        # Test with fallback disabled
        calc_no_fallback = DescriptorCalculator(
            registry=mock_registry, validator=mock_validator, fallback_on_error=False
        )
        ...
        calc_with_conformers = DescriptorCalculator(
            registry=mock_registry, validator=mock_validator, generate_conformers=True
        )
...
class TestDescriptorCalculatorEdgeCases:
    ...
    def test_calculate_single_3d_requirement_without_conformers(
        self, mock_registry, mock_validator
    ):
    ...
    def test_calculate_batch_fallback_disabled_raises_exception(
        self, mock_registry, mock_validator
    ):
...
class TestConformerAndChargeGeneration:
    ...
    def test_compute_charges_adds_gasteiger_charges(self, calculator_with_conformers):
        ...
        has_charges_before = any(atom.HasProp("_GasteigerCharge") for atom in mol.GetAtoms())
        ...
        has_charges_after = any(
            atom.HasProp("_GasteigerCharge") for atom in mol_with_charges.GetAtoms()
        )
