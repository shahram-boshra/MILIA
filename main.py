...
# Handlers
try:
    from milia_pipeline.handlers import create_dataset_handler, DatasetHandler
...
def _get_dataset_config_key(dataset_type: str) -> Optional[str]:
    """
...
def prepare_data_for_task(
    ...
    if not MODELS_TRAINING_AVAILABLE or TaskDataPreparer is None:
        raise RuntimeError(
            "TaskDataPreparer not available. "
            f"Import error: {MODELS_TRAINING_IMPORT_ERROR}"
        )
...
def _discover_and_register_plugins(logger=None):
    ...
    try:
            ...
            for plugin_name in discovered_plugins:
                try:
                    ...
                    if validation_result.get('passed', False):
                        ...
                        logger.warning(f"✗ {plugin_name}: Validation failed - Failed tests: {failed_tests}")
        ...
        if enabled_count > 0:
            ...
            logger.info(f"  Use --list-plugins to see available plugin transforms")
        else:
    ...
    try:
        ...
        for dataset_type in registered_types:
            try:
                ...
                test_handler = create_dataset_handler(
                    test_config, filter_cfg, proc_cfg, logger
                )
...
def validate_transformation_system(logger: logging.Logger, experimental_setup: Optional[str] = None) -> bool:
    """
    ...
    try:
        ...
        try:
            ...
            if isinstance(available_transforms, dict):
                ...
                if total_transforms == 0:
                    ...
                    total_transforms = sum(
                        len(v) for k, v in available_transforms.items() 
                        if isinstance(v, list)
                    )
            else:
                # Old format: might be a list or other structure
                total_transforms = len(available_transforms) if hasattr(available_transforms, '__len__') else 0
            ...
            logger.debug("Transform discovery method not available - continuing with basic validation")
            total_transforms = 0
        ...
        try:
            ...
            has_standard = hasattr(transform_config, 'has_standard_transforms') and transform_config.has_standard_transforms()
            ...
            if not has_experimental and not has_standard:
                ...
                logger.debug("Transform configuration missing 'experimental_setups' and 'standard_transforms' - using legacy format")
            ...
            if hasattr(transform_config, 'default_setup'):
                ...
                if available_setups and default_setup not in available_setups:
                    if has_standard:
                        logger.info(f"Default setup '{default_setup}' not in experimental_setups - will use standard transforms")
                    else:
                        logger.warning(f"Default setup '{default_setup}' not found in available setups")
                else:
...
#---                
                ...
                try:
                    ...
                    if hasattr(gt, 'validate_config'):
                        ...
                        logger.info(f"✅ Experimental setup '{experimental_setup}' validated successfully")
                        ...
                        logger.debug(f"Detailed validation not available for '{experimental_setup}' - using basic checks")
                        logger.info(f"✅ Experimental setup '{experimental_setup}' loaded successfully")
#----                        
                except Exception as setup_error:
                    logger.warning(f"Could not validate experimental setup '{experimental_setup}': {setup_error}")
            ...
            logger.warning("Transformation system validation incomplete - continuing with legacy transforms")
            return True
...
def list_experimental_setups_info(logger: logging.Logger) -> None:
    ...
    try:
        ...
        if hasattr(transform_config, 'has_standard_transforms') and transform_config.has_standard_transforms():
        ...
        setups = list(transform_config.experimental_setups.keys()) if hasattr(transform_config, 'experimental_setups') else []
...
def validate_configuration(
    ...
    try:
        ...
        validate_dataset_specific_configuration(
            config, logger, dataset_config, processing_config
        )
    ...
    try:
        ...
        if not dmc_config:
            raise ConfigurationError(
                message="DMC configuration section missing",
        ...
        if 'scalar_graph_targets' in dmc_properties and 'Etot' not in dmc_properties['scalar_graph_targets']:
...
def validate_dft_configuration(
    ...
    try:
        ...
        if not dft_config:
            raise ConfigurationError(
                message="DFT configuration section missing",
...
def validate_dataset_specific_configuration(
    ...
    if config_key:
        ...
        if not dataset_type_config:
            raise ConfigurationError(
                message=f"{dataset_type} configuration section missing",
...
def _validate_uncertainty_configuration(
    ...
    if uncertainty_config:
        ...
        logger.info(f"  - Uncertainty field: {uncertainty_config.get('uncertainty_field_name', 'std')}")
        logger.info(f"  - Use for loss weighting: {uncertainty_config.get('use_for_loss_weighting', False)}")
        logger.info(f"  - Weighting strategy: {uncertainty_config.get('uncertainty_weighting', 'inverse_variance')}")
        ...
        if max_uncertainty is not None:
            ...
            if max_uncertainty <= 0:
                raise HandlerConfigurationError(
                    ...
                    handler_type=dataset_config.dataset_type,
                    config_validation_errors=[f"max_uncertainty_threshold must be > 0, got {max_uncertainty}"],
...
def validate_common_configuration(
    ...
    if isinstance(transformations, dict) and ('experimental_setups' in transformations or 'standard_transforms' in transformations):
        ...
        for i, transform in enumerate(transformations):
            try:
                ...
                # Handle string-based transform names (rare legacy case)
                    ...
                    logger.warning(f"Transform {i+1}: Unrecognized format {type(transform).__name__}")
...
def print_dataset_info(
    ...
    logger.info(f"Handler pattern support: {'✅“ Available' if HANDLERS_AVAILABLE else 'âœ— Not Available'}")
    ...
    logger.info(f"Transformation system: {'✅ Available' if GRAPH_TRANSFORMS_AVAILABLE else '❌ Not Available'}")
    
    # Plugin System Integration - Plugin system status
    logger.info(f"Plugin system: {'✅ Available' if PLUGIN_SYSTEM_AVAILABLE else '❌ Not Available'}")
    ...
    if PLUGIN_SYSTEM_AVAILABLE:
        try:
            ...
            if plugins_enabled:
                ...
                if enabled_plugins:
                    logger.info(f"Active plugins: {', '.join(enabled_plugins[:3])}" + 
                               (f" (+{len(enabled_plugins)-3} more)" if len(enabled_plugins) > 3 else ""))
            else:
        ...
        try:
            ...
            if CUSTOM_TRANSFORMS_AVAILABLE:
                ...
                if custom_count > 0:
                    ...
                    logger.info(f"Custom transforms: ⚠️  Available but none registered")
            else:
                logger.info(f"Custom transforms: ⚠️  Module not available")
        ...
        try:
            ...
            try:
                ...
                if isinstance(available_transforms, int):
                    ...
                    num_categories = len([k for k, v in available_transforms.items() if isinstance(v, list)])
                    logger.info(f"Available transforms: {total_transforms} across {num_categories} categories")
                    ...
                    logger.info(f"Available transforms: Unable to count (type: {type(available_transforms).__name__})")
            ...
            available_setups = list(transform_config.experimental_setups.keys()) if hasattr(transform_config, 'experimental_setups') else []
            ...
            if experimental_setup:
                ...
                try:
                    ...
                    logger.info(f"  - Combined transforms (standard + experimental): {len(combined_config)}")
    ...
    if variable_props:
        logger.info(f"  Variable-length properties ({len(variable_props)}): {', '.join(variable_props)}")
...
def analyze_dataset_statistics(
    ...
    try:
        if hasattr(dataset, 'get_transform_info'):
            ...
            if transform_info:
                ...
                logger.info(f"Transformation statistics available: transform_count={transform_info.get('transform_count', 0)}")
    except Exception as e:
    ...
    if y_dimensions:
        stats['y_dim'] = y_dimensions[0] if len(set(y_dimensions)) == 1 else f"Variable: {set(y_dimensions)}"
    
    if x_dimensions:
        stats['x_dim'] = x_dimensions[0] if len(set(x_dimensions)) == 1 else f"Variable: {set(x_dimensions)}"
    ...
    logger.info(f"Handler pattern: {'✅“ Used' if stats['handler_pattern_used'] else 'âœ— Legacy fallback'}")
    logger.info(f"Transformation system: {'✅“ Used' if stats['transformation_system_used'] else 'âœ— Legacy fallback'}")
    ...
    if 'transformation_statistics' in stats:
        ...
        # Core statistics
        logger.info(f"  Current experimental setup: {transform_stats.get('current_experimental_setup', 'None')}")
        logger.info(f"  Transform count: {transform_stats.get('transform_count', 0)}")
        logger.info(f"  Transform pipeline active: {transform_stats.get('transform_pipeline_active', False)}")
        
        # Transform details if available
        if 'transforms' in transform_stats:
            logger.info(f"  Transform sequence:")
        ...
        for i in range(num_samples):
            ...
            for attr in handler_attributes:
                if hasattr(data, attr):
                    logger.debug(f"Sample {i}: Handler attribute '{attr}' found: {getattr(data, attr)}")
            
            logger.debug(f"Sample {i}: ✅“ Valid (nodes: {data.num_nodes}, edges: {data.edge_index.shape[1]})")
...
def create_dataset_with_error_handling(
    ...
    try:
        ...
        if len(dataset) > 0:
            setup_info = f" with experimental setup '{experimental_setup}'" if experimental_setup else ""
            logger.info(f"✅“ Dataset created successfully with handler pattern{setup_info} ({len(dataset)} molecules)")
            return dataset
        else:
            # Dataset creation succeeded but no data - this should trigger fallback
            logger.warning(f"Dataset created with handler pattern but is empty (0 molecules)")
        ...
        if experimental_setup:
            ...
            try:
                ...
                if len(dataset) > 0:
                    logger.warning(f"✅“ Dataset created without experimental setup ({len(dataset)} molecules)")
                    return dataset
                else:
                ...
                logger.error(f"Dataset creation without experimental setup failed: {no_setup_error}")
                raise
        else:
...
def handle_predict_mode(
    ...
    try:
        ...
        preds_path_arg = getattr(args, 'preds_path', None) or prediction_config.get('output_path', './predictions.csv')
        ...
        # =================================================================
        batch_size = getattr(args, 'predict_batch_size', None) or prediction_config.get('batch_size', 32)
        device_str = getattr(args, 'predict_device', None) or prediction_config.get('device', 'auto')
        input_format = getattr(args, 'predict_format', None) or prediction_config.get('format', 'auto')
        output_format = getattr(args, 'predict_output_format', None) or prediction_config.get('output_format', 'csv')
        split = getattr(args, 'predict_split', None) or prediction_config.get('split', 'all')
        num_samples = getattr(args, 'predict_num_samples', None) or prediction_config.get('num_samples', None)
        include_inputs = getattr(args, 'predict_include_inputs', False) or prediction_config.get('include_inputs', False)
        uncertainty = getattr(args, 'predict_uncertainty', False) or prediction_config.get('uncertainty', False)
        ...
        if test_path.is_dir():
            ...
            if isinstance(loaded_data, list):
                ...
                if num_graphs is not None and num_graphs > 0:
                    ...
                    raise ValueError(
                        f"Could not determine number of graphs from slices: {slices}"
                    )
        ...
        if data_list:
            ...
            if hasattr(test_sample, 'x') and test_sample.x is not None:
                ...
                logger.info(f"Test data feature dimension: {test_in_channels} (model expects: {model_in_channels})")
...
def _run_standard_training(
    ...
    try:
        ...
        lr = getattr(args, 'learning_rate', None) or training_config.get('optimizer', {}).get('params', {}).get('lr', 0.001)
        ...
        logger.info(f"Task: {task_type}" + (f" (inferred: {inferred_task_type})" if task_type != inferred_task_type else ""))
        ...
        if target_selection_raw is not None and TARGET_SELECTION_AVAILABLE and TargetSelectionConfig is not None:
            target_selection_config = TargetSelectionConfig.from_config(target_selection_raw)
            logger.info(f"Target selection configured: level={target_selection_config.config_level}, source={target_selection_config.config_source}")
        ...
        try:
            ...
            if num_classes_override is not None:
                logger.info(f"Classification: num_classes={num_classes_override} (will be passed to model factory)")
        ...
        if evaluate_only:
            ...
            results = {}
            if hasattr(trainer, 'test') and callable(getattr(trainer, 'test')):
            ...
            if hasattr(trainer, 'validate') and callable(getattr(trainer, 'validate')):
        ...
        if eval_config.get('test_after_training', True):
            ...
            # Otherwise, check if test_loss is already in results from fit()
                ...
                logger.debug("No separate test evaluation method available; test may have been run during fit()")
        ...
        if METRICS_AVAILABLE and hasattr(trainer, 'metrics_history') and trainer.metrics_history:
            try:
                ...
                if viz_enabled:
                    ...
                    logger.info(f"Training visualization saved: {len(saved_plots)} files to {output_dir}")
                    for plot_type, path in saved_plots.items():
                        logger.debug(f"  {plot_type}: {path}")
                else:
                    logger.debug("Visualization disabled in config (evaluation.visualization.enabled: false)")
...
def _run_hpo_training(
    ...
    try:
        ...
        sampler_type = getattr(args, 'sampler', None) or hpo_config_dict.get('sampler', {}).get('type', 'tpe')
        pruner_type = getattr(args, 'pruner', None) or hpo_config_dict.get('pruner', {}).get('type', 'median')
        ...
        if mode == 'ensemble':
            ...
            logger.info(f"Custom architecture config: {len(architecture_config.get('layers', []))} layers")
        ...
        try:
            ...
            logger.warning("HPO results were saved successfully. You can manually retrain with best_params.json")
...
def _get_working_root_dir(config: Dict, logger: logging.Logger) -> Path:
    ...
    """
    working_root_dir: Optional[Path] = None
...
def _get_loss_function(training_config: Dict, task_type: Optional[str] = None):
    """
...
def _get_optimizer(model, training_config: Dict):
    ...
    if not MODELS_TRAINING_AVAILABLE or OptimizerRegistry is None:
        raise RuntimeError(
            "OptimizerRegistry not available. "
            f"Import error: {MODELS_TRAINING_IMPORT_ERROR}"
        )
...
def _get_scheduler(optimizer, training_config: Dict):
    ...
    if not MODELS_TRAINING_AVAILABLE or SchedulerRegistry is None:
        raise RuntimeError(
            "SchedulerRegistry not available. "
            f"Import error: {MODELS_TRAINING_IMPORT_ERROR}"
        )
    ...
    # Registry handles: parameter merging, filtering, validation, instantiation
    return SchedulerRegistry.get_scheduler(
        name=sched_name,
        optimizer=optimizer,
        params=params
    )
...
def _save_training_results(trainer, results: Dict, args, logger, config: Dict):
    ...
    if hasattr(trainer, 'save_results') and callable(getattr(trainer, 'save_results')):
        ...
        logger.warning(
            "Trainer.save_results() not available. Using legacy save logic."
        )
        output_dir.mkdir(parents=True, exist_ok=True)
...
def _save_hpo_results(manager, best_params: Dict, args, logger, config: Dict):
    ...
    if hasattr(manager, 'save_results') and callable(getattr(manager, 'save_results')):
        ...
        logger.warning(
            "HPOManager.save_results() not available. Using legacy save logic."
        )
        import json
    ...
    try:
        ...
        if args.quick_validation:
            success = run_quick_validation(
                str(root_dir), logger, dataset_config, filter_config, 
                processing_config, args.experimental_setup
            )
            sys.exit(0 if success else 1)
        ...
        # Print final summary
        print_final_summary(
            logger, args, dataset, dataset_config, start_time, 
            root_dir, processed_data_path, validate_handlers, validate_transforms
        )
...
def handle_preprocessing_validation(args: argparse.Namespace, logger: logging.Logger) -> int:
    ...
    try:
        ...
        logger.info(f"Configuration built:")
        ...
        preprocessor = preprocessor_class(config, logger)
        logger.info(f"✅ Preprocessor instantiation successful")
...
def handle_transform_validation(
    logger: logging.Logger, 
