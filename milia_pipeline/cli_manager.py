...
_LEGACY_DATASET_TYPES = ['DFT', 'DMC', 'Wavefunction']  # LEGACY - kept for backward compatibility only
...
class CLIManager:
    ...
    def _add_basic_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        basic = parser.add_argument_group(
            'Basic Options',
            'Core dataset processing options'
        )
    ...
    def _add_transformation_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        transforms = parser.add_argument_group(
            'Transformation System',
            'Graph transformation and experimental setup options'
        )
    ...
    def _add_plugin_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        plugins = parser.add_argument_group(
            'Plugin Management',
            'Custom transform plugin operations'
        )
        ...
        plugins.add_argument(
            '--discover-plugins',
            action='store_true',
            help='Discover plugins in registered paths'
        )
        ...
        plugins.add_argument(
            '--validated-only',
            action='store_true',
            help='List/use only validated plugins'
        )
        
        plugins.add_argument(
            '--enabled-only',
            action='store_true',
            help='List/use only enabled plugins'
        )
    ...
    def _add_handler_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        handlers = parser.add_argument_group(
            'Handler System',
            'Handler configuration (required for all operations)'
        )
    ...
    def _add_filter_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        filters = parser.add_argument_group(
            'Molecule Filters',
            'Override filter settings from config.yaml'
        )
    ...
    def _add_validation_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        validation = parser.add_argument_group(
            'Validation & Testing',
            'Configuration and data validation options'
        )
#---        
    ...
    def _add_logging_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        logging_group = parser.add_argument_group(
            'Logging & Output',
            'Control logging verbosity and output files'
        )
    ...
    def _add_advanced_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        advanced = parser.add_argument_group(
            'Advanced Options',
            'Performance tuning and expert configuration'
        )
        ...
        advanced.add_argument(
            '--debug-handlers',
            action='store_true',
            help='Enable detailed handler debugging output'
        )
    ...
    def _add_descriptor_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        descriptor = parser.add_argument_group(
            ...
            'Molecular descriptor calculation and configuration'
        )
        ...
        descriptor.add_argument(
            ...
            nargs='+',
            choices=['constitutional', 'topological', 'electronic', 
    ...
    def _add_training_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        training = parser.add_argument_group(
            'Training System',
            'Model training, HPO, and evaluation options'
        )
        ...
        training.add_argument(
            ...
            type=str,
            default=None,
            help='Resume existing HPO study by name'
        )
    ...
    def _add_prediction_arguments(self, parser: argparse.ArgumentParser) -> None:
        ...
        prediction = parser.add_argument_group(
            'Prediction System',
            'Post-training inference and prediction options (Phase 5b)'
        )
...
For more information, see: https://docs.example.com/milia-cli
    ...
    def _process_arguments(self, args: argparse.Namespace) -> argparse.Namespace:
        ...
        if not any([args.process, args.quick_validation, args.stats_only, args.interactive,
        ...
        if not any([
            args.process, args.quick_validation, args.stats_only,
            args.interactive, args.list_experimental_setups,
            args.validate_transforms_only, args.test_handlers_only,
            args.validate_config, args.list_transforms,
            # Add plugin modes to the check
            args.list_plugins, args.plugin_info, args.validate_plugin,
            args.validate_plugin_comprehensive, args.discover_plugins,
    ...
    def _validate_arguments(self, args: argparse.Namespace) -> None:
        ...
        if args.chunk_size < 100 or args.chunk_size > 50000:
            raise CLIValidationError(
                f"Invalid chunk size: {args.chunk_size}. "
                f"Must be between 100 and 50000."
            )
        
        # Validate test limit
        if args.test_limit is not None and args.test_limit < 1:
            raise CLIValidationError(
                f"Invalid test limit: {args.test_limit}. "
        ...
        if args.no_filters and any([args.max_atoms, args.min_atoms, args.max_uncertainty]):
            active_filters = []
            if args.max_atoms: active_filters.append('--max-atoms')
            if args.min_atoms: active_filters.append('--min-atoms')
            if args.max_uncertainty: active_filters.append('--max-uncertainty')
            raise CLIValidationError(
        ...
        if args.skip_validation:
            ...
            if args.validate_config: conflicting_validation.append('--validate-config')
            if args.validate_handlers: conflicting_validation.append('--validate-handlers')
            if args.validate_transforms_only: conflicting_validation.append('--validate-transforms-only')
        ...
        if hasattr(args, 'validate_preprocessing_only') and args.validate_preprocessing_only:
            ...
            if not (hasattr(args, 'preprocess_config') and args.preprocess_config) and \
               not (hasattr(args, 'preprocess_dataset') and args.preprocess_dataset):
                raise CLIValidationError(
    ...
    def _apply_cli_overrides(self, args: argparse.Namespace) -> None:
        ...
        if hasattr(args, 'preprocess') and args.preprocess:
            ...
            if hasattr(args, 'preprocess_dataset') and args.preprocess_dataset:
                ...
                self.logger.debug(f"CLI override: preprocessing.dataset_type = {args.preprocess_dataset}")
            
            # Override input path
            if hasattr(args, 'preprocess_input') and args.preprocess_input:
                self.config['preprocessing']['input_path'] = args.preprocess_input
                self.logger.debug(f"CLI override: preprocessing.input_path = {args.preprocess_input}")
            
            # Override output path
            if hasattr(args, 'preprocess_output') and args.preprocess_output:
                self.config['preprocessing']['output_path'] = args.preprocess_output
                self.logger.debug(f"CLI override: preprocessing.output_path = {args.preprocess_output}")
            
            # Override num_molecules
            if hasattr(args, 'preprocess_num_molecules') and args.preprocess_num_molecules:
                self.config['preprocessing']['num_molecules'] = args.preprocess_num_molecules
                self.logger.debug(f"CLI override: preprocessing.num_molecules = {args.preprocess_num_molecules}")
            
            # Override feature_tier
            if hasattr(args, 'preprocess_feature_tier') and args.preprocess_feature_tier:
                self.config['preprocessing']['feature_tier'] = args.preprocess_feature_tier
                self.logger.debug(f"CLI override: preprocessing.feature_tier = {args.preprocess_feature_tier}")
            ...
            if hasattr(args, 'preprocess_cleanup'):
                ...
                self.logger.debug(f"CLI override: preprocessing.cleanup_temp = {args.preprocess_cleanup}")
        ...
        if getattr(args, 'train', False):
            ...
            if getattr(args, 'learning_rate', None) is not None:
                self.config['models'].setdefault('training', {}).setdefault('optimizer', {}).setdefault('params', {})['lr'] = args.learning_rate
                self.logger.debug(f"CLI override: models.training.optimizer.params.lr = {args.learning_rate}")
    ...
    def _get_working_root_dir_for_validation(self) -> Path:
        ...
        if working_root_dir is None:
            try:
                ...
                self.logger.debug(f"Using working_root_dir from dataset constants: {working_root_dir}")
            except Exception:
                pass
    ...
    def validate_descriptor_config(self, config: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        if selection_mode == 'category':
            ...
            if not categories:
                issues.append(
                    "selection_mode is 'category' but no categories specified"
                )
        ...
        # Validate explicit mode
            ...
            if not selected:
                issues.append(
                    "selection_mode is 'explicit' but no descriptors specified"
                )
        ...
        if desc_config.get('plugins', {}).get('enabled', False):
            ...
            if not plugin_paths:
                issues.append(
                    "Descriptor plugins enabled but no plugin_paths specified"
                )
        ...
        if batch_size < 1 or batch_size > 10000:
            issues.append(
                f"Invalid batch_size {batch_size}. Must be between 1 and 10000"
            )
    ...
    def run_interactive_mode(self) -> argparse.Namespace:
        ...
        args.root_dir = self._prompt_path(
            "Dataset root directory",
            default=None,
            must_exist=False
        )
        
        args.force_reload = self._prompt_yes_no(
            "Force reprocessing?",
            default=False
        )
        ...
        mode_choice = self._prompt_choice(
        ...
        if TRANSFORMS_AVAILABLE:
            ...
            if not args.disable_transforms:
                ...
                if setups:
                    ...
                    args.experimental_setup = setups[setup_choice] if setup_choice < len(setups) else None
                else:
        ...
        if PLUGIN_SYSTEM_AVAILABLE:
            print("\nPlugin System:")
            use_plugins = self._prompt_yes_no(
                "Enable custom plugin system?",
                default=True
            )
            
            if use_plugins:
                add_plugin_path = self._prompt_yes_no(
                ...
                if add_plugin_path:
                    while True:
                        path = self._prompt_path(
                            "Plugin directory path (empty to finish)",
                            default=None,
                            must_exist=True
                        )
                        if not path:
                ...
                args.discover_plugins = self._prompt_yes_no(
                    "Discover plugins now?",
                    default=False
                )
                
                args.auto_validate = self._prompt_yes_no(
                    "Auto-validate discovered plugins?",
                    default=True
                ) if args.discover_plugins else False
        ...
        # Filters
        print("\nMolecule Filters:")
        use_filters = self._prompt_yes_no(
            "Configure molecule filters?",
            default=True
        )
        
        if use_filters:
            args.max_atoms = self._prompt_int(
                "Maximum atoms per molecule (0 for no limit)",
                default=0,
                min_value=0
            ) or None
            
            args.min_atoms = self._prompt_int(
                "Minimum atoms per molecule (0 for no limit)",
                default=0,
                min_value=0
            ) or None
        else:
        ...
        log_choice = self._prompt_choice(
            "Select log level",
            log_levels,
            default=1
        )
        ...
        if save_log:
            ...
            args.log_file = self._prompt_string(
                "Log file path",
                default=default_log
            )
        else:
    ...
    def print_configuration_summary(self, args: argparse.Namespace) -> None:
        ...
        if PLUGIN_SYSTEM_AVAILABLE and not disable_plugin_system:
            ...
            if discover_plugins:
                ...
                if auto_validate:
                    print(f"  Auto-validation: ✓ Enabled")
        else:
        ...
        if 'preprocessing' in self.config:
            ...
            if 'num_molecules' in preprocessing:
                ...
                self.logger.info(f"  Molecules to process: ALL")
    ...
    def handle_plugin_operations(self, args: argparse.Namespace) -> bool:
        ...
        if args.validate_plugin_comprehensive:
            self._validate_plugin_comprehensive_operation(
                args.validate_plugin_comprehensive, args
            )
            return True
    ...
    def handle_research_api_commands(self, args: argparse.Namespace) -> bool:
        ...
        if hasattr(args, 'run_experiment') and args.run_experiment:
            ...
            try:
                ...
                output_dir = Path(
                    getattr(args, 'experiment_output', './experiments')
                )
                output_dir.mkdir(parents=True, exist_ok=True)
...
""")
    ...
    def _list_available_descriptors(self):
        ...
        try:
            ...
            print(f"  Make sure Phase 1 implementation is complete")
    ...
    def _validate_plugin_comprehensive_operation(
        ...
        try:
            report = PluginValidator.validate_plugin_comprehensive(
                plugin_name,
                run_performance_tests=args.run_performance_tests
            )
    ...
    def _trust_plugin_operation(self, plugin_name: str) -> None:
        ...
        try:
            ...
            self.logger.warning(
                "Trusted plugins bypass security checks. "
...
def create_cli_manager(logger: Optional[logging.Logger] = None) -> CLIManager:
    """
