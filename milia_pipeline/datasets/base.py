...
from pydantic.dataclasses import (
    dataclass,  # Pydantic V2 drop-in replacement with runtime validation
)
...
class BaseDataset(ABC):
    ...
    handler_class: ClassVar[type[DatasetHandlerProtocol] | None] = None
    converter_class: ClassVar[type[DatasetConverterProtocol] | None] = None
    validator_class: ClassVar[type[DatasetValidatorProtocol] | None] = None
    ...
    @classmethod
    def get_config_schema(cls) -> type | None:
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if not isinstance(cls.config_key, str) or not cls.config_key:
            raise TypeError(f"'{cls.__name__}.config_key' must be a non-empty string")
