...
class miliaDataset(InMemoryDataset):
    ...
    def __init__(
        self,
        root: str | None = None,  # Make optional for config_path usage
        logger: logging.Logger | None = None,  # Make optional for config_path usage
        chunk_size: int = 5000,
        transform: Any | None = None,  # Type depends on PyG transform
        pre_filter: Any | None = None,  # Type depends on PyG filter
        force_reload: bool = False,
        dataset_config: DatasetConfig | None = None,
        filter_config: FilterConfig | None = None,
        processing_config: ProcessingConfig | None = None,
        experimental_setup: str | None = None,
        config_path: str | None = None,  # Add this missing parameter
    ):
        ...
        self.raw_npz_download_url: str | None = raw_npz_download_url
        ...
        self.pre_transform_pipeline: Compose | None = self._initialize_pre_transforms(
            experimental_setup=experimental_setup, dataset_config=self._dataset_config
        )
        ...
        if HANDLERS_AVAILABLE:
            ...
            if self._dataset_handler is None:
                self.logger.info(
                    "Handler initialization failed - continuing with enhanced validation mode"
                )
                # _handler_enabled already set to False by _initialize_dataset_handler()
            else:
                self.logger.info(
                    f"Handler successfully initialized for {self._dataset_config.dataset_type}"
                )
            ...
            self.logger.info(
                "Dataset handlers not available - continuing with enhanced validation mode"
            )
            self._handler_enabled = False
        ...
        self.logger.info(
            f"Initializing miliaDataset for {self.dataset_type} with root: {root}, filters: {self.filter_config}, chunk_size: {chunk_size}"
        )
        ...
        if experimental_setup:
            ...
            self.logger.info(
                f"Using default experimental setup with {len(self.pre_transform_pipeline.transforms)} transforms"
            )

            # Log transform sequence for verification
            if transform_count > 0:
                transform_names = [
                    t.__class__.__name__ for t in self.pre_transform_pipeline.transforms
                ]
        ...
        super().__init__(
            root,
            transform,
            pre_transform=self.pre_transform_pipeline,
        ...
        if isinstance(self.slices, dict) and self.slices:
            ...
            self.logger.debug(
                f"Length inferred from first slice key ('{first_slice_key}'): {inferred_len_from_slices}"
            )
        else:
            self.logger.debug(
                "self.slices is not a dictionary or is empty, cannot infer length directly from slices."
            )

        if len(self) == 0:
            self.logger.critical(
                "Dataset is empty after processing/loading! Check errors during processing or if all molecules were filtered out."
            )
            self.logger.warning(
                "Dataset is empty after processing/loading. Refer to ERROR/WARNING logs for details."
            )
        else:
    ...
    def _determine_transform_config_source(
        self,
        experimental_setup: str | None,
        pyg_pre_transforms_config: list[dict[str, Any]] | None,
        ...
        if experimental_setup:
            try:
                ...
                if setup_obj is None:
                    ...
                    if setup_config:
                        self.logger.info(
                            f"Loaded combined transforms for setup '{experimental_setup}' with {len(setup_config)} transforms (standard + experimental)"
                        )
                        return "experimental_setup", setup_config  # GUARANTEED: Always LIST format
                    else:
                        self.logger.warning(
                            f"No transforms returned for setup '{experimental_setup}'"
                        )
                else:
                    ...
                    self.logger.warning(
                        f"Experimental setup '{experimental_setup}' has unexpected format"
                    )

            except Exception as e:
                self.logger.warning(
                    f"Failed to load experimental setup '{experimental_setup}': {e}"
                )
        ...
        try:
            ...
            for i, transform_spec in enumerate(setup_config):
                self.logger.debug(
                    f"Transform {i}: {transform_spec['name']} with {len(transform_spec.get('kwargs', {}))} params"
                )

            self.logger.info(
                f"Loaded default setup '{default_setup}' with {len(setup_config)} combined transforms (standard + experimental)"
            )
        ...
        try:
            ...
            if "transformations" in full_config:
                ...
                if isinstance(transformations_config, dict):
                    ...
                    self.logger.debug(
                        "Found 'transformations' dict in config - attempting legacy extraction"
                    )
                    ...
                    if "experimental_setups" in transformations_config:
                        ...
                        if isinstance(setups, dict) and setups:
                            ...
                            if isinstance(first_setup, dict) and "transforms" in first_setup:
                                ...
                                self.logger.debug(
                                    f"Extracted transforms from setup '{first_setup_name}'"
                                )
            ...
            if legacy_transforms:
                ...
                if not isinstance(legacy_transforms, list):
                    self.logger.warning(
                        f"Legacy transforms config is not a list: {type(legacy_transforms)}"
                    )
                    self.logger.debug(f"Content preview: {str(legacy_transforms)[:200]}")
                    return "none", []

                self.logger.info(
                    f"Using global legacy transform configuration with {len(legacy_transforms)} transform(s)"
                )
    ...
    def _load_and_validate_transform_config(
        self, transform_config: Any, config_source: str, experimental_setup: str | None
    ) -> list[dict[str, Any]]:
        """
        ...
        if config_source in ["experimental_setup", "default_setup"]:
            ...
            self.logger.warning(
                f"Unknown configuration source: {config_source}, disabling transforms"
            )
            return []
    ...
    def _load_and_validate_transform_config_phase2(
        self, transform_config: Any, config_source: str, experimental_setup: str | None
    ) -> list[dict[str, Any]]:
        """
        ...
        if TRANSFORM_VALIDATION_AVAILABLE and self._transform_registry_available:
            validated_config = self._introspect_and_validate_parameters(
                validated_config, experimental_setup
            )
        else:
    ...
    def _introspect_and_validate_parameters(
        self, config: list[dict[str, Any]], experimental_setup: str | None
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            for i, transform_spec in enumerate(config):
                ...
                try:
                    ...
                    self.logger.warning(f"Parameter validation failed for {transform_name}: {e}")
        ...
        for i, transform_spec in enumerate(config):
            if not isinstance(transform_spec, dict):
                validation_errors.append(
                    f"Transform {i}: Must be a dictionary, got {type(transform_spec)}"
                )
                continue
            ...
            if "kwargs" in transform_spec:
                ...
                if not isinstance(kwargs, dict):
                    validation_errors.append(
                        f"Transform {i}: 'kwargs' must be a dictionary, got {type(kwargs)}"
                    )
    ...
    def _convert_legacy_config(self, legacy_config: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for i, transform_spec in enumerate(legacy_config):
            try:
                ...
                if isinstance(transform_spec, str):
                    ...
                    converted_spec = {"name": transform_spec, "kwargs": {}}
        ...
        if converted:
            self.logger.info(
                f"Successfully converted {len(converted)} transform(s) from legacy format"
            )
        else:
    ...
    def _create_transforms_with_new_system(
        self, validated_config: list[dict[str, Any]], experimental_setup: str | None
    ) -> Compose | None:
        """
        ...
        try:
            ...
            if GRAPH_TRANSFORMS_AVAILABLE:
                ...
                if not gt.is_available():
                    self.logger.warning(
                        "Transform system not fully initialized, falling back to legacy"
                    )
                ...
                composed_transforms = gt.create_transform_sequence(
                    validated_config, cache_key=cache_key, validate=True, enable_recovery=True
                )

                self.logger.info(
                    f"Created transform sequence with {len(validated_config)} transforms using enhanced system"
                )
                if experimental_setup:
                    self.logger.info(f"Using experimental setup: {experimental_setup}")
                ...
                self.logger.warning(
                    f"Enhanced transform system not available, using legacy: {GRAPH_TRANSFORMS_IMPORT_ERROR}"
                )
                return self._create_transforms_legacy_fallback(validated_config)
    ...
    def _create_transforms_with_phase2_system(
        self, validated_config: list[dict[str, Any]], experimental_setup: str | None
    ) -> Compose | None:
        """
        ...
        try:
            ...
            if self._transform_registry_available:
                composed_transforms = self._create_from_registry(gt, validated_config, cache_key)
            else:
    ...
    def _create_from_registry(
        self, gt, validated_config: list[dict[str, Any]], cache_key: str
    ) -> Compose:
        """
    ...
    def _validate_cached_sequence(
        self, cached_entry: dict[str, Any], current_config: list[dict[str, Any]]
    ) -> bool:
        """
    ...
    def _create_transforms_legacy_fallback(
        self, validated_config: list[dict[str, Any]]
    ) -> Compose | None:
        """
        ...
        if custom_registry is not None:
            self.logger.debug(
                f"Custom registry available with {len(custom_registry._custom_transforms)} transforms"
            )
            self.logger.debug(
                f"Registered custom transforms: {list(custom_registry._custom_transforms.keys())}"
            )
        else:
            self.logger.warning(
                "No custom registry available - custom transforms will not be found"
            )
            ...
            try:
                ...
                if custom_registry is not None:
                    try:
                        ...
                        self.logger.debug(
                            f"Custom registry lookup failed for '{transform_name}': {e}"
                        )
                ...
                if transform_class is None:
                    creation_errors.append(
                        f"Transform '{transform_name}' not found in custom registry or torch_geometric.transforms"
                    )
                    continue
                ...
                try:
                    ...
                    transforms_list.append(transform_instance)
                    self.logger.debug(
                        f"Legacy creation: {transform_name} with kwargs: {transform_kwargs}"
                    )
                except Exception as e:
        ...
        if creation_errors:
            ...
            if not transforms_list:
                ...
                raise TransformCompositionError(
                    "All legacy transforms failed to create", errors=creation_errors
                )
            else:
    ...
    def _handle_transform_config_error(
        self, error: Exception, config_source: str, experimental_setup: str | None
    ) -> Compose | None:
        """
        ...
        self._processing_statistics["transform_statistics"]["transform_errors"].append(
            {"phase": "configuration", "error": str(error), "context": error_context}
        )

        # Determine if error is recoverable
        if isinstance(
            error, (TransformConfigurationError, TransformValidationError, ExperimentalSetupError)
        ):
    ...
    def _handle_transform_creation_error(
        self,
        error: Exception,
    ...
    def _validate_and_cache_transforms(
        self, composed_transforms: Compose, experimental_setup: str | None
    ) -> None:
        """
        ...
        try:
            ...
            self.logger.info(
                f"Successfully validated transform composition with {transform_count} transforms"
            )
            if experimental_setup:
    ...
    def _validate_and_cache_transforms_phase2(
        self,
        ...
        try:
            ...
            validation_report = self._generate_validation_report_phase2(
                composed_transforms, validated_config, experimental_setup
            )
    ...
    def _generate_validation_report_phase2(
        self,
        composed_transforms: Compose,
        validated_config: list[dict[str, Any]],
        experimental_setup: str | None,
        ...
        try:
            ...
            if GRAPH_TRANSFORMS_AVAILABLE and self._transform_registry_available:
                try:
                    ...
                    report["sequence_validation"] = {"error": str(e), "passed": False}
    ...
    def _generate_recommendations(self, report: dict[str, Any]) -> list[str]:
        ...
        if report.get("transform_count", 0) > 10:
            recommendations.append("⚠️  Large transform sequence detected - consider optimization")
    ...
    def switch_experimental_setup(self, new_setup: str, validate: bool = True) -> bool:
        ...
        try:
            ...
            if validate and TRANSFORM_VALIDATION_AVAILABLE:
                ...
                if not validation_result["valid"]:
                    self.logger.warning(f"Setup validation failed: {validation_result['errors']}")
                    if not validation_result.get("can_proceed", False):
                        return False

            # Create new transforms
            new_transforms = self._initialize_pre_transforms(
                experimental_setup=new_setup, dataset_config=self._dataset_config
            )
            ...
            if new_transforms and TRANSFORM_VALIDATION_AVAILABLE:
                ...
                if transform_config:
                    validation_report = self._generate_validation_report_phase2(
                        new_transforms, transform_config, new_setup
                    )
                    self._transform_validation_results = validation_report
                    self._log_validation_report(validation_report)
            ...
            if new_transforms:
                ...
                self.logger.info(f"✓ Switched to '{new_setup}' with {transform_count} transforms")
            else:
    ...
    def _validate_setup_before_switch(self, setup_name: str) -> dict[str, Any]:
        ...
        """
        result = {"valid": True, "errors": [], "warnings": [], "can_proceed": True}
    ...
    def get_current_experimental_setup(self) -> str | None:
        """
    ...
    def get_transform_validation_report(self) -> dict[str, Any] | None:
        """
    ...
    def validate_transform_configuration(self, config: list[dict[str, Any]]) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if GRAPH_TRANSFORMS_AVAILABLE and self._transform_registry_available:
                try:
                    ...
                    result["sequence_validation"] = {"error": str(e), "passed": False}
    ...
    def _initialize_descriptor_system(self):
        ...
        try:
            ...
            if self._handler_enabled and self._dataset_handler:
                from milia_pipeline.handlers import filter_descriptors_by_handler_support

                registry = DescriptorRegistry.get_instance()
                supported, unsupported = filter_descriptors_by_handler_support(
                    self._dataset_handler, requested_descriptors, registry
                )
            ...
            self._descriptor_calculator = DescriptorCalculator(
                ...
                generate_conformers=desc_config.get("computation", {}).get(
            ...
            self._processing_statistics["descriptor_statistics"]["total_descriptors_expected"] = (
                len(self._selected_descriptors)
            )
    ...
    def _validate_handler_configuration_enhanced(self, handler) -> bool:
        ...
        try:
            ...
            if missing_methods:
                self.logger.warning(f"Handler missing required methods: {missing_methods}")
                return False
            ...
            try:
                ...
                if handler_dataset_type != self._dataset_config.dataset_type:
                    validation_errors.append(
                        f"Dataset type mismatch: {handler_dataset_type} != {self._dataset_config.dataset_type}"
                    )
            except Exception as e:
            ...
            try:
                ...
                if required_properties is None:
                    ...
                    validation_errors.append(
                        f"Required properties must be iterable, got: {type(required_properties)}"
                    )
            except Exception as e:
            ...
            if validation_errors:
                self.logger.warning(f"Handler configuration validation failed: {validation_errors}")
                return False

            self.logger.debug(
                f"Handler configuration validated successfully for {handler.get_dataset_type()}"
            )
            return True
    ...
    def _handle_handler_initialization_error(self, error: Exception) -> None:
        ...
        if is_recoverable_handler_error(error):
            ...
            if suggestions:
                self.logger.info(
                    f"Recovery suggestions for handler error: {suggestions[:3]}"
    ...
    def _collect_handler_statistics(
        self, converter: MoleculeDataConverter, processed_count: int
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if self._dataset_handler and self._dataset_handler != getattr(
                converter, "_dataset_handler", None
            ):
                statistics = self._collect_enhanced_handler_statistics(
            ...
            if (
                not self._handler_enabled
                or not hasattr(converter, "_dataset_handler")
                or not converter._dataset_handler
            ):
    ...
    def _collect_enhanced_handler_statistics(
        self,
        statistics: dict[str, Any],
        handler: Any,
        processed_count: int,
        handler_role: str = "unknown",
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if hasattr(handler, "get_processing_statistics"):
                try:
                    processed_molecules_metadata = self._processing_statistics.get(
                        "processed_molecules_metadata", []
                    )
                    ...
                    statistics = self._extract_handler_insights_enhanced(
                        statistics, handler, handler_stats, handler_role
                    )
                    ...
                    # Track the error
                    self._processing_statistics["handler_error_analysis"][
                        "operation_failures"
                    ].append(error_msg)
            ...
            error_msg = (
                f"Enhanced handler statistics collection failed for {handler_role}: {str(e)}"
            )
            self.logger.debug(error_msg)
            statistics[handler_key] = {"error": error_msg, "role": handler_role, "enabled": False}
            statistics["statistics_source"] = "fallback"

        return statistics

    def _extract_handler_insights_enhanced(
        self,
        statistics: dict[str, Any],
        handler: Any,
        ...
        try:
            ...
            if "uncertainty_statistics" in handler_stats:
                ...
                statistics["uncertainty_enabled"] = (
                    self._dataset_config.is_uncertainty_enabled
                    if hasattr(self, "_dataset_config")
                    else False
                )
            ...
            if "atomization_energy_calculations" in handler_stats:
                statistics["atomization_energy_calculations"] = handler_stats[
                    "atomization_energy_calculations"
                ]
            ...
            if hasattr(handler, "get_dataset_type"):
                try:
                    ...
                    if _get_dataset_feature(dataset_type, "uncertainty_handling"):
                        ...
                        statistics = self._extract_uncertainty_specific_insights(
                    ...
                    if _get_dataset_feature(dataset_type, "vibrational_analysis"):
                        ...
                        statistics = self._extract_vibrational_specific_insights(
                            statistics, handler_stats
                        )

                    if _get_dataset_feature(dataset_type, "orbital_analysis"):
                        statistics["orbital_enabled_specific"] = True
                        statistics = self._extract_orbital_specific_insights(
                            statistics, handler_stats
                        )

                    # Store insight types for reference
                    statistics["applicable_insight_types"] = _get_dataset_specific_insight_types(
                        dataset_type
                    )
    ...
    def _extract_uncertainty_specific_insights(
        self, statistics: dict[str, Any], handler_stats: dict[str, Any]
    ) -> dict[str, Any]:
        """
    ...
    def _extract_vibrational_specific_insights(
        self, statistics: dict[str, Any], handler_stats: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if "vibrational_refinement" in handler_stats:
                ...
                vibrational_insights["vibrational_processing"] = {
                    ...
                    "average_frequency_reduction": vibrational_stats.get(
                        "average_frequency_reduction", 0.0
                    ),
                }
    ...
    def _extract_orbital_specific_insights(
        self, statistics: dict[str, Any], handler_stats: dict[str, Any]
    ) -> dict[str, Any]:
        """
    ...
    def _calculate_error_recovery_effectiveness(
    ...
    def _analyze_error_patterns(
        self, handler_error_analysis: dict[str, list[str]]
    ) -> dict[str, int]:
        """
    ...
    def _log_enhanced_error_analysis(self, error_analysis: dict[str, Any]) -> None:
        ...
        try:
            if error_analysis.get("handler_processing_errors", 0) > 0:
                ...
                self.logger.info(
                    f"  Handler Processing Errors: {error_analysis['handler_processing_errors']}"
                )
                self.logger.info(
                    f"  Enhanced Validation Count: {error_analysis['enhanced_validation_count']}"
                )
                self.logger.info(
                    f"  Handler Success Rate: {error_analysis['handler_success_rate']:.2%}"
                )
                self.logger.info(
                    f"  Recoverable Errors: {error_analysis.get('recoverable_errors', 0)}"
                )
                self.logger.info(
                    f"  Non-Recoverable Errors: {error_analysis.get('non_recoverable_errors', 0)}"
                )
                self.logger.info(
                    f"  Error Recovery Attempts: {error_analysis.get('error_recovery_attempts', 0)}"
                )

                if error_analysis["handler_success_rate"] < 0.9:
                    self.logger.warning(
                        "  ⚠️  Low handler success rate detected - investigate handler errors"
                    )

            if error_analysis.get("initialization_errors", 0) > 0:
                self.logger.warning(
                    f"  ⚠️  Handler initialization errors: {error_analysis['initialization_errors']}"
                )
    ...
    def _log_enhanced_performance_analysis(self, performance_analysis: dict[str, Any]) -> None:
        ...
        try:
            if performance_analysis.get("handler_processing_time", 0.0) > 0:
                ...
                self.logger.info(
                    f"  Handler Processing Time: {performance_analysis['handler_processing_time']:.2f}s"
                )
                self.logger.info(
                    f"  Enhanced Validation Time: {performance_analysis['enhanced_validation_time']:.2f}s"
                )
                self.logger.info(
                    f"  Average Processing Time: {performance_analysis['average_processing_time']:.4f}s"
                )
                self.logger.info(
                    f"  Error Handling Overhead: {performance_analysis.get('error_handling_overhead', 0.0):.2f}s"
                )

                improvement = performance_analysis.get("performance_improvement", {})
                if improvement:
                    self.logger.info(
                        f"  Handler Time Share: {improvement.get('handler_time_percentage', 0):.1f}%"
                    )
                    self.logger.info(
                        f"  Fallback Time Share: {improvement.get('fallback_time_percentage', 0):.1f}%"
                    )
            ...
            if exception_analysis:
                ...
                if recovery_metrics and "recovery_rate" in recovery_metrics:
                    ...
                    if recovery_rate < 0.7:
                        self.logger.warning(
                            "  ⚠️  Low error recovery rate - consider improving error handling"
                        )
                ...
                if error_context and error_context.get("recoverable") is not None:
                    self.logger.info(
                        f"  Handler Errors Recoverable: {error_context.get('recoverable', False)}"
                    )
            ...
            if converter_info.get("enabled", False):
                ...
                if success_rate >= 0.95:
                    ...
                    if recovery_metrics.get("recovery_rate", 0) > 0.8:
                        self.logger.info(
                            f"  Excellent error recovery: {recovery_metrics['recovery_rate']:.1%}"
                        )
                    ...
                    if error_patterns:
                        ...
                        if most_common[0]:
                            self.logger.warning(
                                f"  ⚠️  Most common error: {most_common[0]} ({most_common[1]} occurrences)"
                            )
            else:
                ...
                if self._handler_processing_errors:
                    self.logger.info(
                        f"  ℹ️  Handler disabled due to {len(self._handler_processing_errors)} initialization errors"
                    )
        ...
        try:
            ...
            if statistics.get("uncertainty_enabled_specific", False):
                ...
                if uncertainty_insights and "error" not in uncertainty_insights:
                    ...
                    self.logger.info(
                        f"  Uncertainty Processing: {'Enabled' if uncertainty_insights.get('uncertainty_processing_enabled', False) else 'Disabled'}"
                    )
                    self.logger.info(
                        f"  Uncertainty Count: {uncertainty_insights.get('uncertainty_count', 0)}"
                    )
                    self.logger.info(
                        f"  Mean Uncertainty: {uncertainty_insights.get('mean_uncertainty', 0.0):.6f}"
                    )
                    self.logger.info(
                        f"  High Uncertainty Rate: {uncertainty_insights.get('high_uncertainty_rate', 0.0):.2%}"
                    )

                    outliers = uncertainty_insights.get("statistical_outliers", 0)
                    if outliers > 0:
                        outlier_rate = outliers / max(
                            uncertainty_insights.get("uncertainty_count", 1), 1
                        )
                        self.logger.info(f"  Statistical Outliers: {outliers} ({outlier_rate:.2%})")

                        if outlier_rate > 0.05:  # More than 5% outliers
                            self.logger.warning(
                                "  ⚠️  High outlier rate detected - review uncertainty thresholds"
                            )
            ...
            if statistics.get("vibrational_enabled_specific", False):
                ...
                if vibrational_insights and "error" not in vibrational_insights:
                    ...
                    if vib_processing:
                        self.logger.info(
                            f"  Molecules with Vibrational Refinement: {vib_processing.get('molecules_refined', 0)}"
                        )
    ...
    def _calculate_performance_improvement(
        ...
        try:
            ...
            if total_time > 0:
                ...
                error_overhead_percentage = (
                    (error_overhead / total_time) * 100 if error_overhead > 0 else 0.0
                )

                efficiency_ratio = (
                    handler_time / fallback_time if fallback_time > 0 else float("inf")
                )
    ...
    def _track_molecule_processing(
        self,
        molecule_index: int,
        ...
        try:
            ...
            if used_handler:
                ...
                if pyg_data is not None:
                    ...
                    self._processing_statistics["error_statistics"][
                        "handler_processing_errors"
                    ] += 1
            else:
                self._processing_statistics["error_statistics"]["enhanced_validation_count"] += 1
                self._processing_statistics["performance_metrics"]["enhanced_validation_time"] += (
                    processing_time
                )
            ...
            if self._dataset_handler and pyg_data:
                try:
                    metadata = self._extract_handler_metadata_enhanced(
                        molecule_metadata, pyg_data, raw_properties_dict
                    )
                    molecule_metadata.update(metadata)
                except Exception as e:
                    self.logger.debug(
                        f"Handler metadata extraction failed for molecule {molecule_index}: {e}"
                    )
            ...
            successful_handler_uses = sum(
                1
                for m in self._processing_statistics["processed_molecules_metadata"]
                if m.get("used_handler", False) and m.get("processing_successful", False)
            )

            if total_molecules > 0:
                self._processing_statistics["error_statistics"]["handler_success_rate"] = (
                    successful_handler_uses / total_molecules
                )
                ...
                self._processing_statistics["performance_metrics"]["average_processing_time"] = (
                    total_processing_time / total_molecules
                )

        except Exception as e:
            self.logger.debug(
                f"Enhanced molecule processing tracking failed for molecule {molecule_index}: {e}"
            )

    def _extract_handler_metadata_enhanced(
        self, metadata: dict[str, Any], pyg_data: Data, raw_properties_dict: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if hasattr(self._dataset_handler, "extract_processing_metadata"):
                try:
                    ...
                    self._processing_statistics["handler_error_analysis"][
                        "operation_failures"
                    ].append(f"Metadata extraction failed: {str(e)}")
            else:
                    ...
                    try:
                        ...
                        if _get_dataset_feature(dataset_type, "uncertainty_handling"):
                            additional_metadata.update(
                                self._extract_uncertainty_metadata_fallback_enhanced(pyg_data)
                            )

                        if _get_dataset_feature(dataset_type, "vibrational_analysis"):
                            additional_metadata.update(
                                self._extract_vibrational_metadata_fallback_enhanced(
                                    pyg_data, raw_properties_dict
                                )
                            )

                        if _get_dataset_feature(dataset_type, "orbital_analysis"):
                            additional_metadata.update(
                                self._extract_orbital_metadata_fallback_enhanced(pyg_data)
                            )

                        # Record which features were processed
                        additional_metadata["processed_features"] = (
                            _get_dataset_specific_insight_types(dataset_type)
                        )
            ...
            if hasattr(pyg_data, "uncertainty"):
                try:
                    uncertainty = (
                        ...
                        if hasattr(pyg_data.uncertainty, "item")
                        else float(pyg_data.uncertainty)
                    )
            ...
            if hasattr(pyg_data, "relative_uncertainty"):
                try:
                    rel_uncertainty = (
                        ...
                        if hasattr(pyg_data.relative_uncertainty, "item")
                        else float(pyg_data.relative_uncertainty)
                    )
                    metadata["relative_uncertainty"] = rel_uncertainty
                    metadata["relative_uncertainty_valid"] = is_value_valid_and_not_nan(
                        rel_uncertainty
                    )
                except Exception as e:
    ...
    def _extract_dft_metadata_fallback_enhanced(
        self, pyg_data: Data, raw_properties_dict: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if "freqs" in raw_properties_dict and "vibmodes" in raw_properties_dict:
                try:
                    ...
                    if original_freqs is not None:
                        ...
                        metadata["original_freqs_count"] = (
                            len(original_freqs) if hasattr(original_freqs, "__len__") else 0
                        )
                        metadata["original_freqs_valid"] = is_value_valid_and_not_nan(
                            original_freqs
                        )

                        if hasattr(pyg_data, "freqs"):
                            refined_freqs = pyg_data.freqs
                            metadata["refined_freqs_count"] = (
                                len(refined_freqs) if hasattr(refined_freqs, "__len__") else 0
                            )

                            # Calculate refinement statistics if possible
                            if (
                                metadata["original_freqs_count"] > 0
                                and metadata["refined_freqs_count"] > 0
                            ):
                                reduction = (
                                    metadata["original_freqs_count"]
                                    - metadata["refined_freqs_count"]
            ...
            if hasattr(pyg_data, "atomization_energy") or (
                self._processing_config and self._processing_config.atomization_energy_key_name
            ):
                try:
                    atomization_key = (
                        self._processing_config.atomization_energy_key_name
                        if self._processing_config
                        else "atomization_energy"
                    )
                    if hasattr(pyg_data, atomization_key):
                        ...
                        metadata["atomization_energy_valid"] = is_value_valid_and_not_nan(
                            atomization_value
                        )
                except Exception as e:
    ...
    def _extract_vibrational_metadata_fallback_enhanced(
        self, pyg_data: Data, raw_properties_dict: dict[str, Any]
    ) -> dict[str, Any]:
        """
    ...
    def _extract_orbital_metadata_fallback_enhanced(self, pyg_data: Data) -> dict[str, Any]:
        ...
        try:
            ...
            if hasattr(pyg_data, "homo_lumo_gap"):
                try:
                    gap = (
                        pyg_data.homo_lumo_gap.item()
                        if hasattr(pyg_data.homo_lumo_gap, "item")
                        else float(pyg_data.homo_lumo_gap)
                    )
            ...
            if hasattr(pyg_data, "homo_lumo_gap_eV"):
                try:
                    gap_ev = (
                        pyg_data.homo_lumo_gap_eV.item()
                        if hasattr(pyg_data.homo_lumo_gap_eV, "item")
                        else float(pyg_data.homo_lumo_gap_eV)
                    )
            ...
            if hasattr(pyg_data, "mo_energies"):
                try:
                    ...
                    metadata["mo_energies_count"] = (
                        len(pyg_data.mo_energies) if hasattr(pyg_data.mo_energies, "__len__") else 0
                    )
                except Exception as e:
            ...
            if hasattr(pyg_data, "mo_occupations"):
                try:
                    ...
                    metadata["mo_occupations_count"] = (
                        len(pyg_data.mo_occupations)
                        if hasattr(pyg_data.mo_occupations, "__len__")
                        else 0
                    )
                except Exception as e:
    ...
    def _extract_data_config_from_processing_config(self) -> dict[str, Any]:
        ...
        if self._processing_config:
            data_config["scalar_graph_targets_to_include"] = (
                self._processing_config.scalar_graph_targets
            )
            data_config["node_features_to_add"] = self._processing_config.node_features
            data_config["vector_graph_properties_to_include"] = (
                self._processing_config.vector_graph_properties
            )
            data_config["variable_len_graph_properties_to_include"] = (
                self._processing_config.variable_len_graph_properties
            )
            data_config["calculate_atomization_energy_from"] = (
                self._processing_config.calculate_atomization_energy_from
            )
            data_config["atomization_energy_key_name"] = (
                self._processing_config.atomization_energy_key_name
            )
    ...
    def _initialize_pre_transforms(
        self,
        experimental_setup: str | None = None,
        pyg_pre_transforms_config: list[dict[str, Any]] | None = None,
        dataset_config: DatasetConfig | None = None,
    ) -> Compose | None:
        """
        ...
        if transform_config:
            ...
            self.logger.debug(
                f"Transform config preview: {transform_config[:2] if len(transform_config) > 2 else transform_config}"
            )
        else:
        ...
        try:
            ...
            validated_config = self._load_and_validate_transform_config_phase2(
                transform_config, config_source, experimental_setup
            )
            ...
            if composed_transforms:
                self._validate_and_cache_transforms_phase2(
                    composed_transforms, experimental_setup, validated_config
                )
            ...
            return self._handle_transform_creation_error(
                e, validated_config if "validated_config" in locals() else [], experimental_setup
            )
        except Exception as e:
    ...
    def collate(self, data_list):
        ...
        present_variable_attrs, validation_issues = self._validate_variable_length_data_enhanced(
            data_list, variable_length_attrs
        )
        ...
        if validation_issues:
            self.logger.warning(
                f"Variable-length data validation issues: {len(validation_issues)} found"
            )
            ...
            if self._handler_enabled:
                self._processing_statistics["handler_error_analysis"]["operation_failures"].extend(
                    validation_issues[:3]
                )
        ...
        try:
            ...
            self.logger.debug(
                f"Default collation completed, excluded keys: {present_variable_attrs}"
            )

        except Exception as e:
            # Enhanced error diagnostics with handler integration
            error_context = self._diagnose_collation_error_enhanced(
                data_list, variable_length_attrs, e
            )
        ...
        try:
            ...
            self.logger.debug(
                f"Custom collation completed for: {present_variable_attrs + present_descriptor_attrs}"
            )

        except Exception as e:
            error_context = self._diagnose_variable_length_error_enhanced(
                data_list, present_variable_attrs, e
            )
    ...
    def _validate_variable_length_data_enhanced(self, data_list, variable_length_attrs):
        ...
        if self._handler_enabled and (attr_type_stats or attr_validity_stats):
            self.logger.debug(
                f"Variable-length data statistics: types={attr_type_stats}, validity={attr_validity_stats}"
            )
        ...
        try:
                    ...
                    try:
                        if isinstance(vibmodes, torch.Tensor):
                            ...
                            for vib_tensor in vibmodes:
                                try:
                                    if (
                                        isinstance(vib_tensor, torch.Tensor)
                                        and vib_tensor.numel() > 0
                                    ):
                                    ...
                                    conversion_errors += 1
                                    self.logger.debug(
                                        f"Vibmode tensor conversion failed for molecule {i}: {conv_e}"
                                    )
            ...
            if all_vib_tensors_in_batch:
                try:
                    ...
                    self.logger.debug(
                        f"Successfully padded {len(all_vib_tensors_in_batch)} vibmode tensors"
                    )
                except Exception as e:
            ...
            if conversion_errors > 0:
                self.logger.warning(
                    f"Vibmodes collation: {conversion_errors} conversion errors encountered"
                )
        ...
        try:
                    ...
                    try:
                        if isinstance(freqs, torch.Tensor) and freqs.numel() > 0:
                            ...
                            try:
                                ...
                                conversion_errors += 1
                                self.logger.debug(
                                    f"Freqs tensor conversion failed for molecule {i}: {conv_e}"
                                )
            ...
            if all_freq_tensors_in_batch:
                try:
                    ...
                    self.logger.debug(
                        f"Successfully padded {len(all_freq_tensors_in_batch)} freq tensors"
                    )
                except Exception as e:
            ...
            if conversion_errors > 0:
                self.logger.warning(
                    f"Freqs collation: {conversion_errors} conversion errors encountered"
                )
        ...
        try:
            ...
            if descriptor_vectors:
                ...
                if len(set(descriptor_sizes)) == 1:
                    ...
                    padded_vectors = pad_sequence(
                        descriptor_vectors, batch_first=True, padding_value=0.0
                    )
                ...
                if num_descriptors_list:
                    data_batch.num_descriptors = torch.tensor(
                        num_descriptors_list, dtype=torch.long
                    )
            ...
            if descriptors_list:
                ...
                if len(set(descriptors_sizes)) == 1:
                    ...
                    self.logger.debug(
                        f"Collated 'descriptors' attribute: shape={data_batch.descriptors.shape}"
                    )
                else:
                    # Use padding for variable sizes
                    padded_descriptors = pad_sequence(
                        descriptors_list, batch_first=True, padding_value=0.0
                    )
    ...
    def _diagnose_collation_error_enhanced(self, data_list, variable_length_attrs, error):
        ...
        try:
            ...
            if "'list' object has no attribute 'dim'" in error_str:
                diagnosis.append(
                    "ERROR TYPE: List stored as tensor attribute - tensor conversion required"
                )
            elif "keyerror" in error_str:
                diagnosis.append(
                    f"ERROR TYPE: Missing expected attribute during collation - {error}"
                )
            ...
            if self._handler_enabled:
                diagnosis.append(
                    f"Handler enabled: {self._dataset_handler.get_dataset_type() if self._dataset_handler else 'Unknown'}"
                )
    ...
    def download(self):
        ...
        if self.raw_npz_download_url is None:
            self.logger.info(
                f"No download URL specified for {self.raw_data_filename}. Skipping download."
            )
            return
        ...
        if final_path.exists() and final_path.stat().st_size > 0:
            self.logger.info(
                f"Final file {final_filename} already exists ({final_path.stat().st_size} bytes). Skipping download."
            )
            return
        ...
        if needs_preprocessing:
            ...
            if source_path.exists() and source_path.stat().st_size > 0:
                self.logger.info(
                    f"Source file {source_filename} already exists ({source_path.stat().st_size} bytes)."
                )
            ...
            self.logger.info(
                f"Download complete ({source_path.stat().st_size} bytes). Attempting automatic preprocessing..."
            )
            if self._try_auto_preprocessing(source_path, final_path):
                return
            else:
            ...
            self.logger.info(
                f"Standard download workflow for {dataset_type} dataset: {final_filename}"
            )
    ...
    def _try_auto_preprocessing(self, source_path: Path, target_path: Path) -> bool:
        ...
        try:
            ...
            self.logger.info(
                "To enable automatic preprocessing, ensure Phase 1-3 (preprocessing subsystem) is implemented"
            )
            return False
        ...
        try:
            ...
            if target_path.exists() and target_path.stat().st_size > 0:
                self.logger.info("✅ Automatic preprocessing successful!")
                self.logger.info(
                    f"   Created: {target_path.name} ({target_path.stat().st_size} bytes)"
                )
                return True
            else:
    ...
    def download_file(
        ...
        if destination_path.exists() and destination_path.stat().st_size > 0:
            if logger:
                logger.info(
                    f"File already exists at {destination_path} and is non-empty. Skipping download."
                )
            return
        ...
        if url is None:  # Handle cases where download URL is null for DMC
            if logger:
                logger.info(
                    f"Download URL for {filename} is not specified (null). Skipping download."
                )
            return
            ...
            try:
                ...
                if file_id_match:
                    ...
                    try:
                        ...
                        if logger:
                            logger.warning(
                                f"Figshare API resolution failed: {api_err}. Using original URL."
                            )
        ...
        while retries < max_retries:
            try:
                ...
                with requests.get(
                    actual_url, stream=True, headers=headers, allow_redirects=True, timeout=60
                ) as response:
                    ...
                    with open(destination_path, "ab") as file:
                        try:
                            ...
                            if logger:
                                logger.warning(
                                    "tqdm library not found, progress bar will not be shown. Continuing download without progress bar."
                                )
                            ...
                            raise OSError(f"Error writing downloaded data to file: {e}") from e
                    ...
                    if total_size_in_bytes != 0 and downloaded >= total_size_in_bytes:
                        ...
                        if logger:
                            logger.info(
                                f"Download complete (file might be empty or content-length unknown): {destination_path}"
                            )
                        return
                    else:
                        if logger:
                            logger.warning(
                                f"Download interrupted at {downloaded}/{total_size_in_bytes} bytes. Retrying..."
                            )
                ...
                if retries < max_retries:
                    ...
                    if logger:
                        logger.critical(
                            f"Maximum number of retries reached. Unable to download '{filename}'."
                        )
                    if os.path.exists(destination_path):
            ...
            try:
                ...
                for key in all_required_keys:
                    if key in preloaded_data:
                        ...
                        if self._dataset_handler and hasattr(
                            self._dataset_handler, "process_property_value"
                        ):
                            used_handler = True

                # PHASE 6 FIX PART 3: Add feature_tier to raw_properties_dict for handler tier-awareness
                if "_feature_tier" in preloaded_data:
                    raw_properties_dict_for_current_mol["_feature_tier"] = preloaded_data[
                        "_feature_tier"
                    ]
                ...
                if pyg_data is None:
                    self.logger.warning(
                        f"Molecule {i} (InChI: {original_inchi}) conversion returned None without explicit error. Skipping."
                    )
                    skipped_count += 1
                    continue
                ...
                if self.pre_transform is not None:
                    try:
                        ...
                        self.logger.debug(
                            f"Applied PyG pre_transform to molecule {i} (InChI: {original_inchi})."
                        )
                    except Exception as e:
                ...
                if self._descriptor_enabled and self._descriptor_calculator:
                    try:
                        ...
                        if hasattr(converter, "get_rdkit_mol"):
                            ...
                            self.logger.warning(
                                f"Converter has no rdkit_mol attribute for molecule {i}"
                            )
                            rdkit_mol = None
                        ...
                        if rdkit_mol is not None:
                            ...
                            if i == 0:
                                self.logger.debug(
                                    f"RDKit mol retrieved: type={type(rdkit_mol)}, attempting {len(self._selected_descriptors)} descriptors"
                                )

                            # Calculate descriptors
                            result = self._descriptor_calculator.calculate_batch(
                                rdkit_mol, self._selected_descriptors, original_inchi
                            )
                            ...
                            if result.successful:
                                ...
                                desc_tensor = torch.tensor(
                                    list(result.successful.values()), dtype=torch.float32
                                )
                                ...
                                desc_values = [
                                    result.successful[k] for k in sorted(result.successful.keys())
                                ]
                                pyg_data.descriptors = torch.tensor(
                                    desc_values, dtype=torch.float32
                                )

                                if i == 0:
                                    self.logger.debug("add_descriptors_to_pyg_data completed")
                                ...
                                if result.failed:
                                    ...
                                    self._processing_statistics["descriptor_statistics"][
                                        "molecules_with_incomplete_descriptors"
                                    ] += 1
                                    self._processing_statistics["descriptor_statistics"][
                                        "skipped_due_to_incomplete_descriptors"
                                    ] += 1
                                    self._processing_statistics["descriptor_statistics"][
                                        "incomplete_descriptor_details"
                                    ].append(
                                    ...
                                    self._processing_statistics["descriptor_statistics"][
                                        "molecules_with_complete_descriptors"
                                    ] += 1
                            ...
                            self.logger.warning(
                                f"Molecule {i} ({original_inchi}): RDKit molecule is None - cannot calculate descriptors"
                            )
                        ...
                        self.logger.warning(
                            f"Molecule {i} ({original_inchi}): Descriptor calculation failed with exception: {e}"
                        )
                        if self._descriptor_enabled:
                            self._processing_statistics["descriptor_statistics"][
                                "skipped_due_to_incomplete_descriptors"
                ...
                self._track_molecule_processing(
                    i, raw_properties_dict_for_current_mol, pyg_data, processing_time, used_handler
                )
                ...
                self.logger.info(
                    f"Skipping molecule {e.molecule_index} (InChI: {e.inchi}): {e.reason}"
                )
                skipped_count += 1
                ...
                self.logger.warning(
                    f"Skipping molecule {e.molecule_index} (InChI: {e.inchi}): RDKit Conversion Failed: {e.reason} {e.detail}"
                )
                skipped_count += 1
                recoverable_error = True
                ...
                self.logger.warning(
                    f"Skipping molecule {e.molecule_index} (InChI: {e.inchi}): PyG Data Creation Failed: {e.reason} {e.detail}"
                )
                skipped_count += 1
                recoverable_error = True
                ...
                self.logger.warning(
                    f"Skipping molecule {e.molecule_index} (InChI: {e.inchi}): Property Enrichment Failed for '{e.property_name}': {e.reason} {e.detail}"
                )
                skipped_count += 1
                recoverable_error = True
                ...
                self.logger.warning(
                    f"Skipping molecule {i} (InChI: {original_inchi}): Validation Failed: {e.message}"
                )
                skipped_count += 1
                recoverable_error = True
                ...
                if self._handler_enabled:
                    self._processing_statistics["handler_error_analysis"][
                        "operation_failures"
                    ].append(f"Validation error: {e.message}")

            except HandlerOperationError as e:
                self.logger.warning(
                    f"Skipping molecule {i} (InChI: {original_inchi}): Handler Operation Failed: {e.message}"
                )
                skipped_count += 1
                ...
                self.logger.warning(
                    f"Skipping molecule {e.molecule_index} (InChI: {e.inchi}): {e.reason} {e.details}"
                )
                skipped_count += 1
                recoverable_error = True
                ...
                if recoverable_error:
                    self.logger.warning(
                        f"Recoverable error processing molecule {i} (InChI: {original_inchi}): {e.__class__.__name__} - {e}"
                    )
                    skipped_count += 1
                    ...
                    if recovery_suggestions:
                        self.logger.debug(
                            f"Recovery suggestions: {recovery_suggestions[:2]}"
                    ...
                    self.logger.critical(
                        f"CRITICAL NON-RECOVERABLE ERROR processing molecule {i} (InChI: {original_inchi}): {e.__class__.__name__} - {e}",
                        exc_info=True,
                    )
                    skipped_count += 1
                ...
                if pyg_data is None:
                    self._track_molecule_processing(
                        i,
                        raw_properties_dict_for_current_mol
                        if "raw_properties_dict_for_current_mol" in locals()
                        else {},
                        None,
                        processing_time,
                        used_handler,
    ...
    def create_with_containers(
        ...
        force_reload: bool = False,
        experimental_setup: str | None = None,
    ) -> "miliaDataset":  # Added experimental_setup parameter
        """
    ...
    def _process_property_with_handler(
        self, key: str, value: Any, molecule_index: int, dataset_config: DatasetConfig
    ) -> Any:
        """
        ...
        if self._dataset_handler:
            try:
                ...
                if is_recoverable_handler_error(e):
                    self.logger.debug(
                        f"Recoverable handler property processing failed for {key}: {e}"
                    )
                    recovery_suggestions = get_exception_recovery_suggestions(e)
                    if recovery_suggestions:
                        self.logger.debug(
                            f"Attempting fallback processing due to: {recovery_suggestions[0]}"
                        )
                    return self._enhanced_property_validation(
                        key, value, molecule_index, dataset_config
                    )
                else:
                ...
                try:
                    ...
                    raise PropertyEnrichmentError(
                        ...
                        inchi="N/A",
                        property_name=key,
                        reason="Both handler and fallback processing failed",
    ...
    def _enhanced_property_validation(
        self, key: str, value: Any, molecule_index: int, dataset_config: DatasetConfig
    ) -> Any:
        """
        ...
        try:
            ...
            if self._dataset_handler and hasattr(self._dataset_handler, "get_dataset_type"):
                try:
                    ...
                    if (
                        _get_dataset_feature(dataset_type, "uncertainty_handling")
                        and dataset_config.uncertainty_config
                    ):
            ...
            raise PropertyEnrichmentError(
                ...
                inchi="N/A",
                property_name=key,
                reason="Unexpected error in fallback property processing",
    ...
    def _save_chunk(self, chunk_data: list[Data], chunk_idx: int) -> Path:
        ...
        try:
            self.logger.info(
                f"Saving chunk {chunk_idx} with {len(chunk_data)} molecules to {chunk_path}..."
            )
            ...
            if self._handler_enabled:
                raise HandlerOperationError(
                    message="Handler-enabled dataset chunk save failed",
                    handler_type=self._dataset_config.dataset_type,
                    operation="chunk_save",
    ...
    def _consolidate_chunks(self, chunk_file_paths: list[Path]) -> list[Data]:
            ...
            if chunk_path.exists():
                try:
                    ...
                    if isinstance(chunk_data, list):
                        ...
                        self.logger.debug(
                            f"Successfully loaded chunk {chunk_path} with {len(chunk_data)} molecules"
                        )
                    else:
                        failed_chunks.append(
                            f"{chunk_path}: Invalid chunk data type {type(chunk_data)}"
                        )
                        self.logger.warning(
                            f"Chunk {chunk_path} contains invalid data type: {type(chunk_data)}"
                        )
        ...
        if failed_chunks:
            ...
            if len(failed_chunks) == len(chunk_file_paths):
                ...
                self.logger.warning(
                    f"High chunk failure rate: {len(failed_chunks)}/{len(chunk_file_paths)} chunks failed"
                )

        self.logger.info(
            f"Concatenated {len(all_processed_data)} molecules from {len(chunk_file_paths) - len(failed_chunks)} chunks."
        )
        ...
        if not all_processed_data:
            if self._handler_enabled:
                raise HandlerOperationError(
                    ...
                    operation="chunk_consolidation",
                    recovery_suggestions=[
                        "Check chunk generation process",
                        "Verify molecule processing succeeded",
                    ],
    ...
    def _load_and_prepare_data(
        ...
        try:
            with np.load(raw_npz_path, allow_pickle=True, mmap_mode="r") as data:
                ...
                for count_source in count_sources:
                    if count_source in data.files:
                        try:
                            ...
                            self.logger.debug(
                                f"Total molecule count determined from '{count_source}': {total_molecules}"
                            )
                            break
                        except Exception as e:
                ...
                test_molecule_limit = (
                    processing_config.test_molecule_limit if processing_config else None
                )
                if test_molecule_limit is not None and test_molecule_limit > 0:
                    if test_molecule_limit > total_molecules:
                        self.logger.warning(
                            f"Test limit ({test_molecule_limit}) exceeds available molecules ({total_molecules}), using all available molecules"
                        )
                    else:
                        total_molecules = min(total_molecules, test_molecule_limit)
                        self.logger.warning(
                            f"*** TEMPORARY: Limiting processing to {total_molecules} molecules for testing. Remove 'test_molecule_limit' from config for full dataset processing. ***"
                        )
                ...
                if "metadata" in data.files:
                    try:
                        ...
                        if npz_metadata.shape == (1,) and isinstance(npz_metadata[0], dict):
                            ...
                            metadata_dict = (
                                npz_metadata.item()
                                if hasattr(npz_metadata, "item")
                                else npz_metadata[0]
                            )
                        else:
                            metadata_dict = {}
                        detected_feature_tier = metadata_dict.get("feature_tier")
                ...
                for key in all_required_keys:
                    if key in data.files:
                        try:
                            ...
                            self.logger.debug(
                                f"Successfully loaded key '{key}' with shape {preloaded_data[key].shape}"
                            )
                        except Exception as e:
                        ...
                        if tier_expected_keys is not None:
                            if key not in tier_expected_keys:
                                ...
                                missing_keys.append(
                                    f"{key}: Not in '{detected_feature_tier}' tier (expected)"
                                )
                                self.logger.debug(
                                ...
                                missing_keys.append(
                                    f"{key}: Missing (expected in '{detected_feature_tier}' tier)"
                                )
                                self.logger.warning(
                                    f"Key '{key}' missing from NPZ but expected in '{detected_feature_tier}' tier"
                                )
                        else:
                ...
                if missing_keys:
                    if detected_feature_tier and tier_expected_keys:
                        ...
                        truly_missing = [
                            k for k in missing_keys if "Missing (expected" in k or "Load error" in k
                        ]
                        ...
                        self.logger.warning(
                            f"Missing or failed keys ({len(missing_keys)}): {missing_keys}"
                        )
                    ...
                    try:
                        ...
                        if "metadata" in data.files:
                            try:
                                ...
                                if npz_metadata.shape == (1,) and isinstance(npz_metadata[0], dict):
                                    ...
                                    isinstance(npz_metadata, np.ndarray) and npz_metadata.size == 1
                                ):
                                    metadata_dict = (
                                        npz_metadata.item()
                                        if hasattr(npz_metadata, "item")
                                ...
                                feature_tier = metadata_dict.get("feature_tier")
                                ...
                                if feature_tier:
                                    ...
                                    if feature_tier in FEATURE_TIERS:
                                        ...
                                        try:
                                            id_keys = []
                                            if (
                                                hasattr(self, "_dataset_handler")
                                                and self._dataset_handler is not None
                                                and hasattr(
                                                    self._dataset_handler, "get_identifier_keys"
                                                )
                                            ):
                                                id_key_tuples = (
                                                    self._dataset_handler.get_identifier_keys()
                                                ...
                                                if id_key_tuples:
                                                    id_keys = [
                                                        kt[0]
                                                        for kt in id_key_tuples
                                                        if isinstance(kt, (list, tuple))
                                                        and len(kt) >= 1
                                                    ]
                                            if not id_keys:
                                                from milia_pipeline.config import (
                                                    get_identifier_keys as _get_id_keys,
                                                )

                                                id_key_tuples = _get_id_keys(
                                            ...
                                            self.logger.debug(
                                                f"Could not add identifier keys to critical keys: {id_err}"
                                            )
                                        self.logger.debug(
                                        ...
                                        critical_keys = get_required_properties(
                                            dataset_config.dataset_type
                                        )
                                else:
                                    self.logger.debug(
                        ...
                        self.logger.debug(
                            f"Using critical keys for {dataset_config.dataset_type}: {critical_keys}"
                        )
                    except Exception as e:
                        # Fallback to absolute minimum if configuration lookup fails
                        self.logger.warning(
                            f"Failed to get required properties for {dataset_config.dataset_type}, using fallback: {e}"
                        )
                    ...
                    missing_critical = [
                        key for key in critical_keys if key not in available_npz_keys
                    ]
                        ...
                        if self._handler_enabled:
                            raise HandlerConfigurationError(
                                ...
                                config_validation_errors=[
                                    f"Missing critical keys: {missing_critical}"
                                ],
                ...
                if "metadata" in data.files:
                    try:
                        ...
                        if feature_tier:
                            ...
                            self.logger.info(
                                f"{dataset_config.dataset_type} feature_tier detected: '{feature_tier}'"
                            )
                    except Exception as tier_err:
                        self.logger.debug(
                            f"Could not extract feature_tier from metadata: {tier_err}"
                        )

            self.logger.info(
                f"Successfully loaded {total_molecules} molecules with {len(loaded_keys)} data arrays for {dataset_config.dataset_type} dataset."
            )

            if missing_keys:
                self.logger.info(
                    f"Note: {len(missing_keys)} keys were missing or failed to load, but processing can continue with available data."
                )
            ...
            if self._handler_enabled:
                raise HandlerOperationError(
                    message="Handler-enabled dataset failed to load NPZ data",
                    handler_type=dataset_config.dataset_type,
                    operation="data_loading",
                    recovery_suggestions=[
    ...
    def _determine_required_keys_via_handler_enhanced(
        self, dataset_config: DatasetConfig, processing_config: ProcessingConfig | None
    ) -> list[str]:
        """
        ...
        try:
            ...
            if self._dataset_handler:
                try:
                    ...
                    if isinstance(handler_required_keys, (list, tuple, set)):
                        ...
                        self.logger.debug(
                            f"Handler provided {len(handler_required_keys)} required keys: {handler_required_keys}"
                        )
                    else:
                        raise ValueError(
                            f"Handler returned invalid key type: {type(handler_required_keys)}"
                        )
                    ...
                    try:
                        if hasattr(self._dataset_handler, "get_identifier_keys"):
                            ...
                            if identifier_keys:
                                ...
                                npz_identifier_keys = [
                                    key_tuple[0]
                                    for key_tuple in identifier_keys
                                    if isinstance(key_tuple, (list, tuple)) and len(key_tuple) >= 1
                                ]
                                all_required_keys.extend(npz_identifier_keys)
                                self.logger.debug(
                                    f"Handler provided {len(npz_identifier_keys)} identifier keys: {npz_identifier_keys}"
                                )
                    except Exception as id_err:
                ...
                try:
                    ...
                    try:
                        ...
                        if identifier_keys:
                            ...
                            all_required_keys.extend(npz_identifier_keys)
                            self.logger.debug(
                                f"Fallback: Added {len(npz_identifier_keys)} identifier keys: {npz_identifier_keys}"
                            )
                    except Exception as id_err:
                    ...
                    if processing_config:
                        ...
                        if processing_config.calculate_atomization_energy_from:
                            all_required_keys.append(
                                processing_config.calculate_atomization_energy_from
                            )
                    ...
                    if (
                        ...
                        if uncertainty_field:
                            ...
                            self.logger.debug(
                                f"Added uncertainty field '{uncertainty_field}' to required keys (feature: uncertainty_handling)"
                            )

                    self.logger.debug(
                        f"Fallback determination provided {len(all_required_keys)} required keys"
                    )
            ...
            raise HandlerOperationError(
                message="Unexpected error determining required keys",
                handler_type=dataset_config.dataset_type,
                operation="determine_required_keys",
                recovery_suggestions=[
                    "Use fallback key determination",
    ...
    def _log_processing_results(
        self, total_molecules: int, processed_count: int, skipped_count: int
    ) -> None:
        """
        ...
        """
        self.logger.info("Finished processing loop.")
        ...
        if self._descriptor_enabled:
            ...
            self.logger.info("Descriptor Statistics:")
            ...
            if incomplete > 0:
                ...
                if len(incomplete_details) <= 10:
                    self.logger.info(
                        f"  Incomplete descriptor details: {[(idx, f'{succ}/{exp}') for idx, succ, exp in incomplete_details]}"
                    )
                else:
                    self.logger.info(
                        f"  Incomplete descriptor details (first 10): {[(idx, f'{succ}/{exp}') for idx, succ, exp in incomplete_details[:10]]}"
                    )
        ...
        try:
            ...
            if all_processed_data:
                try:
                    ...
                    self.logger.debug(
                        f"Type of collated_data before saving: {type((data, slices))}"
                    )
                    self.logger.debug(
                        f"Collated data is a tuple (data, slices). Data object type: {type(data)}, Slices object type: {type(slices)}"
                    )
                    ...
                    try:
                        ...
                        if processed_path.exists() and processed_path.stat().st_size > 0:
                            self.logger.debug(
                                f"Validated saved file: {processed_path.stat().st_size} bytes"
                            )
                        else:
                            raise DataProcessingError(
                        ...
                        raise DataProcessingError(
                            message="Permission denied saving final dataset",
                            file_path=str(processed_path),
                        ...
                        raise DataProcessingError(
                            message="OS error saving final dataset",
                            file_path=str(processed_path),
                            operation="final_save",
                    ...
                    if self._handler_enabled:
                        raise HandlerOperationError(
                            ...
                            recovery_suggestions=[
                                ...
                                "Verify collation requirements",
                                "Review variable-length data",
                            ],
                ...
                self.logger.critical(
                    f"{error_msg}. '{PROCESSED_DATA_FILENAME}' will not be created. Check logs for details on skipped molecules."
                )
                ...
                if processed_path.exists():
                    try:
                        ...
                        self.logger.info(
                            f"Removed empty/incomplete processed file: {processed_path}"
                        )
                    except Exception as e:
                        self.logger.warning(
                            f"Could not remove incomplete file {processed_path}: {e}"
                        )
    ...
    def process(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Pre-loading required arrays from {raw_npz_path_for_processing} into memory (mmap_mode='r')..."
            )
            try:
                ...
                if not is_recoverable_handler_error(e):
                    ...
                    try:
                        preloaded_data, total_molecules, all_required_keys = (
            ...
            try:
                ...
                self.logger.debug(
                    f"Processed data chunks will be saved to: {self.processed_chunk_dir}"
                )
            except PermissionError as e:
                raise DataProcessingError(
                    message="Permission denied creating chunk directory",
            ...
            if not self._has_filters_configured():
                self.logger.info(
                    "=== No filters configured in config.yaml. Skipping pre-filtering. ==="
                )
            ...
            if self._descriptor_enabled and self._descriptor_calculator:
                self.logger.info(
                    f"Descriptor calculation active: enabled={self._descriptor_enabled}, calculator={self._descriptor_calculator is not None}, descriptors={len(self._selected_descriptors)}"
                )
                # Log descriptor system confirmation (one-time, not per-molecule)
                self.logger.info(
                    f"Calculation result: successful={len(self._selected_descriptors)}, failed=0"
                )
                self.logger.info(
                    f"Entering add_descriptors block with {len(self._selected_descriptors)} descriptors"
                )
                desc_config = get_descriptor_config()
                output_config = desc_config.get("output", {})
                self.logger.info(
                    f"About to call add_descriptors_to_pyg_data with prefix='{output_config.get('prefix', 'desc_')}'"
                )

            if self.pre_transform is not None:
                transform_names = [t.__class__.__name__ for t in self.pre_transform.transforms]
                self.logger.info(
                    f"Deploying transforms to molecules: {' → '.join(transform_names)}"
                )
            ...
            for i in tqdm(
                range(total_molecules),
                desc=f"Processing {self._dataset_config.dataset_type} Molecules",
            ):
                try:
            ...
            self._log_processing_results(
                total_molecules, processed_total_count, skipped_total_count
            )
            ...
            if self.pre_transform_pipeline:
                ...
                if processed_total_count > 0:
                    self.logger.info(
                        f"All {processed_total_count} molecules successfully transformed"
                    )
            ...
            if processing_errors:
                self.logger.warning(
                    f"Processing completed with {len(processing_errors)} recoverable errors"
                )
            ...
            try:
                processing_statistics = self._collect_handler_statistics(
                    converter, processed_total_count
                )
            ...
            self._log_enhanced_processing_completion(
                processing_statistics if "processing_statistics" in locals() else {}
            )
    ...
    def _log_enhanced_processing_start(self) -> None:
        ...
        if self._handler_enabled and self._dataset_handler:
            try:
                ...
                self.logger.info(
                    f"=== ENHANCED PROCESSING START ({self._dataset_config.dataset_type}) ==="
                )
                self.logger.info(
                    f"Processing {self._dataset_config.dataset_type} dataset with {handler_type} handler"
                )
                ...
                if not self._handler_processing_errors:
                    ...
                    self.logger.warning(
                        f"⚠️  Handler configuration issues detected: {len(self._handler_processing_errors)} errors"
                    )
            ...
            self.logger.info(
                f"=== LEGACY PROCESSING START ({self._dataset_config.dataset_type}) ==="
            )
            self.logger.info(
                f"Processing {self._dataset_config.dataset_type} dataset in legacy mode (handlers disabled)"
            )
            if self._handler_processing_errors:
                self.logger.info(
                    f"Handler errors that led to legacy mode: {len(self._handler_processing_errors)} errors"
                )
    ...
    def _log_enhanced_processing_completion(self, processing_statistics: dict[str, Any]) -> None:
        ...
        if self._handler_enabled and processing_statistics:
            ...
            handler_type = processing_statistics.get("converter_handler_info", {}).get(
                "type", "Unknown"
            )

            processed_count = processing_statistics.get("total_processed", 0)
            self.logger.info(
                f"Successfully processed {processed_count} molecules using {handler_type} dataset handlers"
            )
            ...
            processed_count = (
                processing_statistics.get("total_processed", 0) if processing_statistics else 0
            )
            self.logger.info(
                f"Successfully processed {processed_count} molecules using legacy processing mode"
            )
            if self._handler_processing_errors:
                self.logger.info(
                    f"ℹ️  Legacy mode used due to: {len(self._handler_processing_errors)} handler initialization issues"
                )

    def _create_molecule_converter_enhanced(
        ...
        try:
            ...
            self.logger.debug(
                f"Created MoleculeDataConverter for {dataset_config.dataset_type} dataset with configuration containers"
            )
    ...
    def get_handler_info(self) -> dict[str, Any]:
        ...
        handler_info = {
            ...
            "error_summary": {
                ...
                "processing_error_count": self._processing_statistics.get(
                    "error_statistics", {}
                ).get("handler_processing_errors", 0),
                "fallback_usage_count": self._processing_statistics.get("error_statistics", {}).get(
                    "fallback_processing_count", 0
                ),
                "recoverable_error_count": self._processing_statistics.get(
                    "error_statistics", {}
                ).get("recoverable_errors", 0),
                "non_recoverable_error_count": self._processing_statistics.get(
                    "error_statistics", {}
                ).get("non_recoverable_errors", 0),
            },
        ...
        if self._dataset_handler:
            try:
                ...
                try:
                    validate_dataset_handler_compatibility(
                        self._dataset_handler, self._dataset_config
                    )
    ...
    def get_processing_summary(self) -> dict[str, Any]:
        ...
        if self._descriptor_enabled and self._descriptor_calculator:
            ...
            summary["descriptor_statistics"] = {"enabled": False}
