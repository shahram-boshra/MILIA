...
class HandlerLoggerAdapter(logging.LoggerAdapter):
    ...
    def __init__(
        self, logger: logging.Logger, handler_type: str, extra: dict[str, Any] | None = None
    ):
        """
...
class MigrationLoggerAdapter(logging.LoggerAdapter):
    ...
    def __init__(
        self, logger: logging.Logger, migration_phase: str, extra: dict[str, Any] | None = None
    ):
        """
...
class TransformLoggerAdapter(logging.LoggerAdapter):
    ...
    def __init__(
        ...
        transform_context: str | None = None,
    ...
    if not logger.handlers:
        try:
            ...
            try:
                ...
                log_msg = (
                    f"Logging initialized. Output will be written to console and '{log_file_path}'"
                )
...
def _configure_third_party_loggers() -> None:
    ...
    verbose_loggers = ["matplotlib", "PIL", "urllib3", "requests", "torch", "torch_geometric"]
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if molecule_index is not None:
                logger = HandlerLoggerAdapter(
                    logger.logger, handler_type, {"molecule_index": molecule_index}
                )
            ...
            try:
                ...
                execution_time = (datetime.now() - start_time).total_seconds()
                logger.error(
                    f"Unexpected error in {operation_name} after {execution_time:.3f}s: {e}",
                    exc_info=True,
                )
                raise
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                execution_time = (datetime.now() - start_time).total_seconds()
                logger.error(
                    f"Unexpected error in migration step {step_name} after {execution_time:.3f}s: {e}",
                    exc_info=True,
                )
                raise
...
def log_transform_operation(
    operation_name: str, experimental_setup: str | None = None, transform_context: str | None = None
):
    """
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            logger = create_transform_logger(
                experimental_setup=setup, transform_context=transform_context or operation_name
            )
            ...
            if transform_name:
                logger = TransformLoggerAdapter(
                    logger.logger, setup, transform_context, {"transform_name": transform_name}
                )
            ...
            try:
                ...
                execution_time = (datetime.now() - start_time).total_seconds()
                logger.error(
                    f"Transform error in {operation_name} after {execution_time:.3f}s: {e}"
                )
                raise
...
def log_handler_performance(
    ...
    metrics = {
        ...
        "success_rate": round((molecule_count - error_count) / molecule_count * 100, 2)
        if molecule_count > 0
        else 0,
    }
...
def log_transform_performance(
    transform_logger: logging.Logger,
    operation: str,
    transform_count: int,
    execution_time: float,
    error_count: int = 0,
    ...
    metrics = {
        ...
        "execution_time_sec": round(execution_time, 3),
        "transforms_per_sec": round(transform_count / execution_time, 2)
        if execution_time > 0
        else 0,
        "error_count": error_count,
        "success_rate": round((transform_count - error_count) / transform_count * 100, 2)
...
def log_experimental_setup_switch(
    logger: logging.Logger,
    from_setup: str | None,
    to_setup: str,
    success: bool,
...
def log_transform_validation_results(
    logger: logging.Logger,
    transform_name: str,
    ...
    if validation_results.get("valid", False):
        ...
        if warnings:
            for warning in warnings:
                logger.warning(
                    f"{setup_prefix}Transform '{transform_name}' validation warning: {warning}"
                )
    else:
...
def log_transform_composition_summary(
    logger: logging.Logger,
    ...
    logger.info(
        f"{setup_prefix}Transform composition completed: {transform_count} transforms in {composition_time:.3f}s (cache: {'HIT' if cache_hit else 'MISS'})"
    )
    ...
    if transform_count > 0 and composition_time > 0:
        ...
        logger.debug(
            f"{setup_prefix}Composition performance: {transforms_per_sec:.2f} transforms/sec"
        )
...
def configure_debug_logging_for_handlers() -> None:
    ...
    handler_logger_names = ["handler.dft", "handler.dmc", "handler.generic"]
...
def log_experimental_setup_summary(
    logger: logging.Logger,
    setup_name: str,
...
def setup_basic_logging() -> logging.Logger:
    ...
    return setup_logging(
        enable_handler_logging=False, enable_migration_logging=False, enable_transform_logging=False
    )
