"""
Graph Sparsification & Coarsening Plugin (catalogue Family F.a/F.b, in-scope subset).

Eight unary graph-reduction transforms (NetworkX + NumPy/SciPy; no new
...
PLUGIN_METADATA = {
    ...
    "description": "Graph sparsification and coarsening (Family F.a/F.b, in-scope subset).",
    "license": "MIT",
...
logger = logging.getLogger("milia_Main.PluginSystem.graph_reduction")
...
def _lazy_import_transforms() -> bool:
    ...
    try:
        ...
        logger.debug("Loaded graph_reduction transform classes")
        return True
    except Exception as exc:  # noqa: BLE001
        logger.error(f"Failed to import graph_reduction transforms: {exc}")
        return False
...
def _check_dependencies() -> tuple[bool, list[str]]:
    ...
    missing: list[str] = []
    for module_name in ("networkx", "numpy", "torch", "torch_geometric"):
        try:
...
def enable_plugin() -> bool:
    ...
    logger.info(f"Plugin 'graph_reduction' v{__version__} enabled")
    return True
...
def disable_plugin() -> bool:
    ...
    _plugin_enabled = False
    logger.info("Plugin 'graph_reduction' disabled")
    return True
...
try:
    if not enable_plugin():
        logger.warning("graph_reduction auto-enable encountered issues; continuing")
except Exception as exc:  # noqa: BLE001
    logger.error(f"graph_reduction auto-enable failed: {exc}")
