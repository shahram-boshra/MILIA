"""
Curvature & Differential-Geometric Rewiring Plugin (catalogue Family B, in-scope subset).

Four unary curvature-rewiring transforms (NetworkX + NumPy/SciPy; no new
...
PLUGIN_METADATA = {
    "name": "curvature_rewiring",
    "version": __version__,
    "author": __author__,
    "plugin_type": "user_experimental",
    "description": "Curvature and differential-geometric rewiring (Family B, in-scope subset).",
    "license": "MIT",
...
logger = logging.getLogger("milia_Main.PluginSystem.curvature_rewiring")
...
def _lazy_import_transforms() -> bool:
    ...
    try:
        ...
        logger.debug("Loaded curvature_rewiring transform classes")
        return True
    except Exception as exc:  # noqa: BLE001
        logger.error(f"Failed to import curvature_rewiring transforms: {exc}")
        return False
...
def enable_plugin() -> bool:
    ...
    logger.info(f"Plugin 'curvature_rewiring' v{__version__} enabled")
    return True
...
def disable_plugin() -> bool:
    ...
    _plugin_enabled = False
    logger.info("Plugin 'curvature_rewiring' disabled")
    return True
...
try:
    if not enable_plugin():
        logger.warning("curvature_rewiring auto-enable encountered issues; continuing")
except Exception as exc:  # noqa: BLE001
    logger.error(f"curvature_rewiring auto-enable failed: {exc}")
