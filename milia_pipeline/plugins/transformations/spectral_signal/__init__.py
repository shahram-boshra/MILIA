"""
Spectral & Signal-Decomposition Plugin (catalogue Family E, in-scope subset).

Six unary spectral node-descriptor transforms (NetworkX + NumPy/SciPy; no new
...
PLUGIN_METADATA = {
    "name": "spectral_signal",
    "version": __version__,
    "author": __author__,
    "plugin_type": "user_experimental",
    "description": "Spectral and signal-decomposition node descriptors (Family E, in-scope subset).",
    "license": "MIT",
...
logger = logging.getLogger("milia_Main.PluginSystem.spectral_signal")
...
def _lazy_import_transforms() -> bool:
    ...
    try:
        ...
        logger.debug("Loaded spectral_signal transform classes")
        return True
    except Exception as exc:  # noqa: BLE001
        logger.error(f"Failed to import spectral_signal transforms: {exc}")
        return False
...
def enable_plugin() -> bool:
    ...
    logger.info(f"Plugin 'spectral_signal' v{__version__} enabled")
    return True
...
def disable_plugin() -> bool:
    ...
    _plugin_enabled = False
    logger.info("Plugin 'spectral_signal' disabled")
    return True
...
try:
    if not enable_plugin():
        logger.warning("spectral_signal auto-enable encountered issues; continuing")
except Exception as exc:  # noqa: BLE001
    logger.error(f"spectral_signal auto-enable failed: {exc}")
