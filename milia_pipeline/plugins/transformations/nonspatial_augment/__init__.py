"""
Strict Non-Spatial Augmentations Plugin (catalogue Family G, in-scope subset).

Six unary non-spatial augmentation transforms (NetworkX + NumPy/SciPy; no new
...
PLUGIN_METADATA = {
    "name": "nonspatial_augment",
    "version": __version__,
    "author": __author__,
    "plugin_type": "user_experimental",
    "description": "Strict non-spatial graph augmentations (Family G, in-scope subset).",
    "license": "MIT",
...
logger = logging.getLogger("milia_Main.PluginSystem.nonspatial_augment")
...
def _lazy_import_transforms() -> bool:
    ...
    try:
        ...
        logger.debug("Loaded nonspatial_augment transform classes")
        return True
    except Exception as exc:  # noqa: BLE001
        logger.error(f"Failed to import nonspatial_augment transforms: {exc}")
        return False
...
def _check_dependencies() -> tuple[bool, list[str]]:
    ...
    missing: list[str] = []
    for module_name in ("networkx", "numpy", "scipy", "torch_geometric"):
        try:
...
def enable_plugin() -> bool:
    ...
    logger.info(f"Plugin 'nonspatial_augment' v{__version__} enabled")
    return True
...
def disable_plugin() -> bool:
    ...
    _plugin_enabled = False
    logger.info("Plugin 'nonspatial_augment' disabled")
    return True
...
try:
    if not enable_plugin():
        logger.warning("nonspatial_augment auto-enable encountered issues; continuing")
except Exception as exc:  # noqa: BLE001
    logger.error(f"nonspatial_augment auto-enable failed: {exc}")
