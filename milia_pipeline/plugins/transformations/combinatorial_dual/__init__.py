"""
Combinatorial & Dual Operators Plugin (catalogue Family D).

Thirteen unary combinatorial/dual graph transforms (NetworkX + NumPy/SciPy; no new
...
PLUGIN_METADATA = {
    "name": "combinatorial_dual",
    "version": __version__,
    "author": __author__,
    "plugin_type": "user_experimental",
    "description": "Combinatorial and dual graph operators (Family D).",
    "license": "MIT",
...
logger = logging.getLogger("milia_Main.PluginSystem.combinatorial_dual")
...
def _lazy_import_transforms() -> bool:
    ...
    try:
        ...
        logger.debug("Loaded combinatorial_dual transform classes")
        return True
    except Exception as exc:  # noqa: BLE001
        logger.error(f"Failed to import combinatorial_dual transforms: {exc}")
        return False
...
def enable_plugin() -> bool:
    ...
    logger.info(f"Plugin 'combinatorial_dual' v{__version__} enabled")
    return True
...
def disable_plugin() -> bool:
    ...
    _plugin_enabled = False
    logger.info("Plugin 'combinatorial_dual' disabled")
    return True
...
try:
    if not enable_plugin():
        logger.warning("combinatorial_dual auto-enable encountered issues; continuing")
except Exception as exc:  # noqa: BLE001
    logger.error(f"combinatorial_dual auto-enable failed: {exc}")
