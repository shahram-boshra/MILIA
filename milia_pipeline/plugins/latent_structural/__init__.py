"""
Latent-Space & Self-Supervised Structural Plugin (catalogue Family J, deterministic subset).

Three unary latent-structural transforms (NetworkX + NumPy/SciPy; no new
...
PLUGIN_METADATA = {
    "name": "latent_structural",
    "version": __version__,
    "author": __author__,
    "plugin_type": "user_experimental",
    "description": "Latent-space and self-supervised structural transforms (Family J, deterministic subset).",
    "license": "MIT",
...
logger = logging.getLogger("milia_Main.PluginSystem.latent_structural")
...
def _lazy_import_transforms() -> bool:
    """Lazily import the transform classes."""
    ...
    try:
        ...
        logger.debug("Loaded latent_structural transform classes")
        return True
    except Exception as exc:  # noqa: BLE001
        logger.error(f"Failed to import latent_structural transforms: {exc}")
        return False
...
def _check_dependencies() -> tuple[bool, list[str]]:
    ...
    missing: list[str] = []
    for module_name in ("networkx", "torch", "torch_geometric"):
        try:
...
def _register_transforms() -> bool:
    """Register transform classes with the TransformRegistry (discovery does PluginRegistry)."""
...
def enable_plugin() -> bool:
    ...
    logger.info(f"Plugin 'latent_structural' v{__version__} enabled")
    return True
...
def disable_plugin() -> bool:
    ...
    _plugin_enabled = False
    logger.info("Plugin 'latent_structural' disabled")
    return True
...
try:
    if not enable_plugin():
        logger.warning("latent_structural auto-enable encountered issues; continuing")
except Exception as exc:  # noqa: BLE001
    logger.error(f"latent_structural auto-enable failed: {exc}")
