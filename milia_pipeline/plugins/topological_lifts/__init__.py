"""
Topological & Higher-Order Lifts Plugin (catalogue Family A, zero-dependency subset).

Three unary topological-lift transforms (NetworkX + NumPy/SciPy; no new
...
PLUGIN_METADATA = {
    "name": "topological_lifts",
    "version": __version__,
    "author": __author__,
    "plugin_type": "user_experimental",
    "description": "Topological and higher-order lifts (Family A, zero-dependency subset).",
    "license": "MIT",
...
logger = logging.getLogger("milia_Main.PluginSystem.topological_lifts")
...
def _lazy_import_transforms() -> bool:
    ...
    try:
        ...
        logger.debug("Loaded topological_lifts transform classes")
        return True
    except Exception as exc:  # noqa: BLE001
        logger.error(f"Failed to import topological_lifts transforms: {exc}")
        return False
...
def enable_plugin() -> bool:
    ...
    logger.info(f"Plugin 'topological_lifts' v{__version__} enabled")
    return True
...
def disable_plugin() -> bool:
    ...
    _plugin_enabled = False
    logger.info("Plugin 'topological_lifts' disabled")
    return True
...
try:
    if not enable_plugin():
        logger.warning("topological_lifts auto-enable encountered issues; continuing")
except Exception as exc:  # noqa: BLE001
    logger.error(f"topological_lifts auto-enable failed: {exc}")
