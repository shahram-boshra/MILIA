...
class QDPiDatasetHandler(DatasetHandler):
        ...
        try:
            ...
            if missing_props:
                raise HandlerValidationError(
                    ...
                    details="QDπ molecules must have energy, atoms, and coordinates",
                )
    ...
    def get_molecular_charge(
        ...
        charge_type = raw_properties_dict.get("charge_type")
    ...
    def _infer_charge_from_elements(
        self, atomic_numbers: np.ndarray, mol_identifier: str | None
    ) -> int:
        """
    ...
    def enrich_pyg_data(
        ...
        try:
            ...
            charge = raw_properties_dict.get(
                "molecular_charge", raw_properties_dict.get("charge", 0)
            )
    ...
    def _validate_dataset_specific_transforms(self, transform_names: list[str]) -> list[str]:
        ...
        if not has_geometric:
            warnings.append(
                "QDπ dataset without geometric augmentation - consider adding RandomRotate for invariance"
            )
