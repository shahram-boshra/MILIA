...
class ANI2xDatasetHandler(DatasetHandler):
        ...
        try:
            ...
            if missing_props:
                raise HandlerValidationError(
                    ...
                    details="ANI-2x molecules must have energy, atoms, and coordinates",
                )
        ...
        try:
            ...
            if key == "forces":
                ...
                if not np.all(np.isfinite(arr)):
                    self.logger.warning(
                        f"ANI-2x molecule {molecule_index} has non-finite forces data"
                    )
                    return None
                return arr
        ...
        try:
            for prop_key in self.processing_config.vector_graph_properties:
                try:
                    ...
                    if not is_value_valid_and_not_nan(value):
                        self.logger.debug(
                            f"ANI-2x molecule {molecule_index}: Skipping vector property '{prop_key}' - not available"
                        )
                        continue
        ...
        try:
            ...
            for key in self.processing_config.variable_len_graph_properties:
                try:
                    ...
                    if not is_value_valid_and_not_nan(value):
                        self.logger.debug(
                            f"ANI-2x molecule {molecule_index}: Skipping variable-length property '{key}' - not available"
                        )
                        continue
    ...
    def _calculate_atomization_energy_internal(
        ...
        try:
            ...
            if not is_value_valid_and_not_nan(base_energy_raw):
                self.logger.warning(
                    f"ANI-2x molecule {molecule_index} missing {base_energy_key} for atomization energy"
                )
                return None

            if not hasattr(pyg_data, "z") or pyg_data.z is None:
                self.logger.warning(
                    f"ANI-2x molecule {molecule_index} missing atomic numbers for atomization energy"
                )
                return None
            ...
            if isinstance(base_energy_raw, np.ndarray) and base_energy_raw.size == 1:
                ...
                self.logger.warning(
                    f"ANI-2x molecule {molecule_index}: Cannot convert {base_energy_key} to float"
                )
                return None
            ...
            if HAR2EV is None or not ATOMIC_ENERGIES_HARTREE:
                self.logger.warning(
                    f"ANI-2x molecule {molecule_index}: Missing atomic energies for atomization calculation"
                )
                return None
            ...
            for atomic_num in pyg_data.z.tolist():
                ...
                if atomic_energy is None:
                    self.logger.warning(
                        f"ANI-2x molecule {molecule_index}: Missing atomic energy for element {atomic_num}"
                    )
                    return None
            ...
            self.logger.debug(
                f"ANI-2x molecule {molecule_index} atomization energy: {atomization_energy_eV:.4f} eV"
            )
            return atomization_energy_eV

        except Exception as e:
            self.logger.error(
                f"Error calculating ANI-2x atomization energy for molecule {molecule_index}: {e}"
            )
            return None
    ...
    def _validate_dataset_specific_transforms(self, transform_names: list[str]) -> list[str]:
        ...
        if not has_geometric:
            warnings.append(
                "ANI-2x dataset without geometric augmentation - consider adding RandomRotate for invariance"
            )
    ...
    def _get_transform_recommendations(self, transform_names: list[str]) -> list[str]:
        ...
        if "GCNNorm" in transform_names and "AddSelfLoops" not in transform_names:
            recommendations.append(
                "Consider adding AddSelfLoops before GCNNorm for better performance"
            )
