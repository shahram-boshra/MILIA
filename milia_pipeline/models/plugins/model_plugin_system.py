...
class ModelDeclaration(BaseModel):
    ...
    min_pyg_version: Optional[str] = None
...
class ModelPluginLoader:
    ...
    def _validate_model_declaration(self, declaration: ModelDeclaration) -> List[str]:
        ...
        if not declaration.name:
            errors.append(f"Model declaration missing name")
                ...
                try:
                    ...
                    for pattern in suspicious_patterns:
                        if re.search(pattern, content):
                            issues.append(
                                f"Suspicious code pattern in {py_file.name}: {pattern}"
                            )
                except Exception as e:
    ...
    def _register_plugin_model(
        ...
        if not MODELS_AVAILABLE or registry is None:
            raise PluginError(
                "ModelRegistry not available - cannot register plugin models"
            )
