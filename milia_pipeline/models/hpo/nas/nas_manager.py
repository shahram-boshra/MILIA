...
from ..hpo_config import HPOConfig, SearchSpaceParamConfig, ParamType
...
from milia_pipeline.exceptions import HPOError, SearchSpaceError, ConfigurationError
...
class NASConfig(BaseModel):
    ...
    def validate_n_trials(cls, v: int) -> int:
        ...
        if v < 1:
            raise ValueError(
                f"n_trials must be at least 1, got {v}"
            )
        return v
    ...
    def validate_timeout(cls, v: Optional[int]) -> Optional[int]:
        ...
        if v is not None and v < 1:
            raise ValueError(
                f"timeout must be positive or None, got {v}"
            )
        return v
    ...
    def validate_direction(cls, v: str) -> str:
        ...
        if v not in valid_directions:
            raise ValueError(
                f"Invalid direction: '{v}'. Must be 'minimize' or 'maximize'"
            )
        return v
    ...
    def validate_cv_folds(cls, v: int) -> int:
        ...
        if v < 0:
            raise ValueError(
                f"cv_folds must be non-negative, got {v}"
            )
        return v
...
class NASManager:
    ...
    def __init__(
        ...
        if hpo_config is None:
            ...
            hpo_config = self._create_hpo_config_from_nas_config(
                nas_config, arch_search_space
            )
        else:
    ...
    def _convert_arch_space_to_hpo_format(
        ...
        if arch_space.allow_mixed_layers:
            ...
            has_attention_layers = any(
                lt in arch_space.layer_types for lt in attention_layer_types
            )
    ...
    def search(
        ...
        """
        logger.info(
            f"Starting architecture search with "
            f"{self.hpo_manager.config.n_trials} trials"
        )
    ...
    def build_model(
        ...
        logger.info(
            f"Building heterogeneous model with layer types: {layer_types}"
        )
        ...
        if self.hpo_manager.study is not None:
            try:
                ...
                summary['completed_trials'] = len([
                    t for t in self.hpo_manager.backend.get_all_trials(
                        self.hpo_manager.study
                    ) if t['state'] == 'COMPLETE'
                ])
            except HPOError:
                pass
...
class HeterogeneousGNN(nn.Module):
    ...
    def _create_layer(
        ...
        try:
            from torch_geometric.nn import (
                GCNConv, GATConv, SAGEConv, GINConv, GATv2Conv,
                TransformerConv,
            )
        except ImportError:
            raise HPOError(
        ...
        if layer_type == 'gcn':
            ...
            return GATConv(
                ...
                heads=heads, concat=True, dropout=self.dropout_p
            )
    ...
    def _create_pooling(self, pooling_type: str):
        ...
        if pooling_type == 'mean':
            ...
            gate_nn = nn.Linear(
                self.architecture['hidden_channels'],
                1
            )
...
def create_nas_manager(
    arch_space: Optional[GNNArchitectureSpace] = None,
    n_trials: int = 100,
    **kwargs
) -> NASManager:
    """
