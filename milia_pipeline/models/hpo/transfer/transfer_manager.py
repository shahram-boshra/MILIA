...
def _lazy_import_meta_features():
    ...
    try:
        from .meta_features import MetaFeatureExtractor, MetaFeatureConfig
...
class HPOTransferManager:
    ...
    def __init__(self, config: Optional[TransferConfig] = None):
        ...
        WarmStartStrategy, WarmStartConfig, WarmStartMethod, TransferredTrial = _lazy_import_warm_start()
    ...
    def register_study(
        ...
        try:
            ...
            self._raise_hpo_error(
                f"Failed to extract study information: {e}",
    ...
    def get_study_info(self, study_name: str) -> Optional[RegisteredStudyInfo]:
        """
    ...
    def _compute_similarity(
        ...
        if self._meta_extractor_class is not None:
            return self._meta_extractor_class.compute_similarity(
                features_a, 
                features_b
            )
    ...
    def warm_start_study(
        self,
        target_study: Any,
        source_studies: Optional[List[str]] = None,
        target_dataset: Optional[Any] = None,
        target_search_space: Optional[Dict[str, Any]] = None,
    ) -> int:
        """
        ...
        if source_studies is None:
            if target_dataset is None:
                raise ValueError(
                    "Either source_studies or target_dataset must be provided"
                )
        ...
        n_transferred = 0
        trials_per_source = max(
            1, 
            self.config.n_warm_start_trials // len(source_studies)
        )
        ...
        for source_name in source_studies:
            ...
            if source_study is None:
                logger.warning(
                    f"Study '{source_name}' not in cache, skipping"
                )
                continue
    ...
    def _load_meta_db(self) -> None:
        ...
        try:
            with open(self.config.meta_db_path, 'r') as f:
                data = json.load(f)
            
            self._meta_db = {
                name: RegisteredStudyInfo.from_dict(info)
                for name, info in data.items()
            }
            
            logger.info(
                f"Loaded {len(self._meta_db)} studies from "
                f"{self.config.meta_db_path}"
            )
