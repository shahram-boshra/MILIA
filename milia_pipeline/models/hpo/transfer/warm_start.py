...
class WarmStartStrategy:
    ...
    def weighted_transfer(
        ...
        for i, trial in enumerate(source_trials):
            ...
            transferred.append(TransferredTrial(
                params=params,
                value=value,
                similarity=sim,
                weight=weight,
    ...
    def transfer(
        ...
        if method == WarmStartMethod.ADAPTIVE:
            method = self._select_adaptive_method(
                source_trials, target_search_space, similarities
            )
    ...
    @staticmethod
    def _extract_value(trial: Any) -> Optional[float]:
    ...
    def _filter_invalid_trials(
        ...
        for trial in trials:
            _, is_valid = self._filter_params(
                trial.params, flat_space, scale_to_bounds=False
            )
            if is_valid:
    ...
    def _add_noise_to_trials(
        ...
        for trial in trials:
            ...
            noisy_trials.append(TransferredTrial(
                params=noisy_params,
                value=trial.value,
    ...
    def _select_adaptive_method(
        self,
        source_trials: List[Any],
        target_space: Optional[Dict[str, Any]],
