        ...
        if isinstance(data, dict):
            ...
            for d in directions:
                if d not in valid_directions:
                    raise ValueError(
                        f"Invalid direction: {d}. Must be 'minimize' or 'maximize'"
                    )
            
            # Multi-objective requires at least 2 metrics
            if len(metrics) < 2:
                raise ValueError(
                    f"Multi-objective requires at least 2 metrics, got {len(metrics)}"
                )
...
class HPOConfig(BaseModel, frozen=True):
    ...
    def validate_backend(cls, v: str) -> str:
        ...
        if v not in valid_backends:
            raise ValueError(
                f"Unknown HPO backend: '{v}'. Must be 'optuna' or 'ray_tune'"
            )
        return v
    ...
    def validate_cv_metric_aggregation(cls, v: str) -> str:
        ...
        if v not in valid_aggregations:
            raise ValueError(
                ...
                f"Must be 'mean', 'median', 'min', or 'max'"
            )
        return v
