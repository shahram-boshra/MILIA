...
class AnalysisConfig(BaseModel, frozen=True):
    ...
    n_importance_trials: int | None = None
    ...
    def validate_convergence_window(cls, v: int) -> int:
        ...
        if v < 1:
            raise ValueError(f"convergence_window must be at least 1, got {v}")
        return v

    @field_validator("n_importance_trials")
    @classmethod
    def validate_n_importance_trials(cls, v: int | None) -> int | None:
        """Validate n_importance_trials is at least 1 or None."""
        if v is not None and v < 1:
            raise ValueError(f"n_importance_trials must be at least 1 or None, got {v}")
        return v
    ...
    def validate_percentile_thresholds(self) -> Self:
        ...
        for p in self.percentile_thresholds:
            if not 0 <= p <= 100:
                raise ValueError(f"Percentile values must be between 0 and 100, got {p}")
        return self
...
class StudyAnalyzer:
    ...
    def __init__(
        ...
        if not OPTUNA_AVAILABLE:
            raise HPOError("Optuna is not installed", details="Install with: pip install optuna")

        if study is None:
            raise HPOError("Study cannot be None", details="Provide a valid Optuna Study object")

        self.study = study
        self.config = config or AnalysisConfig()

        # Caches for performance
        self._trials_cache: list[dict[str, Any]] | None = None
        self._importance_cache: dict[str, float] | None = None
        self._completed_trials_cache: list[dict[str, Any]] | None = None
    ...
    def from_manager(
        ...
        if manager.study is None:
            raise HPOError(
                "HPOManager has no study", details="Run optimize() before creating analyzer"
            )
        ...
        if window > 0:
            ...
            if converged and len(improvements) >= window:
                for i in range(len(improvements) - window, -1, -1):
                    if improvements[i] > 0:
                        convergence_trial = (
                            trial_numbers[i + 1] if i + 1 < len(trial_numbers) else None
                        )
                        break
        else:
            convergence_rate = 0.0
            ...
            for name2 in param_names:
                if name1 != name2:
                    param_corr[name1][name2] = pearson_correlation(
                        param_values[name1], param_values[name2]
                    )
    ...
    def get_parallel_coordinate_data(self) -> dict[str, Any]:
        ...
        param_ranges = {name: {"min": float("inf"), "max": float("-inf")} for name in param_names}
    ...
    def export_results(
        ...
        if include_trials:
            export_data["trials"] = self.get_trials(
                states=["COMPLETE", "PRUNED", "FAIL"], use_cache=False
            )
    ...
    def compare_with(
        ...
        return {
            ...
            "improvement": (
                (other_best - self_best)
                if self_best is not None and other_best is not None
                else None
            ),
        }
