...
def _prepare_data_for_task_hpo(
    ...
    val_data,
    task_type: str,
    discretize_config: Optional[Dict[str, Any]] = None,
...
def _prepare_classification_data_hpo(
    ...
    if y.dtype in [torch.int, torch.int32, torch.int64, torch.long]:
        ...
        for data in train_data:
            if hasattr(data, 'y') and data.y is not None:
                all_y.append(data.y)
        if all_y:
            combined_y = torch.cat([yi.flatten() if yi.dim() > 0 else yi.unsqueeze(0) for yi in all_y])
    ...
    if y.dtype in [torch.float, torch.float32, torch.float64]:
        logger.info(f"{task_type}: Float targets detected (dtype={y.dtype}), discretization required")
...
def _prepare_edge_regression_data_hpo(
    ...
    if (target_selection_config is not None and 
        _TARGET_SELECTION_AVAILABLE and
    ...
    train_data = _extract_targets_from_source(
        train_data, source_attr, indices, 'edge_y', "train"
    )
    val_data = _extract_targets_from_source(
        val_data, source_attr, indices, 'edge_y', "val"
    )
...
def _prepare_node_level_data_hpo(
    ...
    # Determine which indices to extract
    if (target_selection_config is not None and 
        target_selection_config.resolved_indices is not None):
        indices = target_selection_config.resolved_indices
    elif (target_selection_config is not None and
          target_selection_config.indices is not None):
        indices = target_selection_config.indices
    else:
    ...
    train_data = _extract_targets_from_source(
        train_data, source_attr, indices, 'y', "train"
    )
    val_data = _extract_targets_from_source(
        val_data, source_attr, indices, 'y', "val"
    )
        ...
        if y is not None:
            ...
            if y.dtype in [torch.float, torch.float32, torch.float64]:
                ...
                if unique_ratio > 0.5:
                    ...
                    logger.debug("Inferred 'graph_classification' from discrete target distribution")
                    return 'graph_classification'
...
def _create_loss_from_registry(
    task_type: str,
...
class HPOManager:
    ...
    def optimize(
        ...
        config_dict: Optional[Dict[str, Any]] = None,  # For target selection
    ) -> Dict[str, Any]:
        """
        ...
        def objective(trial) -> float:
            ...
            try:
                ...
                if config.task_type is not None:
                    ...
                    if task_type != inferred_task_type and not hasattr(objective, '_task_type_warned'):
                        logger.warning(
                ...
                # ============================================================
                target_selection_raw = models_config.get('selection', {}).get('target_selection', {})
                ...
                # ============================================================
                discretize_config = models_config.get('transforms', {}).get('DiscretizeTargets', None)
                if discretize_config is None:
                    # Fallback: check under preprocessing
                    discretize_config = models_config.get('preprocessing', {}).get('discretize', None)
                ...
                #-------
                    ...
                    batch_size = training_params.get(
                        'batch_size', 
                        trainer_kwargs.get('batch_size', 32)
                    )
                    ...
                    train_data_prepared, val_data_prepared, num_classes_override = _prepare_data_for_task_hpo(
                    ...
                    if factory is not None:
                        model, model_info = factory.create_model_with_info(
                            ...
                            task_type=task_type,
                            sample_data=train_data_prepared[0] if train_data_prepared else dataset[0],
    ...
    def _filter_search_space_for_model(
        ...
        **M4 Fix (2025-12-10)**:
        ...
        if metadata is None:
            logger.warning(
                f"No metadata found for model '{model_name}'. "
                f"Using unfiltered search space."
            )
            return deepcopy(search_space)
        ...
        if task_type is not None and LossRegistry is not None:
            try:
                ...
                logger.debug(
                    f"Loss '{loss_name}' accepts parameters: {sorted(valid_loss_params)}"
                )
            except Exception as e:
                logger.debug(
    ...
    def train_final_model(
        ...
        if Trainer is None:
            raise HPOError(
                "Trainer not available",
                details="Cannot train final model without Trainer class"
            )
        ...
        if get_factory is None:
            raise HPOError(
                ...
                details="Cannot create model without ModelFactory"
            )
        ...
        test_loader = PyGDataLoader(test_data, batch_size=batch_size, shuffle=False) if len(test_data) > 0 else None
        ...
        if training_config.get('scheduler', {}).get('enabled', True):
            scheduler = _create_scheduler_from_registry(
                optimizer,
                scheduler_params,
                scheduler_name=training_config.get('scheduler', {}).get('name', 'reduce_on_plateau')
            )
    ...
    def resume_study(
        ...
        if self.backend is None:
            raise HPOError(
                "Backend not initialized",
                details="Initialize HPOManager with enabled=True"
            )
    ...
    def save_results(
        ...
        try:
            ...
            raise HPOError(
                f"Failed to save best parameters: {e}",
                details=f"Target path: {best_params_path}"
            )
        ...
        try:
            ...
            raise HPOError(
                f"Failed to save study statistics: {e}",
                details=f"Target path: {statistics_path}"
            )
        ...
        try:
            ...
            raise HPOError(
                f"Failed to save trials: {e}",
                details=f"Target path: {trials_path}"
            )
...
def _extract_param_categories(
    ...
    optimizer_keys = {
        ...
        'betas', 'eps', 'amsgrad', 'nesterov'
    }
    scheduler_keys = {
        'factor', 'patience', 'step_size', 'gamma', 'T_max',
        'eta_min', 'cooldown', 'min_lr', 'T_0', 'T_mult'
    }
    loss_keys = {
        'alpha', 'gamma', 'reduction', 'weight', 'label_smoothing',
        'pos_weight', 'ignore_index'
    }
    training_keys = {
        'batch_size', 'epochs', 'max_epochs', 'num_epochs',
        'gradient_clip_val', 'accumulate_grad_batches', 
        'log_every_n_steps', 'shuffle'
    }
...
def get_best_params(manager: HPOManager) -> Dict[str, Any]:
    ...
    if manager.best_params is None:
        raise HPOError(
            ...
            details="Run optimize() before getting best parameters"
        )
