...
class OptunaPruningCallback(Callback):
    ...
    def __init__(
        ...
        if not OPTUNA_AVAILABLE:
            raise ImportError(
                "Optuna is required for OptunaPruningCallback. Install with: pip install optuna"
            )
        ...
        logger.debug(
            f"OptunaPruningCallback initialized: monitor='{monitor}', report_every={report_every}"
        )
    ...
    def on_train_begin(self, trainer) -> None:
        ...
        """
        logger.debug(f"Training started for trial {self.trial.number}, monitoring '{self.monitor}'")
    ...
    def on_epoch_end(self, trainer, epoch: int, metrics: dict[str, float]) -> None:
        ...
        if epoch not in self._reported_steps:
            ...
            logger.debug(f"Trial {self.trial.number}, Epoch {epoch}: {self.monitor}={value:.6f}")
        else:
            logger.debug(
        ...
        if self.trial.should_prune():
            logger.info(
                f"Trial {self.trial.number} pruned at epoch {epoch} ({self.monitor}={value:.6f})"
            )
            raise optuna.TrialPruned(f"Trial pruned at epoch {epoch} with {self.monitor}={value}")
    ...
    def on_train_end(self, trainer) -> None:
        ...
        logger.debug(
            f"Training ended for trial {self.trial.number}, final {self.monitor}={final_value}"
        )
...
def create_hpo_callback(
    trial: "optuna.Trial", monitor: str = "val_loss", report_every: int = 1, backend: str = "optuna"
) -> Callback:
    """
    ...
    """
    if backend == "optuna":
        return OptunaPruningCallback(trial=trial, monitor=monitor, report_every=report_every)
    elif backend == "ray_tune":
        # Future: RayTuneReportCallback
        raise NotImplementedError("Ray Tune callback not yet implemented. Use 'optuna' backend.")
    else:
