...
def validate_pyg_data(data: Data, strict: bool = False) -> Dict[str, Any]:
    ...
    if hasattr(data, 'edge_index') and data.edge_index is not None:
        ...
        if data.edge_index.size(0) != 2:
            errors.append(f"edge_index should have shape [2, num_edges], got {data.edge_index.shape}")
...
def infer_num_features(data: Union[Data, Dataset, DataLoader]) -> Dict[str, Optional[int]]:
    """
                            ...
                            if isinstance(data, Dataset):
                                for i in range(min(len(data), 1000)):  # Sample up to 1000
                                    if hasattr(data[i], 'y') and data[i].y is not None:
                                        all_labels.extend(data[i].y.tolist() if data[i].y.dim() > 0 else [data[i].y.item()])
...
def infer_out_channels(
    data: Union[Data, Dataset, DataLoader],
    task_type: str,
    ...
    if task_lower == 'edge_regression':
        ...
        if hasattr(sample, 'edge_value') and sample.edge_value is not None:
            ...
            logger.debug(f"edge_regression: out_channels={out_channels} from edge_value.shape={list(ev.shape)}")
            return out_channels
...
def compute_dataset_statistics(dataset: Dataset, max_samples: Optional[int] = None) -> Dict[str, Any]:
    """
    ...
    if stats['edge_counts']:
        ...
        stats['avg_degree'] = (2 * stats['avg_num_edges']) / stats['avg_num_nodes'] if stats['avg_num_nodes'] > 0 else 0
...
def get_batch_info(batch: Union[Data, Batch]) -> Dict[str, Any]:
    """
...
def to_device(data: Union[Data, Batch], device: torch.device) -> Union[Data, Batch]:
    """
...
def detach_data(data: Union[Data, Batch]) -> Union[Data, Batch]:
    """
