...
try:
    ...
    warnings.warn(
        "config_loader not available - using fallback YAML loading",
        UserWarning
    )
    import yaml
...
class CallbacksConfig(BaseModel):
    ...
    early_stopping: CallbackConfig = Field(default_factory=lambda: CallbackConfig(
        enabled=True,
        params={"monitor": "val_loss", "patience": 20, "mode": "min", "min_delta": 0.0001}
    ))
    model_checkpoint: CallbackConfig = Field(default_factory=lambda: CallbackConfig(
        enabled=True,
        params={"monitor": "val_loss", "save_top_k": 3, "mode": "min", "save_last": True}
    ))
    tensorboard: CallbackConfig = Field(default_factory=lambda: CallbackConfig(
        enabled=True,
        params={"log_dir": None}
    ))
    lr_monitor: CallbackConfig = Field(default_factory=lambda: CallbackConfig(
        enabled=True,
        params={"logging_interval": "epoch"}
    ))
    progress_bar: CallbackConfig = Field(default_factory=lambda: CallbackConfig(
        enabled=True,
        params={"refresh_rate": 1}
    ))
...
class HPOConfigBridge(BaseModel):
    ...
    def validate_backend(cls, v: str) -> str:
        ...
        if v not in ("optuna", "ray_tune"):
            raise ValueError(
                f"Unknown HPO backend: '{v}'. "
                f"Must be 'optuna' or 'ray_tune'"
            )
        return v
    ...
    def validate_cv_metric_aggregation(cls, v: str) -> str:
        ...
        if v not in valid_values:
            raise ValueError(
                f"Invalid cv_metric_aggregation: '{v}'. "
...
class ModelConfig(BaseModel):
    ...
    """
    enabled: bool = False
    selection: ModelSelectionConfig = Field(default_factory=lambda: ModelSelectionConfig(
    ...
    def from_dict(cls, config_dict: Dict[str, Any]) -> 'ModelConfig':
        ...
        def get_config(key: str, default_factory):
            return config_dict.get(key, default_factory() if callable(default_factory) else default_factory)
        ...
        callbacks = CallbacksConfig(
            early_stopping=CallbackConfig(**callbacks_dict.get('early_stopping', {})) 
                if 'early_stopping' in callbacks_dict else CallbackConfig(),
            model_checkpoint=CallbackConfig(**callbacks_dict.get('model_checkpoint', {}))
                if 'model_checkpoint' in callbacks_dict else CallbackConfig(),
            tensorboard=CallbackConfig(**callbacks_dict.get('tensorboard', {}))
                if 'tensorboard' in callbacks_dict else CallbackConfig(),
            lr_monitor=CallbackConfig(**callbacks_dict.get('lr_monitor', {}))
                if 'lr_monitor' in callbacks_dict else CallbackConfig(),
            progress_bar=CallbackConfig(**callbacks_dict.get('progress_bar', {}))
                if 'progress_bar' in callbacks_dict else CallbackConfig()
        )
    ...
    @classmethod
    def from_yaml(cls, config_path: Optional[Path] = None) -> 'ModelConfig':
        """
...
def get_models_config(config_path: Optional[Path] = None) -> ModelConfig:
    """
...
def is_models_enabled(config_path: Optional[Path] = None) -> bool:
    """
...
def get_model_selection(config_path: Optional[Path] = None) -> ModelSelectionConfig:
    """
...
def get_training_config(config_path: Optional[Path] = None) -> TrainingConfig:
    """
...
def get_acceleration_config(config_path: Optional[Path] = None) -> AccelerationConfig:
    """
...
def get_deployment_config(config_path: Optional[Path] = None) -> DeploymentConfig:
    """
...
def get_plugins_config(config_path: Optional[Path] = None) -> PluginsConfig:
    """
...
def get_hpo_config(config_path: Optional[Path] = None) -> HPOConfigBridge:
    """
...
def is_hpo_enabled(config_path: Optional[Path] = None) -> bool:
    """
...
def validate_models_config(config_path: Optional[Path] = None) -> bool:
    """
