...
class ModelComposer:
    ...
    def add_model(
        ...
        if not isinstance(model, nn.Module):
            raise TypeError(
                f"model must be nn.Module instance, got {type(model).__name__}"
            )
        ...
        logger.debug(
            f"Added model '{model_name}' with weight={weight}, level={level}"
        )
        return self
    ...
    def remove_model(self, index: int) -> 'ModelComposer':
        ...
        if index < 0 or index >= len(self.models):
            raise ValueError(
                f"Invalid index: {index}. Valid range: 0-{len(self.models)-1}"
            )
    ...
    def validate_composition(self) -> Dict[str, Any]:
        ...
        if self.strategy == "sequential":
            ...
            if len(unique_levels) < 2:
                ...
                suggestions.append(
                    "Assign different levels to models or use 'parallel' strategy"
                )
            ...
            if missing_levels:
                ...
                suggestions.append(
                    "Assign models to missing levels or adjust level numbers"
                )
        ...
        if self.fusion == "voting":
            if "classification" not in self.task_type.lower():
                ...
                suggestions.append(
                    "Use 'mean' or 'weighted' fusion for regression tasks"
                )
        ...
        if self.fusion == "weighted":
            ...
            if abs(total_weight - 1.0) > 0.01:
                ...
                suggestions.append(
                    "For manual control, ensure weights sum to 1.0"
                )
    ...
    def build(self) -> nn.Module:
        ...
        if self.fusion == "weighted":
            ...
            if abs(total_weight - 1.0) > 0.01:
                logger.info(
                    f"Normalizing weights (sum={total_weight:.3f}) to sum to 1.0"
                )
    ...
    def from_config(
        cls,
        config: Union[EnsembleConfig, Dict[str, Any]]
    ) -> 'ModelComposer':
        """
    ...
    def summary(self) -> str:
        ...
        for i, spec in enumerate(self.models):
            ...
            lines.append(
                f"  [{i}] {model_name}: "
                f"weight={spec.weight:.3f}, level={spec.level}"
            )
...
class ParallelEnsemble(nn.Module):
    ...
    def __init__(
        ...
        logger.debug(
            f"Initialized ParallelEnsemble: {len(models)} models, "
            f"fusion={fusion}"
        )
    ...
    def _call_model_with_signature(
        ...
        batch: Optional[torch.Tensor],
        edge_label_index: Optional[torch.Tensor],
        **kwargs
    ) -> torch.Tensor:
        """
        ...
        logger.debug(f"[DIAGNOSTIC] _call_model_with_signature: model={type(inner_model).__name__}, "
            ...
            try:
                if edge_attr is not None and batch is not None:
                    result = model(x, edge_index, edge_attr=edge_attr, 
                                batch=batch, edge_label_index=edge_label_index)
        ...
        # =================================================================
        use_encode_method = hasattr(model, 'encode') and callable(getattr(model, 'encode'))
        ...
        try:
            ...
            if edge_attr is not None and batch is not None:
                logger.debug(f"[DIAGNOSTIC] Strategy 2a: model(x, edge_index, edge_attr=edge_attr, batch=batch)")
                ...
                logger.debug(f"[DIAGNOSTIC] Strategy 2b: model(x, edge_index, batch=batch)")
                if use_encode_method:
                    result = model.encode(x, edge_index, batch=batch)
                else:
                    result = model(x, edge_index, batch=batch)
            elif edge_attr is not None:
                logger.debug(f"[DIAGNOSTIC] Strategy 2c: model(x, edge_index, edge_attr=edge_attr)")
                if use_encode_method:
                    result = model.encode(x, edge_index, edge_attr=edge_attr)
                else:
                    result = model(x, edge_index, edge_attr=edge_attr)
            else:
                logger.debug(f"[DIAGNOSTIC] Strategy 2d: model(x, edge_index)")
                if use_encode_method:
            ...
            # Handle tuple output from variational autoencoders (Fix 30)
            if isinstance(result, tuple):
                logger.debug(f"[DIAGNOSTIC] Strategy 2: Got tuple output, using first element (mu)")
                result = result[0]
            return result
    ...
    def forward(
        ...
        **kwargs
        ...
        if hasattr(x, 'x') and hasattr(x, 'edge_index'):
            ...
            logger.debug(f"[ParallelEnsemble] Extracted tensors from DataBatch: "
        ...
        for i, model in enumerate(self.models):
            ...
            logger.debug(f"[DIAGNOSTIC] Calling model {i}: {type(inner).__name__}, edge_attr={'None' if edge_attr is None else f'shape={list(edge_attr.shape)}'}")
            ...
            try:
                ...
                import traceback
                logger.error(
                    f"Model {i} ({type(inner).__name__}) forward FAILED: {e}"
                )
                logger.debug(f"[DIAGNOSTIC] Full traceback:\n{traceback.format_exc()}")
                raise
                raise
            ...
            for i, pred in enumerate(predictions):
                ...
                if current_dim != max_out_dim:
                    ...
                    if proj_key not in self._output_projections:
                        ...
                        logger.info(f"[ParallelEnsemble] Created output projection for model {i}: "
                                   f"{current_dim} -> {max_out_dim}")
                    
                    # Apply projection
                    pred = self._output_projections[proj_key](pred)
                    logger.debug(f"[ParallelEnsemble] Projected model {i} output: "
                               f"{current_dim} -> {max_out_dim}")
    ...
    def __repr__(self) -> str:
        """String representation."""
        return (
            f"{self.name}(models={len(self.models)}, fusion='{self.fusion}')"
        )
...
class SequentialStack(nn.Module):
    ...
    def _call_model_with_signature(
        ...
        **kwargs
        ...
        # Log entry with edge_attr status (DEBUG level for production)
        logger.debug(f"[SequentialStack] _call_model_with_signature: model={type(inner_model).__name__}, "
                   f"edge_attr={'None' if edge_attr is None else f'shape={list(edge_attr.shape)}'}")
        ...
        if is_3d_model:
            ...
            logger.debug(f"[SequentialStack] Calling 3D model {type(inner_model).__name__} with z, pos, batch")
            # Call with 3D signature
            return model(z=z, pos=pos, batch=batch)
        ...
        try:
            if edge_attr is not None and batch is not None:
                logger.debug(f"[SequentialStack] Strategy 2a: model(x, edge_index, edge_attr=edge_attr, batch=batch)")
                if use_encode_method:
                    result = model.encode(x, edge_index, edge_attr=edge_attr, batch=batch)
                else:
                    result = model(x, edge_index, edge_attr=edge_attr, batch=batch)
            elif batch is not None:
                logger.debug(f"[SequentialStack] Strategy 2b: model(x, edge_index, batch=batch)")
                if use_encode_method:
                    result = model.encode(x, edge_index, batch=batch)
                else:
                    result = model(x, edge_index, batch=batch)
            elif edge_attr is not None:
                logger.debug(f"[SequentialStack] Strategy 2c: model(x, edge_index, edge_attr=edge_attr)")
                if use_encode_method:
                    result = model.encode(x, edge_index, edge_attr=edge_attr)
                else:
                    result = model(x, edge_index, edge_attr=edge_attr)
            else:
                logger.debug(f"[SequentialStack] Strategy 2d: model(x, edge_index)")
                if use_encode_method:
                    result = model.encode(x, edge_index)
                else:
                    result = model(x, edge_index)
            # Handle tuple output from variational autoencoders (Fix 30)
            if isinstance(result, tuple):
                logger.debug(f"[SequentialStack] Strategy 2: Got tuple output, using first element (mu)")
                result = result[0]
            return result
        except TypeError as te:
    ...
    def forward(
        ...
        **kwargs
        ...
        if hasattr(x, 'x') and hasattr(x, 'edge_index'):
            ...
            logger.debug(f"[SequentialStack] Extracted tensors from DataBatch: "
                        f"x={x.shape if x is not None else None}, "
                        f"edge_index={edge_index.shape if edge_index is not None else None}, "
                        f"batch={batch.shape if batch is not None else None}")
        ...
        for i, model in enumerate(self.models):
            ...
            logger.debug(f"[SequentialStack] Processing model {i}: {type(inner_model).__name__}, "
            ...
            try:
                ...
                logger.debug(f"[SequentialStack] Model {i} ({type(inner_model).__name__}) "
...
class HierarchicalComposition(nn.Module):
    ...
    def __init__(
        ...
        logger.debug(
            f"Initialized HierarchicalComposition: {len(self.levels)} levels, "
            f"fusion={fusion}"
        )
    ...
    def forward(
        ...
        **kwargs
        ...
        if hasattr(x, 'x') and hasattr(x, 'edge_index'):
            ...
            logger.debug(f"[HierarchicalComposition] Extracted tensors from DataBatch: "
                        f"x={x.shape if x is not None else None}, "
                        f"edge_index={edge_index.shape if edge_index is not None else None}, "
                        f"batch={batch.shape if batch is not None else None}")
    ...
    def __repr__(self) -> str:
        """String representation."""
        return (
            f"{self.name}(levels={len(self.levels)}, fusion='{self.fusion}')"
        )
