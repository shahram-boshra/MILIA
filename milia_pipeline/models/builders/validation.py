...
from .layer_registry import LayerRegistry, LayerCategory, registry as layer_registry
from .architecture_builder import LayerConfig, ArchitectureConfig, ArchitectureError
...
class ArchitectureValidator:
    ...
    def validate_channel_flow(
        ...
        if current_channels != out_channels:
            ...
            suggestions.append(
                f"Add Linear layer at end: "
                f".add_layer('Linear', out_features={out_channels})"
            )
    ...
    def validate_task_compatibility(
        ...
        if 'graph' in task_type.lower():
            ...
            if not has_pooling:
                errors.append(
                    f"Task '{task_type}' requires pooling layer for graph-level output"
                )
                suggestions.append(
                    "Add pooling layer before final layer: "
                    ".insert_layer(-1, 'global_mean_pool')"
                )
        ...
        # Check node-level task requirements
            ...
            pooling_layers = [
                (i, l.type) for i, l in enumerate(layers)
            ...
            if pooling_layers:
                ...
                suggestions.append(
                    f"Remove pooling layers for node-level tasks"
                )
        ...
        for i in range(len(layers) - 1):
            ...
            if current_meta.category == LayerCategory.POOLING:
                if next_meta.category == LayerCategory.CONVOLUTIONAL:
                    ...
                    suggestions.append(
                        f"Consider moving pooling layer later in architecture"
                    )
