    ...
    def forward(
        ...
        if self.requires_edge_index:
            if edge_index is None:
                raise ValueError(
                    f"{self.func_name} requires edge_index but none provided"
                )
        ...
        if self.requires_batch:
            if batch is None:
                raise ValueError(
                    f"{self.func_name} requires batch but none provided"
                )
            args.append(batch)
...
class LayerNotFoundError(ModelError):
    ...
    def __init__(self, layer_name: str, available_layers: Optional[List[str]] = None):
...
class LayerRegistry:
    ...
    def _register_pooling_layers(self):
        ...
        for name, (func, description, req_batch, req_edge_idx, req_edge_attr) in global_pooling_funcs.items():
            # Wrap the function
            wrapped_class = type(
                f"Wrapped_{name}",
    ...
    def register_custom_layer(
        ...
        if not callable(layer_class):
            raise TypeError(
                f"layer_class must be callable, got {type(layer_class)}"
            )
        ...
        with self._lock:
            ...
            if metadata is None:
                ...
                metadata = LayerMetadata(
                    name=name,
                    category=category,
                    class_path=f"{layer_class.__module__}.{layer_class.__name__}" if hasattr(layer_class, '__module__') else "custom",
...
def list_layers(category: Optional[LayerCategory] = None) -> List[str]:
