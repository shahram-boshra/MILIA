    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'ArchitectureConfig':
        ...
        return cls(
            ...
            residual_connections=[
                ResidualConnection.from_dict(rc) 
                for rc in data.get('residual_connections', [])
            ]
        )
...
class ArchitectureBuilder:
    ...
    def __init__(
        ...
        logger.debug(
            f"Initialized ArchitectureBuilder: {task_type}, "
    ...
    def validate_architecture(self) -> Dict[str, Any]:
        ...
        if 'graph' in self.task_type:
            ...
            if not has_pooling:
                errors.append(
                    f"Task '{self.task_type}' requires pooling layer "
                    f"(e.g., global_mean_pool)"
                )
                suggestions.append(
    ...
    def from_config(
        cls,
        config: Union[ArchitectureConfig, Dict[str, Any]],
...
class CustomArchitecture(nn.Module):
        ...
        for layer_config in self.layer_configs:
            ...
            try:
                ...
                if metadata.is_functional:
                    ...
                    error_msg += (
                        f"  Hint: Check the layer documentation for required parameters.\n"
                    )
    ...
    def _create_projection_if_needed(
        self,
        rc_idx: int,
        start_shape: torch.Size,
        end_shape: torch.Size,
        device: torch.device
    ) -> Optional[nn.Module]:
        """
