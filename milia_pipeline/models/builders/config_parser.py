...
class ArchitectureConfigParser:
    ...
    def __init__(
        self,
        validator: Optional[ArchitectureValidator] = None,
        templates: Optional[ArchitectureTemplates] = None,
    ...
    def parse_custom_architecture(
        ...
        logger.info(
            f"Parsed custom architecture '{name}' with {len(builder)} layers "
            f"for task '{task}'"
        )
    ...
    def _parse_template_based(
        ...
        try:
            builder = self.templates.get_template(
                template_name,
                task_type=task,
                **template_params
            )
        ...
        logger.info(
            f"Parsed template-based architecture using '{template_name}' "
            f"with {len(builder)} layers"
        )
        ...
        if validate:
            ...
            if not validation_result['valid']:
                if self.strict_validation:
                    ...
                    logger.warning(f"Ensemble configuration has warnings: {validation_result['warnings']}")
    ...
    def _load_config(self, source: Union[Path, str]) -> Dict[str, Any]:
        """
    ...
    def save_config(
        self,
        config: Union[ArchitectureBuilder, ModelComposer, Dict[str, Any]],
        path: Union[Path, str],
...
def validate_config(
    config: Dict[str, Any],
    config_type: str = 'custom_architecture'
) -> Dict[str, Any]:
    """
    Validate configuration structure.
