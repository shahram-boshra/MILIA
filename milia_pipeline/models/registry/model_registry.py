...
class ModelRegistry:
            ...
            for name in discovered_names:
                ...
                try:
                    ...
                    self._register_internal(
                        name=name, model_class=model_class, metadata=metadata, is_builtin=True
                    )
                    discovered_count += 1
    ...
    def _import_pyg_model(self, name: str, metadata: ModelMetadata) -> type[torch.nn.Module] | None:
        """
    ...
    def register_model(
        ...
        with self._lock:
            ...
            if name in self._models and not force:
                logger.warning(f"Model '{name}' already registered. Use force=True to override.")
                return False
    ...
    def get_model(self, name: str) -> type[torch.nn.Module] | None:
        """
    ...
    def list_available_models(
        ...
        supports_heterogeneous: bool | None = None,
        ...
        with self._lock:
            ...
            if task_type:
                models = [
                    m for m in models if task_type in self._models[m].metadata.supported_tasks
                ]
            ...
            if tags:
                models = [
                    m for m in models if all(tag in self._models[m].metadata.tags for tag in tags)
                ]

            return sorted(models)

    def get_registration(self, name: str) -> ModelRegistration | None:
        """
    ...
    def search_models(self, query: str, search_in: list[str] | None = None) -> list[str]:
        """
    ...
    def get_builtin_models(self) -> list[str]:
        ...
        """
        with self._lock:
            return sorted([name for name, reg in self._models.items() if reg.is_builtin])
    ...
    def get_custom_models(self) -> list[str]:
        ...
        """
        with self._lock:
            return sorted([name for name, reg in self._models.items() if not reg.is_builtin])
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            by_category = {
                cat.value: len(names) for cat, names in self._by_category.items() if len(names) > 0
            }
        ...
        with self._lock:
            ...
            report = {
                ...
                "success_rate": (
                    ...
                    if self._discovery_stats["total_attempted"] > 0
                    else 0
                ),
            }
    ...
    def log_availability_summary(self):
        ...
        if report["failed_models"]:
            logger.info(f"\nUnavailable in Current PyG Version: {len(report['failed_models'])}")
            if len(report["failed_models"]) <= 10:
                for name, reason in report["failed_models"]:
                    logger.debug(f"  - {name}: {reason}")
            else:
                logger.debug(f"  (First 10 shown, {len(report['failed_models']) - 10} more)")
...
def get_model(name: str) -> type[torch.nn.Module] | None:
    """
...
def list_models(category: ModelCategory | None = None, task_type: str | None = None) -> list[str]:
    """
