...
try:
    from milia_pipeline.exceptions import TrainingError, CheckpointError
except ImportError:
...
class Trainer:
    ...
    def _validate_configuration(self):
        ...
        if self.optimizer is None:
            raise TrainingError(
                ...
                "Please provide optimizer argument."
            )
    ...
    def _train_epoch(self) -> Dict[str, float]:
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"[DIAGNOSTIC] _train_epoch: Moving batch to device")
                batch = batch.to(self.device)
                logger.debug(f"[DIAGNOSTIC] _train_epoch: Batch moved to device, calling _forward_pass")
    ...
    def _get_forward_signature_params(self) -> List[str]:
        ...
        try:
            ...
            if params:
                logger.info(f"Introspected forward signature for {type(inner_model).__name__}: {params}")
            else:
                logger.warning(f"Empty forward signature introspected for {type(inner_model).__name__}")
    ...
    def _forward_with_dynamic_signature(
        self, 
        ...
        if not forward_params:
            logger.debug("Dynamic forward: No forward parameters introspected, skipping dynamic approach")
            return None
        ...
        try:
            sig = inspect.signature(self.model.forward if not hasattr(self.model, 'model') 
                                    else self.model.model.forward)
        ...
        try:
            ...
            if 'edge_attr' in kwargs:
                ea = kwargs['edge_attr']
                logger.debug(f"[DIAGNOSTIC] edge_attr in kwargs: shape={list(ea.shape) if ea is not None else None}")
            else:
                logger.debug(f"[DIAGNOSTIC] edge_attr NOT in kwargs (correct for ensembles)")
    ...
    def _forward_pass(self, batch: Union[Data, Batch]) -> torch.Tensor:
        """
        ...
        has_edge_label_index = hasattr(batch, 'edge_label_index') and batch.edge_label_index is not None
        ...
        if use_edge_features and has_edge_attr:
            ...
            return self._forward_without_edge_features(batch, has_batch, has_edge_attr, has_edge_label_index)
    ...
    def _should_use_edge_features(self, has_edge_attr: bool) -> bool:
        ...
        if self._uses_edge_features is True:
            ...
            logger.debug(
                "No model_info provided, defaulting to not using edge features"
            )
            return False
    ...
    def _forward_with_edge_features(
        ...
        try:
            if has_batch:
                return self.model(batch.x, batch.edge_index, batch.edge_attr, batch=batch.batch, **extra_kwargs)
            else:
        ...
        try:
            if has_batch:
                return self.model(batch.x, batch.edge_index, edge_attr=batch.edge_attr, batch=batch.batch, **extra_kwargs)
            else:
                return self.model(batch.x, batch.edge_index, edge_attr=batch.edge_attr, **extra_kwargs)
    ...
    def _get_target(self, batch: Union[Data, Batch]) -> torch.Tensor:
        """
        ...
        if not self._is_edge_level_task():
            ...
            if (self._is_graph_level_task() and 
                not self._is_classification_task and  # Don't reshape classification targets
                self._out_channels is not None and
                self._out_channels > 1 and 
                target is not None and
                target.dim() == 1 and
                hasattr(batch, 'num_graphs') and 
                batch.num_graphs is not None):
        ...
        if task_lower == 'link_prediction':
            ...
            if hasattr(batch, 'edge_label') and batch.edge_label is not None:
                ...
                logger.warning(
                    "link_prediction task but batch.edge_label not found. "
    ...
    def _apply_target_selection(
        ...
        if target_last_dim == num_selected:
            ...
            original_total = self._original_out_channels or self._target_selection.get('total_available')
        ...
        total_targets = self._original_out_channels or self._target_selection.get('total_available', len(indices))
    ...
    def _prepare_target_for_metrics(self, target: torch.Tensor) -> torch.Tensor:
        ...
        if self._task_type is not None:
            task_lower = self._task_type.lower()
            is_classification = (
                task_lower == 'link_prediction' or
                'classification' in task_lower
            )
        ...
        try:
            ...
            if data_info is not None:
                ...
                checkpoint['data_info'] = {
                    'requires_edge_features': self.model_info.get('requires_edge_features', False) if self.model_info else False,
                    'uses_edge_features': self.model_info.get('uses_edge_features', False) if self.model_info else False,
                }
    ...
    def load_checkpoint(self, filepath: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            core_keys = {
                ...
                'scheduler_state_dict', 'metrics_history', 'best_val_loss'
            }
    ...
    def get_checkpoint_info(filepath: Path) -> Dict[str, Any]:
        ...
        return {
            ...
            'model_name': hyper_params.get('model_name') or hyper_params.get('model_info', {}).get('name'),
            'task_type': hyper_params.get('task_type') or hyper_params.get('model_info', {}).get('task_type'),
    ...
    def save_results(
        ...
        try:
            ...
            raise IOError(f"Failed to save training results: {e}") from e
        ...
        for callback in self.callbacks:
            try:
                if hasattr(callback, 'should_stop'):
                    ...
                    should_stop_attr = getattr(callback, 'should_stop')
                ...
                logger.warning(
                    f"Callback {callback.__class__.__name__}.should_stop failed: {e}"
                )
        return False
