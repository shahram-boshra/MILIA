...
class TaskDataPreparer:
    ...
    def prepare_for_task(
        ...
        if task_lower in cls._task_handlers:
            ...
            return handler(train_data, val_data, test_data, log, target_selection_config)
    ...
    def _prepare_graph_classification(
        ...
        if target.dtype in [
            ...
            logger.info(
                f"graph_classification: Targets are already integer class indices (num_classes={num_classes})"
            )
        ...
        if hasattr(sample, "targets_discretized") and sample.targets_discretized:
            logger.info(
                "graph_classification: Data already marked as discretized (no transform needed)"
            )
            return train_data, val_data, test_data, None
        ...
        logger.info(
            "graph_classification: DiscretizeTargets fitted with global bin edges. "
            "All splits will use consistent discretization."
        )

        # Apply transform to all splits using the fitted bin edges
        train_data = cls._apply_discretize_to_subset(
            train_data, discretize_transform, logger, "train"
        )
        ...
        logger.info(
            f"graph_classification: Data discretized into {n_bins} classes (num_classes={n_bins})"
        )
    ...
    def _prepare_node_classification(
        ...
        if target.dtype in [
            ...
            num_classes = len(all_classes) if all_classes else None
            logger.info(
                f"node_classification: Targets are already integer class indices (num_classes={num_classes})"
            )
            return train_data, val_data, test_data, num_classes
        ...
        if hasattr(sample, "y_discretized") and sample.y_discretized:
            logger.info(
                "node_classification: Data already marked as discretized (no transform needed)"
            )
            return train_data, val_data, test_data, None
        ...
        logger.info(
            "node_classification: DiscretizeTargets fitted with global bin edges. "
            "All splits will use consistent discretization."
        )
        ...
        # Log class distribution info
        logger.info(
            f"node_classification: Data discretized into {n_bins} classes (num_classes={n_bins})"
        )
    ...
    def _prepare_edge_regression(
        ...
        if target_selection_config is not None:
            if (
                ...
                source_total_count = (
                    source_tensor.size(-1) if source_tensor.dim() > 1 else source_tensor.size(0)
                )
                target_selection_config.resolve(
                    available_names=None, total_count=source_total_count
                )
                indices = target_selection_config.resolved_indices
                logger.debug(
    ...
    def _prepare_edge_classification(
        ...
        if target.dtype in [
            ...
            num_classes = len(all_classes) if all_classes else None
            logger.info(
                f"edge_classification: Targets are already integer class indices (num_classes={num_classes})"
            )
            return train_data, val_data, test_data, num_classes

        # Check if data was already discretized (metadata flag)
        if hasattr(sample, f"{target_attr}_discretized") and getattr(
            sample, f"{target_attr}_discretized"
        ):
            logger.info(
                "edge_classification: Data already marked as discretized (no transform needed)"
            )
            return train_data, val_data, test_data, None
        ...
        logger.info(
            "edge_classification: DiscretizeTargets fitted with global bin edges. "
            "All splits will use consistent discretization."
        )
        ...
        # Log class distribution info
        logger.info(
            f"edge_classification: Data discretized into {n_bins} classes (num_classes={n_bins})"
        )
    ...
    def _prepare_link_prediction(
        ...
        train_data = cls._apply_transform_to_subset(
            train_data, link_split_transform, logger, "train"
        )
    ...
    def _extract_targets_from_source(
        ...
        logger.debug(
            f"Extracted targets to '{target_attr}' for {len(modified_data)} graphs in {split_name} split"
        )
    ...
    def _apply_discretize_to_subset(
        ...
        logger.debug(
            f"Applied DiscretizeTargets to {len(transformed)} graphs in {split_name} split"
        )
        return transformed
...
logger.info(
    f"data_preparation module loaded - {len(TaskDataPreparer._task_handlers)} task types supported"
)
