...
class OptimizerRegistry:
    ...
    def register_custom_optimizer(
        ...
        if name in cls._optimizers and not overwrite:
            raise ValueError(
                f"Optimizer '{name}' already registered. Use overwrite=True to replace."
            )
...
def get_optimizer(
    name: str, model_parameters: Iterator[nn.Parameter], params: dict[str, Any] | None = None
) -> torch.optim.Optimizer:
    """
...
logger.info(f"optimizers module loaded - {len(OptimizerRegistry._optimizers)} optimizers available")
