...
class MetricsRegistry:
    ...
    _classification_metrics: Set[str] = {
        'accuracy', 'precision', 'recall', 'f1', 'auroc', 'auprc'
    }
    _regression_metrics: Set[str] = {
    ...
    def register_custom_metric(
        ...
        if not (isinstance(metric_class, type) and 
                (issubclass(metric_class, nn.Module) or 
                 (TORCHMETRICS_AVAILABLE and issubclass(metric_class, Metric)))):
            raise TypeError(
    ...
    def get_metrics_for_task(
        ...
        if not metrics:
            raise RuntimeError(
                f"No metrics could be created for task '{task_type}'. "
                f"Tried: {final_metrics}"
            )
...
if TORCHMETRICS_AVAILABLE:
    logger.info(f"metrics module loaded - {len(MetricsRegistry._metrics)} metrics available (TorchMetrics backend)")
else:
    logger.warning(
        f"metrics module loaded - TorchMetrics not available. "
        f"Install with: pip install torchmetrics>=1.0.0"
    )
