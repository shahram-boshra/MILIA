...
class SchedulerRegistry:
    ...
    def register_custom_scheduler(
        ...
        if name in cls._schedulers and not overwrite:
            raise ValueError(
                f"Scheduler '{name}' already registered. "
...
def create_warmup_scheduler(
    ...
    after_scheduler_params: Optional[Dict[str, Any]] = None
    ...
    main_scheduler = SchedulerRegistry.get_scheduler(
        after_scheduler_name,
        optimizer,
        after_params
    )
    ...
    logger.info(
        f"Created warmup scheduler: {warmup_epochs} epochs warmup, "
        f"then {after_scheduler_name}"
    )
...
logger.info(
    f"schedulers module loaded - {len(SchedulerRegistry._schedulers)} schedulers available"
)
