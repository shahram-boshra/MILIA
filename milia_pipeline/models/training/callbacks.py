...
class ModelCheckpoint(Callback):
    ...
    def on_epoch_end(self, trainer, epoch: int, metrics: Dict[str, float]):
        ...
        try:
            ...
            is_new_best = (
                self._best_model_score is None or
                (self.mode == "min" and current_best_score < self._best_model_score) or
                (self.mode == "max" and current_best_score > self._best_model_score)
            )
            ...
            if is_new_best:
                ...
                if self.save_best:
                    ...
                    try:
                        self._save_checkpoint(trainer, best_path, epoch, current_best_score, is_best=True)
                        if self.verbose:
                            logger.info(
...
class CallbackFactory:
    ...
    def from_config(
        ...
        callback_logger: Optional[logging.Logger] = None
    ) -> List[Callback]:
        """
        ...
        for callback_name, callback_class in cls._callback_registry.items():
            ...
            try:
                ...
                log.debug(f"CallbackFactory: Created {callback_name} with params: {filtered_params}")
            except Exception as e:
    ...
    def _filter_params(cls, callback_class: type, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if ignored:
                logger.debug(f"CallbackFactory: Ignored unsupported params for {callback_class.__name__}: {ignored}")
    ...
    def register_custom_callback(
        ...
        if not issubclass(callback_class, Callback):
            raise TypeError(f"callback_class must be subclass of Callback, got {type(callback_class)}")
        
        if name in cls._callback_registry and not overwrite:
            raise ValueError(f"Callback '{name}' already registered. Use overwrite=True to replace.")
...
logger.info(f"callbacks module loaded - {len(CallbackFactory._callback_registry)} callback types available")
