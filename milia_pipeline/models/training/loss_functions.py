...
class LossRegistry:
    ...
    def register_custom_loss(
        ...
        if not issubclass(loss_class, nn.Module):
            raise TypeError(
                f"loss_class must be a subclass of nn.Module, "
                f"got {type(loss_class)}"
            )
        
        if name in cls._losses and not overwrite:
            raise ValueError(
                f"Loss '{name}' already registered. "
...
def get_loss(name: str, params: Optional[Dict[str, Any]] = None) -> nn.Module:
    """
