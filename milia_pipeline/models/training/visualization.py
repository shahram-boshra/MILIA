...
class TrainingVisualizer:
    ...
    def plot_metrics(
        ...
        if metric_names is None:
            metric_names = [
                k for k in self.metrics_history.keys()
    ...
    def plot_interactive(
        ...
        fig = make_subplots(
            rows=2, cols=1,
            subplot_titles=('Loss Curves', 'Metrics'),
        ...
        if save_path:
            ...
            elif save_path.suffix == '.png' and KALEIDO_AVAILABLE:
                fig.write_image(str(save_path))
            elif save_path.suffix == '.pdf' and KALEIDO_AVAILABLE:
                fig.write_image(str(save_path))
            else:
