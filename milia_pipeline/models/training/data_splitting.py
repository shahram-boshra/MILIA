...
class DataSplitter:
    ...
    def stratified_split(
        ...
        try:
            ...
            raise DataError(
                "stratified_split requires scikit-learn. "
        ...
        if label_getter is None:
            ...
            label_getter = lambda data: data.y.item() if data.y.dim() == 0 else data.y.argmax().item()
    ...
    def scaffold_split(
        ...
        test_ratio: float = 0.1,
        random_seed: int = 42,
        mol_getter: Optional[callable] = None,
        ...
        for idx in range(n):
            try:
                ...
                raise DataError(
                    "Dataset samples missing 'mol' attribute. "
                    "Provide custom mol_getter function."
                )
            except Exception as e:
    ...
    def k_fold_split(
        ...
        if n < n_splits:
            raise DataError(
                f"Dataset too small for {n_splits}-fold split. "
                f"Dataset size: {n}"
            )
        ...
        logger.info(
            f"K-fold split: {n_splits} folds, "
            f"~{fold_size} samples per fold "
            f"(seed={random_seed})"
        )
...
def random_split(
    ...
    """
    return DataSplitter.random_split(
...
def stratified_split(
    ...
    """
    return DataSplitter.stratified_split(
        dataset, train_ratio, val_ratio, test_ratio, random_seed
    )
...
def k_fold_split(
    ...
    Example:
        ...
        ...     print(f"Fold {fold_idx}: Train={len(train)}, Val={len(val)}")
    """
    return DataSplitter.k_fold_split(
        dataset, n_splits, random_seed, shuffle
    )
