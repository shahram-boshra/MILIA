...
class ModelOptimizer:
    ...
    def quantize_model(
        ...
        try:
            if quant_type == "dynamic":
                ...
                quantized = self._static_quantization(
                    model, calibration_data, example_inputs
                )
            elif quant_type == "qat":
                ...
                if self.verbose:
                    logger.info(
                        ...
                        "Train the model, then call finalize_qat()"
                    )
    ...
    def _remove_pruning_reparameterization(
        self,
        model: nn.Module
    ) -> nn.Module:
    ...
    def get_sparsity(self, model: nn.Module) -> Dict[str, float]:
        ...
        return {
            ...
            'compression_ratio': 1.0 / (1.0 - global_sparsity) if global_sparsity < 1.0 else float('inf')
        }
    ...
    def optimize_for_mobile(
        ...
        try:
            ...
            raise ExportError(
                "Mobile optimization requires PyTorch with mobile support"
            )
        except Exception as e:
