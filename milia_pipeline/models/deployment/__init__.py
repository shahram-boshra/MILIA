...
def deploy_model_to_cloud(
    ...
    try:
        ...
        if verbose:
            logger.info(
                f"Model prepared for {target} deployment at {model_path}"
            )
...
def create_production_monitor(
    ...
    try:
        ...
        if verbose:
            logger.info(
                f"Production monitor created for '{model_name}' - "
                f"All monitoring features enabled"
            )
...
logger.info(
    f"Deployment module loaded (v{__version__}) - "
    f"Available targets: {list_deployment_targets()}"
)
