...
class ComputationOptimizer:
    ...
    def compile_model(
        ...
        if not hasattr(torch, 'compile'):
            warnings.warn(
                "torch.compile requires PyTorch 2.0+. Skipping compilation."
            )
            return model
    ...
    def compare_optimizations(
        ...
        for i, config in enumerate(optimization_configs):
            ...
            # Optimize model
            optimized_model = optimizer.optimize_model(model.clone() if hasattr(model, 'clone') else model)
            ...
            if self.verbose:
                logger.info(
                    f"{config_name}: {bench_results['avg_time_ms']:.2f}ms"
                )
