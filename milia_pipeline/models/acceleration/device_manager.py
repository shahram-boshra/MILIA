...
try:
    from milia_pipeline.exceptions import DeviceNotAvailableError, HardwareError, ModelError
except ImportError:
...
class DeviceManager:
    ...
    def _validate_and_set_device(self, device: str | torch.device) -> torch.device:
        """
        ...
        if device.type == "cuda":
            ...
            if not self._is_tpu_available():
                if self.allow_fallback:
                    ...
                    raise DeviceNotAvailableError("TPU requested but not available.")
    ...
    def _get_device_info(self, device: torch.device) -> DeviceInfo:
        ...
        if device.type == "cuda":
            ...
            return DeviceInfo(
                device_type="mps", name="Apple MPS", is_available=True, is_default=True
            )

        elif device.type == "tpu":
            return DeviceInfo(
                device_type="tpu", name="Google TPU", is_available=True, is_default=True
            )
    ...
    def get_available_devices(self, device_type: str | None = None) -> list[DeviceInfo]:
        """
        ...
        if device_type is None or device_type == "cpu":
            devices.append(DeviceInfo(device_type="cpu", name="CPU", is_available=True))
        ...
        if (device_type is None or device_type == "mps") and self._is_mps_available():
            devices.append(DeviceInfo(device_type="mps", name="Apple MPS", is_available=True))

        # TPU device
        if (device_type is None or device_type == "tpu") and self._is_tpu_available():
            devices.append(DeviceInfo(device_type="tpu", name="Google TPU", is_available=True))

        return devices

    def move_to_device(self, model: nn.Module, non_blocking: bool = False) -> nn.Module:
        """
    ...
    @contextmanager
    def device_context(self, device: str | torch.device | None = None):
        """
    ...
    def print_device_summary(self):
        ...
        for device in devices:
            ...
            if device.compute_capability:
                print(
                    f"  Compute Capability: {device.compute_capability[0]}.{device.compute_capability[1]}"
                )
...
def get_default_device(
    device: str | torch.device | None = None, verbose: bool = False
) -> torch.device:
    """
