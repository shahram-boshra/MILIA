...
class MemoryOptimizer:
    ...
    def _validate_config(self):
        ...
        if self.config.precision == "bf16":
            if self.device.type == 'cuda':
                if not torch.cuda.is_bf16_supported():
                    warnings.warn(
                        "BF16 not supported on this device, falling back to FP16"
                    )
        ...
        if self.config.mixed_precision and self.device.type not in ['cuda', 'cpu']:
            warnings.warn(
                f"Mixed precision not supported on {self.device.type}, disabling"
            )
    ...
    def get_grad_scaler(self) -> Optional[GradScaler]:
        """
    ...
    def step(self):
        ...
        if (self.config.empty_cache_interval > 0 and 
            self._step_count % self.config.empty_cache_interval == 0):
            self.empty_cache()
        
        # Periodic garbage collection
        if (self.config.garbage_collect_interval > 0 and
            self._step_count % self.config.garbage_collect_interval == 0):
    ...
    def check_memory_usage(self, threshold: float = 0.9) -> bool:
        ...
        if usage > threshold:
            if self.verbose:
                logger.warning(
                    f"Memory usage high: {usage*100:.1f}% "
