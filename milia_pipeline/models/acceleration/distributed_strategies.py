...
class DistributedManager:
    ...
    def __init__(
        self,
        strategy: Union[str, DistributedStrategy] = "none",
        backend: Union[str, DistributedBackend] = "auto",
        find_unused_parameters: bool = False,
        gradient_as_bucket_view: bool = True,
        static_graph: bool = False,
        cpu_offload: bool = False,
    ...
    def wrap_model(
        ...
        if not self._is_initialized:
            raise DistributedError(
                "Must call setup() before wrapping model. "
                "Call manager.setup() first."
            )
        ...
        if strategy == DistributedStrategy.NONE:
            ...
            if not torch.cuda.is_available():
                raise DistributedError(
                    "DataParallel requires CUDA. No CUDA devices found."
                )
            ...
            if self.verbose:
                logger.info(
                    f"Wrapped model with DataParallel - "
            ...
            if self.verbose and self.is_main_process():
                logger.info(
                    f"Wrapped model with DDP - "
                    f"Device: cuda:{self.config.local_rank}"
                )
            ...
            try:
                ...
                raise DistributedError(
                    ...
                    "Upgrade PyTorch or use DDP instead."
                )
