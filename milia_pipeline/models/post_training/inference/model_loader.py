...
from ...factory.model_factory import ModelFactory, get_factory
...
class ModelLoader:
    ...
    def _load(
        ...
        if hyperparameters is not None:
            ...
            logger.info("Using hyperparameters from checkpoint['hyper_parameters']['hyperparameters']")
        else:
            ...
            if resolved_hyperparams:
                logger.info("Using hyperparameters from checkpoint['hyper_parameters']['model_info']['hyperparameters_values']")
            else:
                logger.warning(
        ...
        found_params = {k: resolved_hyperparams.get(k) for k in key_params if k in resolved_hyperparams}
        ...
        has_output_projection = any(
            'output_projection' in k for k in aligned_state_dict.keys()
        )
        ...
        if has_output_projection:
            ...
            if weight_key is not None:
                ...
                if wrapper is not None:
                    ...
                    wrapper.output_projection = torch.nn.Linear(
                        in_features, out_features
                    ).to(device)
                    wrapper._model_out_dim = in_features
                    logger.debug(
                        f"Initialized output_projection on {type(wrapper).__name__}"
                    )
                else:
                    logger.warning(
...
def load_model_only(
    ...
    **kwargs
    ...
    Example:
        ...
        >>> predictions = model(data)
    """
    model, _ = load_model(checkpoint_path, working_root_dir=working_root_dir, device=device, **kwargs)
    return model
