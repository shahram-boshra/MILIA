...
class Predictor:
    ...
    def _forward(self, data: Union[Data, Batch]) -> torch.Tensor:
        """
        ...
        try:
            ...
            if is_3d_molecular_model:
                ...
                if not hasattr(data, 'z') or data.z is None:
                    raise ValueError(
                        f"3D molecular model requires 'z' (atomic numbers) in data. "
                        f"Ensure your input format provides atomic numbers (e.g., XYZ file)."
                    )
                if not hasattr(data, 'pos') or data.pos is None:
                    raise ValueError(
                        f"3D molecular model requires 'pos' (3D coordinates) in data. "
                        f"Ensure your input format provides 3D positions (e.g., XYZ file)."
                    )
                ...
                if 'edge_index' in valid_params and hasattr(data, 'edge_index') and data.edge_index is not None:
            ...
            if hasattr(data, 'z') and data.z is not None and hasattr(data, 'pos') and data.pos is not None:
