...
class FineTuner:
    ...
    def __init__(self, model: nn.Module, hyper_parameters: dict[str, Any], working_root_dir: Path):
        """
    ...
    @classmethod
    def from_checkpoint(
        cls, checkpoint_path: str | Path, working_root_dir: Path, device: torch.device | None = None
    ) -> "FineTuner":
        """
        ...
        model, model_info = ModelLoader.load_from_checkpoint(
            checkpoint_path=resolved_path, working_root_dir=working_root_dir, device=device
        )
    ...
    def prepare_for_finetuning(
        self,
        new_out_channels: int | None = None,
    ...
    def _apply_freeze_strategy(
        self, model: nn.Module, strategy: FreezeStrategy, freeze_layers: int | None = None
    ):
    ...
    def _replace_output_head(self, model: nn.Module, new_out_channels: int) -> nn.Module:
        ...
        new_linear = nn.Linear(last_linear.in_features, new_out_channels)
        ...
        logger.info(f"Replaced output head: {last_linear.out_features} -> {new_out_channels}")
