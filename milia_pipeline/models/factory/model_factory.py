...
from ..registry.pyg_introspector import (
    ...
    introspect_forward_signature,  # NEW: Forward signature introspection
    get_required_data_attributes,  # NEW: Required data attributes
    KNOWN_CONV_KWARGS,  # NEW: Known conv layer kwargs for passthrough
    model_accepts_kwargs,  # NEW: Check if model accepts **kwargs
            ...
            if model_out_dim != self.out_channels:
                if self.output_projection is None or self._model_out_dim != model_out_dim:
                    ...
                    self.output_projection = torch.nn.Linear(
                        model_out_dim, self.out_channels
                    ).to(out.device)
                    logger.info(
...
class EdgeLevelModelWrapper(torch.nn.Module):
    ...
    def __init__(
        ...
        model_out_channels: Optional[int] = None
    ):
        """
        Initialize wrapper.
    ...
    def _is_edge_level_task(self) -> bool:
        ...
        if self.task_type is None:
            return False
        task_lower = self.task_type.lower()
        return task_lower in ['link_prediction', 'edge_regression'] or \
               task_lower.startswith('link_') or task_lower.startswith('edge_')
    ...
    def _decode_edges(
        ...
        if self.decoder_method == 'dot_product':
            ...
            logger.warning(
                f"Unknown decoder method '{self.decoder_method}', using 'dot_product'"
            )
            return (z_src * z_dst).sum(dim=-1)
    ...
    def forward(
        ...
        **kwargs
        ...
        use_encode_method = hasattr(self.model, 'encode') and callable(getattr(self.model, 'encode'))
        ...
        if isinstance(z, tuple):
            ...
            logger.debug(
                f"EdgeLevelModelWrapper: Received tuple output (likely variational encoder), "
                f"using first element (mu) for decoding"
            )
            z = z[0]
        ...
        logger.debug(f"EdgeLevelModelWrapper: task_type={self.task_type}, _is_edge_level_task=False, returning z unchanged")
        return z
        ...
        for param, value in hyperparameters.items():
            ...
            if "options" in spec:
                ...
                if value not in options:
                    errors.append(
                        f"Parameter '{param}' must be one of {options}, got '{value}'"
                    )
    ...
    def validate_data_compatibility(
        ...
        if metadata.requires_edge_features:
            if not hasattr(data, 'edge_attr') or data.edge_attr is None:
                errors.append(
                    f"Model requires edge features, but data has no edge_attr"
                )
        ...
        if metadata.requires_edge_weights:
            if not hasattr(data, 'edge_weight') or data.edge_weight is None:
                errors.append(
                    f"Model requires edge weights, but data has no edge_weight"
                )
        ...
        if hasattr(data, 'node_type') or hasattr(data, 'edge_type'):
            ...
            if not metadata.supports_heterogeneous:
                errors.append(
                    f"Model does not support heterogeneous graphs, "
                    f"but data appears to be heterogeneous"
                )
...
class ModelFactory:
    ...
    def create_model(
        ...
        hyperparameters: Dict[str, Any],
        task_type: str,
        sample_data: Optional[Data] = None,
        ...
        if name.lower() == "custom":
            ...
            return self._create_custom_model(
                hyperparameters, task_type, sample_data, device
            )
        ...
        if name.lower() == "ensemble":
            ...
            num_classes_override = (
                hyperparameters.get('out_channels') or 
                hyperparameters.get('num_classes')
            )
        ...
        try:
            ...
            raise ModelInstantiationError(
                f"Failed to get metadata for model '{name}': {e}",
                model_name=name
            )
        ...
        if metadata.hyperparameters:
            try:
                self.validator.validate_hyperparameters(
                    processed_params, metadata.hyperparameters
                )
            except HyperparameterError as e:
                raise HyperparameterError(
                    f"Hyperparameter validation failed for model '{name}': {e}",
                    model_name=name
                )
        ...
        if model_class is not None:
            try:
                is_valid, errors = validate_params_against_signature(
                    model_class, processed_params
                )
                if not is_valid:
                    # Log as warning rather than error - introspected schema may be permissive
                    logger.warning(
                        f"Signature validation warnings for '{name}': {errors}"
                    )
            except Exception as e:
        ...
        if sample_data is not None:
            try:
                ...
                logger.warning(
                    f"Data compatibility warning for model '{name}': {e}"
                )
        ...
        try:
            ...
            raise ModelInstantiationError(
                f"Failed to get model class for '{name}': {e}",
                model_name=name
            )
        ...
        if self._is_graph_level_task(task_type) and not _skip_wrapper:
            ...
            elif sample_data is not None and hasattr(sample_data, 'y') and sample_data.y is not None:
                y = sample_data.y
                if y.dim() == 0:
        ...
        # CRITICAL: Skip wrapping when _skip_wrapper=True (used for ensemble models)
            ...
            if task_type.lower() == 'edge_regression':
                ...
                if edge_out_channels is None and sample_data is not None:
                    ...
                    if hasattr(sample_data, 'edge_attr') and sample_data.edge_attr is not None:
                        ...
                        logger.info(f"Inferred edge_out_channels={edge_out_channels} from sample_data.edge_attr")
                ...
                if edge_out_channels is not None and edge_out_channels > 1:
                    ...
                    logger.info(f"Using {decoder_method} decoder for edge_regression with {edge_out_channels} output channels")
            ...
            # For edge_classification, use MLP decoder with num_classes output channels
                ...
                edge_out_channels = processed_params.get('out_channels') or processed_params.get('num_classes')
                ...
                if edge_out_channels is None:
                    ...
                    logger.warning(f"edge_classification: num_classes not specified, defaulting to {edge_out_channels}")
                
                # Use MLP decoder for classification (outputs class logits)
                decoder_method = processed_params.get('decoder_method', 'hadamard_mlp')
                logger.info(f"Using {decoder_method} decoder for edge_classification with {edge_out_channels} output classes")
        ...
        logger.info(
            f"Model created successfully: {name} "
            f"({param_count:,} parameters) in {elapsed:.2f}s"
        )
    ...
    def create_model_with_info(
        ...
        sample_data: Optional[Data] = None,
        device: Optional[torch.device] = None,
        target_selection_config: Optional[Dict[str, Any]] = None,  # For regression target selection
        num_classes_override: Optional[int] = None,  # For classification with discretized targets
        ...
        if target_selection_config is not None and not is_classification:
            ...
            if ts_config.resolved_indices is not None:
                ...
                if sample_data is not None:
                    ...
                    # ================================================================
                    config_source = ts_config.config_source.lower() if ts_config.config_source else 'auto'
                    
                    # Determine which attribute to get property names from
                    if config_source == 'x' and hasattr(sample_data, 'x_property_names') and sample_data.x_property_names is not None:
                        available_names = list(sample_data.x_property_names)
                        logger.debug(f"Found x_property_names in sample_data: {available_names}")
                    elif config_source == 'edge_attr' and hasattr(sample_data, 'edge_attr_property_names') and sample_data.edge_attr_property_names is not None:
                        available_names = list(sample_data.edge_attr_property_names)
                        logger.debug(f"Found edge_attr_property_names in sample_data: {available_names}")
                    elif hasattr(sample_data, 'y_property_names') and sample_data.y_property_names is not None:
                    ...
                    if config_source == 'x':
                        if hasattr(sample_data, 'x') and sample_data.x is not None:
                            ...
                            if len(x_shape) > 0:
                                ...
                                logger.debug(f"Using x.shape for total_count: {total_count} (config_source='x')")
                    elif config_source == 'edge_attr':
                        if hasattr(sample_data, 'edge_attr') and sample_data.edge_attr is not None:
                            ea_shape = sample_data.edge_attr.shape
                            if len(ea_shape) > 0:
                                total_count = ea_shape[-1] if len(ea_shape) > 1 else ea_shape[0]
                                logger.debug(f"Using edge_attr.shape for total_count: {total_count} (config_source='edge_attr')")
                    else:
        ...
        edge_dim_configured = any(
            hyperparameters.get(param) is not None
            for param in detected_edge_params
        )
        ...
        model_info['original_out_channels'] = original_out_channels if target_selection else None  # NEW
        model_info['num_classes_override'] = num_classes_override  # For classification with discretization
        ...
        if hasattr(actual_model, 'in_channels') and actual_model.in_channels is not None:
            ...
            if 'in_channels' not in processed_hyperparams or processed_hyperparams.get('in_channels') != actual_model.in_channels:
        ...
        if hasattr(actual_model, 'out_channels') and actual_model.out_channels is not None:
            if 'out_channels' not in processed_hyperparams or processed_hyperparams.get('out_channels') != actual_model.out_channels:
        ...
        if hasattr(actual_model, 'hidden_channels') and actual_model.hidden_channels is not None:
            if 'hidden_channels' not in processed_hyperparams or processed_hyperparams.get('hidden_channels') != actual_model.hidden_channels:
        ...
        if hasattr(actual_model, 'num_layers') and actual_model.num_layers is not None:
            if 'num_layers' not in processed_hyperparams or processed_hyperparams.get('num_layers') != actual_model.num_layers:
    ...
    def _create_custom_model(
        ...
        try:
            ...
            raise ModelInstantiationError(
                f"Failed to parse architecture config: {e}",
                model_name="custom"
            )
        ...
        try:
            ...
            raise ModelInstantiationError(
                f"Failed to build custom architecture: {e}",
                model_name="custom"
            )
    ...
    def _create_ensemble_model(
        ...
        try:
            ...
            raise ModelInstantiationError(
                f"Failed to parse ensemble config: {e}",
                model_name="ensemble"
            )
            ...
            try:
                ...
                for level in sorted(models_by_level.keys()):
                    ...
                    for model_spec in models_by_level[level]:
                        ...
                        if num_classes_override is not None and is_classification and not is_edge_classification:
                            if 'out_channels' not in model_hparams:
                        ...
                        if needs_dimension_chaining:
                            model_out = None
                            actual_model = individual_model.model if hasattr(individual_model, 'model') else individual_model
                            ...
                            if model_out is None:
                                model_out = model_hparams.get('out_channels') or model_hparams.get('hidden_channels')
                ...
                raise ModelInstantiationError(
                    f"Failed to add models to ensemble: {e}",
                    model_name="ensemble"
                )
        ...
        try:
            ...
            if not validation['valid']:
                logger.warning(
                    f"Ensemble validation warnings: {validation.get('warnings', [])}"
                )
                if validation.get('errors'):
                    raise ModelInstantiationError(
                        f"Ensemble validation failed: {validation['errors']}",
                        model_name="ensemble"
                    )
        except Exception as e:
        ...
        if self._is_graph_level_task(task_type):
            ...
            if num_classes_override is not None:
                ...
                logger.debug(f"Graph-level ensemble: out_channels={wrapper_out_channels} from num_classes_override")
            elif 'out_channels' in hyperparameters:
                # Explicit out_channels in hyperparameters
                wrapper_out_channels = hyperparameters['out_channels']
                logger.debug(f"Graph-level ensemble: out_channels={wrapper_out_channels} from hyperparameters")
            elif sample_data is not None and hasattr(sample_data, 'y') and sample_data.y is not None:
                # Infer from sample_data for multi-target regression
                y = sample_data.y
                if y.dim() == 0:
                    ...
                    wrapper_out_channels = y.size(-1)
                logger.debug(f"Graph-level ensemble: out_channels={wrapper_out_channels} inferred from sample_data.y")
            
            model = GraphLevelModelWrapper(
                model, task_type, pooling_method, out_channels=wrapper_out_channels
            )
            logger.info(f"Wrapped ensemble for graph-level task: {task_type} (out_channels={wrapper_out_channels})")
        ...
        # =====================================================================
            ...
            if 'models' in config and config['models']:
                ...
                model_out_channels = (
                    first_model_hparams.get('out_channels') or 
                    first_model_hparams.get('hidden_channels') or
                    64  # Default hidden_channels
                )
                logger.debug(f"Inferred model_out_channels={model_out_channels} for edge-level ensemble")
            
            if task_type.lower() == 'edge_classification':
                edge_out_channels = num_classes_override
                if edge_out_channels is None:
                    edge_out_channels = 10  # Default
                    logger.warning(f"edge_classification ensemble: num_classes not specified, defaulting to {edge_out_channels}")
                decoder_method = 'hadamard_mlp'
                logger.info(f"Wrapping ensemble for edge_classification with {decoder_method} decoder ({edge_out_channels} classes)")
            
            elif task_type.lower() == 'edge_regression':
                # Infer from sample_data
                if sample_data is not None and hasattr(sample_data, 'edge_attr') and sample_data.edge_attr is not None:
                    edge_out_channels = sample_data.edge_attr.shape[-1]
                if edge_out_channels is not None and edge_out_channels > 1:
                    decoder_method = 'hadamard_mlp'
            ...
            model = EdgeLevelModelWrapper(
                model, task_type, decoder_method, edge_out_channels, 
        ...
        logger.info(
            f"Ensemble created with {num_models} models ({param_count:,} total parameters)"
        )
    ...
    def _process_hyperparameters(
        ...
        # =====================================================================
        if (filtered_params.get('add_self_loops') is True and 
            sample_data is not None):
            
            # Check if data has multi-dimensional edge_attr
            has_multidim_edge_attr = (
                hasattr(sample_data, 'edge_attr') and 
                sample_data.edge_attr is not None and
                sample_data.edge_attr.dim() >= 2 and
                sample_data.edge_attr.size(-1) > 1
            )
        ...
        # FUTURE-PROOF: Works for any new PyG model with any signature
        # =====================================================================
        is_edge_level_task = task_type.lower().startswith('edge_') or task_type.lower().startswith('link_')
        ...
        if is_pna_model:
            ...
            if 'aggregators' not in processed:
                ...
                logger.info(
                    f"PNA: Using default aggregators={processed['aggregators']}"
                )
            ...
            if 'scalers' not in processed:
                ...
                logger.info(
                    f"PNA: Using default scalers={processed['scalers']}"
                )
            ...
            if 'deg' not in processed:
                if sample_data is not None and hasattr(sample_data, 'edge_index'):
                    try:
                        ...
                        num_nodes = sample_data.num_nodes if hasattr(sample_data, 'num_nodes') else None
                        ...
                        max_degree = int(node_degrees.max().item()) if node_degrees.numel() > 0 else 0
                        deg_histogram = torch.bincount(
                            node_degrees, 
                    ...
                    logger.warning(
                        f"PNA: Cannot compute 'deg' without sample_data. "
                        f"PNA model requires degree histogram for normalization. "
                        f"Please provide 'deg' parameter explicitly or ensure sample_data is available."
                    )
        ...
        is_3d_molecular_model = any(pattern in model_name_lower for pattern in [
            'schnet', 'dimenet', 'painn', 'egnn', 'visnet', 'comenet', 
            'spherenet', 'gemnet', 'equiformer', 'mace', 'nequip'
        ])
        ...
        if is_3d_molecular_model:
            ...
            if sample_data is not None:
                ...
                if not hasattr(sample_data, 'z') or sample_data.z is None:
                    missing_attrs.append(
                        "'z' (atomic numbers) - required for atomic embeddings"
                    )
                ...
                if missing_attrs:
                    error_msg = (
                        ...
                        f"  - " + "\n  - ".join(missing_attrs) + "\n"
                        f"This model uses forward(z, pos, batch) signature instead of "
                        f"forward(x, edge_index, ...). Ensure your dataset contains:\n"
                        f"  - data.z: Atomic numbers tensor [num_atoms] (e.g., 6 for Carbon, 8 for Oxygen)\n"
                        f"  - data.pos: 3D coordinates tensor [num_atoms, 3] (in Angstroms)\n"
                        f"Common molecular datasets with these attributes: QM9, MD17, ANI, GEOM."
                    )
                    raise DataCompatibilityError(error_msg)
            ...
            if is_schnet:
                ...
                logger.debug(
                    f"SchNet: Using cutoff={cutoff_value}Å for interatomic interactions"
                )
        ...
        if is_autoencoder_model and not is_adversarial_autoencoder:
            ...
            encoder_hidden_channels = hyperparameters.get(
                ...
                hyperparameters.get('hidden_channels', 64)
            )
            ...
            encoder_out_channels = hyperparameters.get(
                ...
                hyperparameters.get('out_channels', 16)
            )
            encoder_dropout = hyperparameters.get(
                'encoder_dropout',
                hyperparameters.get('dropout', 0.0)
            )
            encoder_act = hyperparameters.get(
                'encoder_act',
                hyperparameters.get('act', 'relu')
            )
            ...
            encoder_config_params = [
                'encoder_type', 'encoder_hidden_channels', 'encoder_num_layers',
                'encoder_out_channels', 'encoder_dropout', 'encoder_act',
                'encoder_in_channels', 'in_channels', 'hidden_channels',
                'num_layers', 'dropout', 'act', 'out_channels'
            ]
            ...
            encoder_out_channels = hyperparameters.get(
                'encoder_out_channels',
                hyperparameters.get('out_channels', 32)
            )
            encoder_dropout = hyperparameters.get(
                'encoder_dropout',
                hyperparameters.get('dropout', 0.0)
            ...
            encoder_config_params = [
                ...
                'encoder_in_channels', 'in_channels', 'hidden_channels',
                'num_layers', 'dropout', 'act', 'out_channels',
                'discriminator_hidden_channels', 'discriminator_out_channels'
            ]
            for param in encoder_config_params:
                processed.pop(param, None)
    ...
    @staticmethod
    def _is_graph_level_task(task_type: Optional[str]) -> bool:
        """
    ...
    @staticmethod
    def _is_edge_level_task(task_type: Optional[str]) -> bool:
        """
        ...
        try:
            ...
            if hasattr(pyg_models, encoder_type_normalized):
                ...
                if hasattr(pyg_nn, encoder_type_normalized):
                    ...
                    raise AttributeError(
                        f"Encoder type '{encoder_type}' not found in PyG models"
                    )
        except Exception as e:
            raise ModelInstantiationError(
