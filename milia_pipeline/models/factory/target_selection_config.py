...
class TargetSelectionConfig(BaseModel):
    ...
    config_level: str | None = "auto"  # "auto", "graph", "node", "edge"
    config_source: str | None = "auto"  # "auto", "y", "x", "edge_attr", or custom

    # Resolved level and source (after inference/validation)
    resolved_level: TargetLevel | None = None
    resolved_source: TargetSource | None = None
    resolved_source_attr: str | None = None  # Actual attribute name (for custom sources)

    @classmethod
    def from_config(cls, config: dict[str, Any] | None) -> "TargetSelectionConfig":
        """
    ...
    @classmethod
    def infer_level_from_task_type(cls, task_type: str | None) -> TargetLevel:
        """
        ...
        if task_lower.startswith("node_"):
            ...
            logger.warning(f"Unknown task_type pattern '{task_type}', defaulting to GRAPH level")
            return TargetLevel.GRAPH

    @classmethod
    def infer_source_from_level(
        cls, level: TargetLevel, task_type: str | None = None
    ) -> TargetSource:
        """
    ...
    def resolve_for_task(
        ...
        if self.config_level and self.config_level != "auto":
            ...
            logger.debug(
                f"Auto-inferred level from task_type '{task_type}': {self.resolved_level.name}"
            )
    ...
    def _resolve_source_auto(
        ...
        if hasattr(data_sample, "y") and data_sample.y is not None:
            ...
            if level == TargetLevel.GRAPH:
                ...
                num_edges = (
                    data_sample.edge_index.size(1) if hasattr(data_sample, "edge_index") else None
                )
    ...
    def resolve(
        self, available_names: list[str] | None, total_count: int
    ) -> "TargetSelectionConfig":
        """
    ...
    def _resolve_properties(self, available_names: list[str] | None, total_count: int):
    ...
    def _resolve_indices(self, total_count: int, available_names: list[str] | None):
        ...
        self.resolved_indices = resolved_indices
        self.resolved_names = (
            [available_names[i] for i in resolved_indices] if available_names else None
        )

    def _resolve_range(self, total_count: int, available_names: list[str] | None):
        ...
        try:
            ...
            self.resolved_names = (
                [available_names[i] for i in self.resolved_indices] if available_names else None
            )
