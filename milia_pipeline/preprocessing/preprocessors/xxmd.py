...
class XXMDPreprocessor(BasePreprocessor):
    ...
    def _validate_config(self) -> None:
        ...
        for key in required_keys:
            if key not in self.config:
                raise ConfigurationError(
                    f"xxMD preprocessor missing required config key: {key}",
                    config_key=key
                )
        ...
        if not archive_path.exists():
            raise ConfigurationError(
                f"xxMD archive not found: {archive_path}",
        ...
        if not archive_path.name.endswith('.zip'):
            self.logger.warning(
                f"xxMD archive does not have .zip extension: {archive_path.name}"
            )
        ...
        if 'molecules_to_include' in self.config:
            ...
            if molecules is not None:
                ...
                if invalid_molecules:
                    valid_with_names = [f"{abbr} ({XXMD_MOLECULE_FULL_NAMES[abbr]})" 
                                        for abbr in XXMD_MOLECULES]
                    raise ConfigurationError(
    ...
    def preprocess(self) -> Path:
        ...
        try:
            ...
            self.logger.info(f"Unit conversion: eV → Hartree")
            self.logger.info("=" * 60)
            ...
            raise DataProcessingError(
                f"xxMD preprocessing failed: {e}",
                operation="xxmd_preprocessing"
            ) from e
    ...
    def _extract_archive(self, archive_path: Path) -> Path:
        ...
        try:
            ...
            if not xxmd_dft_dir:
                raise DataProcessingError(
                    "Could not find xxMD-DFT directory in archive",
            ...
            if xxmd_dft_dir.exists():
                ...
                self.logger.info(f"  xxMD-DFT directory contents: {[p.name for p in actual_contents]}")
                
                # Also check for case-insensitive matches
                actual_dirs = {p.name.lower(): p.name for p in actual_contents if p.is_dir()}
                self.logger.info(f"  Available subdirectories (lowercase): {list(actual_dirs.keys())}")
            ...
            for mol in found_molecules:
                ...
                if inner_zip.exists():
                    ...
                    try:
                        ...
                        extracted_files = [f.name for f in mol_dir.iterdir() if f.is_file() and f.suffix == '.xyz']
            ...
            raise DataProcessingError(
                f"Failed to extract xxMD archive: {e}",
                operation="archive_extraction"
            ) from e
    ...
    def _parse_xxmd_xyz_files(
        self,
        extracted_dir: Path,
        molecules_to_include: Optional[List[str]] = None,
        ...
        for molecule in molecules_to_include:
            ...
            for split in XXMD_SPLITS:
                ...
                if xyz_path is None:
                    self.logger.debug(f"    Split file not found for {split}, tried: {possible_patterns}")
                    continue
