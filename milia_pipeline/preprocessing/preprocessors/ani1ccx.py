...
class ANI1ccxPreprocessor(BasePreprocessor):
    ...
    def preprocess(self) -> Path:
        ...
        try:
            ...
            self.logger.info("NOTE: Same HDF5 file as ANI-1x, filtering to CC subset")
            ...
            raise DataProcessingError(
                f"ANI-1ccx preprocessing failed: {e}", operation="ani1ccx_preprocessing"
            ) from e
    ...
    def _parse_ani1ccx_h5(
        ...
        for data in iter_data_buckets_ccx(str(h5_path), keys=property_keys):
            ...
            if "wb97x_dz.hirshfeld_charges" in data:
                hirshfeld_charges_list.append(
                    np.ascontiguousarray(data["wb97x_dz.hirshfeld_charges"], dtype=np.float32)
                )
            else:
            ...
            if "wb97x_dz.cm5_charges" in data:
                cm5_charges_list.append(
                    np.ascontiguousarray(data["wb97x_dz.cm5_charges"], dtype=np.float32)
                )
            else:
        ...
        if any(e is not None for e in dft_energy_list):
            ...
            dft_energy_array = np.array(
                [e if e is not None else np.nan for e in dft_energy_list], dtype=np.float64
            )
    ...
    def _build_npz(
        ...
        self.logger.info(
            f"  Total ANI-1ccx conformers: {metadata.get('total_conformers', 'N/A'):,}"
        )
        self.logger.info("  Primary energy: ccsd_energy (CCSD(T)/CBS)")
