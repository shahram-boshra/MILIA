...
class RMD17Preprocessor(BasePreprocessor):
    ...
    def _validate_config(self) -> None:
        ...
        for key in required_keys:
            if key not in self.config:
                raise ConfigurationError(
                    f"rMD17 preprocessor missing required config key: {key}",
                    config_key=key
                )
        ...
        if not archive_path.exists():
            raise ConfigurationError(
                f"rMD17 archive not found: {archive_path}",
    ...
    def preprocess(self) -> Path:
        ...
        try:
            ...
            self.logger.info(f"Unit conversion: kcal/mol → Hartree")
            self.logger.info("=" * 60)
            ...
            raise DataProcessingError(
                f"rMD17 preprocessing failed: {e}",
                operation="rmd17_preprocessing"
            ) from e
    ...
    def _extract_archive(self, archive_path: Path) -> Path:
        ...
        try:
            ...
            if len(npz_files) == 0:
                raise DataProcessingError(
                    f"No rmd17_*.npz files found after extraction",
            ...
            raise DataProcessingError(
                f"Failed to extract rMD17 archive: {e}",
                operation="archive_extraction"
            ) from e
        ...
        for molecule in molecules_to_include:
            ...
            try:
                ...
                for conf_idx in range(n_conformers):
                    ...
                    atoms_list.append(
                        np.ascontiguousarray(nuclear_charges, dtype=np.uint8)
                    )
