    ...
    with h5py.File(h5filename, 'r') as f:
        for formula_name in f.keys():
            ...
            if 'type.raw' not in formula_group:
                logger.warning(f"Skipping {formula_name}: no 'type.raw' key found (not DeePMD-kit format)")
                continue
            ...
            atomic_numbers = np.array([
                ELEMENT_TO_Z.get(type_map[int(at)], 0) 
...
class QDPiPreprocessor(BasePreprocessor):
        ...
        try:
            ...
            if not h5_files:
                raise DataProcessingError(
                    "No HDF5 files found in QDπ dataset",
                    operation="find_h5_files"
                )
            
            self.logger.info(f"Found {len(h5_files)} HDF5 files to process")
            
            # Step 3: Parse all HDF5 files and extract data
            features, metadata = self._parse_qdpi_h5_files(
                h5_files, hdf5_keys, max_conformers
            )
            ...
            raise DataProcessingError(
                f"QDπ preprocessing failed: {e}",
                operation="qdpi_preprocessing"
            ) from e
    ...
    def _get_data_path(self, archive_path: Path) -> Path:
        ...
        for candidate in [
            ...
            if candidate.exists() and (
                (candidate / 'neutral').exists() or 
                (candidate / 'charged').exists()
            ):
    ...
    def _find_h5_files(
        ...
        if include_neutral and neutral_dir.exists():
            ...
            for h5_file in neutral_dir.glob('*.h5'):
                h5_files.append((h5_file, 'neutral'))
            self.logger.info(f"Found {len([f for f, t in h5_files if t == 'neutral'])} neutral HDF5 files")
    ...
    def _parse_qdpi_h5_files(
        ...
        for h5_path, charge_type in h5_files:
            ...
            for data in iter_data_buckets_qdpi(str(h5_path), keys=property_keys, charge_type=charge_type):
            ...
            self.logger.info(f"  → Extracted {file_conformer_count:,} conformers from {h5_path.name}")
