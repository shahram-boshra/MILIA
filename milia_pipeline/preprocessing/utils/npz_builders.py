...
def build_npz(
    ...
    if not features:
        raise DataProcessingError(
            "Cannot create .npz with empty features",
    ...
    for key, array in features.items():
        if key in ['compounds', 'atoms', 'coordinates'] and len(array) != num_molecules:
            raise DataProcessingError(
                f"Shape mismatch: '{key}' has {len(array)} entries, "
                f"expected {num_molecules}",
                operation="npz_creation"
            )
    ...
    try:
        ...
        if not output_path.exists():
            raise DataProcessingError(
                f"File not created: {output_path}",
                operation="npz_creation"
            )
