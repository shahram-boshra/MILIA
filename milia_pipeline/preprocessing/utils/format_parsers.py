...
FEATURE_TIERS = {
    'basic': [
        ...
        'homo_energy_eV', 'homo_index', 'lumo_energy_eV', 'lumo_index', 'homo_lumo_gap_eV'
    ],
    'standard': [
    ...
    'complete': [
        ...
        'n_occupied_orbitals', 'n_virtual_orbitals', 'n_shells',
...
def parse_molden_files(
    ...
    if feature_tier not in FEATURE_TIERS:
        raise DataProcessingError(
            f"Invalid feature tier '{feature_tier}'. "
            f"Available: {list(FEATURE_TIERS.keys())}"
        )
...
#---    
    ...
    for molden_file in molden_files:
        try:
            ...
            mol_features = _extract_molecule_features(
                mol_data, 
                compound_name,
                feature_tier
            )
    ...
    if feature_tier == 'complete':
        if 'homo_energy_eV' in features and 'lumo_energy_eV' in features:
            ...
            if features['chemical_hardness_eV'] != 0:
                features['electrophilicity_eV'] = (
                    features['chemical_potential_eV']**2 / (2 * features['chemical_hardness_eV'])
                )
...
def _convert_to_numpy_arrays(features: Dict[str, list]) -> Dict[str, np.ndarray]:
    ...
    OBJECT_DTYPE_FEATURES = {
        'compounds', 'atoms', 'coordinates', 'n_atoms', 'n_electrons',
        'mo_energies', 'mo_occupations', 'mo_coefficients',
        'molecular_formula', 'mo_kind'
    }
    
    FLOAT64_FEATURES = {
        'homo_energy_eV', 'lumo_energy_eV', 'homo_lumo_gap_eV',
        'total_energy_eV', 'total_energy_Hartree',
        'molecular_weight', 'ionization_potential_eV', 
        'electron_affinity_eV', 'chemical_hardness_eV',
        'chemical_potential_eV', 'electrophilicity_eV',
        'mo_energy_mean_eV', 'mo_energy_std_eV',
        'mo_energy_min_eV', 'mo_energy_max_eV'
    }
    
    INT64_FEATURES = {
        'homo_index', 'lumo_index', 'n_occupied_orbitals',
        'n_virtual_orbitals', 'n_basis_functions', 'n_shells'
    }
