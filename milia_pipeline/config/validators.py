...
import inspect
import json
from typing import Any, Union, Dict, List, Optional, Tuple, Set, Callable, Type
...
try:
    from milia_pipeline.exceptions import (
        MoleculeProcessingError, PropertyEnrichmentError, ValidationError,
        HandlerError, HandlerValidationError, HandlerConfigurationError,
        HandlerCompatibilityError, DatasetSpecificHandlerError,
        UncertaintyProcessingError, StructuralFeatureError,
        # Transformation exceptions
        TransformationError, TransformValidationError, TransformCompositionError,
        ConfigurationError
    )
except ImportError:
    ...
    class UncertaintyProcessingError(MoleculeProcessingError):
        def __init__(self, message: str, dataset_type: str = "", molecule_index: int = None, 
                     inchi: str = None, detail: str = None, uncertainty_property_name: str = None, **kwargs):
...
try:
    from milia_pipeline.config.config_containers import (
        TransformationConfig, ExperimentalSetup, TransformSpec
    )
...
def _get_available_dataset_types() -> List[str]:
    ...
    try:
        ...
        if implementations_dir.exists():
            ...
            if discovered_types:
                logger.debug(f"Validators: Dynamically discovered dataset types: {discovered_types}")
    ...
    logger.warning("Validators: No dataset types available - registry not initialized and dynamic discovery failed")
    return []
...
def wrap_pydantic_validation_error(pydantic_error: 'PydanticValidationError') -> ValidationResult:
    ...
    if not PYDANTIC_AVAILABLE:
        raise RuntimeError(
            "Pydantic is not available. Install pydantic>=2.0 to use this function."
        )
...
class TransformValidator:
    ...
    def validate_transform_config(
        ...
        """
        self.issues.clear()
        location_prefix = f"experimental_setup[{experimental_setup}]" if experimental_setup else "transforms"
    ...
    def _get_transform_metadata(self, transform_name: str) -> Optional[Dict[str, Any]]:
        """
    ...
    def _check_type(self, value: Any, expected_type: str) -> bool:
        ...
        expected = type_map.get(expected_type, None)
        ...
        try:
            ...
            for idx, issue_msg in enumerate(semantic_issues):
                ...
                issues.append(ValidationIssueDetail(
                    severity=severity,
                    message=issue_msg,
                    location=f"transforms.sequence",
            ...
            if arr_value.dtype.kind in ['U', 'S']:
                for element in arr_value.flat:
                    ...
                    try:
                        ...
                        if check_finite and not np.isfinite(float_val):
                            return False
                        elif not check_finite and np.isnan(float_val):
                            return False
                    except (ValueError, TypeError):
                        return False
                return True
...
def validate_molecular_structure(
    ...
    if not is_value_valid_and_not_nan(atoms):
        raise StructuralFeatureError(
            message=f"Invalid or missing atoms data",
    ...
    if not is_value_valid_and_not_nan(coordinates):
        raise StructuralFeatureError(
            message=f"Invalid or missing coordinates data",
            molecule_index=molecule_index,
            inchi=identifier,
...
def convert_to_scalar(
    value: Union[np.ndarray, torch.Tensor, float, int, None],
    ...
    if value is None:
        if allow_none:
            ...
            raise ValidationError(
                message=f"Value is None but allow_none=False",
    ...
    # Handle numpy arrays
        ...
        if value.size == 0:
            ...
            if allow_none:
                logger.debug(f"{error_context}: Cannot convert array of size {value.size} to scalar")
                return None
            else:
                raise ValidationError(
    ...
    # Handle PyTorch tensors
        ...
        if value.numel() == 0:
            ...
            if not np.isfinite(scalar_val):
                if allow_none:
                    logger.debug(f"{error_context}: Tensor scalar value is not finite: {scalar_val}")
                    return None
                else:
                    raise ValidationError(
            ...
            if allow_none:
                logger.debug(f"{error_context}: Cannot convert tensor of size {value.numel()} to scalar")
                return None
            else:
                raise ValidationError(
    ...
    # Handle lists and tuples
        ...
        if len(value) == 0:
            ...
            if allow_none:
                logger.debug(f"{error_context}: Cannot convert list/tuple of length {len(value)} to scalar")
                return None
            else:
                raise ValidationError(
    ...
    # Handle string values (attempt numeric conversion)
        ...
        try:
            ...
            if not np.isfinite(scalar_val):
                if allow_none:
                    logger.debug(f"{error_context}: String converted to non-finite value: {scalar_val}")
                    return None
                else:
                    raise ValidationError(
...
def validate_uncertainty_data(
    ...
    if uncertainty_scalar is None:
        error_msg = f"{context}: {uncertainty_field_name} is None or could not be converted to scalar"
        if uncertainty_field_name in ['std', 'variance', 'uncertainty']:
            raise UncertaintyProcessingError(
                message=error_msg,
                dataset_type="",
    ...
    if require_positive and uncertainty_scalar < 0:
        error_msg = f"{context}: {uncertainty_field_name} must be non-negative, got {uncertainty_scalar}"
        if uncertainty_field_name in ['std', 'variance', 'uncertainty']:
            raise UncertaintyProcessingError(
                message=error_msg,
                dataset_type="",
...
def validate_molecular_data_dict(
    ...
    identifier: str = "N/A",
    dataset_type: Optional[str] = None,
    property_validators: Optional[Dict[str, callable]] = None,
    strict: bool = False,  
    ...
    """
    errors = []
    context = f"Molecule {molecule_index} ({identifier})" if molecule_index is not None else f"Molecule ({identifier})"
    ...
    if dataset_type and _get_dataset_feature(dataset_type, 'uncertainty_handling'):
        ...
        if 'Etot' in molecular_data:
            ...
            if etot_value is None:
                ...
                try:
                    etot_scalar = convert_to_scalar(
                        etot_value, 'Etot', context, allow_none=False
                    )
                    if etot_scalar is None:
                        ...
                        errors.append(f"{context}: {dataset_type} energy magnitude unusually large: {etot_scalar}")
                except ValidationError as e:
    ...
    if dataset_type and _get_dataset_feature(dataset_type, 'vibrational_analysis'):
        ...
        if 'freqs' in molecular_data and 'vibmodes' in molecular_data:
            ...
            if (isinstance(freqs, (list, np.ndarray)) and 
                isinstance(vibmodes, (list, np.ndarray))):
    ...
    if 'atoms' in molecular_data and 'coordinates' in molecular_data:
        ...
        if (is_value_valid_and_not_nan(atoms) and 
            is_value_valid_and_not_nan(coordinates)):
            try:
...
def validate_coordinates_3d(
    ...
    if not validate_array_shape(coordinates, expected_shape=(num_atoms, 3), allow_empty=allow_empty):
    ...
    if not validate_numeric_range(coordinates, min_val=-1000, max_val=1000, name=f"{identifier} coordinates"):
...
def validate_atomic_numbers(
    atomic_numbers: Union[np.ndarray, List[int]],
    identifier: str = "molecule"
) -> bool:
    """
    ...
    return validate_numeric_range(
        ...
        min_val=1, 
        max_val=118, 
        name=f"{identifier} atomic numbers"
    )
...
def validate_batch_consistency(
    ...
    property_shapes: Optional[Dict[str, Tuple[int, ...]]] = None,
    allow_empty_arrays: Optional[Set[str]] = None
) -> Tuple[bool, List[str]]:
    """
                ...
                if key in data:
                    ...
                    if hasattr(value, 'shape'):
                        ...
                        if len(actual_shape) != len(expected_shape):
                            ...
                            for j, (actual, expected) in enumerate(zip(actual_shape, expected_shape)):
                                if expected != -1 and actual != expected:
                                    errors.append(
...
def is_valid_molecule_identifier(
    identifier: str,
...
def validate_handler_molecular_batch(
    ...
    for i, mol_data in enumerate(molecular_data_list):
        ...
        is_mol_valid, mol_errors = validate_molecular_data_dict(
            mol_data, required_properties, molecule_index=i, 
            identifier=identifier, dataset_type=dataset_type
        )
...
def validate_handler_compatibility(
    ...
    if compatibility.get('supports_uncertainty', False):
        ...
        if 'freqs' in vector_props or 'freqs' in var_props:
            issues.append(f"{handler_type} handler incompatible with vibrational frequency processing")
    ...
    if compatibility.get('supports_vibrational', False):
        ...
        if atomization_key and not atomization_from:
            issues.append(f"{handler_type} atomization energy key specified but no base energy source")
...
def create_validation_report(
    ...
    if include_recommendations:
        ...
        if 'total_molecules' in validation_results and validation_results['total_molecules'] > 0:
            valid_rate = (validation_results['valid_molecules'] / 
                         validation_results['total_molecules']) * 100
            
            if valid_rate < 90:
                recommendations.append("Low validation success rate - review data preprocessing")
            elif valid_rate < 99:
                recommendations.append("Some validation failures - consider filtering invalid molecules")
...
def validate_transform_spec(
    transform_spec: Union[Dict[str, Any], 'TransformSpec'],
...
def validate_experimental_setup(
    ...
    if TRANSFORMATION_CONTAINERS_AVAILABLE and isinstance(experimental_setup, ExperimentalSetup):
        setup_dict = {
            ...
            'transforms': [{'name': t.name, 'kwargs': t.kwargs, 'enabled': t.enabled} 
    ...
    if not transforms:
        if strict_mode:
            ...
            logger.debug(f"Experimental setup '{setup_name}' has no experimental transforms (standard transforms may still apply)")
...
def validate_transformation_config(
    transformation_config: Union[Dict[str, Any], 'TransformationConfig'],
    strict_mode: bool = True
) -> Tuple[bool, List[str]]:
    """
    ...
    if TRANSFORMATION_CONTAINERS_AVAILABLE and isinstance(transformation_config, TransformationConfig):
        config_dict = {
            'experimental_setups': {name: [{'name': t.name, 'kwargs': t.kwargs} 
                                           for t in setup.transforms]
        ...
        if hasattr(transformation_config, 'standard_transforms') and transformation_config.standard_transforms:
    ...
    if not has_experimental_setups and not has_standard_transforms:
        errors.append("Missing required field: 'experimental_setups' or 'standard_transforms' (at least one required)")
        return False, errors
    ...
    if has_experimental_setups:
        if not isinstance(experimental_setups, dict):
            errors.append(f"'experimental_setups' must be a dictionary, got {type(experimental_setups)}")
            return False, errors
    
    # At least one transform source must have content in strict mode
    if not experimental_setups and not standard_transforms:
        if strict_mode:
            errors.append("At least one transform must be defined in 'experimental_setups' or 'standard_transforms'")
...
def validate_transform_composition_rules(
    ...
    if last_structural_idx > first_augmentation_idx:
        warnings.append("Structural transforms should typically come before augmentation transforms")
    ...
    if any(t in transform_names for t in geometric_transforms):
        warnings.append("Geometric transforms require 3D coordinate data - ensure data is available")
    ...
    try:
        ...
        if _get_dataset_feature(dataset_type, 'orbital_analysis'):
            ...
            if category == 'electronic':
                logger.info(
                    f"{dataset_type} dataset: Electronic descriptors can use quantum data"
                )
            elif category == 'geometric':
                warnings.append(
                    "Geometric descriptors will use optimized coordinates"
                )
        else:
...
def validate_transforms_with_fallback(
    ...
    logger: Optional[logging.Logger] = None,
    detailed: bool = False
) -> Union[bool, Tuple[bool, List[ValidationIssueDetail]]]:
    """
...
def run_validation_diagnostics() -> Dict[str, Any]:
    ...
    try:
        results["convert_to_scalar"] = (
            convert_to_scalar(3.14) == 3.14 and
            convert_to_scalar(np.array([2.71])) == 2.71 and
            abs(convert_to_scalar(torch.tensor(1.41)) - 1.41) < 1e-6 and
            convert_to_scalar(None, allow_none=True) is None  
        )
    except Exception as e:
...
def run_handler_validation_tests() -> Dict[str, bool]:
    ...
    try:
        ...
        # ====================================================================
        available_types = _get_available_dataset_types()
        results["dynamic_types_available"] = len(available_types) >= 0  # Dynamic discovery, count varies
        ...
        for handler_type in available_types:
            ...
            config = {
                ...
                'uncertainty_config': {'uncertainty_field_name': 'std'} 
                    if compatibility.get('supports_uncertainty') else None,
                'is_uncertainty_enabled': compatibility.get('supports_uncertainty', False)
            }
            
            processing = {
                'scalar_graph_targets': ['Etot'],
                'node_features': ['atomic_number'] if not compatibility.get('supports_uncertainty') else [],
            }
            ...
            results[f"{handler_type.lower()}_handler_compatible"] = is_compatible and len(issues) == 0
        ...
        results["unknown_type_rejected_with_available"] = (
            not is_compatible and 
            any('Available' in issue for issue in issues)
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        none_error_found = any('None' in err or 'missing' in err.lower() or 'invalid' in err.lower() 
                              for err in errors)
        
        if not is_valid and none_error_found:
            pitfall_tests["molecular_dict_none_handling"] = True
            print(f"  ✓ PASS: Correctly detected invalid None energy")
    ...
    try:
        ...
        if result_rejected and result_allowed:
            ...
            print(f"  ✗ FAIL: Empty list handling incorrect: rejected={result_rejected}, allowed={result_allowed}")
    except Exception as e:
        pitfall2_tests["empty_list_handling"] = False
        print(f"  ✗ FAIL: Unexpected exception: {type(e).__name__}: {e}")
    ...
    try:
        ...
        result_fail = validate_property_value(empty_freqs, 'freqs', expected_type=np.ndarray, allow_empty=False)
        
        # Should pass with allow_empty
        result_pass = validate_property_value(empty_freqs, 'freqs', expected_type=np.ndarray, allow_empty=True)
        ...
        if not result_fail and result_pass:
            ...
            print(f"  ✗ FAIL: Expected fail=False, pass=True, got fail={result_fail}, pass={result_pass}")
    except Exception as e:
        pitfall2_tests["property_value_empty"] = False
        print(f"  ✗ FAIL: Unexpected exception: {type(e).__name__}: {e}")
    ...
    try:
        ...
        if not is_valid_fail and is_valid_pass:
            ...
            print(f"  ✗ FAIL: Expected fail=False, pass=True")
    ...
    try:
        ...
        if is_valid:
            ...
            print(f"  ✗ FAIL: Vibrational-analysis validation should accept empty vibrations")
    ...
    try:
        ...
        if result_rejected and result_allowed:
            ...
            print(f"  ✗ FAIL: Empty tensor handling incorrect")
    except Exception as e:
        pitfall2_tests["empty_tensor_handling"] = False
        print(f"  ✗ FAIL: Unexpected exception: {type(e).__name__}: {e}")
    ...
    def validate_and_require(
        ...
        **kwargs
        ...
        if isinstance(result, tuple) and len(result) >= 2:
            ...
            if not is_valid:
                msg = error_message or f"Validation failed: {'; '.join(errors[:3]) if errors else 'Unknown error'}"
                raise exception_class(msg)
    ...
    class ValidationContext:
        ...
        def check(self, is_valid: bool, errors: Optional[List[str]] = None) -> bool:
