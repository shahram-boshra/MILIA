...
from typing import Dict, Optional, Union, Any, List, Tuple, Type
...
def _get_config_value(config: Dict[str, Any], key: str, expected_type, parent_key: Optional[str] = None):
    """
...
def _get_dataset_constants_local() -> Tuple[str, Optional[str], str]:
...
_TEMP_CONFIG: Optional[Dict[str, Any]] = None
...
@property 
def RAW_DATA_DOWNLOAD_URL() -> Optional[str]:
...
SUPPORTED_HANDLER_TYPES: List[str] = ['DFT', 'DMC', 'Wavefunction', 'QM9', 'ANI1x', 'ANI1CCX', 'RMD17', 'ANI2x', 'XXMD', 'QDPi']  # LEGACY - use get_supported_handler_types()
...
HANDLER_OPTIONAL_PROPERTIES: Dict[str, List[str]] = {
    ...
    'QM9': ['A', 'B', 'C', 'mu', 'alpha', 'homo', 'lumo', 'gap', 'r2', 'zpve', 'U', 'H', 'G', 'Cv', 'freqs', 'Qmulliken'],
    'ANI1x': ['forces', 'hirshfeld_charges', 'cm5_charges', 'dipole', 'molecule_id'],
    'ANI1CCX': ['dft_energy', 'forces', 'hirshfeld_charges', 'cm5_charges', 'dipole', 'molecule_id'],
...
def _get_handler_specific_constants(handler_type: str, dataset_config: Dict[str, Any]) -> Dict[str, Any]:
    """
...
def _validate_dmc_handler_config(config: Dict[str, Any]) -> List[str]:
    ...
    if isinstance(uncertainty_config, dict):
        ...
        if validation_mode not in valid_modes:
            errors.append(f"Invalid uncertainty validation_mode: {validation_mode}. Must be one of {valid_modes}")
...
TRANSFORMATION_SYSTEM_AVAILABLE: Optional[bool] = None
...
def _check_transformation_system_availability() -> bool:
    ...
    try:
        ...
        from milia_pipeline.config.config_containers import TransformationConfig, ExperimentalSetup
...
def _get_config_transformation_constants() -> Dict[str, Any]:
    ...
    from milia_pipeline.config.config_accessors import get_transformation_config, list_experimental_setups
    ...
    try:
        ...
        return {
            ...
            'default_setup': transform_config.default_setup if hasattr(transform_config, 'default_setup') else DEFAULT_EXPERIMENTAL_SETUP_NAME,
            'validation_enabled': transform_config.validation.enabled if hasattr(transform_config, 'validation') else True,
...
def _get_transform_registry_info() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            'total_transforms': sum(len(transforms) for transforms in available_transforms.values()),
            'transforms_by_category': {cat: len(transforms) for cat, transforms in available_transforms.items()},
...
def get_handler_transform_compatibility(handler_type: str, 
...
def get_compatible_transforms_for_handler(handler_type: str, 
                                         include_warnings: bool = False) -> List[str]:
    """
    ...
    for transform_name, status in handler_compat.items():
        ...
        elif status == 'warning' and include_warnings:
            compatible.append(transform_name)
...
def validate_experimental_setup_for_handler(handler_type: str, 
                                           setup_name: str) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for transform_spec in setup.transforms:
            transform_name = transform_spec.name if hasattr(transform_spec, 'name') else transform_spec.get('name')
...
def create_handler_config_from_constants(handler_type: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if validation_errors:
            raise HandlerConfigurationError(
                f"Handler configuration validation failed",
                handler_type=handler_type,
        ...
        raise HandlerConfigurationError(
            f"Failed to create handler configuration",
            handler_type=handler_type,
            details=str(e)
        )
...
def validate_handler_environment() -> Dict[str, bool]:
    ...
    try:
        ...
        try:
            validation_results['atomic_energies_available'] = bool(const_module.ATOMIC_ENERGIES_HARTREE)
        except Exception:
        ...
        try:
            validation_results['dataset_constants_available'] = bool(const_module._dataset_constants)
        except Exception:
        ...
        validation_results['migration_compatibility_available'] = bool(get_migration_compatibility_constants())
...
def get_legacy_compatible_constants(handler_type: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    for handler_type in get_supported_handler_types():
        try:
            ...
            debug_info['handler_constants'][handler_type] = {
                ...
                'molecule_creation_strategy': constants.get('molecule_creation_strategy', 'unknown')
            }
        except Exception as e:
            debug_info['handler_constants'][handler_type] = {
...
def validate_complete_environment() -> Dict[str, bool]:
    ...
    try:
        validation_results['transformation_system_available'] = _check_transformation_system_availability()
        ...
        if validation_results['transformation_system_available']:
            ...
            validation_results['experimental_setups_available'] = transform_constants.get('setup_count', 0) > 0
        else:
...
def __getattr__(name: str):
    ...
    if name == 'ATOMIC_ENERGIES_HARTREE':
        if name not in _CONSTANTS_CACHE:
            _CONSTANTS_CACHE[name] = _get_config_value(_TEMP_CONFIG, 'atomic_energies_hartree', dict)
        return _CONSTANTS_CACHE[name]
    
    elif name == 'HEAVY_ATOM_SYMBOLS_TO_Z':
        if name not in _CONSTANTS_CACHE:
            _CONSTANTS_CACHE[name] = _get_config_value(_TEMP_CONFIG, 'heavy_atom_symbols_to_z', dict)
        return _CONSTANTS_CACHE[name]
