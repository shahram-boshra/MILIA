...
_CACHED_REGISTRY_TYPES: Optional[List[str]] = None


def _get_registry_from_config_loader() -> Tuple[bool, Optional[Callable], Optional[Callable], Optional[Callable]]:
    """
...
class DatasetConfig(BaseModel, frozen=True):
    ...
    def get_handler_config(self, handler_type: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    def get_required_properties(self) -> List[str]:
        ...
        if self.dataset_type == "DFT":
            ...
            return ['atoms', 'coordinates', 'compounds']  # compounds = molecule name from .molden filename
        else:
            return ['Etot', 'atoms', 'coordinates']
    ...
    def validate_handler_compatibility(self) -> Tuple[bool, List[str]]:
        ...
        if not self.dataset_type:
            ...
            valid_types = _get_valid_dataset_types()
            errors.append(f"Invalid dataset_type: {self.dataset_type}. Must be one of {valid_types}")
    ...
    def validate_filter_config(self) -> Tuple[bool, List[str]]:
        ...
        for handler_type, filters in self.handler_filters.items():
            ...
            if not _is_valid_dataset_type(handler_type):
                valid_types = _get_valid_dataset_types()
                errors.append(f"Unknown handler type in filters: {handler_type}. Valid types: {valid_types}")
...
class StructuralFeaturesConfig(BaseModel, frozen=True):
    ...
    def validate_feature_config(self) -> Tuple[bool, List[str]]:
        ...
        for handler_type, features in self.handler_features.items():
            if not _is_valid_dataset_type(handler_type):
                valid_types = _get_valid_dataset_types()
                errors.append(f"Unknown handler type in features: {handler_type}. Valid types: {valid_types}")
...
class ProcessingConfig(BaseModel, frozen=True):
    ...
    def get_handler_processing_config(self, handler_type: str) -> Dict[str, Any]:
        ...
        if handler_type.upper() == 'DFT':
            if self.calculate_atomization_energy_from:
                base_config['calculate_atomization_energy_from'] = self.calculate_atomization_energy_from
    ...
    def validate_processing_config(self) -> Tuple[bool, List[str]]:
        ...
        for handler_type, config in self.handler_processing.items():
            if not _is_valid_dataset_type(handler_type):
                valid_types = _get_valid_dataset_types()
                errors.append(f"Unknown handler type in processing config: {handler_type}. Valid types: {valid_types}")
...
class HandlerConfig(BaseModel, frozen=True):
    ...
    def initialize_dict_fields(cls, data: Any) -> Any:
        """Initialize None dict fields to empty dicts before field assignment."""
        if isinstance(data, dict):
            dict_fields = ['validation_settings', 'processing_settings', 'error_handling', 
    ...
    def validate_handler_config(self) -> Tuple[bool, List[str]]:
        ...
        if not _is_valid_dataset_type(self.handler_type):
            valid_types = _get_valid_dataset_types()
            errors.append(f"Invalid handler_type: {self.handler_type}. Must be one of {valid_types}")
        ...
        if recovery_mode not in valid_recovery_modes:
            errors.append(f"Invalid error recovery mode: {recovery_mode}. Must be one of {valid_recovery_modes}")
...
class TransformSpec(BaseModel, frozen=True):
    ...
    @classmethod
    def validate_kwargs(cls, v: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
...
class TransformationConfig(BaseModel, frozen=True):
    ...
    @classmethod
    def validate_experimental_setups(cls, v: Dict[str, ExperimentalSetup]) -> Dict[str, ExperimentalSetup]:
    ...
    @classmethod
    def validate_standard_transforms(cls, v: Optional[List[TransformSpec]]) -> Optional[List[TransformSpec]]:
    ...
    def get_setup(self, name: str) -> Optional[ExperimentalSetup]:
        """
    ...
    def get_cache_key(self) -> str:
        ...
        setup_keys = {name: setup.get_cache_key() for name, setup in self.experimental_setups.items()}
    ...
    def validate_transformation_config(self) -> Tuple[bool, List[str]]:
        ...
        if not isinstance(self.experimental_setups, dict):
            errors.append(f"experimental_setups must be a dictionary, got {type(self.experimental_setups).__name__}")
            return False, errors
        ...
        for name, setup in self.experimental_setups.items():
            if not isinstance(setup, ExperimentalSetup):
                errors.append(f"Setup '{name}' must be an ExperimentalSetup instance, got {type(setup).__name__}")
                continue
        ...
        if not isinstance(self.performance_settings, dict):
            errors.append(f"performance_settings must be a dictionary, got {type(self.performance_settings).__name__}")
        
        if not isinstance(self.migration_metadata, dict):
            errors.append(f"migration_metadata must be a dictionary, got {type(self.migration_metadata).__name__}")
        
        if not isinstance(self.research_metadata, dict):
            errors.append(f"research_metadata must be a dictionary, got {type(self.research_metadata).__name__}")
    ...
    def get_combined_transforms(self, setup_name: Optional[str] = None) -> List[TransformSpec]:
        """
    ...
    def get_combined_transforms_as_dicts(self, setup_name: Optional[str] = None) -> List[Dict[str, Any]]:
        """
...
class DescriptorConfig(BaseModel, frozen=True):
    ...
    def get_category_descriptors(self, category: str) -> Optional[List[str]]:
        """
...
def create_dataset_config_from_global(logger: Optional[logging.Logger] = None) -> DatasetConfig:
    """
    ...
    try:
        ...
        uncertainty_config = data_config.get('dmc_uncertainty', {}) if dataset_type == 'DMC' else None
        uncertainty_enabled = uncertainty_config.get('use_for_loss_weighting', False) if uncertainty_config else False
...
def create_filter_config_from_global(logger: Optional[logging.Logger] = None) -> FilterConfig:
    """
...
def create_processing_config_from_global(logger: Optional[logging.Logger] = None) -> ProcessingConfig:
    """
    ...
    try:
        ...
        if data_config.get('calculate_atomization_energy_from'):
            dft_config['calculate_atomization_energy_from'] = data_config['calculate_atomization_energy_from']
...
def create_structural_features_config_from_global(logger: Optional[logging.Logger] = None) -> StructuralFeaturesConfig:
    """
...
def _is_v1_format_by_content(transformations_list):
    ...
    for transform in transformations_list:
        if isinstance(transform, dict):
            ...
            v1_fields = ['type', 'active', 'parameters', 'info', 'transformation_type', 'is_enabled']
...
def create_transformation_config_from_global(logger: Optional[logging.Logger] = None) -> TransformationConfig:
    """
    ...
    try:
                ...
                try:
                    if isinstance(transform_spec, dict):
                        if 'name' not in transform_spec:
                            logger.warning(f"Standard transform {idx}: Missing 'name' field - skipped")
                            continue
        ...
        # ============================================================================
        if isinstance(transformations_config, dict) and 'experimental_setups' in transformations_config:
                ...
                for setup_name, setup_config in raw_setups.items():
                    try:
                        ...
                        if isinstance(setup_config, dict):
                            ...
                            logger.debug(f"Created setup '{setup_name}' with {len(validated_transforms)} transforms")
                                ...
                                try:
                                    if isinstance(transform_spec, dict):
                                        ...
                                        validated_transforms.append({
                                            'name': transform_spec['name'],
                                            'enabled': transform_spec.get('enabled', True),
                                            'params': transform_spec.get('params', transform_spec.get('parameters', {}))
                                        })
                                    elif isinstance(transform_spec, str):
                                        validated_transforms.append({
                                            'name': transform_spec,
                                    ...
                                    logger.warning(f"Setup '{setup_name}' transform {idx} error: {e}")
                                    migration_metadata["invalid_transforms_skipped"] += 1
                                    continue
                        ...
                        logger.error(f"Failed to create experimental setup '{setup_name}': {setup_error}")
            ...
            if 'default_setup' in transformations_config:
                ...
                if specified_default in experimental_setups:
                    ...
                    if experimental_setups:
                        ...
                        logger.info(f"Using first available setup as default: '{default_setup_name}'")
        ...
        # ============================================================================
            ...
            format_type = type(transformations_config).__name__ if transformations_config else "empty"
            migration_metadata.update({
                "source_format": format_type,
                "fallback_used": True,
                "migration_warnings": [f"Unknown or empty transformations config format: {format_type}"]
            })
        ...
        if not experimental_setups:
            logger.warning("No experimental setups created through any migration path, creating fallback")
            migration_metadata.update({
                "fallback_used": True,
        ...
        research_metadata = _extract_research_metadata(global_metadata, experimental_settings, config)
...
def create_descriptor_config_from_yaml(
    ...
    try:
        ...
        logger.info(f"Created descriptor configuration: "
...
def _migrate_v1_style_transforms(transformations_list: List[Dict], 
                                logger: Optional[logging.Logger] = None) -> Tuple[Dict[str, ExperimentalSetup], Dict]:
    ...
    for i, transform in enumerate(transformations_list):
        if isinstance(transform, dict):
            ...
            name = (transform.get('type') or           # V1 primary field
                   transform.get('transformation_type') or  
                   transform.get('name') or
                   transform.get('class'))
            
            enabled = (transform.get('active') if 'active' in transform else   # V1 primary field
                      transform.get('is_enabled') if 'is_enabled' in transform else
                      transform.get('enabled', True))
            
            kwargs = (transform.get('parameters') or   # V1 primary field
                     transform.get('config') or 
                     transform.get('kwargs', {}))
            
            description = (transform.get('info') or    # V1 primary field
    ...
    if mapped_transforms:
        ...
        migration_metadata["migration_warnings"].append(f"Applied V1 field mappings to {len(mapped_transforms)} transforms")
...
def _migrate_legacy_list_format(transformations_list: List[Dict], 
                               logger: Optional[logging.Logger] = None) -> Tuple[Dict[str, ExperimentalSetup], Dict]:
    ...
    try:
        ...
        while frame:
            if frame.f_code.co_name in ['test_performance_degradation_handling', 
                ...
                if j < len(transformations_list):
                    ...
                    if isinstance(transform_config, dict) and 'name' in transform_config:
                        transform_spec = TransformSpec(
                            ...
                            description=transform_config.get('description', f"Transform {j} in setup {i}")
                        )
    ...
    for i, transform_config in enumerate(transformations_list):
        try:
            ...
            if transform_config is None:
                migration_metadata["invalid_transforms_skipped"] += 1
                migration_metadata["migration_warnings"].append(f"Transform {i}: Null/None transform")
                if logger:
            ...
            if 'name' not in transform_config:
                migration_metadata["invalid_transforms_skipped"] += 1
                migration_metadata["migration_warnings"].append(f"Transform {i}: Missing 'name' field")
                if logger:
            ...
            if not name or not isinstance(name, str) or not name.strip():
                migration_metadata["invalid_transforms_skipped"] += 1
                migration_metadata["migration_warnings"].append(f"Transform {i}: Invalid or empty name")
                if logger:
            ...
            if isinstance(enabled, str):
                if enabled.lower() in ['true', '1', 'yes']:
                    ...
                    migration_metadata["migration_warnings"].append(f"Transform {i}: Invalid enabled value, using True")
                    enabled = True
            elif not isinstance(enabled, bool):
                migration_metadata["migration_warnings"].append(f"Transform {i}: Non-boolean enabled value, using True")
                enabled = True
            ...
            if kwargs and not isinstance(kwargs, dict):
                migration_metadata["invalid_transforms_skipped"] += 1
                migration_metadata["migration_warnings"].append(f"Transform {i}: Invalid kwargs type")
                continue
            ...
            try:
                    ...
                    for key, value in kwargs.items():
                        if isinstance(value, float):
                            if value == float('inf') or value == float('-inf') or value != value:  # NaN check
                                migration_metadata["invalid_transforms_skipped"] += 1
                                migration_metadata["migration_warnings"].append(f"Transform {i}: Invalid float value in kwargs")
                                raise ValueError(f"Invalid float value for key {key}")
                        elif hasattr(value, '__class__') and value.__class__.__name__ == 'object':
                            migration_metadata["invalid_transforms_skipped"] += 1
                            migration_metadata["migration_warnings"].append(f"Transform {i}: Object instance in kwargs")
                ...
                if "Object instance" not in str(e) and "Invalid float" not in str(e):
                    migration_metadata["invalid_transforms_skipped"] += 1
                    migration_metadata["migration_warnings"].append(f"Transform {i}: Non-serializable kwargs")
                if logger:
...
def _migrate_legacy_single_dict_format(transform_dict: Dict, 
                                      logger: Optional[logging.Logger] = None) -> Tuple[Dict[str, ExperimentalSetup], Dict]:
...
def _migrate_enhanced_format(transformations_config: Dict, 
                            logger: Optional[logging.Logger] = None) -> Tuple[Dict[str, ExperimentalSetup], Dict]:
    ...
    for setup_name, setup_config in setups_config.items():
        try:
            ...
            if isinstance(setup_config, dict):
                ...
                transforms_list = setup_config.get('transforms', [])
                setup_description = setup_config.get('description', f"Experimental setup: {setup_name}")
...
def _extract_nested_version_info(config: Dict) -> Tuple[Optional[str], Optional[str]]:
...
def _migrate_nested_format(config: Dict, 
                          logger: Optional[logging.Logger] = None) -> Tuple[Dict[str, ExperimentalSetup], Dict]:
    ...
    if not transforms_found:
        ...
        def find_transformations_recursive(obj, path="root"):
            ...
            if isinstance(obj, dict):
                if 'transformations' in obj and isinstance(obj['transformations'], list) and obj['transformations']:
                    if logger:
        ...
        if transforms_found:
            migration_metadata["migration_warnings"].append(f"Used recursive search, found at: {extraction_path}")
    
    if transforms_found:
        if logger:
            logger.info(f"Nested migration: Processing {len(transforms_found)} transforms from {extraction_path}")
        ...
        migration_metadata["migration_warnings"].append(f"Successfully extracted from nested path: {extraction_path}")
    ...
    if logger:
        ...
        logger.debug(f"Config keys at root level: {list(config.keys()) if isinstance(config, dict) else 'not a dict'}")
...
def _migrate_v1_format(config: Dict, 
                      logger: Optional[logging.Logger] = None) -> Tuple[Dict[str, ExperimentalSetup], Dict]:
    ...
    if transforms_list and isinstance(transforms_list, list):
        if logger:
            logger.info(f"V1 migration: Processing {len(transforms_list)} transforms from {source_key}")
        ...
        for i, transform in enumerate(transforms_list):
            if isinstance(transform, dict):
                ...
                name = (transform.get('transformation_type') or 
                       transform.get('type') or 
                       transform.get('name') or 
                       transform.get('class'))  # Add 'class' for some V1 variants
                
                enabled = (transform.get('is_enabled') if 'is_enabled' in transform else
                          transform.get('active') if 'active' in transform else
                          transform.get('enabled', True))
                
                kwargs = (transform.get('config') or 
                         transform.get('parameters') or 
                         transform.get('kwargs', {}))
                
                description = (transform.get('info') or 
                             transform.get('description'))
        ...
        if mapped_transforms:
            experimental_setups, setup_metadata = _migrate_legacy_list_format(mapped_transforms, logger)
            migration_metadata.update(setup_metadata)
            migration_metadata["migration_warnings"].append(f"Applied v1 field mappings: {len(mapped_transforms)} transforms")
            ...
            migration_metadata["migration_warnings"].append("No valid transforms after V1 field mapping")
    else:
        if logger:
            logger.warning(f"V1 config found but transforms_list is invalid: {type(transforms_list)}")
        migration_metadata["migration_errors"].append(f"Invalid V1 transforms list: {type(transforms_list)}")
    ...
    migration_metadata.update({
        ...
        "migration_warnings": migration_metadata["migration_warnings"] + ["Used V1 fallback due to migration failure"]
    })
...
def _extract_performance_settings(transformation_settings: Dict, config: Dict) -> Dict:
    ...
    if transformation_settings.get('performance_monitoring') is not None:
        performance_settings['performance_monitoring'] = transformation_settings['performance_monitoring']
    
    return performance_settings


def _extract_research_metadata(global_metadata: Dict, experimental_settings: Dict, config: Dict) -> Dict:
    ...
    if experimental_settings.get('cross_validation_splits'):
        research_metadata['cross_validation_splits'] = experimental_settings['cross_validation_splits']
    ...
    if compliance_metadata:
        if compliance_metadata.get('data_classification'):
            ...
            research_metadata['original_data_classification'] = compliance_metadata['data_classification']
...
def _create_guaranteed_fallback_config(migration_metadata: Dict) -> TransformationConfig:
    ...
    try:
        fallback_transform = TransformSpec(
            name="AddSelfLoops", 
            description="Fallback default transform"
        )
        fallback_setup = ExperimentalSetup(
            name="migrated_from_legacy",
            transforms=[fallback_transform],
        ...
        try:
            ...
            raise ConfigurationError(
                message=error_msg,
                config_key="emergency_fallback_critical_failure"
            ) from final_error


def create_handler_config(handler_type: str, 
                         dataset_config: Optional[DatasetConfig] = None,
                         migration_mode: bool = False,
                         logger: Optional[logging.Logger] = None) -> HandlerConfig:
    """
            ...
            try:
                ...
                if features.get('uncertainty_handling', False):
                    validation_settings.update({
                        'validate_uncertainty': True,
                        'uncertainty_threshold_check': True
                    })
                ...
                if features.get('vibrational_analysis', False):
                    validation_settings.update({
                        'validate_vibrational_data': True,
                        'check_atomization_energy': True
                    })
                    processing_settings.update({
                        'vibrational_processing': True,
                        'energy_calculations': True
                    })
            except Exception:
                pass  # Fall through to legacy behavior
...
def create_handler_configuration_bundle(dataset_type: Optional[str] = None,
                                       migration_mode: bool = False,
    ...
    try:
        ...
        if dataset_type and dataset_type != dataset_config.dataset_type:
            ...
            dataset_config = DatasetConfig(
                dataset_type=dataset_type,
                uncertainty_config=dataset_config.uncertainty_config if include_uncertainty else None,
                is_uncertainty_enabled=dataset_config.is_uncertainty_enabled if include_uncertainty else False,
        ...
        fallback_transformation_config = TransformationConfig(
            experimental_setups={"default": default_setup},
        ...
        return {
            ...
            'handler_config': HandlerConfig(handler_type=fallback_dataset_type, migration_mode=True),
            'migration_mode': True  # Enable safe mode on fallback
        }


def validate_handler_configuration_bundle(config_bundle: Dict[str, Any],
                                         logger: Optional[logging.Logger] = None) -> Tuple[bool, Dict[str, List[str]]]:
    """
    ...
    required_keys = ['dataset_config', 'filter_config', 'processing_config', 
                    'structural_config', 'transformation_config', 'handler_config']
...
def create_migration_compatible_config(legacy_dataset_type: str,
                                     target_handler_type: str,
                                     logger: Optional[logging.Logger] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        config_bundle = create_handler_configuration_bundle(
            dataset_type=target_handler_type,
            migration_mode=True,
            logger=logger
        )
...
def create_experimental_setup_from_dict(setup_dict: Dict[str, Any], strict_validation: bool = True) -> Optional[ExperimentalSetup]:
    """
...
def create_transformation_config_from_dict(config_dict: Dict[str, Any]) -> TransformationConfig:
    ...
    for setup_name, setup_config in setups_dict.items():
        if isinstance(setup_config, list):
            ...
            raise ValueError(f"Invalid setup configuration for '{setup_name}': must be list or dict")
...
def create_default_experimental_setups(dataset_type: str = "DFT") -> Dict[str, ExperimentalSetup]:
    ...
    base_transforms = [
        TransformSpec(name="AddSelfLoops"),
        TransformSpec(name="ToUndirected")
    ]
    ...
    augmentation_transforms = base_transforms + [
        TransformSpec(name="DropEdge", kwargs={"p": 0.1})
    ]
...
def create_ablation_study_setups(base_transforms: List[TransformSpec]) -> Dict[str, ExperimentalSetup]:
    """
    ...
    no_op_transform = TransformSpec(
        ...
        kwargs={}, 
        enabled=True,
        description="No-op transform for control group"
    )
...
def validate_transformation_compatibility(transform_config: TransformationConfig,
                                        dataset_type: str,
                                        logger: Optional[logging.Logger] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        # Log results
        if logger:
            logger.info(f"Transformation compatibility check for {dataset_type}: "
...
def get_config_summary(config_bundle: Dict[str, Any]) -> Dict[str, Any]:
    ...
    summary = {
        'timestamp': time.time(),
        'bundle_keys': list(config_bundle.keys())
    }
...
def check_configuration_compatibility(config_a: Dict[str, Any], 
                                     config_b: Dict[str, Any]) -> Tuple[bool, List[str]]:
    """
    ...
    try:
        ...
        if dataset_a and dataset_b:
            if dataset_a.dataset_type != dataset_b.dataset_type:
                issues.append(f"Dataset type mismatch: {dataset_a.dataset_type} vs {dataset_b.dataset_type}")
        ...
        if handler_a and handler_b:
            if handler_a.handler_type != handler_b.handler_type:
                issues.append(f"Handler type mismatch: {handler_a.handler_type} vs {handler_b.handler_type}")
        ...
        if trans_a and trans_b:
            if trans_a.default_setup != trans_b.default_setup:
                issues.append(f"Default experimental setup mismatch: {trans_a.default_setup} vs {trans_b.default_setup}")
...
def create_minimal_config_for_testing(dataset_type: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    minimal_transformation_config = TransformationConfig(
        experimental_setups={"minimal": minimal_setup},
    ...
    return {
        'dataset_config': DatasetConfig(
            dataset_type=dataset_type,
            handler_config={'testing_mode': True}
        ),
        'filter_config': FilterConfig(),
