...
def _get_dataset_config_schema_class(dataset_type: str) -> Optional[type]:
    """
...
def _get_dataset_feature_support(dataset_type: str) -> Optional[Dict[str, bool]]:
    """
...
class TransformationSchema(BaseModel):
    ...
    standard_transforms: Optional[List[Dict[str, Any]]] = None  # NEW: Standard transforms applied before experimental
    legacy_transforms: Optional[List[Dict[str, Any]]] = None
    research_metadata: Optional[Dict[str, Any]] = None
    dataset_optimization: Optional[Dict[str, Any]] = None
    ...
    def validate_schema_definition(self) -> Self:
        ...
        has_experimental = isinstance(self.experimental_setups, dict) and bool(self.experimental_setups)
        ...
        if not has_experimental and not has_standard:
            raise ValueError("At least one of 'experimental_setups' or 'standard_transforms' must be defined")
...
class PluginConfigSchema(BaseModel):
    ...
    def validate_max_plugins(cls, v: int) -> int:
        ...
        if v < 1 or v > 1000:
            raise ValueError(
                f"max_plugins must be between 1 and 1000, got {v}"
            )
        return v
...
class WavefunctionProcessingConfigSchema(BaseModel, frozen=True):
    ...
    def validate_feature_tier(cls, v: str) -> str:
        ...
        if v not in valid_tiers:
            raise ValueError(
                f"feature_tier must be one of {valid_tiers}, got '{v}'"
            )
        return v
...
class WavefunctionConfigSchema(BaseModel, frozen=True):
    ...
    raw_data_download_url: Optional[str] = None
    dataset_root_dir: str = '~/Chem_Data/milia_PyG_Dataset'
    processing_config: Optional[WavefunctionProcessingConfigSchema] = None
    uncertainty_handling: Optional[WavefunctionUncertaintyConfigSchema] = None
    ...
    def validate_raw_npz_filename(cls, v: str) -> str:
        ...
        if not v.endswith('.npz'):
            raise ValueError(
                f"raw_npz_filename must end with '.npz', got '{v}'"
            )
        return v
...
def validate_wavefunction_config(config: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    if config_key not in config:
        errors.append(
            f"{config_key} section is required when dataset_type is '{dataset_type}'"
        )
        return False, errors
...
class DescriptorConfigSchema(BaseModel, frozen=True):
    ...
    def validate_error_handling(cls, v: str) -> str:
        ...
        if v not in valid_error_modes:
            raise ValueError(
                f"error_handling must be one of {valid_error_modes}, "
                f"got '{v}'"
            )
        return v
    ...
    def validate_num_workers(cls, v: int) -> int:
        ...
        if v < 1:
            raise ValueError(
                f"num_workers must be at least 1, got {v}"
            )
        return v
...
class DescriptorCategoryConfigSchema(BaseModel, frozen=True):
    ...
    @classmethod
    def validate_descriptors(cls, v: Optional[List[str]]) -> Optional[List[str]]:
...
class YAMLSchemaValidator:
    ...
    def validate_config(self, config: Dict[str, Any], validation_config: Optional[ValidationConfig] = None) -> Dict[str, Any]:
        ...
        if format_detected == 'invalid':
            ...
            # Must have at least one transform source
            if not has_experimental and not has_standard:
                result['errors'].append("Enhanced format requires 'experimental_setups' or 'standard_transforms' (at least one)")
                return result
                    ...
                    for i, transform in enumerate(standard_transforms):
                        if isinstance(transform, dict):
                            if 'name' not in transform:
                                result['warnings'].append(f"standard_transforms[{i}] missing 'name' field")
                        elif not isinstance(transform, str):
                            result['warnings'].append(f"standard_transforms[{i}] has unexpected format")
            ...
            if has_experimental:
                ...
                if len(setups) == 0 and not has_standard:
                    result['errors'].append("experimental_setups cannot be empty when no standard_transforms defined")
                    return result
                ...
                for setup_name, setup_data in setups.items():
                    ...
                    if validation_config.require_descriptions and validation_config.strict_mode:
                        if 'description' not in setup_data:
                            result['errors'].append(f"Setup '{setup_name}' missing required description")
                    
                    # Check for transforms - but be flexible about format
                    if 'transforms' in setup_data:
                        transforms = setup_data['transforms']
                        if not isinstance(transforms, list):
                            result['errors'].append(f"Setup '{setup_name}' transforms must be a list")
                            continue
                        ...
                        for i, transform in enumerate(transforms):
                            if isinstance(transform, dict):
                                if 'name' not in transform:
                                    result['warnings'].append(f"Setup '{setup_name}' transform {i} missing 'name' field")
                                ...
                                if 'params' in transform and 'kwargs' not in transform:
                                    ...
                                    result['warnings'].append(f"Setup '{setup_name}' transform {i} has both 'params' and 'kwargs' - 'kwargs' will take precedence")
                            elif not isinstance(transform, str):
                                result['warnings'].append(f"Setup '{setup_name}' transform {i} has unexpected format")
            ...
            if 'default_setup' not in transformations:
                if validation_config.strict_mode:
                    ...
                    result['errors'].append("Enhanced format missing default_setup (required in strict mode)")
                else:
            ...
            if not result['errors']:
                result['valid'] = True
                result['summary']['experimental_setups_count'] = len(setups) if has_experimental else 0
                if has_standard:
                    result['summary']['standard_transforms_count'] = len(standard_transforms)
            ...
            result['valid'] = True
            result['warnings'].append(f"Configuration is in {format_detected} format and should be migrated")
    ...
    def validate_config_with_plugins(
        ...
        plugin_result = plugin_validator.validate_plugin_config(
            config['plugins'],
            validation_level=plugin_validation_level
        )
        ...
        combined_result['valid'] = (
            len(combined_result['errors']) == 0 and 
            base_result['valid'] and 
            plugin_result['valid']
        )
    ...
    def validate_plugin_config(
        ...
        if not self.plugin_system_available:
            ...
            result['suggestions'].append(
                "Ensure plugin_system.py is installed and importable"
            )
        ...
        if not isinstance(plugin_paths, list):
            result['errors'].append(f"'plugin_paths' must be a list, got {type(plugin_paths).__name__}")
        else:
            if not plugin_paths:
        ...
        bool_flags = [
            'auto_discover', 'auto_validate', 'allow_experimental',
            'require_metadata', 'enforce_checksums', 'security_scanning'
        ]
        ...
        if not isinstance(max_plugins, int):
            result['errors'].append(f"'max_plugins' must be integer, got {type(max_plugins).__name__}")
...
class ExperimentSchemaValidator:
    ...
    def validate_experiment_config(
        ...
        if not isinstance(random_seed, int):
            result['errors'].append(f"'random_seed' must be an integer, got {type(random_seed).__name__}")
        ...
        if not ablations and not parameter_sweeps:
            if strict_mode:
                ...
                result['errors'].append("Experiment must have at least one ablation or parameter sweep (strict mode requires variants for comparison)")
            else:
                # Non-strict: Basic structure check only
                result['warnings'].append("Experiment has no variants configured (consider adding for meaningful comparisons)")
        ...
        if not self.research_api_available:
            ...
            result['suggestions'].append(
                "Ensure research_api.py is installed and importable"
            )
        
        # Build experiment summary
        result['experiment_summary'] = {
            'name': name,
            'base_transforms_count': len(base_transforms) if isinstance(base_transforms, list) else 0,
        ...
        try:
            ...
            if 'error_handling' in config:
                ...
                if config['error_handling'] not in valid_modes:
                    result['errors'].append(
                        f"Invalid error_handling mode. Valid: {valid_modes}"
                    )
            ...
            if 'validation_mode' in config:
                ...
                if config['validation_mode'] not in valid_modes:
                    result['errors'].append(
                        f"Invalid validation_mode. Valid: {valid_modes}"
                    )
            ...
            if strict_mode:
                if config.get('enabled', False):
                    ...
                    if 'categories' not in config or not config['categories']:
                        result['warnings'].append(
                            "Strict mode: No category configurations defined"
                        )
        ...
        try:
            ...
            if 'enabled' in category_config:
                if not isinstance(category_config['enabled'], bool):
                    result['errors'].append(
                        f"Category '{category_name}': enabled must be boolean"
                    )
    ...
    def validate_with_dataset_type(
        ...
        try:
            ...
            if _dataset_supports_feature(dataset_type, 'orbital_analysis'):
                ...
                logger.debug(f"{dataset_type} has orbital_analysis, suggested electronic descriptors")
            ...
            # For datasets without specific features, provide generic note
                ...
                logger.debug(f"{dataset_type} does not have special features, generic coordinate note")
...
class ConfigMigration:
    ...
    def migrate_to_enhanced(self, config: Dict[str, Any], target_version: str = 'v3', preserve_original: bool = True) -> Tuple[Dict[str, Any], List[str]]:
            ...
            enhanced_config['transformations'] = {
                'experimental_setups': {
                    'migrated_default': {
                        ...
                        'transforms': [
                            {**transform, 'enabled': True} if isinstance(transform, dict) else {'name': transform, 'enabled': True}
        ...
        # Convert legacy dict format - To handle named setups properly
            ...
            if any(isinstance(v, list) for v in transformations.values() if v is not None):
                ...
                enhanced_config['transformations'] = {
                    ...
                    'default_setup': list(experimental_setups.keys())[0] if experimental_setups else 'default',
                    'validation': {
                        'enabled': True,
                        'strict_mode': False,
                        'warn_on_unknown': True
                ...
                experimental_setups['migrated_default'] = {
                    ...
                    'transforms': [
                        {**transformations, 'enabled': True} if 'name' in transformations else {'name': 'Unknown', 'enabled': True}
                    ]
                }
...
def _load_transformation_config_enhanced(config_dict: Dict[str, Any], 
                                        validate: bool = True,
    ...
    try:
        ...
        if format_type in ['legacy_list', 'legacy_dict'] and migrate_legacy:
            ...
            try:
                ...
                migration_validation = migrator.validate_migration_result(migrated_config, config_dict)
                ...
                if validation_config.strict_mode:
                    ...
                    config_logger.warning("Continuing with original configuration (migration disabled)")
                    migrated_config = config_dict
        ...
        if 'plugins' in migrated_config:
            try:
                ...
                if not plugin_validation['valid']:
                    error_msg = "Plugin configuration validation failed: " + \
                               "; ".join(plugin_validation['errors'])
                ...
                if 'transformations' in migrated_config and summary.get('enabled'):
                    compat_result = plugin_validator.validate_plugin_compatibility(
                        migrated_config['plugins'],
                        migrated_config['transformations']
                    )
                    
                    if not compat_result['compatible']:
                        compat_error_msg = "Plugin-transform compatibility issues: " + \
...
def load_and_validate_yaml_config(config_path: Union[str, Path], 
                                 validation_config: Optional[ValidationConfig] = None,
                                 migrate_legacy: bool = True) -> Dict[str, Any]:
...
def validate_yaml_config_string(config_string: str, 
                               validation_config: Optional[ValidationConfig] = None) -> Dict[str, Any]:
...
def get_enhanced_transformation_config(validate: bool = True, 
                                      migrate_legacy: bool = True,
                                      validation_config: Optional[ValidationConfig] = None) -> Dict[str, Any]:
...
def validate_current_config(validation_config: Optional[ValidationConfig] = None) -> Dict[str, Any]:
...
def merge_plugin_configs(
    ...
    simple_keys = [
        'enabled', 'auto_discover', 'auto_validate', 'validation_level',
        'allow_experimental', 'max_plugins', 'require_metadata',
        'enforce_checksums', 'security_scanning'
    ]
...
if __name__ == "__main__":
    ...
    print(f"   ✓ Default experiment created")
    ...
    print(f"   ✓ Example experiments created")
    ...
    single_validation = experiment_validator.validate_experiment_config(
        example_experiments['transform_ablation'],
        strict_mode=False
    )
    ...
    if 'experiment_validation' in full_validation:
        ...
        print(f"   ✓ Valid experiments: {exp_val['summary']['valid_experiments']}/{exp_val['summary']['total_experiments']}")
    ...
    try:
        ...
        print(f"   ✓ ExperimentSchema created successfully")
