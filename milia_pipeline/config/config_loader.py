...
def _normalize_dataset_keyed_sections(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if 'data_config' in config and isinstance(config['data_config'], dict):
            if 'property_selection' in config['data_config'] and isinstance(config['data_config']['property_selection'], dict):
                config['data_config']['property_selection'] = _normalize_dict_keys(
                    config['data_config']['property_selection'], type_lookup, 'data_config.property_selection'
                )
...
def _normalize_dict_keys(d: Dict[str, Any], type_lookup: Dict[str, str], section_name: str) -> Dict[str, Any]:
    """
    ...
    for key, value in d.items():
        ...
        if key_upper in type_lookup:
            ...
            if canonical_key != key:
                logger.debug(
                    f"Normalized key in {section_name}: '{key}' -> '{canonical_key}'"
                )
...
_ENHANCED_TRANSFORMATION_CONFIG: Optional[Dict[str, Any]] = None
    ...
    with _cache_lock:
        ...
        try:
            ...
            if _normalization_skipped:
                logger.debug("Skipping config cache due to re-entrant call during registry initialization")
                _CONFIG = config
                return config
            ...
            if enable_enhancement:
                try:
                    ...
                    config, validation_results, migration_report = _apply_transformation_enhancements(
                        config, 
                        enable_migration, 
                        enable_validation,
                        validation_level,
...
def _validate_enhanced_format(transforms_section: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not has_experimental_setups and not has_standard_transforms:
        ...
        validation['errors'].append("Missing 'experimental_setups' or 'standard_transforms' key (at least one required)")
        return validation
    ...
    if has_experimental_setups:
        ...
        if len(setups) == 0 and not has_standard_transforms:
            validation['valid'] = False
            validation['errors'].append("'experimental_setups' is empty and no 'standard_transforms' defined")
            return validation
        ...
        if 'default_setup' not in transforms_section:
            validation['warnings'].append("Missing 'default_setup' key (recommended even with only standard_transforms)")
...
def _apply_transformation_enhancements(config, enable_migration, enable_validation, 
                                      validation_level, report_validation):
    """
    ...
    try:
        ...
        is_already_enhanced = (format_detected == 'enhanced' and 
                              (format_info == 'enhanced') if isinstance(format_info, str) else False)
        
        if format_detected == 'enhanced' and not (format_info == 'enhanced') if isinstance(format_info, str) else False:
            logger.warning(
                f"Enhanced format detected but invalid: {[]}"
            )
    ...
    if enable_migration and not is_already_enhanced:
        needs_migration = (format_detected in ['legacy_list', 'legacy_dict', 'unknown'] or
                          not is_already_enhanced)
        ...
        if needs_migration:
            try:
                ...
                from milia_pipeline.config.config_schemas import ConfigMigration
                migration = ConfigMigration()
                migrated_config, migration_warnings = migration.migrate_to_enhanced(config, preserve_original=True)
    ...
    if enable_validation:
        try:
            ...
            from milia_pipeline.config.config_schemas import YAMLSchemaValidator, ValidationConfig
            ...
            val_results = validator.validate_config(enhanced_config, validation_config=validation_cfg)
...
def _apply_configuration_enhancements(config, enable_migration=True, enable_validation=True):
    ...
    try:
        ...
        transforms_section = config.get('transformations', {})
        is_already_enhanced = (
            isinstance(transforms_section, dict) and 
        ...
        if enable_migration and 'transformations' in config:
            ...
            needs_migration = (format_info not in ['enhanced', 'invalid']) if isinstance(format_info, str) else False
            ...
            if needs_migration:
                try:
                    ...
                    from milia_pipeline.config.config_schemas import ConfigMigration
                    migration = ConfigMigration()
                    migrated_config, migration_warnings = migration.migrate_to_enhanced(enhanced_config, preserve_original=True)
                    if migrated_config and isinstance(migrated_config, dict):
                        enhanced_config = migrated_config
...
def _optimized_validation(config):
    ...
    if isinstance(transforms_section, dict):
        ...
        total_experimental = sum(len(transforms) for transforms in setups.values() if isinstance(transforms, list))
        ...
        if len(setups) > 50 or total_transforms > 500:
            logger.info(f"Large config detected ({len(setups)} setups, {total_transforms} transforms). Using optimized validation.")
            ...
            if 'default_setup' not in transforms_section:
                ...
                if not standard_transforms:
                    logger.warning(f"Default setup '{transforms_section['default_setup']}' not found in experimental_setups")
    ...
    if 'transformations' in config:
        ...
        if isinstance(transforms, dict) and 'experimental_setups' in transforms:
            ...
            total_transforms = sum(
                len(setup_transforms) for setup_transforms in setups.values()
                if isinstance(setup_transforms, list)
            )
            default_setup = transforms.get('default_setup', 'unknown')
            
            info_parts.append(f"transforms: {setup_count} experimental setups, {total_transforms} total transforms, default: {default_setup}")
            ...
            info_parts.append(f"transforms: 1 transform (legacy format)")
...
def get_enhanced_transformation_config(force_reload=False):
    ...
    try:
        ...
        raise ConfigurationError(
            f"Failed to load transformation configuration: {str(e)}",
            config_key='transformations'
        )

def validate_config_file(config_path: str = 'config.yaml',
...
def create_example_config(config_path='config.example.yaml', dataset_type=None, 
                         include_experimental_setups=True):
    """
    ...
    try:
        ...
        if not include_experimental_setups:
            if 'transformations' in example_config and 'experimental_setups' in example_config['transformations']:
                ...
                'transformations': {
                    'experimental_setups': {
                        'basic': [
                            ...
                            {'name': 'ToUndirected', 'kwargs': {}, 'enabled': True, 'description': 'Convert to undirected graph'}
                        ],
                        'advanced': [
                            {'name': 'AddSelfLoops', 'kwargs': {}, 'enabled': True, 'description': 'Add self-loops'},
                            {'name': 'GCNNorm', 'kwargs': {'add_self_loops': False}, 'enabled': True, 'description': 'GCN normalization'}
                        ]
                    },
                    'default_setup': 'basic'
                }
...
def migrate_legacy_config(input_path, output_path=None, dataset_type=None, 
                         backup=True, report=True):
    """
    ...
    if not os.path.exists(input_path):
        raise ConfigurationError(
            f"Input configuration file not found: {input_path}",
    ...
    try:
        ...
        raise ConfigurationError(
            f"Failed to parse input configuration: {str(e)}",
            config_key='yaml_parsing'
        )
    
    if not isinstance(legacy_config, dict):
        raise ConfigurationError(
            "Legacy configuration must be a dictionary",
            config_key='legacy_format'
        )
    ...
    try:
        ...
        if migrated_config is None or not isinstance(migrated_config, dict):
            raise ConfigurationError(
                "Migration failed during configuration migration",
                config_key='migration_process'
            )
...
def get_experimental_setup(setup_name: str, config: Optional[Dict[str, Any]] = None) -> Optional[List[Dict[str, Any]]]:
    """
...
def list_experimental_setups(config: Optional[Dict[str, Any]] = None) -> List[str]:
    """
...
def get_migration_report() -> Optional[Dict[str, Any]]:
    """
...
def print_config_diagnostics():
    ...
    if stats['last_load_time']:
        print(f"\nTiming:")
        print(f"  Last load: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(stats['last_load_time']))}")
        if stats.get('last_validation_time'):
            print(f"  Last validation: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(stats['last_validation_time']))}")
    
    print("\n" + "=" * 70 + "\n")


def validate_and_report(config_path: str = 'config.yaml', 
                       validation_level: str = 'NORMAL') -> bool:
    """
        ...
        if needs_migration:
            ...
            if True:
                result['recommendations'].append(
                    f"Run: migrate_legacy_config('{config_path}')"
                )
                result['recommendations'].append(
                ...
                result['recommendations'].append(
                    "Migration will be applied automatically on load"
                )
                result['recommendations'].append(
                    "Using fallback migration (Not available)"
                )
        else:
        ...
        if format_type == 'enhanced':
            ...
            if isinstance(transforms, dict):
                ...
                if 'standard_transforms' in transforms:
                    ...
                    result['standard_transforms_count'] = len(standard_transforms) if isinstance(standard_transforms, list) else 0
                else:
...
def recommend_validation_level(config_path: str = 'config.yaml') -> str:
    ...
    try:
        ...
        if isinstance(transforms, dict) and ('experimental_setups' in transforms or 'standard_transforms' in transforms):
            setups = transforms.get('experimental_setups', {})
            standard_transforms = transforms.get('standard_transforms', [])
            ...
            total_experimental_transforms = sum(
                len(t) for t in setups.values() if isinstance(t, list)
            ) if setups else 0
