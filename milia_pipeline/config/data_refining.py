...
def _init_registry() -> bool:
    ...
    try:
        ...
        _REGISTRY_AVAILABLE = False
        logger.debug(f"Phase 6: Registry not available for data_refining, using legacy fallback: {e}")
        return False
...
def _get_available_dataset_types() -> list:
    ...
    try:
        ...
        if implementations_dir.exists():
            ...
            if discovered_types:
                logger.debug(f"DataRefining: Dynamically discovered dataset types: {discovered_types}")
    ...
    logger.warning("DataRefining: No dataset types available - registry not initialized and dynamic discovery failed")
    return []
        ...
        try:
            if hasattr(item, 'flatten'):
                ...
                for val in flat_item:
                    try:
                        if val is not None and not (isinstance(val, (list, tuple)) and len(val) == 0):
                            float_val = float(val)
                            if np.isfinite(float_val):
                                extracted_values.append(float_val)
                    except:
                        ...
                        for element in arr.flat:
                            ...
                            if isinstance(element, list):
                                ...
                                for sub_element in element:
                                    try:
                                        if isinstance(sub_element, (np.float64, np.number, int, float)):
...
def _deep_convert_to_float(item: Any) -> Union[float, List[float], None]:
    ...
    if isinstance(item, (int, float, np.number)):
        ...
        logger.debug(f"Unsupported type encountered during deep conversion to float: {type(item).__name__} with value '{str(item)[:100]}'. Returning None.")
        return None
    ...
    try:
        ...
        if hasattr(freqs, '__len__'):
            for freq_item in freqs:
                if _is_value_valid_and_not_nan(freq_item) and not np.isclose(freq_item, 0.0, atol=comparison_tolerance):
        ...
        if len(extracted_freqs) == len(extracted_vibmodes) and len(extracted_freqs) > 0:
            logger.debug(f"Molecule {molecule_index}: Perfect match after extraction: {len(extracted_freqs)} pairs")
...
def _validate_and_reshape_vibmode_data(vibmode_raw: Any, expected_atoms: Optional[int] = None, 
                                      molecule_index: int = -1) -> Optional[np.ndarray]:
    """
...
def _normalize_vibmode(vibmode_entry: Any, molecule_index: int = -1) -> np.ndarray:
    ...
    normalized_vibmode = _validate_and_reshape_vibmode_data(vibmode_entry, molecule_index=molecule_index)
    ...
    if normalized_vibmode is None:
        ...
        vibmode_info = {
            ...
            'size': getattr(vibmode_entry, 'size', len(vibmode_entry) if hasattr(vibmode_entry, '__len__') else 'N/A')
        }
...
def log_vibration_refinement_status(
    ...
    if not freqs_available and not vibmodes_available:
        logger.debug(f"Molecule {molecule_index}: Skipping vibrational data refinement: 'freqs' and 'vibmodes' are both not chosen to be processed.")
    elif not freqs_available:
        logger.debug(f"Molecule {molecule_index}: Skipping vibrational frequencies data refinement: 'freqs' is not not chosen to be processed.")
    elif not vibmodes_available:
        logger.debug(f"Molecule {molecule_index}: Skipping vibrational modes data refinement: 'vibmodes' is not chosen to be processed.")
    else:
        pass
...
def _handler_refine_molecular_data(
    ...
    try:
        ...
        if hasattr(handler, 'refine_molecule_data'):
            ...
            result = handler.refine_molecule_data(
                raw_properties_dict, 
                molecule_index, 
                identifier
            )
...
def _handler_get_refinement_statistics(
    ...
    try:
        if hasattr(handler, 'get_refinement_statistics'):
            ...
            # Fallback statistics
            logger.warning(f"Handler {type(handler).__name__} lacks 'get_refinement_statistics' method, using fallback")
            return {
                'dataset_type': handler.get_dataset_type() if hasattr(handler, 'get_dataset_type') else 'unknown',
                'total_molecules': len(refinement_results),
                'refined_molecules': sum(1 for r in refinement_results if r.get('is_refined', False))
            }
    except DatasetSpecificHandlerError as e:
...
def refine_molecular_vibrations(
    ...
    if meaningful_freq_count != meaningful_vibmode_count:
        logger.warning(f"Molecule {molecule_index}: Meaningful data count mismatch: {meaningful_freq_count} freqs vs {meaningful_vibmode_count} vibmodes")
        ...
        logger.debug(f"Molecule {molecule_index}: Resolved count mismatch - using {len(working_freqs)} paired freq/vibmode entries")
    else:
        # Counts match - proceed with original data
        working_freqs = freqs
        working_vibmodes = vibmodes
        logger.debug(f"Molecule {molecule_index}: Meaningful counts match: {meaningful_freq_count} freq/vibmode pairs")
    ...
    try:
        ...
        if not freqs_valid:
            ...
            meaningful_count = sum(1 for f in working_freqs 
                                   if _is_value_valid_and_not_nan(f) and not np.isclose(f, 0.0, atol=comparison_tolerance))
            logger.warning(f"Molecule {molecule_index}: {invalid_count} out of {meaningful_count} meaningful frequencies are not finite")
    except Exception as e:
    ...
    meaningful_freq_count = sum(1 for f in working_freqs 
                                if _is_value_valid_and_not_nan(f) and not np.isclose(f, 0.0, atol=comparison_tolerance))
    logger.debug(f"Molecule {molecule_index}: Processing {meaningful_freq_count} meaningful frequency entries (from {len(working_freqs)} total) and {len(working_vibmodes)} vibmode entries")
    ...
    for i, freq_entry in enumerate(working_freqs):
        try:
            ...
            if not is_freq_valid:
                ...
                logger.debug(f"Molecule {molecule_index}: Frequency at index {i} is invalid or near zero ({freq_entry}). Skipping this pair.")
                continue
            ...
            try:
                ...
                if _is_value_valid_and_not_nan(normalized_vibmode, allow_empty_array=False) and normalized_vibmode.size > 0:
                    ...
                    logger.debug(f"Molecule {molecule_index}: Successfully processed freq/vibmode pair {i}")
                else:
                    processing_stats['invalid_vibmodes'] += 1
                    logger.debug(f"Molecule {molecule_index}: Normalized vibmode at index {i} is not valid or is empty. Skipping this pair.")
                    
            except VibrationRefinementError as e:
                processing_stats['normalization_failures'] += 1
                logger.debug(f"Molecule {molecule_index}: Skipping vibmode at index {i} due to normalization error: {e}")
    ...
    rejection_rate = (processing_stats['total_pairs'] - processing_stats['valid_pairs']) / processing_stats['total_pairs'] if processing_stats['total_pairs'] > 0 else 0
    if rejection_rate > 0.5:  # More than 50% rejected
        logger.warning(f"Molecule {molecule_index}: High rejection rate: {rejection_rate:.2%} of vibmode pairs rejected")
    ...
    for freq, vibmode in zip(cleaned_freqs, cleaned_vibmodes):
        try:
            ...
            vibmode_shape_str = f"{vibmode.shape[0]}x{vibmode.shape[1]}" if len(vibmode.shape) >= 2 else f"{vibmode.shape[0]}"
            ...
            vibmode_fingerprint = f"{vibmode_shape_str}_{vibmode_sum:.8f}_{vibmode_std:.8f}_{vibmode_max:.8f}"
    ...
    if uniqueness_stats['duplicate_pairs'] > 0:
        duplicate_rate = uniqueness_stats['duplicate_pairs'] / uniqueness_stats['before_deduplication'] if uniqueness_stats['before_deduplication'] > 0 else 0
        logger.debug(f"Molecule {molecule_index}: Removed {uniqueness_stats['duplicate_pairs']} duplicate pairs ({duplicate_rate:.2%})")

    # Step 3: CRITICAL PHYSICAL VALIDATION
    # Ensure final freq count == final vibmode count (physical requirement)
    is_accepted = (len(final_unique_freqs) > 0 and len(final_unique_freqs) == len(final_unique_vibmodes))

    if is_accepted:
        logger.debug(f"Molecule {molecule_index}: Refinement complete: Accepted. Number of unique freqs = {len(final_unique_freqs)}, Number of unique vibmodes = {len(final_unique_vibmodes)}")
    else:
...
def detect_dmc_statistical_outliers(
    energy_value: Union[float, np.ndarray],
    ...
    relative_uncertainty = abs(uncertainty_value / energy_scalar) if energy_scalar != 0 else float('inf')
    ...
    is_extreme_uncertainty = uncertainty_value > outlier_threshold_sigma  # Absolute uncertainty threshold
    ...
    if is_statistical_outlier:
        ...
        if is_extreme_uncertainty:
            reasons.append(f"extreme absolute uncertainty ({uncertainty_value:.6f} > {outlier_threshold_sigma})")
        outlier_info['outlier_reason'] = "; ".join(reasons)
        
        logger.warning(f"DMC molecule {molecule_index} flagged as statistical outlier: {outlier_info['outlier_reason']}")
    else:
...
def calculate_dmc_uncertainty_weights(
    ...
    logger.debug(f"Calculated {len(weights)} uncertainty weights using {weighting_strategy} strategy")
    return weights
...
def refine_molecular_data(
    ...
    dataset_config: Optional[DatasetConfig] = None,
    processing_config: Optional[ProcessingConfig] = None,
    logger: Optional[logging.Logger] = None  # ADDED - was missing
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.debug(
            f"Successfully refined {dataset_type} molecule {molecule_index} using handler"
        )
...
def validate_refined_data_quality(
    ...
    dataset_config: Optional[DatasetConfig] = None,
    logger: Optional[logging.Logger] = None  # ADDED
) -> bool:
    """
    ...
    try:
        ...
        logger.warning(
            f"Handler error during validation for molecule {molecule_index} "
            f"(InChI: {inchi}): {e}"
        )
        return False
    except Exception as e:
    ...
    if refinement_category == 'uncertainty':
        ...
        if dataset_config.is_uncertainty_enabled and dataset_config.uncertainty_config:
            ...
            if uncertainty_available and energy_available:
                logger.debug(f"{dataset_type} molecule {molecule_index}: Ready for uncertainty-based refinement")
            elif not uncertainty_available:
                logger.debug(f"{dataset_type} molecule {molecule_index}: Skipping refinement - uncertainty data missing")
            elif not energy_available:
                logger.debug(f"{dataset_type} molecule {molecule_index}: Skipping refinement - energy data missing")
        else:
            logger.debug(f"{dataset_type} molecule {molecule_index}: Skipping refinement - uncertainty handling disabled")
        ...
        logger.debug(f"{dataset_type} molecule {molecule_index}: Preprocessed by external tools (no additional refinement required)")
    
    else:
        # Generic datasets without specific refinement requirements
        if _is_dataset_type_registered(dataset_type):
            logger.debug(f"{dataset_type} molecule {molecule_index}: No dataset-specific refinement required")
        else:
...
def apply_dataset_specific_refinement(
    ...
    try:
        ...
        refinement_result = refine_molecular_data(
            raw_properties_dict, molecule_index, inchi, data_config, 
            dataset_config, processing_config, handler
        )
        ...
        logger.debug(f"Dataset-specific refinement completed successfully for molecule {molecule_index}")
        return refinement_result
    
    except (PropertyEnrichmentError, VibrationRefinementError, ConfigurationError, DataProcessingError):
...
#---    
    ...
    # Catch any remaining unexpected errors
        ...
        logger.error(f"Unexpected error during refinement for molecule {molecule_index}: {type(e).__name__}: {e}")
        raise DataProcessingError(
...
def create_refinement_handler(
    ...
    # Catch import errors - handler module not found
        ...
        raise HandlerNotAvailableError(
            message=f"Could not import required handler module",
            requested_dataset_type=dataset_config.dataset_type,
    ...
    # Catch attribute errors - handler interface mismatch
        ...
        raise HandlerConfigurationError(
            message=f"Handler does not implement required interface",
    ...
    # Catch type errors - wrong parameter types
        ...
        raise HandlerConfigurationError(
            message=f"Invalid parameters for handler creation",
            handler_type=dataset_config.dataset_type,
    ...
    # Catch value errors - invalid configuration values
        ...
        raise HandlerConfigurationError(
            message=f"Invalid configuration values for handler",
            handler_type=dataset_config.dataset_type,
    ...
    # Catch truly unexpected errors
        ...
        if any(keyword in error_msg_lower for keyword in ["not found", "not available", "missing", "cannot find"]):
            raise HandlerNotAvailableError(
                message=f"Could not create refinement handler: {str(e)}",
                requested_dataset_type=dataset_config.dataset_type,
                details=f"Unexpected {type(e).__name__}: {str(e)}"
            ) from e
        elif any(keyword in error_msg_lower for keyword in ["config", "setting", "parameter", "option"]):
            raise HandlerConfigurationError(
...
def get_refinement_statistics_with_handler(
    ...
    try:
        ...
        logger.error(
            f"Direct handler statistics collection failed: {type(e).__name__}: {e}",
            exc_info=True
        )
        raise
...
def migrate_refinement_call_to_handler(
    ...
    # Handler operation error - may be recoverable
        ...
        logger.error(
            f"Handler operation error during migration for molecule {molecule_index}: {e}"
        )
...
def get_migration_benefits() -> Dict[str, str]:
    ...
    return {
        'code_organization': 
            'Dataset-specific refinement logic is now centralized in handlers '
            'instead of scattered conditionals throughout the module.',
        
        'maintainability':
            'Adding new dataset types only requires implementing handler methods. '
            'No need to modify existing refinement functions.',
        
        'testability':
            'Handler methods can be unit tested in isolation. Mock handlers '
            'can be used for testing refinement logic without dataset dependencies.',
        
        'performance':
            'Handler instances can be reused across multiple molecules, '
            'reducing configuration overhead in batch processing.',
        
        'consistency':
            'All dataset-specific operations follow the same handler interface, '
            'ensuring consistent behavior across the pipeline.',
        
        'extensibility':
            'New refinement capabilities can be added to handlers without '
            'affecting the main refinement interface.',
        
        'separation_of_concerns':
            'Refinement logic is clearly separated from dataset-specific logic, '
            'making both easier to understand and modify.',
        
        'error_handling':
            'Comprehensive handler exception handling provides clear error reporting '
            'and recovery strategies for different failure scenarios.',
        
        'reliability':
            'Multiple fallback mechanisms ensure processing can continue even when '
            'handlers are unavailable or encounter errors.',
        
        'migration_safety':
            'Migration tracking and error handling ensure smooth transition from '
            'legacy code to handler pattern without data loss.',
        
        'vibrational_data_robustness':
            'VQM24: Enhanced vibrational mode processing handles complex nested structures '
            'found in VQM24 dataset, reducing processing failures and improving data quality.',
        
        'vqm24_compatibility':
            'VQM24: Specific improvements for handling the complex data structures in '
            'DFT_all.npz, including linear molecules and mixed object arrays.',
        
        'debugging_capabilities':
            'VQM24: Enhanced logging and diagnostics help identify and resolve '
...
def diagnose_vibrational_data_structure(
            ...
            if i < len(raw_vibmodes):
                try:
                    ...
                    sample_info = {
                        ...
                        'size': getattr(sample, 'size', len(sample) if hasattr(sample, '__len__') else 'N/A'),
                        'content_preview': str(sample)[:100] + '...' if len(str(sample)) > 100 else str(sample)
                    }
    ...
    try:
        ...
        diagnostics['extraction_test'] = {
            ...
            'extracted_sample': extracted_values[:10] if len(extracted_values) > 10 else extracted_values,
            'divisible_by_3': len(extracted_values) % 3 == 0,
            'potential_atoms': len(extracted_values) // 3 if len(extracted_values) % 3 == 0 else 'N/A'
        }
    except Exception as e:
        diagnostics['extraction_test'] = {
            'success': False,
