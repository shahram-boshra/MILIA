...
def _init_registry() -> bool:
    ...
    try:
        ...
        logger.warning(
            f"Dataset registry not available in config_accessors - using legacy validation: {e}"
        )
        return False
        ...
        _REGISTRY_IMPORT_ERROR = str(e)
        logger.warning(
            f"Dataset registry import failed in config_accessors - using legacy validation: {e}"
        )
        return False
...
def _registry_list_all_safe() -> list[str]:
    ...
    try:
        ...
        if implementations_dir.exists():
            ...
            if discovered_types:
                logger.debug(
                    f"ConfigAccessors: Dynamically discovered dataset types: {discovered_types}"
                )
    ...
    logger.warning(
        "ConfigAccessors: No dataset types available - registry not initialized and dynamic discovery failed"
    )
    return []
...
def validate_dataset_config(config: dict[str, Any]) -> tuple[bool, list[str]]:
    ...
    if not _is_valid_dataset_type(dataset_type):
        valid_types = _get_valid_dataset_types()
        errors.append(
            f"Invalid dataset_type '{dataset_type}'. Valid types: {', '.join(valid_types)}"
        )
        return False, errors
...
class EnhancedConfigAccessor:
    ...
    def _get_raw_value(self, key: str, default: Any = None) -> Any:
        ...
        elif CONTAINERS_AVAILABLE and isinstance(
            self._config, (DatasetConfig, TransformationConfig, HandlerConfig)
        ):
    ...
    def _validate_value(
        self, key: str, value: Any, expected_type: type | None = None
    ) -> tuple[bool, list[str]]:
        """
    ...
    def _get_intelligent_default(self, key: str, expected_type: type | None = None) -> Any:
        """
                    ...
                    if len(parts) >= 2:
                        ...
                        try:
                            param_metadata = self._gt.get_parameter_info(
                                potential_transform, param_name
                            )
    ...
    def _generate_default_from_type(self, type_hint: type) -> Any:
        ...
        return type_defaults.get(type_hint)
...
def get_transform_config(
    ...
    accessor = EnhancedConfigAccessor(
        config, auto_validate=validate, dataset_context=dataset_context
    )
    ...
    if validate and GRAPH_TRANSFORMS_AVAILABLE:
        gt = get_graph_transforms()
        validation_result = gt.validate_config([transform_config], dataset_type=dataset_context)
...
def get_transform_parameter(
    ...
    try:
        ...
        intelligent_default = accessor._get_intelligent_default(
            f"{transform_name}_{parameter_name}", expected_type
        )
                ...
                try:
                    ...
                    if isinstance(param_metadata, ParameterMetadata):
                        if param_metadata.has_default:
                            ...
                            value = accessor._get_intelligent_default(parameter_name, expected_type)
                    else:
                        value = None
                except (TransformNotFoundError, TransformValidationError):
                    value = None
    ...
    if validate and GRAPH_TRANSFORMS_AVAILABLE:
        ...
        try:
            is_valid, errors = gt.validate_parameter(
                transform_name, parameter_name, value, check_constraints=True
            )
...
def get_dataset_specific_config(
    ...
    """
    accessor = EnhancedConfigAccessor(config, auto_validate=validate, dataset_context=dataset_type)
...
def get_all_transforms(
    ...
    """
    accessor = EnhancedConfigAccessor(
        config, auto_validate=validate_each, dataset_context=dataset_context
    )
    ...
    if validate_each and GRAPH_TRANSFORMS_AVAILABLE:
        gt = get_graph_transforms()
        validation_result = gt.validate_config(transforms, dataset_type=dataset_context)

        if not validation_result["valid"]:
            logger.warning(f"Transform validation found issues: {validation_result['errors']}")
    ...
    if GRAPH_TRANSFORMS_AVAILABLE and isinstance(config, dict):
        ...
        if transforms:
            ...
            validation_result = gt.validate_config_comprehensive(
                transforms, dataset_type=dataset_context, validation_level=validation_level
            )
...
def _get_config_value(
    ...
    if key not in config_dict:
        raise ConfigurationError("Missing required configuration key.", config_key=full_key)
...
def get_dataset_type() -> str:
    ...
    try:
        ...
        raise ConfigurationError(
            f"Failed to get dataset type: {str(e)}", config_key="dataset_type"
        ) from e


def get_handler_type(dataset_type: str | None = None) -> str:
    """
...
def is_handler_type(handler_type: str, dataset_type: str | None = None) -> bool:
    """
...
def get_optional_properties(dataset_type: str | None = None) -> list[str]:
    """
...
def get_supported_features(dataset_type: str | None = None) -> dict[str, bool]:
    """
...
def is_feature_supported(feature_name: str, dataset_type: str | None = None) -> bool:
    """
...
def get_dataset_config(dataset_type: str | None = None) -> dict[str, Any]:
    """
...
def get_raw_data_info(dataset_type: str | None = None) -> dict[str, Any]:
    """
...
def get_data_config() -> dict[str, Any]:
    ...
    dataset_specific_config = _get_config_value(
        property_selection, dataset_type, dict, parent_key="data_config.property_selection"
    )

    # Merge common settings with dataset-specific settings
    common_settings = _get_config_value(
        data_config, "common_settings", dict, parent_key="data_config"
    )
...
def get_property_availability() -> dict[str, Any]:
    ...
    return _get_config_value(
        property_availability, dataset_type, dict, parent_key="property_availability"
    )


# Uncertainty Configuration (DMC-specific)
def get_uncertainty_config(dataset_type: str | None = None) -> dict[str, Any] | None:
    """
...
def is_uncertainty_enabled() -> bool:
    ...
    return uncertainty_config is not None and uncertainty_config.get(
        "use_for_loss_weighting", False
    )


# Structural Features Configuration
def get_structural_features_config() -> dict[str, Any] | None:
    """
...
def get_structural_features_preprocessing_config() -> dict[str, Any] | None:
    """
...
def get_charge_handling_config() -> dict[str, Any]:
    ...
    return preprocessing_config.get(
        "charge_handling",
        {
            "prefer_mulliken": True,
            "compute_gasteiger_fallback": True,
...
def get_stereochemistry_config() -> dict[str, Any]:
    ...
    if preprocessing_config is None:
        # Return sensible defaults
        return {"assign_stereochemistry": True, "cleanup_stereochemistry": True}
    return preprocessing_config.get(
        "stereochemistry", {"assign_stereochemistry": True, "cleanup_stereochemistry": True}
    )
...
def get_graph_transforms():
    ...
    try:
        from milia_pipeline.transformations.graph_transforms import (
            get_graph_transforms as gt_get_transforms,
        )
...
def create_experimental_setup_from_dict(
    setup_name: str, setup_config: dict[str, Any]
    ...
    try:
        ...
        from milia_pipeline.config.config_containers import (
            create_experimental_setup_from_dict as container_create,
        )
...
def save_experimental_setup(
    setup: ExperimentalSetup, persist_to_config: bool = False
) -> dict[str, Any]:
    """
    ...
    """
    result = {"success": False, "setup_name": setup.name, "persisted": False, "warnings": []}
    ...
    try:
        ...
        if transformation_config is None:
            raise ConfigurationError(
                ...
                suggestions=[
                    "Check config.yaml transformations section",
        ...
        if not experimental_setups:
            raise ConfigurationError(
                ...
                suggestions=["Configuration migration may have failed", "Check config.yaml format"],
            )
        ...
        for setup_name, setup in experimental_setups.items():
            if not hasattr(setup, "transforms"):
                ...
                if has_std_transforms:
                    ...
                    logger.warning(
                        f"Setup '{setup_name}' has empty transforms list and no standard_transforms configured"
                    )
...
def get_experimental_setup(
    ...
    if hasattr(setup_obj, "transforms"):
        ...
        if not setup_transforms and setup_obj.transforms:
            ...
            setup_transforms = (
                list(setup_obj.transforms) if isinstance(setup_obj.transforms, list) else []
            )
...
def get_default_experimental_setup() -> ExperimentalSetup | None:
    """
...
def get_combined_transforms_as_dicts(setup_name: str | None = None) -> list[dict[str, Any]]:
    """
...
def get_experimental_setups_for_dataset(dataset_type: str) -> dict[str, ExperimentalSetup]:
    ...
    try:
        ...
        logger.error(
            f"Unexpected error getting experimental setups for dataset {dataset_type}: {e}"
        )
        return {}
...
def get_transform_info(transform_name: str) -> dict[str, Any] | None:
    """
...
def get_transform_registry_info() -> dict[str, Any] | None:
    """
...
def get_transformation_config_summary() -> dict[str, Any]:
    ...
    try:
        ...
        logger.debug(
            f"Checked transformation compatibility for {len(registered_types)} dataset types: {registered_types}"
        )
    ...
    try:
        ...
        try:
            ...
            if enhanced_config is None:
                compatibility["issues"].append(
                    "Enhanced transformation configuration not accessible"
                )
        ...
        if compatibility["enhanced_support"] and compatibility["legacy_support"]:
            compatibility["recommendations"].append(
                "System fully compatible - both legacy and enhanced modes available"
            )

        # Overall compatibility
        compatibility["compatible"] = (
            compatibility["legacy_support"] or compatibility["enhanced_support"]
        )
    ...
    try:
        ...
        try:
            ...
            if available and available.get("count", 0) > 0:
                logger.info(
                    f"✅“ Transformation system validated: {available['count']} transforms available"
                )
...
def should_pass_mulliken_charges_to_structural_features(dataset_type: str | None = None) -> bool:
    """
    ...
    features = _get_dataset_features(dataset_type)
    supports_mulliken = features.get(
        "mulliken_charges_support", True
...
def get_dataset_appropriate_structural_features(
    dataset_type: str | None = None, structural_config: dict[str, Any] | None = None
) -> dict[str, list[str]]:
    """
    ...
    try:
        ...
        if removed_atom:
            logger.info(
                f"Filtered out unsupported atom features for {dataset_type}: {list(removed_atom)}"
            )
        if removed_bond:
            logger.info(
                f"Filtered out unsupported bond features for {dataset_type}: {list(removed_bond)}"
            )
        ...
        if not supports_mulliken:
            ...
            if "mulliken_charge" in atom_features:
                ...
                logger.info(
                    f"Legacy fallback: Removed 'mulliken_charge' for {dataset_type} (not supported)"
                )
            if "partial_charge" in atom_features:
                atom_features.remove("partial_charge")
                logger.info(
                    f"Legacy fallback: Removed 'partial_charge' for {dataset_type} (not supported)"
                )
...
def validate_structural_features_for_dataset(
    features: dict[str, list[str]], dataset_type: str | None = None
) -> tuple[bool, list[str]]:
    """
...
def get_feature_compatibility_report(dataset_type: str | None = None) -> dict[str, Any]:
    """
...
def get_dataset_constants() -> tuple[str, str | None, str]:
    """
...
def get_raw_source_filename(dataset_type: str | None = None) -> str | None:
    """
...
def get_preprocessing_config(
    dataset_type: str,
    raw_tar_path_override: str | None = None,
    output_npz_path_override: str | None = None,
) -> dict[str, Any]:
    """
    ...
    raw_tar_path = (
        Path(raw_tar_path_override).expanduser() if raw_tar_path_override else default_raw_path
    )
    output_npz_path = (
        Path(output_npz_path_override).expanduser()
        if output_npz_path_override
...
def get_required_properties(dataset_type: str | None = None) -> list[str]:
    """
...
def get_identifier_keys(dataset_type: str | None = None) -> list[tuple[str, str]]:
    """
...
def get_coordinate_units(dataset_type: str | None = None) -> str:
    """
...
def get_energy_units(dataset_type: str | None = None) -> str:
    """
...
def get_molecule_creation_strategy(dataset_type: str | None = None) -> str:
    """
...
def create_handler_compatible_config(
    dataset_type: str, base_config: dict[str, Any]
) -> dict[str, Any]:
    """
