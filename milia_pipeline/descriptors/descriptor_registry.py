...
class DescriptorRegistry:
    ...
    def auto_discover_rdkit_descriptors(self) -> int:
        ...
        for name in known_descriptor_names:
            ...
            try:
                ...
                if func is None:
                    func, found_location = self._try_case_insensitive_search(name, Descriptors, Descriptors3D)
                    if func is not None:
        ...
        if case_mismatch_candidates:
            logger.info(f"  → {len(case_mismatch_candidates)} descriptors found with case differences:")
        ...
        if failed_descriptors:
            logger.info(f"  → {len(failed_descriptors)} descriptors not available in current RDKit version")
            logger.debug(f"      Missing descriptors: {[name for name, _ in failed_descriptors[:10]]}")
    ...
    def register_mol_method_descriptors(self) -> int:
        ...
        logger.info(f"Mol method registration complete: {registered_count} registered, {failed_count} failed")
        return registered_count

    def _try_case_insensitive_search(self, name: str, *modules) -> Tuple[Optional[Callable], Optional[str]]:
        """
            ...
            for attr_name in dir(module):
                if attr_name.lower() == name_lower and not attr_name.startswith('_'):
                    ...
                    if callable(obj):
                        ...
                        logger.debug(f"Found {name} as {attr_name} in {module_name} (case mismatch)")
    ...
    def register_descriptor(
        ...
        name: str,
        function: Callable,
        metadata: Optional[DescriptorMetadata] = None,
        ...
        if not name:
            raise DescriptorValidationError(
                "Descriptor name cannot be empty",
                validation_type="name_check"
            )
    ...
    def get_descriptor(self, name: str) -> Optional[Callable]:
        """
    ...
    def get_descriptor_registration(self, name: str) -> Optional[DescriptorRegistration]:
        """
    ...
    def list_available_descriptors(
        self,
        category: Optional[DescriptorCategory] = None,
        ...
        with self._lock:
            ...
            if not include_plugins or not include_builtins:
                names = [
                    ...
                    if (include_builtins and self._descriptors[name].is_builtin) or
                       (include_plugins and not self._descriptors[name].is_builtin)
                ]
    ...
    def get_3d_descriptors(self) -> List[str]:
        ...
        with self._lock:
            return [
                name for name, reg in self._descriptors.items()
                if reg.metadata.requires_3d
            ]
    ...
    def get_charge_dependent_descriptors(self) -> List[str]:
        ...
        with self._lock:
            return [
                name for name, reg in self._descriptors.items()
                if reg.metadata.requires_charges
            ]
    ...
    def get_metadata(self, name: str) -> Optional[DescriptorMetadata]:
        """
    ...
    def get_availability_report(self) -> Dict[str, Any]:
        ...
        mol_methods = [name for name, reg in self._descriptors.items() 
                       if reg.metadata.rdkit_module == "Mol"]
        ...
        report = {
            ...
            'by_category': by_category,
            'success_rate': len(self._descriptors) / total_requested * 100 if total_requested > 0 else 0
        }
    ...
    def log_availability_summary(self):
        ...
        logger.info(f"Total Registered: {report['total_registered']} / {report['total_requested']} "
...
def get_descriptor(name: str) -> Optional[Callable]:
...
def list_descriptors(category: Optional[DescriptorCategory] = None) -> List[str]:
