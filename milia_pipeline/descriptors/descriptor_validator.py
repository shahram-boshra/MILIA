...
class DescriptorValidator:
    ...
    def validate_batch_values(
        self, descriptor_values: dict[str, float], **validation_kwargs
    ) -> ValidationResult:
        """
        ...
        for desc_name, value in descriptor_values.items():
            is_valid, error_msg = self.validate_value(desc_name, value, **validation_kwargs)
    ...
    def check_requirements(self, mol: Chem.Mol, descriptor_name: str) -> tuple[bool, list[str]]:
        """
    ...
    def filter_by_requirements(self, mol: Chem.Mol, descriptor_names: list[str]) -> dict[str, Any]:
        """
    ...
    def validate_configuration(self, config: dict[str, Any]) -> ValidationResult:
        ...
        if "selection_mode" in config:
            ...
            if config["selection_mode"] not in valid_modes:
                errors.append(
                    f"Invalid selection_mode: {config['selection_mode']}, must be one of {valid_modes}"
                )
        ...
        if "plugins" in config:
            ...
            if "plugin_paths" in plugin_config and not isinstance(
                plugin_config["plugin_paths"], list
            ):
