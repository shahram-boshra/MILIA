...
from typing import Dict, List, Optional, Set, Any, Tuple, Type, Callable
...
from .descriptor_registry import DescriptorRegistry, registry as descriptor_registry
from .descriptor_categories import DescriptorMetadata, DescriptorCategory
...
class DescriptorPluginLoader:
        ...
        for search_path in self._plugin_paths:
            ...
            for plugin_yaml in search_path.glob("*/plugin.yaml"):
                try:
                    ...
                    logger.info(f"Discovered plugin: {plugin_meta.plugin_name} v{plugin_meta.version}")
    ...
    def _load_plugin_metadata_from_yaml(
        self,
        yaml_path: Path
    ) -> Optional[DescriptorPluginMetadata]:
        """
    ...
    def _register_descriptor_from_declaration(
        ...
        try:
            ...
            metadata = DescriptorMetadata(
                ...
                description=declaration.description or f"Plugin descriptor from {plugin_meta.plugin_name}"
            )
            ...
            logger.warning(
                f"⚠ {descriptor_name}: Module not found "
                f"[Plugin: {plugin_meta.plugin_name}]"
            )
            return {
                'registered': False,
            ...
            logger.error(
                f"✗ {descriptor_name}: Registration failed: {e} "
                f"[Plugin: {plugin_meta.plugin_name}]"
            )
    ...
    def _load_descriptor_function(
        ...
        if not hasattr(module, function_name):
            raise AttributeError(
                f"Function '{function_name}' not found in {module_file}"
            )
    ...
    def validate_plugin(self, plugin_name: str) -> Tuple[bool, List[str]]:
        ...
        plugin_meta.validation_results = {
            'errors': errors,
            'warnings': warnings
        }
    ...
    def _log_plugin_discovery_summary(
        ...
        if plugin_meta.missing_implementations:
            ...
            logger.info(f"  Status: ✓ All declared descriptors successfully registered")
        else:
            logger.info(f"  Status: ℹ No descriptors declared or registered")
...
def discover_plugins(paths: Optional[List[Path]] = None, auto_validate: bool = False) -> List[str]:
