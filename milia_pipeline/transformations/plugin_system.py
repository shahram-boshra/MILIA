...
from dataclasses import field  # Kept for potential backward compatibility elsewhere
...
def _import_custom_transforms():
    ...
    try:
        from milia_pipeline.transformations.custom_transforms import (
            CustomTransformBase as _CTB,
            MolecularTransformBase as _MTB,
            QuantumTransformBase as _QTB,
            TransformMetadata as _TM
        )
...
class PluginMetadata(BaseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        base_dict['transform_declarations'] = [decl.to_dict() for decl in self.transform_declarations]
...
class PluginRegistry:
        ...
        for search_path in instance._plugin_paths:
            ...
            for plugin_yaml in search_path.glob("*/plugin.yaml"):
                try:
                    ...
                    plugin_type_label = "PyG Fallback" if plugin_meta.plugin_type == "pyg_fallback" else "User Experimental"
                    logger.info(f"Discovered plugin from YAML: {plugin_meta.plugin_name} [{plugin_type_label}]")
    ...
    @classmethod
    def _load_plugin_metadata_from_yaml(cls, yaml_path: Path) -> Optional[PluginMetadata]:
        """
            ...
            if 'transforms' in data and isinstance(data['transforms'], list):
                for transform_dict in data['transforms']:
                    try:
                        ...
                        logger.warning(
                            f"Failed to parse transform declaration in {yaml_path}: {e}"
                        )
    ...
    def _register_transform_with_fallback(
        ...
        try:
            ...
            if isinstance(e, FileNotFoundError):
                logger.warning(f"⚠ {transform_name}: Module not found [Plugin: {plugin_meta.plugin_name}]")
            elif isinstance(e, AttributeError):
                logger.warning(f"⚠ {transform_name}: Class '{declaration.class_name}' not found [Plugin: {plugin_meta.plugin_name}]")
            elif isinstance(e, ImportError):
                logger.error(f"✗ {transform_name}: Import failed: {e} [Plugin: {plugin_meta.plugin_name}]")
                import traceback
                logger.debug(f"Import traceback:\n{traceback.format_exc()}")
        except Exception as e:
            # Unexpected errors during loading
            logger.error(f"✗ {transform_name}: Unexpected load error: {e} [Plugin: {plugin_meta.plugin_name}]")
            import traceback
        ...
        if transform_class is not None:
            try:
                ...
                if not (hasattr(transform_class, 'forward') and 
                        hasattr(transform_class, 'get_metadata') and
                        callable(getattr(transform_class, 'forward')) and
                        callable(getattr(transform_class, 'get_metadata'))):
                    raise TypeError(
                ...
                if TransformRegistry is not None:
                    ...
                    try:
                        ...
                        if registered_transform is None:
                            raise Exception(f"Registration failed - transform not in registry")
                        logger.debug(f"✓ Verified {transform_name} successfully registered in TransformRegistry")
                        ...
                        logger.debug(f"✓ {transform_name} registered (verification skipped due to registry internals)")
                ...
                logger.error(f"✗ {transform_name}: Registration failed: {e} [Plugin: {plugin_meta.plugin_name}]")
                import traceback
...
#---                
    ...
    def _log_plugin_discovery_summary(
        ...
        if plugin_meta.missing_implementations:
            ...
            logger.info(f"  Status: ✓ All declared transforms successfully registered")
        else:
            logger.info(f"  Status: ℹ No transforms declared or registered")
    ...
    @classmethod
    def _load_plugin_from_module(cls, module_path: Path) -> Optional[PluginMetadata]:
        """
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                f"{module_name}.__plugin__",
                module_path / "__plugin__.py"
            )
#---            
            if spec is None or spec.loader is None:
            ...
            if not hasattr(module, 'PLUGIN_METADATA'):
                raise PluginError(
                    f"__plugin__.py missing PLUGIN_METADATA in {module_path}"
                )
            ...
            if not metadata.transforms:
                ...
                logger.debug(f"Transforms already defined in PLUGIN_METADATA for {metadata.plugin_name}, skipping directory scan")
    ...
    @classmethod
    def _load_plugin_from_standalone(cls, py_file: Path) -> Optional[PluginMetadata]:
        """
...
#---             
    ...
    @classmethod
    def _load_transforms_from_directory(
        cls,
        directory: Path,
...
#---                
    ...
    def validate_plugin(cls, plugin_name: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"Validation {'PASSED' if results['passed'] else 'FAILED'} "
    ...
    def _test_transform_instantiation(cls, metadata: PluginMetadata) -> Dict:
        ...
        if TransformRegistry is None:
            return {
                'passed': False,
                'failures': [{'error': 'TransformRegistry not available'}]
            }
        ...
        for transform_name in metadata.registered_transforms:
            try:
                ...
                logger.error(f"❌ Instantiation failed for {transform_name}: {type(e).__name__}: {e}")
        ...
        if results['failures']:
            import logging
            logger = logging.getLogger(__name__)
            logger.error(f"Instantiation test failures: {len(results['failures'])} transform(s) failed")
    ...
    def _test_parameter_validation(cls, metadata: PluginMetadata) -> Dict:
        ...
        for transform_name in metadata.registered_transforms:
            try:
                ...
                issues.append({
                    'transform': transform_name,
                    'issue': f"Parameter validation failed: {e}"
                })
    ...
    def _test_data_compatibility(cls, metadata: PluginMetadata) -> Dict:
        ...
        try:
            ...
            return {
                'passed': False,
                'failures': [{'error': 'PyTorch Geometric not available'}]
            }
        _import_graph_transforms()
        if TransformRegistry is None:
            return {
        ...
        for transform_name in metadata.registered_transforms:
            try:
                ...
                if result is not None and not isinstance(result, Data):
                    raise TypeError(
                        f"Transform returned {type(result)}, expected Data or None"
                    )
                ...
                import traceback
                logger = logging.getLogger(__name__)
                logger.error(f"❌ Compatibility test failed for {transform_name}: {type(e).__name__}: {e}")
                logger.debug(f"Full traceback:\n{traceback.format_exc()}")
    ...
    def list_plugins(
        ...
        if validated_only:
            plugins = [
                ...
                if instance._plugins[name].is_validated
            ]
        
        if enabled_only:
            plugins = [
                name for name in plugins
                if name in instance._enabled_plugins
            ]
        
        return plugins
    
    @classmethod
    def get_plugin_info(cls, plugin_name: str) -> Optional[Dict]:
...
class PluginValidator:
    ...
    def validate_plugin_comprehensive(
        ...
        report['sections']['code_quality'] = PluginValidator._check_code_quality(
            metadata
        )
        
        # Section 2: Documentation
        logger.info(f"Checking documentation for {plugin_name}...")
        report['sections']['documentation'] = PluginValidator._check_documentation(
            metadata
        )
        ...
        if run_performance_tests:
            ...
            report['sections']['performance'] = PluginValidator._benchmark_performance(
                metadata
            )
