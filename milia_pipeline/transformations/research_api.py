    ...
    def validate_experiment_configuration(self) -> "ExperimentConfiguration":
        ...
        if not self.name:
            raise ConfigurationError(
                message="Experiment name is required", config_key="experiment.name"
            )
    ...
    def from_dict(cls, data: dict[str, Any]) -> "ExperimentConfiguration":
        ...
        for t in data.get("base_transforms", []):
            if isinstance(t, dict):
                ...
                base_transforms.append(TransformSpec(name=str(t), kwargs={}, enabled=True))
...
class AblationStudyBuilder:
    ...
    def remove_transform(
        self, transform_name: str, variant_name: str | None = None
        ...
        self.variants[variant_name] = [t for t in self.baseline_transforms if t != transform_name]
        logger.debug(f"Created variant '{variant_name}': {self.variants[variant_name]}")
        return self

    def keep_only(
        self, transform_names: list[str], variant_name: str = "minimal"
    ) -> "AblationStudyBuilder":
        """
        ...
        Returns:
            Self for method chaining
        """
        self.variants[variant_name] = [t for t in self.baseline_transforms if t in transform_names]
        logger.debug(f"Created variant '{variant_name}': {self.variants[variant_name]}")
        return self

    def add_variant(
        self, variant_name: str, additional_transforms: list[str], position: int = -1
    ) -> "AblationStudyBuilder":
        """
    ...
    def replace_transform(
        self, old_transform: str, new_transform: str, variant_name: str | None = None
    ) -> "AblationStudyBuilder":
        """
        ...
        self.variants[variant_name] = [
            new_transform if t == old_transform else t for t in self.baseline_transforms
        ]
        logger.debug(f"Created variant '{variant_name}': {self.variants[variant_name]}")
        return self

    def with_metadata(
        self, hypothesis: str, expected_outcome: str, paper_section: str | None = None
    ) -> "AblationStudyBuilder":
        """
    ...
    def build(self) -> ExperimentConfiguration:
        ...
        if not self.baseline_transforms:
            raise ConfigurationError(
                message="Baseline transforms not set", config_key="ablation_study.baseline"
            )

        # Convert to ablation format
        ablations = [
            {"name": name, "transforms": transforms} for name, transforms in self.variants.items()
        ]

        base_specs = [
            TransformSpec(name=t, kwargs={}, enabled=True) for t in self.baseline_transforms
        ]
...
class ParameterSweepBuilder:
    ...
    def __init__(self, sweep_name: str):
        ...
        self.target_transform: str | None = None
    ...
    def sweep_parameter(self, param_name: str, values: list[Any]) -> "ParameterSweepBuilder":
        """
    ...
    def with_baseline_transforms(self, transforms: list[str]) -> "ParameterSweepBuilder":
        """
    ...
    def with_metadata(self, hypothesis: str, expected_outcome: str) -> "ParameterSweepBuilder":
        """
        ...
        Returns:
            Self for method chaining
        """
        self.metadata.update({"hypothesis": hypothesis, "expected_outcome": expected_outcome})
        return self
    ...
    def build(self) -> ExperimentConfiguration:
        ...
        if not self.parameter_sweeps:
            raise ConfigurationError(
                message="No parameters to sweep", config_key="parameter_sweep.parameters"
            )
        ...
        for i, params in enumerate(param_combinations):
            variant_name = f"sweep_{i}_" + "_".join(f"{k}={v}" for k, v in params.items())

            # Build transform sequence
            transforms = self.baseline_transforms.copy()
            transforms.append({"name": self.target_transform, **params})

            variants.append({"name": variant_name, "transforms": transforms, "parameters": params})
...
class ComparativeStudyBuilder:
    ...
    def with_metadata(
        self, research_question: str, expected_best: str
    ) -> "ComparativeStudyBuilder":
        """
        Add study metadata.
        ...
        """
        self.metadata.update(
            {"research_question": research_question, "expected_best": expected_best}
        )
        return self
    ...
    def build(self) -> ExperimentConfiguration:
        ...
        variants = [
            {"name": name, "transforms": transforms} for name, transforms in self.approaches.items()
        ]
        ...
        logger.info(f"Built comparative study '{self.study_name}' with {len(variants)} approaches")
        return config
...
class ExperimentRunner:
    ...
    def __init__(self, config: ExperimentConfiguration, output_dir: str | Path, seed: int = 42):
        """
            ...
            for run_idx in range(num_runs):
                ...
                try:
                    ...
                    dataset = dataset_loader(
                        transforms=variant["transforms"], seed=self.seed + run_idx
                    )
                    ...
                    logger.info(f"    ✓ Eval MAE: {eval_metrics.get('mae', 'N/A'):.4f}")

                except Exception as e:
                    logger.error(f"    ✗ Run {run_idx} failed: {e}")
                    variant_results.append(
                        {"run": run_idx, "variant": variant_name, "error": str(e)}
                    )
    ...
    def _analyze_results(self) -> dict[str, Any]:
        ...
        for variant_name, runs in self.results.items():
            ...
            if not successful_runs:
                summary["variants"][variant_name] = {"num_runs": len(runs), "status": "all_failed"}
                continue

            # Extract evaluation metrics (assuming 'mae' as primary)
            eval_scores = [run["eval_metrics"].get("mae", float("inf")) for run in successful_runs]
        ...
        if valid_variants:
            best_variant = min(valid_variants.items(), key=lambda x: x[1]["mean"])
            summary["best_variant"] = {"name": best_variant[0], "score": best_variant[1]["mean"]}
        else:
...
def create_ablation_study(
    study_name: str, baseline_transforms: list[str], transforms_to_ablate: list[str], **metadata
...
def create_parameter_sweep(
    ...
    **metadata,
    ...
    """
    builder = (
        ParameterSweepBuilder(sweep_name)
...
def create_comparative_study(
    study_name: str, approaches: dict[str, list], evaluation_metrics: list[str], **metadata
) -> ExperimentConfiguration:
    """
...
def get_experiment(
    experiment_name: str, config_path: Path | None = None
) -> ExperimentConfiguration:
    """
...
def list_available_experiments(config_path: Path | None = None) -> list[str]:
    """
