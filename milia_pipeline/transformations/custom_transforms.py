...
def _lazy_import_graph_transforms():
    ...
    try:
        from .graph_transforms import (
            TransformRegistry as _TR,
            ValidationLevel as _VL,
            ValidationScope as _VS,
            get_transform_info as _GTI,
            validate_comprehensive as _VC
        )
...
try:
    ...
    class TransformExecutionError(Exception):
        ...
        def __init__(self, message: str, transform_name: str = None, original_error: Exception = None):
            super().__init__(message)
            self.transform_name = transform_name
...
class TransformMetadata(BaseModel):
    ...
    def __str__(self) -> str:
        ...
        return (f"TransformMetadata(name='{self.name}', version='{self.version}', "
...
class CustomTransformBase(BaseTransform, ABC):
    ...
    def __call__(self, data: Data) -> Optional[Data]:
        ...
        try:
            ...
            self._error_count += 1
            error_msg = (f"Transform '{self._metadata.name}' failed on sample "
...
class MolecularTransformBase(CustomTransformBase):
    ...
    def validate_molecular_structure(self, data: Data) -> Tuple[bool, List[str]]:
        ...
        if hasattr(data, 'x') and data.x is not None:
            ...
            invalid_atoms = [
                z.item() for z in atomic_nums 
                if z.item() not in self.VALID_ATOMIC_NUMBERS
            ]
            if invalid_atoms:
                issues.append(f"Invalid atomic numbers found: {invalid_atoms[:5]}" + 
        ...
        if hasattr(data, 'edge_attr') and data.edge_attr is not None:
            ...
            invalid_bonds = [
                b.item() for b in bond_types 
                if b.item() not in self.VALID_BOND_TYPES
            ]
            if invalid_bonds:
        ...
        if hasattr(data, 'pos') and data.pos is not None:
            if data.pos.shape[0] != data.num_nodes:
                issues.append(
                    f"Coordinate count mismatch: {data.pos.shape[0]} != {data.num_nodes}"
                )
    ...
    def validate_quantum_properties(self, data: Data) -> Tuple[bool, List[str]]:
        ...
        if hasattr(data, 'charges') and data.charges is not None:
            if data.charges.shape[0] != data.num_nodes:
                issues.append(
                    f"Charge count mismatch: {data.charges.shape[0]} != {data.num_nodes}"
                )
            ...
            if hasattr(data, 'total_charge'):
                ...
                expected_charge = data.total_charge if isinstance(data.total_charge, torch.Tensor) else torch.tensor(data.total_charge)
                ...
                if not torch.isfinite(expected_charge):
                    ...
                    issues.append(f"Charge not conserved: sum={charge_sum:.3f}, expected={expected_charge:.3f}")
        ...
        if hasattr(data, 'forces') and data.forces is not None:
            if data.forces.shape[0] != data.num_nodes:
                issues.append(
                    f"Forces count mismatch: {data.forces.shape[0]} != {data.num_nodes}"
                )
...
class NormalizeVibrationalModes(QuantumTransformBase):
    ...
    def transform(self, data: Data) -> Data:
        ...
        if data.vibmodes.dim() != 3:
            self._logger.error(f"Expected 3D vibmodes [n_modes, n_atoms, 3], got {data.vibmodes.dim()}D")
            raise TransformExecutionError(
        ...
        if data.vibmodes.shape[2] != 3:
            self._logger.error(f"Expected 3D coordinates in vibmodes, got shape {data.vibmodes.shape}")
            raise TransformExecutionError(
                f"Invalid vibmodes shape: {data.vibmodes.shape}",
                transform_name=self._metadata.name
            )
        ...
        if self.normalize_per_mode:
            norms = torch.norm(
                data.vibmodes.view(data.vibmodes.shape[0], -1),
                dim=1,
                keepdim=True
            )
            ...
            if norms.shape != expected_norm_shape:
                ...
                raise TransformExecutionError(
                    "Shape mismatch in normalization",
                    transform_name=self._metadata.name
                )
            ...
            if data.vibmodes.shape != original_shape:
                self._logger.error(f"Vibmodes shape changed: {original_shape} -> {data.vibmodes.shape}")
                raise TransformExecutionError(
                    "Shape changed during normalization",
                    transform_name=self._metadata.name
                )
    ...
    def transform(self, data: Data) -> Data:
        ...
        if self.center:
            ...
            if charges.shape != original_shape:
                self._logger.error(f"Charges shape changed after centering: {original_shape} -> {charges.shape}")
                raise TransformExecutionError(
                    "Shape mismatch after centering",
                    transform_name=self._metadata.name
                )
        ...
        if data.charges.shape != original_shape:
            self._logger.error(f"Charges shape changed after scaling: {original_shape} -> {data.charges.shape}")
            raise TransformExecutionError(
                "Shape mismatch after scaling",
                transform_name=self._metadata.name
            )
...
class DiscretizeTargets(QuantumTransformBase):
    ...
    def __init__(
        ...
        fitted_bin_edges: Optional[Dict[str, torch.Tensor]] = None,
    ...
    def fit(self, data_list, attrs: Optional[List[str]] = None) -> 'DiscretizeTargets':
        """
                ...
                if hasattr(data, attr) and getattr(data, attr) is not None:
                    ...
                    if values.dim() > 1:
                        ...
                        if last_dim_size > 1:
                            ...
                            self._logger.debug(
                                f"fit(): Single-feature 2D target (shape[-1]=1): squeezed"
                            )
                    ...
                    # For consistency with transform(), we extract a single value
                        ...
                        if self.target_level != 'auto':
                            ...
                            if num_edges is None and hasattr(data, 'edge_index') and data.edge_index is not None:
                        ...
                        if not is_node_level and not is_edge_level:
                            if first_dim > 1:
                                ...
                                self._logger.debug(
                                    f"fit(): Graph-level 1D single-target: using value as-is"
                                )
    ...
    def get_fitted_bin_edges(self, attr: str = 'y') -> Optional[torch.Tensor]:
    ...
    def transform(self, data: Data) -> Data:
        ...
        for attr in self.attrs:
            ...
            num_nodes = data.num_nodes if hasattr(data, 'num_nodes') else (
                data.x.size(0) if hasattr(data, 'x') and data.x is not None else None
            )
            
            # Get num_edges
            num_edges = data.num_edges if hasattr(data, 'num_edges') else (
                data.edge_index.size(1) if hasattr(data, 'edge_index') and data.edge_index is not None else None
            )
            ...
            if values.dim() > 1:
                ...
                if num_targets > 1:
                    ...
                    values = values[self.target_column:self.target_column+1]  # [num_targets] -> [1]
                    self._logger.debug(
                    ...
                    self._logger.debug(
                        f"Graph-level single-target detected (1D): keeping shape [1] for "
                        f"PyG batching consistency"
                    )
            ...
            was_1d_graph_target = (len(original_shape) == 1 and original_shape[0] >= 1 and 
                                   target_level == 'graph' and multi_target_handled)
            
            if was_scalar:
                # Original was scalar, restore to scalar
                class_indices = class_indices.squeeze(0) if class_indices.dim() > 0 else class_indices
    ...
    def compute_class_weights(
        ...
        if method == 'balanced':
            ...
            raise ValueError(f"Unknown method '{method}'. Choose from: 'balanced', 'inverse', 'sqrt_inverse'")
...
if __name__ == "__main__":
    ...
    print(f"milia Custom Transforms Module v1.0")
    ...
    print(f"  - CustomTransformBase")
    print(f"  - MolecularTransformBase")
    print(f"  - QuantumTransformBase")
    print(f"\nExample transforms:")
    print(f"  - NormalizeVibrationalModes")
    print(f"  - FilterByDMCUncertainty")
    print(f"  - ScaleMullikenCharges")
...
if __name__ == "__main__":
    ...
    # Check original was NOT modified
    assert torch.allclose(data3.charges, original_charges_copy), "❌ FAILED: Original charges modified!"
    assert not torch.allclose(result3.charges, original_charges_copy), "❌ FAILED: Result not transformed!"
