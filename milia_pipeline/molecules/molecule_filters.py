...
def validate_filter_compatibility_with_transforms(
    ...
    if filter_config is None or transform_config is None:
        validation_results["warnings"].append(
            "Incomplete configuration - skipping compatibility check"
        )
    ...
    try:
        ...
        if has_structural_transforms and filter_config.heavy_atom_filter:
            ...
            if detailed_reporting:
                validation_results["detailed_analysis"]["structural_heavy_atom"] = {
                    "transforms": [
                        t.get("name") for t in transforms if t.get("name") in structural_transforms
                    ],
        ...
        node_modifying_present = [
            t for t in transforms if t.get("name") in node_modifying_transforms
        ]
        ...
        edge_modifying_present = [
            t for t in transforms if t.get("name") in edge_modifying_transforms
        ]
            ...
            if detailed_reporting:
                ...
                for t in edge_modifying_present:
                    ...
                    if t_name == "DropEdge":
                        ...
                        severity = (
                            "high" if drop_prob > 0.5 else "medium" if drop_prob > 0.2 else "low"
                        )
                ...
                validation_results["detailed_analysis"]["edge_modification_analysis"] = (
                    edge_analysis
                )
        ...
        if has_augmentation and filter_config.dmc_uncertainty_filter:
            ...
            if uncertainty_filter.get("filter_invalid_uncertainties", True):
                ...
                if detailed_reporting:
                    aug_transforms = [
                        t for t in transforms if t.get("name") in augmentation_transforms
                    ]
        ...
        if experimental_setup:
            ...
            logger.debug(
                f"Validated filter-transform compatibility for experimental setup: {experimental_setup}"
            )
        ...
        if include_parameter_introspection:
            param_introspection = introspect_transform_filter_parameters(
                filter_config=filter_config, transform_config=transform_config, logger=logger
            )
...
def introspect_transform_filter_parameters(
    ...
    try:
        ...
        transform_params = []
        for t in transforms:
            transform_info = {"name": t.get("name"), "params": t.get("params", {})}
        ...
        if filter_config.dmc_uncertainty_filter:
            augmentation_transforms = [
                t for t in transforms if t.get("name") in ["DropEdge", "DropNode", "MaskFeatures"]
            ]
        ...
        geometric_transforms = [
            t
            for t in transforms
            if t.get("name") in ["RandomRotate", "RandomScale", "RandomTranslate", "RandomFlip"]
        ]
        ...
        logger.debug(
            f"Parameter introspection completed: "
        ...
        introspection_results["parameter_conflicts"].append(
            {"error": str(e), "type": "introspection_error"}
        )
...
def apply_atom_count_filters(
    pyg_data: Data, filter_config: FilterConfig | None = None, logger: logging.Logger | None = None
) -> None:
    """
    ...
    try:
        ...
        if (
            filter_config.max_atoms is not None
            and filter_config.min_atoms is not None
            and filter_config.min_atoms > filter_config.max_atoms
        ):
            raise ConfigurationError(
...
def apply_heavy_atom_filters(
    ...
    heavy_atom_filter: dict[str, str | list[str]] | None = filter_config.heavy_atom_filter
    if not heavy_atom_filter:
        return

    try:
        mode: str | None = heavy_atom_filter.get("mode")  # 'include' or 'exclude'
        ...
        for symbol in filter_symbols:
            try:
                ...
                atomic_num: int | None = HEAVY_ATOM_SYMBOLS_TO_Z.get(normalized_symbol)
                if atomic_num:
        ...
        if not molecule_heavy_zs:
            if mode == "include":
                ...
                logger.debug(
                    f"Molecule {mol_idx} passed heavy atom exclude filter (no heavy atoms to exclude)"
                )
        else:
            if mode == "include":
                if not molecule_heavy_zs.issubset(target_heavy_zs_for_filter):
                    unallowed_atoms: list[int] = list(
                        molecule_heavy_zs - target_heavy_zs_for_filter
                    )
                    raise MoleculeFilterRejectedError(
                        molecule_index=mol_idx,
                        inchi=inchi,
                ...
                overlap_atoms: list[int] = list(
                    molecule_heavy_zs.intersection(target_heavy_zs_for_filter)
                )
                ...
                raise AtomFilterError(
                    ...
                    filter_config=heavy_atom_filter,
                    details="Invalid mode value in filter configuration",
                )
...
def validate_filter_configuration(
    ...
    if filter_config is None or filter_config is None:
        from milia_pipeline.config.config_containers import create_filter_config_from_global
    ...
    try:
        ...
        if _get_dataset_feature(dataset_config.dataset_type, "uncertainty_handling"):
            ...
            if max_uncertainty is not None and (
                not isinstance(max_uncertainty, (int, float)) or max_uncertainty < 0
            ):
                raise ConfigurationError(
        ...
        if transform_config is not None:
            try:
                ...
                logger.warning(
                    "Could not validate filter-transform compatibility - proceeding with caution"
                )
...
def create_handler_aware_filter_stats() -> dict[str, Any]:
    ...
    stats = {
        "filter_types_available": ["atom_count", "heavy_atom", "dataset_specific"],
        "handler_integration": {
...
def apply_pre_filters(
    ...
    if filter_config is None or (
        filter_config.max_atoms is None
        and filter_config.min_atoms is None
        and filter_config.heavy_atom_filter is None
        and filter_config.dmc_uncertainty_filter is None
    ):
    ...
    try:
        ...
        if not isinstance(
            e, (ConfigurationError, HandlerIntegrationError, TransformConfigurationError)
        ):
            raise HandlerIntegrationError(
    ...
    try:
        ...
        apply_dataset_specific_filters(pyg_data, dataset_config, filter_config, logger, handler)
        ...
        TransformConfigurationError,
        ValueError,
    ):  # Added ValueError for handler requirement
        # Re-raise expected exceptions as-is
        raise
    except Exception as e:
...
class MoleculeFilter:
    ...
    def __init__(
        ...
        if transform_config is not None and filter_config is not None:
            try:
                ...
                if compatibility_results.get("parameter_introspection"):
                    for conflict in compatibility_results["parameter_introspection"].get(
                        "parameter_conflicts", []
                    ):
                        if isinstance(conflict, dict) and conflict.get("severity") == "high":
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        status = {
            ...
            "handler_details": {
                ...
                "integration_working": (
                    self.enable_handler_integration
                    and self._handler is not None
                    and self._stats.get("molecules_processed", 0) > 0
                    and self._stats.get("handler_operations", 0) > 0
                ),
                "usage_statistics": {
                    ...
                    "handler_usage_rate": (
                        ...
                        if self._stats.get("molecules_processed", 0) > 0
                        else 0.0
                    ),
                },
            },
    ...
    def get_optimization_suggestions(self, include_applied: bool = False) -> list[dict[str, Any]]:
        """
    ...
    def has_unacknowledged_suggestions(self) -> bool:
        ...
        return len(self._stats.get("optimization_suggestions", [])) > 0 and not self._stats.get(
            "suggestions_acknowledged", False
        )
    ...
    def validate_configuration(
        ...
        if self.transform_config is not None and self.filter_config is not None:
            try:
                ...
                validation_result["detailed_analysis"] = compatibility_results.get(
                    "detailed_analysis"
                )
                validation_result["parameter_introspection"] = compatibility_results.get(
                    "parameter_introspection"
                )
                ...
                if detailed_reporting and compatibility_results.get("detailed_analysis"):
                    score_info = compatibility_results["detailed_analysis"].get(
                        "compatibility_score"
                    )
                    if score_info:
                        validation_result["configuration_status"]["compatibility_score"] = (
                            score_info
                        )
    ...
    def apply_filters(
        self,
        pyg_data: Data,
        override_dataset_config: DatasetConfig | None = None,
        override_filter_config: FilterConfig | None = None,
    ) -> bool:
        """
    ...
    def apply_atom_count_filters(
        self, pyg_data: Data, override_filter_config: FilterConfig | None = None
    ) -> bool:
        """
    ...
    def check_molecule_compatibility(self, pyg_data: Data) -> dict[str, Any]:
        ...
        try:
                ...
                if self.filter_config:
                    if self.filter_config.min_atoms and num_atoms < self.filter_config.min_atoms:
                        ...
                        compatibility["issues"].append(
                            f"Too few atoms: {num_atoms} < {self.filter_config.min_atoms}"
                        )
                    elif self.filter_config.max_atoms and num_atoms > self.filter_config.max_atoms:
                        compatibility["filter_predictions"]["atom_count"] = "would_reject"
                        compatibility["issues"].append(
                            f"Too many atoms: {num_atoms} > {self.filter_config.max_atoms}"
                        )
                    else:
            ...
            if (
                self.dataset_config
                and _get_dataset_feature(self.dataset_config.dataset_type, "uncertainty_handling")
                and self.dataset_config.is_uncertainty_enabled
            ):
            ...
            if (
                self.filter_config
                and self.filter_config.heavy_atom_filter
                and hasattr(pyg_data, "z")
                and pyg_data.z is not None
            ):
                ...
                if mode and target_symbols:
                    ...
                    for symbol in target_symbols:
                        normalized = symbol[0].upper() + (
                            symbol[1].lower() if len(symbol) == 2 else ""
                        )
                    ...
                    if mode == "include":
                        if heavy_atom_set.issubset(target_z):
                            ...
                            compatibility["issues"].append(
                                f"Contains unallowed heavy atoms: {unallowed}"
                            )
                    elif mode == "exclude":
                        ...
                        if not overlap:
                            ...
                            compatibility["filter_predictions"]["heavy_atom"] = "would_reject"
                            compatibility["issues"].append(
                                f"Contains excluded heavy atoms: {list(overlap)}"
                            )
    ...
    def check_handler_usage(
        self, min_handler_usage_rate: float = 0.9, min_molecules_for_check: int = 10
    ) -> dict[str, Any]:
        """
    ...
    def warn_if_low_handler_usage(
        self, min_handler_usage_rate: float = 0.9, min_molecules: int = 10
    ) -> bool:
        """
    ...
    def get_registry_integration_status(self) -> dict[str, Any]:
        ...
        status = {
            ...
            "current_dataset_registered": _is_dataset_type_registered(dataset_type)
            if dataset_type
            else None,
            "phase_6_complete": True,
        }
...
def create_molecule_filter(
    ...
    if validate_on_init and filter_config is not None:
        try:
            ...
            if validation_results.get("parameter_introspection"):
                conflicts = validation_results["parameter_introspection"].get(
                    "parameter_conflicts", []
                )
                high_severity = [
                    c for c in conflicts if isinstance(c, dict) and c.get("severity") == "high"
                ]
    ...
    if check_handler_status:
        ...
        if enable_handler_integration and not handler_details["handler_type"]:
            ...
            logger.info(f"✓ Handler integration active: {handler_details['handler_type']}")
