...
def _init_registry() -> bool:
    ...
    try:
        ...
        logger.debug("Phase 6: Registry integration initialized successfully for molecule_feature_enricher")
        return True
        
    except ImportError as e:
        _REGISTRY_IMPORT_ERROR = str(e)
        logger.debug(f"Phase 6: Registry not available for molecule_feature_enricher, using legacy fallback: {e}")
        return False
        
    except Exception as e:
        _REGISTRY_IMPORT_ERROR = str(e)
        logger.debug(f"Phase 6: Registry import failed for molecule_feature_enricher, using legacy fallback: {e}")
        return False
...
def estimate_molecular_properties(pyg_data: Data,
                                handler: 'DatasetHandler') -> Dict[str, float]:
    """
    ...
    try:
        ...
        if hasattr(pyg_data, 'pos') and pyg_data.pos is not None:
            ...
            if total_mass > 0:
                ...
                rog_squared = torch.sum(masses.unsqueeze(1) * torch.sum((pyg_data.pos - com.unsqueeze(0))**2, dim=1, keepdim=True)) / total_mass
        ...
        if _is_dataset_type_registered(dataset_type_str):
            ...
            try:
                ...
                if (_get_dataset_feature(dataset_type_str, 'vibrational_analysis') and 
                    handler_capabilities.get('vibrational_refinement', False)):
                    if hasattr(handler, 'refine_vibrational_data') and hasattr(pyg_data, 'freqs') and pyg_data.freqs is not None:
                        try:
                ...
                if (_get_dataset_feature(dataset_type_str, 'uncertainty_handling') and 
                    handler_capabilities.get('uncertainty_processing', False)):
                    if hasattr(handler, 'process_uncertainty_data') and hasattr(pyg_data, 'uncertainty') and pyg_data.uncertainty is not None:
                        try:
            ...
            if enrichment_category == 'vibrational':
                ...
                if hasattr(pyg_data, 'relative_uncertainty') and pyg_data.relative_uncertainty is not None:
                ...
                if hasattr(pyg_data, 'high_uncertainty') and pyg_data.high_uncertainty is not None:
                    properties['statistical_quality'] = 'low' if pyg_data.high_uncertainty.item() else 'high'
        ...
        if hasattr(pyg_data, 'edge_attr') and pyg_data.edge_attr is not None:
            properties['edge_feature_dim'] = pyg_data.edge_attr.shape[1] if pyg_data.edge_attr.ndim > 1 else 1
            ...
            if pyg_data.edge_attr.numel() > 0:
                bond_sparsity = float(torch.sum(pyg_data.edge_attr == 0).item()) / pyg_data.edge_attr.numel()
                properties['bond_feature_sparsity'] = bond_sparsity
                
                # Check bond feature quality
                if bond_sparsity > 0.9:
                    logger.warning(f"Very sparse bond features detected (sparsity: {bond_sparsity:.2%})")
        ...
        if hasattr(pyg_data, 'edge_index') and pyg_data.edge_index is not None:
            num_possible_edges = properties.get('num_atoms', 0) * (properties.get('num_atoms', 0) - 1)
            if num_possible_edges > 0:
                ...
                if connectivity_density < 0.01:
                    ...
                    logger.warning(f"Unusually high connectivity density: {connectivity_density:.4f}")
            ...
            avg_degree = float(pyg_data.edge_index.shape[1]) / max(1, properties.get('num_atoms', 1))
...
def get_molecule_identifiers(pyg_data: Data,
                           handler: 'DatasetHandler') -> Dict[str, Any]:
    """
    ...
    if hasattr(pyg_data, 'idx') and pyg_data.idx is not None:
        identifiers['index'] = int(pyg_data.idx.item()) if hasattr(pyg_data.idx, 'item') else int(pyg_data.idx)
...
def get_structural_feature_summary(pyg_data: Data,
                                 handler: 'DatasetHandler') -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if summary['has_atom_features']:
            ...
            if pyg_data.x.numel() > 0:
                ...
                summary['atom_feature_stats'] = {'min': 0.0, 'max': 0.0, 'mean': 0.0, 'nonzero_fraction': 0.0}
                summary['atom_feature_quality'] = 'empty'
        else:
            summary['atom_feature_quality'] = 'missing'
       
        if summary['has_bond_features']:
            summary['bond_feature_dim'] = pyg_data.edge_attr.shape[1] if pyg_data.edge_attr.ndim > 1 else 1
            if pyg_data.edge_attr.numel() > 0:
                ...
                bond_nonzero = float(torch.sum(pyg_data.edge_attr != 0).item()) / pyg_data.edge_attr.numel()
                ...
                if bond_max - bond_min < 1e-6:
                    ...
                    logger.warning(f"Bond features have no variation")
                ...
                summary['bond_feature_stats'] = {'min': 0.0, 'max': 0.0, 'mean': 0.0, 'nonzero_fraction': 0.0}
                summary['bond_feature_quality'] = 'empty'
        else:
            summary['bond_feature_quality'] = 'missing'
       
        # Enhanced structural analysis
        if hasattr(pyg_data, 'edge_index') and pyg_data.edge_index is not None and summary.get('num_nodes', 0) > 1:
...
def get_feature_extraction_diagnostics(pyg_data: Data,
                                     handler: 'DatasetHandler') -> Dict[str, Any]:
    """
    ...
    try:
        ...
        diagnostics = {
            ...
            'unexpected_features': [],
            'feature_quality': {
                'atom_features': 'not_available',
                'bond_features': 'not_available'
            }
        }
        ...
        if is_structural_features_enabled():
            ...
            if enrichment_category == 'vibrational':
                ...
                capabilities['charge_handling'] = {
                    'preference': 'mulliken' if charge_config.get('prefer_mulliken', True) else 'gasteiger',
                ...
                capabilities['vibrational_specific'] = {
                    ...
                    'electronic_property_estimation': hasattr(handler, 'estimate_electronic_properties')
                }
                ...
                if handler_caps.get('vibrational_processing', False):
                    ...
                    capabilities['vibrational_specific']['vibrational_preprocessing'] = handler_caps.get('vibrational_preprocessing', False)
                
                if handler_caps.get('electronic_structure', False):
                    capabilities['vibrational_specific']['homo_lumo_available'] = True
                    capabilities['vibrational_specific']['orbital_analysis'] = handler_caps.get('orbital_analysis', False)
                
                if handler_caps.get('charge_analysis', False):
                    capabilities['vibrational_specific']['charge_decomposition'] = True
                    capabilities['vibrational_specific']['charge_methods'] = handler_caps.get('available_charge_methods', ['mulliken'])
                ...
                if handler_caps.get('uncertainty_handling', False):
                    capabilities['uncertainty_specific']['uncertainty_weighting'] = handler_caps.get('uncertainty_weighting', False)
                    capabilities['uncertainty_specific']['uncertainty_filtering'] = handler_caps.get('uncertainty_filtering', False)
                    capabilities['uncertainty_specific']['uncertainty_methods'] = handler_caps.get('available_uncertainty_methods', ['std'])
                
                if handler_caps.get('statistical_quality', False):
                    capabilities['uncertainty_specific']['quality_metrics'] = handler_caps.get('quality_metrics', [])
                    capabilities['uncertainty_specific']['quality_thresholds'] = handler_caps.get('quality_thresholds', {})
                
                # Check uncertainty configuration from handler only if capability exists
                if (handler_caps.get('uncertainty_handling', False) and 
                    hasattr(handler, 'dataset_config') and 
                    handler.dataset_config is not None and
                    getattr(handler.dataset_config, 'is_uncertainty_enabled', False)):
                    ...
                    if uncertainty_config:
                        capabilities['uncertainty_configuration'] = {
                            ...
                            'weighting_enabled': uncertainty_config.get('use_for_loss_weighting', False),
                        ...
                        if handler_caps.get('advanced_uncertainty', False):
                            ...
                            capabilities['uncertainty_configuration']['uncertainty_propagation'] = handler_caps.get('uncertainty_propagation', False)
            ...
            if should_pass_coordinates_to_structural_features():
                ...
                capabilities['geometric_features'] = {
                    ...
                    'bond_lengths': '3d_bond_features' in capabilities.get('configured_bond_features', []),
                    'angles': '3d_angle_features' in capabilities.get('configured_bond_features', []),
...
def create_molecular_fingerprint(pyg_data: Data) -> Dict[str, Any]:
    ...
    try:
        ...
        if fingerprint['features']['has_atom_features']:
            fingerprint['features']['atom_feature_dim'] = pyg_data.x.shape[1] if pyg_data.x.ndim > 1 else 1
            ...
            if pyg_data.x.numel() > 0:
                ...
                fingerprint['quality_indicators']['quality_status'] = 'poor' if quality_issues else 'good'
            else:
                fingerprint['quality_indicators']['quality_status'] = 'empty'
       
        if fingerprint['features']['has_bond_features']:
            fingerprint['features']['bond_feature_dim'] = pyg_data.edge_attr.shape[1] if pyg_data.edge_attr.ndim > 1 else 1
        ...
        if fingerprint.get('features', {}).get('has_edges', False):
            try:
                ...
                max_node_idx = pyg_data.edge_index.max().item() if pyg_data.edge_index.numel() > 0 else -1
        ...
        if _get_dataset_feature(dataset_type, 'vibrational_analysis'):
            ...
            fingerprint['vibrational_specific'] = {
                ...
                'has_mulliken_charges': hasattr(pyg_data, 'Qmulliken') and pyg_data.Qmulliken is not None,
                'has_electronic_properties': any(hasattr(pyg_data, prop) for prop in ['homo', 'lumo', 'gap'])
            }
            ...
            if hasattr(pyg_data, 'atomization_energy'):
                ...
                fingerprint['vibrational_specific']['atomization_energy'] = float(pyg_data.atomization_energy.item())
                
        if _get_dataset_feature(dataset_type, 'uncertainty_handling'):
            # Uncertainty-enabled datasets fingerprint data
            fingerprint['uncertainty_specific'] = {
                'has_uncertainty': hasattr(pyg_data, 'uncertainty') and pyg_data.uncertainty is not None,
                'has_uncertainty_weight': hasattr(pyg_data, 'uncertainty_weight') and pyg_data.uncertainty_weight is not None,
                'has_relative_uncertainty': hasattr(pyg_data, 'relative_uncertainty') and pyg_data.relative_uncertainty is not None
            }
            
            if fingerprint['uncertainty_specific']['has_uncertainty']:
                fingerprint['uncertainty_specific']['uncertainty_value'] = float(pyg_data.uncertainty.item())
                
            if fingerprint['uncertainty_specific']['has_relative_uncertainty']:
                fingerprint['uncertainty_specific']['relative_uncertainty_value'] = float(pyg_data.relative_uncertainty.item())
                
            if hasattr(pyg_data, 'high_uncertainty') and pyg_data.high_uncertainty is not None:
                fingerprint['uncertainty_specific']['is_high_uncertainty'] = bool(pyg_data.high_uncertainty.item())
...
def estimate_properties_with_handler(handler: 'DatasetHandler',
                                   pyg_data: Data) -> Dict[str, float]:
    """
...
def create_handler_compatible_fingerprint(handler: 'DatasetHandler',
                                        pyg_data: Data) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if hasattr(handler, 'processing_config') and handler.processing_config is not None:
            processing_config = {
                ...
                'variable_properties': getattr(handler.processing_config, 'variable_len_graph_properties', [])
            }
        ...
        if handler_caps.get('custom_properties', True):  # Default to True for backward compatibility
            if hasattr(handler, 'estimate_additional_properties'):
                try:
                    additional_props = handler.estimate_additional_properties(pyg_data)
                    if additional_props is not None:
        ...
        # PHASE 6: Use feature-based fingerprinting instead of type checks
        if (_get_dataset_feature(handler_type, 'vibrational_analysis') and 
            handler_caps.get('vibrational_processing', False)):
            if hasattr(pyg_data, 'freqs') and pyg_data.freqs is not None:
        ...
        if (_get_dataset_feature(handler_type, 'uncertainty_handling') and 
...
def validate_feature_extraction_with_handler(handler: 'DatasetHandler',
                                           pyg_data: Data) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        validation['available_validation_methods'] = handler_caps.get('validation_methods', ['basic'])
        
        # Check handler requirements only if handler reports having them
        if handler_caps.get('has_required_properties', True):  # Default True for backward compatibility
            if hasattr(handler, 'get_required_properties'):
                try:
