...
def _create_handler_specific_error(
    ...
    # Check if this dataset type has vibrational analysis (DFT-like)
...
def validate_molecular_structure(
    ...
    raw_properties_dict: Optional[Dict[str, Any]] = None  
...
```
    ...
    def _handler_validate_molecular_structure(
        ...
        try:
            ...
            from milia_pipeline.config.validators import validate_molecular_structure as centralized_validator
            ...
            logger.debug(f"Molecule {molecule_index} passed {handler.get_dataset_type()} structural validation")
...
def convert_symbols_to_atomic_numbers(symbols: np.ndarray,
                                    molecule_index: int,
                                    inchi: str = "N/A") -> np.ndarray:
    """
...
def check_dataset_compatibility(raw_properties_dict: Dict[str, Any],
                              dataset_type: str,
                              molecule_index: int,
                              inchi: str = "N/A",
                              handler: Optional[DatasetHandler] = None) -> bool:
    """
    ...
    def _handler_check_dataset_compatibility(handler, raw_properties_dict, dataset_type, molecule_index, inchi):
        """
        ...
        try:
            ...
            if dataset_type != handler_type:
                logger.warning(f"Dataset type mismatch: expected {dataset_type}, handler is {handler_type}")
    ...
    return _handler_check_dataset_compatibility(handler, raw_properties_dict, dataset_type, molecule_index, inchi)


def validate_uncertainty_data(pyg_data: Data,
                            molecule_index: Union[int, str],
                            smiles: str,
                            handler: Optional[DatasetHandler] = None) -> Optional[float]:
    """
    ...
    def _handler_validate_uncertainty_data(handler, pyg_data, molecule_index, smiles):
        ...
        if not _get_dataset_feature(dataset_type, 'uncertainty_handling'):
            logger.debug(f"Uncertainty validation skipped for {dataset_type} dataset (no uncertainty_handling feature)")
            return None
        
        # Check if uncertainty is enabled in handler config
        if not getattr(handler.dataset_config, 'is_uncertainty_enabled', False):
            logger.debug(f"Uncertainty validation skipped for {dataset_type} dataset (uncertainty not enabled)")
            return None
        ...
        try:
            ...
            try:
                ...
                if validated_uncertainty is not None:
                    ...
                    logger.debug(f"Validated uncertainty {uncertainty_scalar} for molecule {molecule_index}")
...
def validate_pyg_data_completeness(pyg_data: Data, 
                                  dataset_type: str,
                                  molecule_index: Optional[int] = None,
                                  handler: Optional[DatasetHandler] = None) -> Dict[str, bool]:
    """
    ...
    def _handler_validate_pyg_data_completeness(handler, pyg_data, dataset_type, molecule_index):
        ...
        try:
            ...
            validation_results = _original_validate_pyg_data_completeness(pyg_data, dataset_type, molecule_index)
            ...
            if _get_dataset_feature(handler_dataset_type, 'orbital_analysis'):
                has_orbital_data = (hasattr(pyg_data, 'mo_energies') or 
                                   hasattr(pyg_data, 'homo_lumo_gap_eV'))
                validation_results['has_orbital_data'] = has_orbital_data
            
            logger.debug(f"Handler-based validation for molecule {molecule_index}: {validation_results}")
...
def _original_validate_pyg_data_completeness(pyg_data: Data, 
                                           dataset_type: str,
                                           molecule_index: Optional[int] = None) -> Dict[str, bool]:
    """
...
def create_validator_with_handler(dataset_config: Optional[DatasetConfig] = None) -> DatasetHandler:
    """
...
def validate_molecule_with_handler(raw_properties_dict: Dict[str, Any],
                                 molecule_index: int,
                                 inchi: str = "N/A",
                                 handler: Optional[DatasetHandler] = None) -> bool:
    """
    ...
    try:
        ...
        # 2. Dataset compatibility validation
        try:
            check_dataset_compatibility(raw_properties_dict, dataset_type, molecule_index, inchi, handler)
        except HandlerError as e:
        ...
        logger.error(f"Handler validation failed for molecule {molecule_index}: {validation_errors}")
        raise
    except MoleculeProcessingError as e:
...
def get_validation_summary(validation_results: Dict[str, bool]) -> str:
    ...
    # Categorize results
    critical_checks = ['has_basic_structure', 'has_coordinates', 'has_atomic_numbers', 'has_target_values']
    handler_checks = ['has_handler_required_props', 'has_uncertainty', 'has_vibrational_data', 
                      'has_atomization_energy', 'has_orbital_data']
    ...
    def format_check_group(check_names, group_title):
        ...
        for check_name in check_names:
            if check_name in validation_results and isinstance(validation_results[check_name], bool):
    ...
    if handler_type and _get_dataset_feature(handler_type, 'vibrational_analysis'):
        if not validation_results.get('has_vibrational_data', True):
            recommendations.append(f"• Consider adding vibrational data for enhanced {handler_type} analysis")
        if not validation_results.get('has_atomization_energy', True):
            if _get_dataset_feature(handler_type, 'atomization_energy'):
                recommendations.append(f"• Consider adding atomization energy for {handler_type} dataset")
...
def validate_molecule_legacy(raw_properties_dict: Dict[str, Any],
                            molecule_index: int,
                            inchi: str = "N/A") -> bool:
    """
    ...
    try:
        ...
        return check_dataset_compatibility(raw_properties_dict, dataset_type, molecule_index, inchi, handler)
...
def get_dataset_requirements(dataset_type: str, 
                           handler: Optional[DatasetHandler] = None) -> Dict[str, List[str]]:
    """
    ...
    try:
        ...
        if _get_dataset_feature(handler_dataset_type, 'vibrational_analysis'):
            requirements['optional_properties'] = _get_dataset_optional_properties(handler_dataset_type)
            if _get_dataset_feature(handler_dataset_type, 'atomization_energy'):
                requirements['derived_properties'] = ['atomization_energy']
        
        # Orbital requirements (orbital_analysis feature)
        if _get_dataset_feature(handler_dataset_type, 'orbital_analysis'):
            requirements['orbital_properties'] = _get_dataset_optional_properties(handler_dataset_type)
        
        # Add validation requirements
        requirements['structural_requirements'] = ['atoms', 'coordinates']
        requirements['energy_requirements'] = ['Etot']
        
        logger.debug(f"Retrieved requirements for {handler.get_dataset_type()} dataset: {len(requirements['required_properties'])} properties")
        return requirements
...
def create_validation_context(handler: DatasetHandler,
                            molecule_index: int,
                            inchi: str = "N/A") -> Dict[str, Any]:
    """
    ...
    if _get_dataset_feature(dataset_type, 'vibrational_analysis'):
        ...
        context['supports_atomization_energy'] = _get_dataset_feature(dataset_type, 'atomization_energy')
...
def validate_with_detailed_feedback(raw_properties_dict: Dict[str, Any],
                                  molecule_index: int,
                                  inchi: str = "N/A",
                                  handler: Optional[DatasetHandler] = None) -> Dict[str, Any]:
    """
    ...
    validation_steps = [
        ('structure_validation', lambda: validate_molecular_structure(
            raw_properties_dict.get('atoms'), raw_properties_dict.get('coordinates'), 
            molecule_index, inchi, handler)),
        ('compatibility_validation', lambda: check_dataset_compatibility(
            raw_properties_dict, handler.get_dataset_type(), molecule_index, inchi, handler)),
        ('handler_validation', lambda: handler.validate_molecule_data(
            raw_properties_dict, molecule_index, inchi))
    ]
    ...
    passed_steps = sum(1 for step in detailed_results['validation_steps'].values() if step['passed'])
