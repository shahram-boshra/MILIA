...
from milia_pipeline.config.config_accessors import (
    ...
    get_identifier_keys,
    # Transformation system accessors
...
from milia_pipeline.config.validators import validate_uncertainty_data
...
class MoleculeDataConverter:
    ...
    def __init__(self,
                 ...
                 structural_features_config: Optional[Dict[str, Any]] = None,
                 atomic_energies_hartree: Optional[Dict[str, float]] = None,
                 har2ev: Optional[Union[int, float]] = None,
                 heavy_atom_symbols_to_z: Optional[Dict[str, int]] = None,
        ...
        self.structural_features_config = structural_features_config or get_dataset_appropriate_structural_features()
        ...
        self._transformation_config = transformation_config or self._create_transformation_config_safe()
        ...
        if not RDKIT_AVAILABLE:
            self.logger.warning("RDKit not available. Structural features will be limited to basic atomic properties.")
    ...
    def _create_transformation_config_safe(self) -> Optional[TransformationConfig]:
        """
    ...
    def _validate_config_containers(self,
                                    dataset_config: Any,
                                    filter_config: Any,
    ...
    def _initialize_transform_system_awareness(self) -> None:
        ...
        try:
            ...
            self._transform_capabilities = {
                'available_transforms': gt.get_available_transforms() if hasattr(gt, 'get_available_transforms') else {},
            ...
            if self._transformation_config:
                self.logger.info(f"Transform system initialized with {len(list_experimental_setups())} experimental setups")
            else:
    ...
    def get_transform_capabilities(self) -> Dict[str, Any]:
        ...
        if not is_available:
            ...
            capabilities['reason'] = "; ".join(reasons) if reasons else "Transform system unavailable"
        ...
        if self._transformation_config:
            try:
                if isinstance(self._transformation_config, dict):
                    capabilities['experimental_setups'] = list(self._transformation_config.get('experimental_setups', {}).keys())
                    capabilities['default_setup'] = self._transformation_config.get('default_setup', 'unknown')
                    capabilities['validation_enabled'] = self._transformation_config.get('validation', {}).get('enabled', True)
                else:
                    capabilities['experimental_setups'] = list(self._transformation_config.experimental_setups.keys())
                    ...
                    if isinstance(self._transformation_config.validation, dict):
                        capabilities['validation_enabled'] = self._transformation_config.validation.get('enabled', True)
                    else:
                        capabilities['validation_enabled'] = self._transformation_config.validation.enabled
            except AttributeError:
    ...
    def validate_transform_compatibility(self, transform_specs: List[TransformSpec]) -> Dict[str, Any]:
        """
        ...
        if self._enhanced_transform_available and self._cache_manager:
            ...
            if cached_result is not None:
                self.logger.debug(f"Using cached validation result for {len(transform_specs)} transforms")
        ...
        if not self._transform_system_available:
            validation_results['warnings'].append("Transform system not available - limited validation")
        ...
        try:
            ...
            for i, spec in enumerate(transform_specs):
                ...
                if spec.name not in all_transforms and self._enhanced_transform_available and self._dynamic_discovery:
                ...
                if self._enhanced_transform_available and self._parameter_introspector and transform_result['valid']:
                    try:
                        ...
                        transform_result['warnings'].append(f"Could not introspect parameters: {str(e)}")
    ...
    def get_transform_parameter_info(self, transform_name: str) -> Optional[Dict[str, Any]]:
        """
    ...
    def discover_available_transforms(self, 
        ...
        if not self._enhanced_transform_available or not self._dynamic_discovery:
            ...
            if self._transform_capabilities:
                discovery_results['by_category'] = self._transform_capabilities.get('available_transforms', {})
        ...
        try:
            ...
            self.logger.debug(f"Discovered {discovered.total_count} transforms ({len(discovered.custom)} custom)")
    ...
    def _validate_transformation_configuration(self) -> None:
        ...
        try:
            if not self._transformation_config.experimental_setups:
                self.logger.warning("No experimental setups defined in transformation configuration")
                return
            
            # Validate default setup exists
            if self._transformation_config.default_setup not in self._transformation_config.experimental_setups:
            ...
            for setup_name, setup in self._transformation_config.experimental_setups.items():
                if not setup.transforms:
                    ...
                    self.logger.debug(f"Experimental setup '{setup_name}' has no experimental transforms (standard transforms may still apply)")
        ...
        try:
            ...
            if handler_type != self._dataset_config.dataset_type:
                raise HandlerCompatibilityError(
                    ...
                    incompatible_features=[f"dataset_type_mismatch"],
            ...
            raise HandlerCompatibilityError(
                ...
                handler_type=getattr(self._dataset_handler, 'get_dataset_type', lambda: 'unknown')(),
                details=f"Validation error: {type(e).__name__}: {str(e)}"
            ) from e


#---
    def _validate_with_handler(self,
                              molecule_index: int,
        ...
        if self._use_handlers and self._dataset_handler:
            try:
                ...
                if molecule_index < 3:
                    self.logger.debug(f"Used {self._dataset_handler.get_dataset_type()} handler validation")
...
#---                
                ...
                if molecule_index < 3:
                    self.logger.debug(f"Unexpected handler validation error for molecule {molecule_index}: {e}")
                raise MoleculeProcessingError(
                    molecule_index=molecule_index,
                    inchi=identifier,
...
#---
    ...
    def _legacy_validation_fallback(self,
                                  molecule_index: int,
                                  raw_properties_dict: Dict[str, Any],
                                  identifier: str) -> None:
        """
        ...
        try:
            ...
            if self._dataset_handler is not None:
                try:
                    ...
                    essential_props = [p for p in handler_props if p in raw_properties_dict or 
                                      p in ['Etot', 'energy', 'U0', 'atoms', 'coordinates', 'compounds']]
                    if molecule_index < 3:
                        self.logger.debug(f"Legacy fallback using handler properties for {dataset_type}: {essential_props}")
                except Exception as e:
            ...
            if not essential_props:
                try:
                    ...
                    if molecule_index < 3:
                        self.logger.debug(f"Legacy fallback using config_constants properties for {dataset_type}: {essential_props}")
                except Exception as e:
    ...
    def _legacy_dmc_validation(self,
                             molecule_index: int,
                             raw_properties_dict: Dict[str, Any],
                             identifier: str) -> None:
        """
        ...
        try:
            ...
            if etot is not None:
                try:
                    ...
                    if abs(energy_val) > 10000:  # Hartree
                        self.logger.warning(f"{self._dataset_config.dataset_type} molecule {molecule_index} has unusually large energy: {energy_val}")
            ...
            if self._dataset_config.is_uncertainty_enabled and self._dataset_config.uncertainty_config:
    ...
    def _validate_dmc_uncertainty(self,
                                molecule_index: int,
                                raw_properties_dict: Dict[str, Any],
                                identifier: str) -> None:
        """
        ...
        if uncertainty_value is not None:
            try:
                ...
                if validated_uncertainty is not None:
                    uncertainty_scalar = float(validated_uncertainty.item()) if hasattr(validated_uncertainty, 'item') else float(validated_uncertainty)
    ...
    def _legacy_dft_validation(self,
                             molecule_index: int,
                             raw_properties_dict: Dict[str, Any],
                             identifier: str) -> None:
        """
        ...
        try:
            ...
            if atomization_base:
                ...
                if not is_value_valid_and_not_nan(base_energy):
                    self.logger.warning(f"{self._dataset_config.dataset_type} molecule {molecule_index}: missing {atomization_base} for atomization energy calculation")
    ...
    def _process_properties_with_handler(self,
                                        raw_properties_dict: Dict[str, Any],
                                        molecule_index: int,
                ...
                if key in raw_properties_dict:
                    if key in ['atoms', 'coordinates', 'inchi', 'compounds']:
                        ...
                        try:
                            ...
                            if molecule_index < 3:
                                self.logger.debug(f"Handler property processing failed for {key}: {e}")
                            # Decide whether to use original value or raise error
                            if e.recovery_suggestions and 'use_original_value' in e.recovery_suggestions:
                                processed[key] = raw_properties_dict[key]
                            else:
    ...
    def _enrich_with_handler(self,
        ...
        try:
            ...
            if molecule_index < 3:
                self.logger.debug(f"Applied {self._dataset_handler.get_dataset_type()} handler enrichment")
    ...
    def _validate_structural_features_configuration(self) -> None:
        ...
        if should_pass_coordinates_to_structural_features():
            ...
            if not geometric_config.get('enable_3d_features', True):
                self.logger.warning("Coordinates will be passed to structural features but 3D features are disabled")
        ...
        if should_enable_stereochemistry_preprocessing():
            ...
            if not stereo_config.get('assign_stereochemistry', True):
                self.logger.warning("Stereochemistry preprocessing enabled but assignment is disabled")

    def _ensure_pyg_data_tensors(self, pyg_data: Data, molecule_index: int, identifier: str) -> Data:
        """
        ...
        def _convert_to_tensor(value: Any, attr_name: str, default_dtype: torch.dtype = torch.float32) -> torch.Tensor:
            ...
            try:
                ...
                self.logger.debug(f"Could not convert {attr_name} to tensor for molecule {molecule_index}: {e}")
        ...
        try:
                    ...
                    if attr_value is not None and not isinstance(attr_value, torch.Tensor):
                        ...
                        if isinstance(tensor_value, torch.Tensor):
                            ...
                            if molecule_index < 3:  # Only log for first few molecules
                                self.logger.debug(f"Converted {attr_name} from {type(attr_value)} to tensor for molecule {molecule_index}")
            
            # REFINED: Skip internal PyG storage objects (don't try to convert these)
            skip_attrs = {
                'edge_stores', 'node_stores', 'stores',  # PyG internal storage
                '_store', '_edge_store', '_node_store',   # PyG internal storage
                'freqs', 'vibmodes',                      # Special vibrational data
                'inchi', 'smiles', 'dataset_type',        # String attributes
                'original_mol_idx', 'num_nodes', 'num_edges'  # Scalar attributes
            }
            ...
            self.logger.error(f"Error ensuring tensor conversion for molecule {molecule_index}: {e}")
            self.logger.debug(f"Error context: {error_context}")
            return pyg_data
    ...
    def convert(self,
                ...
                dataset_config: Optional[DatasetConfig] = None,
                filter_config: Optional[FilterConfig] = None) -> Optional[Data]:
        """
        ...
        try:
            self.logger.debug(f"Converting {dataset_config.dataset_type} molecule {current_mol_index}")
            ...
            pyg_data = self._ensure_pyg_data_tensors(pyg_data, current_mol_index, current_mol_identifier)
            ...
            self.logger.debug(f"Successfully converted {dataset_config.dataset_type} molecule {current_mol_index}")
            return pyg_data
        ...
        # Handler-Based Pattern Development: Enhanced exception handling with handler-specific errors
            ...
            if is_recoverable:
                self.logger.warning(f"Recoverable handler error for molecule {current_mol_index}: {error_summary}")
                self.logger.debug(f"Error context: {error_context}")
            else:
                self.logger.error(f"Non-recoverable handler error for molecule {current_mol_index}: {error_summary}")
                self.logger.debug(f"Error context: {error_context}")
                recovery_suggestions = get_exception_recovery_suggestions(e)
    ...
    def _create_pyg_data_original_approach(self,
                                         ...
                                         dataset_config: Optional[DatasetConfig] = None) -> Data:
        """
        ...
        try:
            if RDKIT_AVAILABLE:
                ...
                atomic_numbers, coords = validate_molecular_structure(
                    atomic_numbers, coordinates, current_mol_index, current_mol_identifier,
                    handler=self._dataset_handler  # Add handler parameter
                )
    ...
    def _add_structural_features_enhanced(self,
                                        pyg_data: Data,
                                        current_mol_index: int,
                                        current_mol_identifier: str,
                                        raw_properties_dict: Dict[str, Any]) -> Data:
        """
        ...
        try:
            ...
            if not self.is_structural_features_enabled or not self.structural_features_config:
                self.logger.info(f"No structural features configured for molecule {current_mol_index}, identifier '{current_mol_identifier}'. Skipping feature addition.")
            ...
            if not RDKIT_AVAILABLE:
                self.logger.warning(f"RDKit not available for molecule {current_mol_index}. Cannot add structural features.")
                pyg_data.x = None
                pyg_data.edge_attr = None
                return pyg_data
            
            self.logger.debug(f"Adding enhanced structural features for mol index {current_mol_index}, identifier '{current_mol_identifier}'")
#---            
            ...
            if atoms is None or coordinates is None:
                self.logger.warning(f"Cannot add structural features to molecule {current_mol_index}: missing atoms or coordinates")
                pyg_data.x = None
                pyg_data.edge_attr = None
                return pyg_data
            ...
            if should_pass_coordinates_to_structural_features():
                ...
                self.logger.debug(f"Passing coordinates to structural features for molecule {current_mol_index}")
            ...
            if should_pass_mulliken_charges_to_structural_features():
                ...
                if qmulliken is not None and is_value_valid_and_not_nan(qmulliken):
                    ...
                    self.logger.debug(f"Passing Mulliken charges to structural features for molecule {current_mol_index}")
                else:
                    self.logger.debug(f"Mulliken charges requested but not available for molecule {current_mol_index}")
            
            # Apply stereochemistry preprocessing if enabled
            if should_enable_stereochemistry_preprocessing():
                try:
                    # Use already imported Chem module instead of importing rdMolOps
                    Chem.AssignStereochemistry(rdkit_mol, cleanIt=self.stereochemistry_config.get('cleanup_stereochemistry', True))
                    self.logger.debug(f"Applied stereochemistry preprocessing for molecule {current_mol_index}")
                except Exception as e:
                    self.logger.warning(f"Failed to apply stereochemistry preprocessing for molecule {current_mol_index}: {e}")
            ...
            if hasattr(enhanced_data, 'x') and enhanced_data.x is not None:
                if not isinstance(enhanced_data.x, torch.Tensor):
                    self.logger.debug(f"Converting x from {type(enhanced_data.x)} to tensor for molecule {current_mol_index}")
            ...
            if hasattr(enhanced_data, 'edge_attr') and enhanced_data.edge_attr is not None:
                if not isinstance(enhanced_data.edge_attr, torch.Tensor):
                    self.logger.debug(f"Converting edge_attr from {type(enhanced_data.edge_attr)} to tensor for molecule {current_mol_index}")
            ...
            if hasattr(enhanced_data, 'x') and enhanced_data.x is not None:
                ...
                self.logger.debug(f"Added {atom_feature_dim} atom features for molecule {current_mol_index}")
            
            if hasattr(enhanced_data, 'edge_attr') and enhanced_data.edge_attr is not None:
                bond_feature_dim = enhanced_data.edge_attr.shape[1] if enhanced_data.edge_attr.ndim > 1 else 1
                num_edges = enhanced_data.edge_attr.shape[0]
                self.logger.debug(f"Added {bond_feature_dim} bond features for {num_edges} edges in molecule {current_mol_index}")
    ...
    def _attach_identifiers_original_logic(self,
                                         pyg_data: Data,
                                         current_mol_identifier: str,
                                         mol_id_type: str,
                                         current_mol_index: int) -> None:
    ...
    def _validate_final_pyg_data(self,
                               pyg_data: Data,
                               molecule_index: int,
                               smiles: str,
                               dataset_config: Optional[DatasetConfig] = None) -> None:
        """
        ...
        try:
            ...
            if self.is_structural_features_enabled:
                self._validate_structural_features_in_pyg_data(pyg_data, molecule_index, validation_results)
            
            self.logger.debug(f"Final validation passed for {dataset_config.dataset_type} molecule {molecule_index}")
    ...
    def _legacy_validate_dmc_pyg_data(self, 
                                    pyg_data: Data, 
                                    molecule_index: int,
        ...
        try:
            ...
            if dataset_config.is_uncertainty_enabled:
                ...
                uncertainty_field = uncertainty_config.get('uncertainty_field_name', 'std')
                has_uncertainty = (hasattr(pyg_data, 'uncertainty') or
                                 hasattr(pyg_data, uncertainty_field) or
                                 hasattr(pyg_data, 'uncertainty_metadata'))
                if not has_uncertainty:
                    self.logger.warning(f"{dataset_config.dataset_type} molecule {molecule_index} missing uncertainty metadata despite being enabled")
                ...
                if use_weighting and not hasattr(pyg_data, 'uncertainty_weight'):
                    self.logger.warning(f"{dataset_config.dataset_type} molecule {molecule_index} missing uncertainty_weight despite loss weighting enabled")
            ...
            if hasattr(pyg_data, 'y') and pyg_data.y is not None:
                ...
                if isinstance(y_values, torch.Tensor):
                    if torch.any(y_values > 1000):
                        self.logger.warning(f"{dataset_config.dataset_type} molecule {molecule_index} has unusually high energy values")
            ...
            if HANDLERS_AVAILABLE:
                ...
                self.logger.warning(f"{dataset_config.dataset_type} PyG data validation error for molecule {molecule_index}: {e}")
        ...
        try:
            ...
            if hasattr(pyg_data, 'atomization_energy'):
                ...
                if isinstance(atom_energy, torch.Tensor) and torch.any(atom_energy > 0):
                    self.logger.warning(f"{self._dataset_config.dataset_type} molecule {molecule_index} has positive atomization energy (unusual)")
            ...
            if HANDLERS_AVAILABLE:
                ...
                self.logger.warning(f"{self._dataset_config.dataset_type} PyG data validation error for molecule {molecule_index}: {e}")

    def _validate_structural_features_in_pyg_data(self, pyg_data: Data, molecule_index: int, validation_results: Dict[str, bool]) -> None:
        """
        ...
        try:
            ...
            if hasattr(pyg_data, 'x') and pyg_data.x is not None:
                if isinstance(pyg_data.x, torch.Tensor):
                    if torch.any(torch.isnan(pyg_data.x)) or torch.any(torch.isinf(pyg_data.x)):
                        self.logger.warning(f"Molecule {molecule_index} has NaN/Inf values in atom structural features")
                    ...
                    if expected_atom_features > 0:
                        ...
                        if atom_feature_dim == 0:
                            self.logger.warning(f"Molecule {molecule_index} has zero-dimensional atom features despite configuration")
            ...
            if hasattr(pyg_data, 'edge_attr') and pyg_data.edge_attr is not None:
                if isinstance(pyg_data.edge_attr, torch.Tensor):
                    if torch.any(torch.isnan(pyg_data.edge_attr)) or torch.any(torch.isinf(pyg_data.edge_attr)):
                        self.logger.warning(f"Molecule {molecule_index} has NaN/Inf values in bond structural features")
                    
                    expected_bond_features = len(self.structural_features_config.get('bond', []))
                    if expected_bond_features > 0:
                        bond_feature_dim = pyg_data.edge_attr.shape[1] if pyg_data.edge_attr.ndim > 1 else 1
                        if bond_feature_dim == 0:
                            self.logger.warning(f"Molecule {molecule_index} has zero-dimensional bond features despite configuration")
            ...
            if configured_atom_features and (not hasattr(pyg_data, 'x') or pyg_data.x is None):
                self.logger.warning(f"Molecule {molecule_index} configured for atom features but pyg_data.x is None")
            
            if configured_bond_features and (not hasattr(pyg_data, 'edge_attr') or pyg_data.edge_attr is None):
                self.logger.warning(f"Molecule {molecule_index} configured for bond features but pyg_data.edge_attr is None")
                
        except Exception as e:
            self.logger.warning(f"Error validating structural features for molecule {molecule_index}: {e}")

    def _prepare_molecule_data(self,
                              molecule_index: int,
                              raw_properties_dict: Dict[str, Any],
                              dataset_config: Optional[DatasetConfig] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if mol_identifier is None:
                ...
                mol_id_type = 'index'
                self.logger.debug(f"No identifier found for molecule {molecule_index}, using index-based identifier")
            ...
            self._validate_with_handler(
                molecule_index, raw_properties_dict, mol_identifier
            )
    ...
    def _homogenize_rots_property(self,
                                 molecule_index: int,
                                 mol_identifier: Optional[str],
                                 raw_properties_dict: Dict[str, Any]) -> None:
        """
    ...
    def _create_base_pyg_data(self,
                            molecule_index: int,
        ...
        if _should_use_enhanced_utils(dataset_config.dataset_type) and RDKIT_AVAILABLE:
            try:
                ...
                self.logger.debug(f"Enhanced utils failed for molecule {molecule_index}, trying fallback: {e}")
    ...
    def _log_structural_features_capabilities(self) -> None:
        ...
        try:
            ...
            invalid_atom_features = [
                f for f in configured_atom_features 
                if f not in available_features['atom']
            ]
            invalid_bond_features = [
                f for f in configured_bond_features 
                if f not in available_features['bond']
            ]
    ...
    def _create_pyg_data_with_enhanced_utils(self,
                                           current_mol_index: int,
                                           raw_properties_dict: Dict[str, Any],
                                           current_mol_identifier: str,
                                           mol_id_type: str,
...
#---
            ...
            try:
                if isinstance(self._transformation_config, dict):
                    stats['transformation_config'] = {
                        'experimental_setups': list(self._transformation_config.get('experimental_setups', {}).keys()),
                        'default_setup': self._transformation_config.get('default_setup', 'unknown'),
                        'validation_enabled': self._transformation_config.get('validation', {}).get('enabled', True)
                    }
                else:
                    stats['transformation_config'] = {
                        'experimental_setups': list(self._transformation_config.experimental_setups.keys()),
                        'default_setup': self._transformation_config.default_setup,
                        'validation_enabled': self._transformation_config.validation.get('enabled', True) if isinstance(self._transformation_config.validation, dict) else self._transformation_config.validation.enabled
                    }
            except AttributeError:
        ...
        if self._enhanced_transform_available:
            ...
            if self._dynamic_discovery:
                try:
                    stats['enhanced_transform_statistics']['discovery'] = {
                        'total_discovered': len(self._dynamic_discovery.get_discovered_transforms()),
                        'custom_transforms': len(self._dynamic_discovery.get_custom_transforms()),
                        'discovery_enabled': True
                    }
                except Exception as e:
                    stats['enhanced_transform_statistics']['discovery'] = {'enabled': False, 'error': str(e)}
            ...
            if self._cache_manager:
                try:
                    ...
                    stats['enhanced_transform_statistics']['cache'] = {'enabled': False, 'error': str(e)}
    ...
    def validate_configuration_compatibility(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not _is_dataset_type_registered(dataset_type):
                ...
                available_types = _get_available_dataset_types()
                diagnostics['errors'].append(f"Invalid dataset type: {dataset_type}. Available: {available_types}")
            ...
            if not handler_validation['handlers_available']:
                diagnostics['warnings'].append("Dataset handlers not available, using legacy validation")
            
            if handler_validation['errors']:
                diagnostics['errors'].extend([f"Handler: {err}" for err in handler_validation['errors']])
            
            if handler_validation['warnings']:
                diagnostics['warnings'].extend([f"Handler: {warn}" for warn in handler_validation['warnings']])
            
            # Validate structural features
            if self.is_structural_features_enabled:
                if not RDKIT_AVAILABLE:
                    diagnostics['warnings'].append("RDKit not available, structural features limited")
                ...
                try:
                    ...
                    invalid_atom_features = [f for f in configured_atom_features if f not in available_features['atom']]
                    invalid_bond_features = [f for f in configured_bond_features if f not in available_features['bond']]
                    
                    if invalid_atom_features or invalid_bond_features:
                        diagnostics['structural_features_valid'] = False
                        diagnostics['errors'].append(f"Invalid features configured: atoms={invalid_atom_features}, bonds={invalid_bond_features}")
                except Exception as e:
                    diagnostics['structural_features_valid'] = False
            ...
            if self._transformation_config:
                try:
                    ...
                    diagnostics['transform_diagnostics']['experimental_setups'] = list(self._transformation_config.experimental_setups.keys())
    ...
    def get_registry_integration_status(self) -> Dict[str, Any]:
        ...
        if _is_dataset_type_registered(self.dataset_type):
            ...
            status['molecule_creation_strategy'] = _get_dataset_molecule_creation_strategy(self.dataset_type)
