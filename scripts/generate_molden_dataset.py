...
def _generate_standard_molden(molecule_name: str, num_atoms: int) -> str:
    ...
    for i in range(num_atoms):
        ...
        content.append(
            f"{atom}  {i+1}  {atom_numbers[atom]}  {x:.6f}  {y:.6f}  {z:.6f}"
        )
...
def create_test_dataset(
    ...
    output_path: Path,
    complexity: str = 'simple',
    atoms_per_molecule: Optional[Tuple[int, int]] = None,
    compression: str = 'gz'
) -> Path:
    """
...
Examples:
  ...
  python create_test_dataset.py --validate ./test_data/dataset.tar.gz
    ...
    parser.add_argument(
        '--output',
        type=Path,
        help='Output path for .tar.gz file'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help='Minimum atoms per molecule (default: 2)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help='Maximum atoms per molecule (default: 10)'
    )
    
    parser.add_argument(
        '--preset',
        choices=['small', 'medium', 'large', 'all'],
        help='Create preset dataset'
    )
