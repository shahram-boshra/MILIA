...
class SafeWavefunctionPreprocessor:
            ...
            with tarfile.open(self.tar_path, 'r:gz') as tar:
                ...
                for member in tar:
                    ...
                    if member.name.endswith('.molden') and member.isfile():
                        ...
                        logger.info(f"  [{molden_count}/{self.num_molecules}] Extracted: {member.name}")
    ...
    def parse_molden_file(self, filepath: Path) -> Optional[Dict]:
        """
    ...
    def _extract_features(self, mol_data, compound_name: str) -> Dict:
        ...
        if hasattr(mol_data, 'mo') and mol_data.mo is not None:
            ...
            if 'homo_energy' in features and 'lumo_energy' in features:
                ...
                features['chemical_potential'] = (features['homo_energy'] + features['lumo_energy']) / 2.0
        ...
        atomic_masses = {
            ...
            14: 28.085, 15: 30.974, 16: 32.06, 17: 35.45, 35: 79.904
        }
        ...
        # Molecular weight
        total_mass = sum(atomic_masses.get(z, 0) * count 
    ...
    def process_files(self, molden_files: List[Path]) -> Dict[str, np.ndarray]:
        ...
        *** THIS IS WHERE THE BUG WAS FIXED ***
        ...
        for i, filepath in enumerate(molden_files, 1):
            ...
            for stat_key in ['occupied_energy_mean', 'occupied_energy_std', 
                           'occupied_energy_min', 'occupied_energy_max',
                           'virtual_energy_mean', 'virtual_energy_std',
                           'virtual_energy_min', 'virtual_energy_max']:
                if stat_key in features:
                    data[stat_key].append(features[stat_key])
            
            # TIER 4: Chemical Descriptors (NEW!)
            for chem_key in ['ionization_potential_approx', 'electron_affinity_approx',
        ...
        for key, values in data.items():
            ...
            if key in ['compounds', 'mo_kind', 'molecular_formula']:
                npz_data[key] = np.array(values, dtype=object)
            
            # OBJECT ARRAYS: Variable-length arrays (ragged arrays)
            elif key in ['atoms', 'coordinates', 'mo_energies', 'mo_occupations', 'mo_coefficients']:
                npz_data[key] = np.array(values, dtype=object)
            
            # INTEGER DATA: Scalar integers per molecule
            elif key in ['n_atoms', 'n_basis', 'n_occupied_orbitals', 'n_virtual_orbitals', 'n_shells']:
                npz_data[key] = np.array(values, dtype=np.int32)
            
            # FLOAT DATA: Scalar floats per molecule
            # Core energies
            elif key in ['n_electrons', 'homo_energy', 'lumo_energy', 
                        'homo_lumo_gap', 'homo_lumo_gap_eV']:
                npz_data[key] = np.array(values, dtype=np.float64)
            
            # Orbital energy statistics (NEW!)
            elif key in ['occupied_energy_mean', 'occupied_energy_std',
                        'occupied_energy_min', 'occupied_energy_max',
                        'virtual_energy_mean', 'virtual_energy_std',
                        'virtual_energy_min', 'virtual_energy_max']:
                npz_data[key] = np.array(values, dtype=np.float64)
            ...
            # Chemical descriptors (NEW!)
            elif key in ['ionization_potential_approx', 'electron_affinity_approx',
                        'chemical_hardness', 'chemical_potential']:
                npz_data[key] = np.array(values, dtype=np.float64)
            
            # Other scalars (NEW!)
            elif key in ['total_energy', 'molecular_weight']:
                npz_data[key] = np.array(values, dtype=np.float64)
